...
public class SunPKCS11Wrapper implements SecurityProvider {
    ...
    /**
     * Indicates the name and library used to install pkcs provider. This variable may be change when
     * user want to log in with <code>OpenDNIe</code> or another pcks provider. It is system dependent
     * (.dll file in Windows or .so file in Linux and UNIX) so, user should check before using. By
     * default it is initialized to {@link #DLL_LIBRARY}.
     */
