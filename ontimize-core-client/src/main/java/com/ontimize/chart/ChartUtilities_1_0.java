...
public class ChartUtilities_1_0 implements IChartUtilities {
	...
	protected XYDataset getXYDataset(String colX, String[] colY, String[] series, int timeInterval, boolean fillZeros, int[] operations) {
		...
		for (int i = 0; i < series.length; i++) {
			...
			TimeSeriesCollection serie = (TimeSeriesCollection) this.getXYDataset(colX, colY[i], series[i], timeInterval, fillZeros, operation, (CategoryData) null);
			if (set == null) {
				set = serie;
			} else if (serie != null) {
	...
	protected XYDataset getXYDataset(String colX, String colY[], String series[], int timeInterval, boolean fillCeros, int operations[], String columnSeriesValuesGen[]) {
		TimeSeriesCollection set = null;
		for (int i = 0; i < series.length; i++) {
			...
			TimeSeriesCollection serie = (TimeSeriesCollection) this.getXYDataset(colX, colY[i], series[i], timeInterval, fillCeros, iOperation,
					this.getCategoryData(colX, colY[i], iOperation, columnSeriesValuesGen == null ? null : columnSeriesValuesGen[i], ChartInfo.NONE));
			if (set == null) {
				set = serie;
				continue;
			}
	 ...
	 * @param fillZeros
	 * @param operation
	 *            Accepts values SUM, MAX and MIN. AVG value not available
	 * @return
	 */
	protected XYDataset getXYDataset(String colX, String colY, String series, int timeInterval, boolean fillZeros, int operation, CategoryData categoryData) {
		...
		if (yDefDataList != null) {
			...
			for (int k = 0; k < yDefDataList.length; k++) {
				...
				if (categoryData.getSerie() != null) {
					if (serieValue != null) {
						series = series + "(" + ApplicationManager.getTranslation(categoryData.getSerie(), this.t.getResourceBundle()) + "=" + serieValue + ")";
					} else {
						series = series + "(" + ApplicationManager.getTranslation(categoryData.getSerie(), this.t.getResourceBundle()) + "= )";
					}
				}
	...
	protected CategoryDataset getCategoryDataset(String colX, String[] colsY, String[] seriesNames, int[] operations, int timeGrouping) {
		return this.getCategoryDataset(colX, colsY, seriesNames, operations, null, timeGrouping);
	}

	protected CategoryDataset getCategoryDataset(String colX, String colsY[], String seriesNames[], int operations[], String columnSeriesValuesGen[], int timeGrouping) {
		if (!Table.isChartEnabled()) {
			return null;
		}
		// Data vectors
		...
		for (int i = 0; i < colsY.length; i++) {
			...
			CategoryData categoryData = this.getCategoryData(colX, colsY[i], iOperation, columnSeriesValuesGen == null ? null : columnSeriesValuesGen[i], timeGrouping);
	...
	protected PieDataset getPieDataset(String colX, String colY, int operation, int timeGrouping) {
		...
		PieDataset set = DatasetUtilities.createPieDatasetForRow(this.getCategoryDataset(colX, new String[] { colY }, new String[] { colY }, timeGrouping), 0);
		return set;
	}
	...
	public JFreeChart getChart(String description) {
		...
		if (info.hasIntervals()) {
			XYDataset xySet = this.getXYDataset(info.colX, info.colsY, info.series, info.intervalType, info.fillZeros(), info.getOperations(), info.getColumnSeriesValuesGen());
			JFreeChart chart = ChartFactory.createTimeSeriesChart(description, info.xLabel, info.yLabel, xySet, true, true, true);
			((ChartRepository_1_0) this.charts).addChart(chart, description);
		} else {
			int type = info.type;
			switch (type) {
			case PIE:
				CategoryDataset set = this.getCategoryDataset(info.colX, info.colsY, info.series, info.getOperations(), info.getColumnSeriesValuesGen(), info.intervalType);
				// PieDataset pieData =
				// DatasetUtilities.createPieDatasetForRow(set, 0);
				JFreeChart chart = ChartFactory.createMultiplePieChart(description, set, TableOrder.BY_ROW, true, true, true);
				...
				break;
			case PIE_3D:
				set = this.getCategoryDataset(info.colX, info.colsY, info.series, info.getOperations(), info.getColumnSeriesValuesGen(), info.intervalType);
				// PieDataset set2 =
				// DatasetUtilities.createPieDatasetForRow(set, 0);
				chart = ChartFactory.createMultiplePieChart3D(description, set, TableOrder.BY_ROW, true, true, true);
				((ChartRepository_1_0) this.charts).addChart(chart, description);
				break;
			case BAR:
				set = this.getCategoryDataset(info.colX, info.colsY, info.series, info.getOperations(), info.getColumnSeriesValuesGen(), info.intervalType);
				chart = ChartFactory.createBarChart(description, info.xLabel, info.yLabel, set, PlotOrientation.VERTICAL, true, true, true);
			...
			case BAR_3D:
				set = this.getCategoryDataset(info.colX, info.colsY, info.series, info.getOperations(), info.getColumnSeriesValuesGen(), info.intervalType);
				chart = ChartFactory.createBarChart3D(description, info.xLabel, info.yLabel, set, PlotOrientation.VERTICAL, true, true, true);
				if (chart.getPlot() instanceof CategoryPlot) {
					CategoryItemRenderer cRenderer = ((CategoryPlot) chart.getPlot()).getRenderer();
					ItemLabelPosition position1 = new ItemLabelPosition(ItemLabelAnchor.OUTSIDE12, TextAnchor.CENTER_LEFT, TextAnchor.CENTER_LEFT, -Math.PI / 4);
					cRenderer.setBasePositiveItemLabelPosition(position1);
					ItemLabelPosition position2 = new ItemLabelPosition(ItemLabelAnchor.OUTSIDE12, TextAnchor.CENTER_LEFT, TextAnchor.CENTER_LEFT, -Math.PI / 4);
			...
			case STACKED_3D:
				set = this.getCategoryDataset(info.colX, info.colsY, info.series, info.getOperations(), info.getColumnSeriesValuesGen(), info.intervalType);
				chart = ChartFactory.createStackedBarChart3D(description, info.xLabel, info.yLabel, set, PlotOrientation.VERTICAL, true, true, true);
				if (chart.getPlot() instanceof CategoryPlot) {
					CategoryItemRenderer cRenderer = ((CategoryPlot) chart.getPlot()).getRenderer();
			...
			case LINE:
				set = this.getCategoryDataset(info.colX, info.colsY, info.series, info.getOperations(), info.getColumnSeriesValuesGen(), info.intervalType);
				chart = ChartFactory.createLineChart(description, info.xLabel, info.yLabel, set, PlotOrientation.VERTICAL, true, true, true);
	...
	public void configureChart(String xLabel, String yLabel, String xColumn, String[] yColumns, String[] series, String descr, int type, int[] operations) {
		if (!Table.isChartEnabled()) {
			return;
		}
		if (this.charts == null) {
	...
	public void configureChart(String xLabel, String yLabel, String xColumn, String yColumns[], String series[], String descr, int type, int operations[],
		...
		this.configureChart(xLabel, yLabel, xColumn, yColumns, series, descr, type, operations, columnSeriesValuesGen, ChartInfo.NONE);
	}
	...
	public void configureChart(String xLabel, String yLabel, String xColumn, String yColumns[], String series[], String descr, int type, int operations[],
		...
		ChartInfo info = new ChartInfo(xLabel, yLabel, xColumn, yColumns, series, type, operations, columnSeriesValuesGen);
	...
	public void configureChartXDate(String xLabel, String yLabel, String xColumn, String yColumns[], String series[], String descr, int interval, boolean fillZeros,
		...
		ChartInfo info = new ChartInfo(xLabel, yLabel, xColumn, yColumns, series, 0, interval, fillZeros, operations, columnSeriesValuesGen);
		if (this.chartInfo == null) {
			this.chartInfo = new ChartInfoRepository();
		}
		((ChartInfoRepository) this.chartInfo).addChartInfo(info, descr);
	...
	/**
	 * Show a modal dialog to configure custom charts with all data in a
	 * table.<br>
	 ...
	 *
	 * @param configuration
	 *            Default chart configuration to show
	 */
	@Override
	...
	private static class DefaultChartDialog extends EJDialog implements Internationalization {
		...
		private final JRadioButtonMenuItem averageOperationMenu = new JRadioButtonMenuItem(DefaultChartDialog.averageOpKey);
		...
		private final JRadioButtonMenuItem withoutBreakdownMenu = new JRadioButtonMenuItem(DefaultChartDialog.withoutBreakDownOpKey);

		private SelectableItem currentItem = null;

		private final JLabel labelNoChart = new JLabel("chartutilities.no_column_has_been_selected_for_the_y_axis", JLabel.CENTER);
		...
		protected CategoryItemLabelGenerator itemValuePercentLabelGenerator = new StandardCategoryItemLabelGenerator("{2} ({3})", NumberFormat.getInstance());

		protected CategoryItemLabelGenerator itemPercentLabelGenerator = new StandardCategoryItemLabelGenerator("{3}", NumberFormat.getInstance());

		protected CategoryItemLabelGenerator itemValueLabelGenerator = new StandardCategoryItemLabelGenerator("{2}", NumberFormat.getInstance());

		protected StandardPieSectionLabelGenerator pieLabelGenerator = new StandardPieSectionLabelGenerator();

		protected StandardPieSectionLabelGenerator pieValueLabelGenerator = new StandardPieSectionLabelGenerator("{0} = {1}");

		protected StandardPieSectionLabelGenerator piePercentLabelGenerator = new StandardPieSectionLabelGenerator("{0} ({2})");

		protected StandardPieSectionLabelGenerator pieValuePercentLabelGenerator = new StandardPieSectionLabelGenerator("{0} = {1} ({2})");
		...
		private final ActionListener changeXListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Class columnClass = DefaultChartDialog.this.t.getJTable().getColumnClass(DefaultChartDialog.this.t.getJTable().convertColumnIndexToView(modelIndex));
				if ((columnClass == Date.class) || (columnClass == java.sql.Date.class) || (columnClass == java.sql.Timestamp.class)) {
		...
		private class ListenerPopup extends MouseAdapter {
			...
			public void mouseClicked(MouseEvent e) {
				if ((e.getClickCount() == 1) && SwingUtilities.isRightMouseButton(e)) {
					...
					int index = DefaultChartDialog.this.yList.locationToIndex(e.getPoint());
					if (index >= 0) {
						DefaultChartDialog.this.currentItem = (SelectableItem) DefaultChartDialog.this.yList.getModel().getElementAt(index);
						...
						do {
							...
							if ((DefaultChartDialog.this.breakdownMenu.getItem(i) != null) && (breakdownColumn != null)
									&& breakdownColumn.equals(DefaultChartDialog.this.breakdownMenu.getItem(i).getActionCommand())) {
						...
						DefaultChartDialog.this.operationTypePopup.setLabel(DefaultChartDialog.this.currentItem.toString());
		...
		private class SelectableItem extends TranslatedItem implements Internationalization {
			...
			public String toString() {
				...
				if (this.column == null) {
					...
					return super.translatedText + " - " + this.opText + " (" + ApplicationManager.getTranslation(this.column, super.res) + ")";
				}
			}
		...
		private void setXColumns(Vector v) {
			...
			Class columnClass = this.t.getJTable().getColumnClass(this.t.getJTable().convertColumnIndexToView(iModelIndex));
			if ((columnClass == Date.class) || (columnClass == java.sql.Date.class) || (columnClass == java.sql.Timestamp.class)) {
			...
			if (v != null) {
				...
				for (int i = 0; i < v.size(); i++) {
					JRadioButtonMenuItem m = new JRadioButtonMenuItem(ApplicationManager.getTranslation(v.get(i).toString(), this.resource));
		...
		private void init() {
			...
			controlPanel.add(this.loadButton, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 2, 2, 2), 0, 0));
			controlPanel.add(this.saveButton, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 2, 2, 2), 0, 0));

			this.configurationPanel.add(controlPanel,
					new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(8, 5, 2, 2), 0, 0));

			this.pX = new JPanel(new BorderLayout());
			this.pX.setBorder(new TitledBorder("chartutilities.x_axis"));
			this.pX.add(this.comboX);
			this.configurationPanel.add(this.pX, new GridBagConstraints(0, 1, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
			...
			this.configurationPanel.add(this.pY, new GridBagConstraints(0, 2, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
			...
			this.chartTitle.getDocument().addDocumentListener(new DocumentListener() {
				...
				private void updateTitle() {
					if ((DefaultChartDialog.this.panelChart != null) && (DefaultChartDialog.this.panelChart.getChart() != null)) {
			...
			this.configurationPanel.add(this.chartTypePanel,
					new GridBagConstraints(0, 3, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
			this.configurationPanel.add(this.periodTypePanel,
					new GridBagConstraints(0, 4, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(2, 15, 2, 2), 0, 0));

			this.configurationPanel.add(this.chartTitlePanel,
					new GridBagConstraints(0, 5, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 0, 0));
					...
					if (p instanceof CategoryPlot) {
						if (((CategoryPlot) p).getRenderer() instanceof AbstractRenderer) {
							...
							if (render instanceof AbstractCategoryItemRenderer) {
								((AbstractCategoryItemRenderer) render).setBaseItemLabelGenerator(DefaultChartDialog.this.getCategoryItemLabelGenerator());
							}
							render.setBaseItemLabelsVisible(DefaultChartDialog.this.paintLabelValues.isSelected() || DefaultChartDialog.this.paintPercentValues.isSelected());
							DefaultChartDialog.this.panelChart.repaint();
						}
			...
			this.configurationPanel.add(this.paintLabelValues,
					new GridBagConstraints(0, 6, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 2, 2, 2), 0, 0));

			this.configurationPanel.add(this.paintPercentValues,
					new GridBagConstraints(0, 7, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 2, 2, 2), 0, 0));

			this.fontLabel = new JLabel(DefaultChartDialog.valueFontKey);
			this.configurationPanel.add(this.fontLabel,
					new GridBagConstraints(0, 8, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 2, 2, 2), 0, 0));

			this.fontfield = new FontDisplayField(this.valueFont);
			JPanel fontPanel = new JPanel(new GridBagLayout());

			fontPanel.add(this.fontfield, new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 2, 2, 2), 0, 0));
			this.selectFontButton = new JButton(DefaultChartDialog.selectFontKey);
			fontPanel.add(this.selectFontButton, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.CENTER, new Insets(5, 2, 2, 2), 0, 0));
			this.configurationPanel.add(fontPanel, new GridBagConstraints(0, 9, 1, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL, new Insets(5, 2, 2, 2), 0, 0));
			this.selectFontButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					int result = JOptionPane.showConfirmDialog((JComponent) e.getSource(), panel, "Font_Selection", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);

					if (result == JOptionPane.OK_OPTION) {
						DefaultChartDialog.this.valueFont = panel.getSelectedFont();
						DefaultChartDialog.this.fontfield.setText(DefaultChartDialog.this.valueFont.getFontName() + " " + DefaultChartDialog.this.valueFont.getSize());
			...
			this.configurationPanel.add(this.closeButton,
					new GridBagConstraints(0, 10, 1, 1, 1, 1, GridBagConstraints.SOUTH, GridBagConstraints.NONE, new Insets(10, 2, 2, 2), 0, 0));
		...
		protected class ExtendChartButton extends ChartButton {
			...
			protected void showConfigChartDialog() {
				...
				String conf = this.storedChartConfigDialog.show(this.getChartConfiguration(), DefaultChartDialog.this.t.getResourceBundle());
				if (conf != null) {
					...
					this.listener = new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							if (e.getSource() instanceof AbstractButton) {
								DefaultChartDialog.this.loadChartConfiguration(((AbstractButton) e.getSource()).getActionCommand());
							}
		...
		public boolean loadChartConfiguration(String conf) {
			...
			if ((preferenceKey != null) && (prefs != null)) {
				String p = prefs.getPreference(((ClientReferenceLocator) ApplicationManager.getApplication().getReferenceLocator()).getUser(), preferenceKey);
				if (p != null) {
			...
			while (token.hasMoreElements()) {
				...
				if (currentToken.indexOf("~charttype~") != -1) {
					...
					while (chartTypeEnum.hasMoreElements()) {
						...
						if ((element instanceof JRadioButton) && this.compareActionCommandAndPreference(((JRadioButton) element).getActionCommand(), chartType)) {
				...
				if (currentToken.indexOf("~timeconf~") != -1) {
					...
					while (timeConfigEnum.hasMoreElements()) {
						...
						if ((element instanceof JRadioButton) && this.compareActionCommandAndPreference(((JRadioButton) element).getActionCommand(), timeType)) {
							((JRadioButton) element).setSelected(true);
							break;
						}
					}
		...
		protected void saveButtonAction(ActionEvent e) {
			try {
				Object s = MessageDialog.showInputMessage(SwingUtilities.getWindowAncestor((Component) e.getSource()), "chartutilities.m_set_configuration_name",
				...
				if (s != null) {
					...
					if (window instanceof Frame) {
						...
						MessageDialog.showMessage(f, "chartutilities.m_save_successfully", JOptionPane.INFORMATION_MESSAGE, this.t.getResourceBundle());
					} else if (window instanceof Dialog) {
						Dialog d = (Dialog) window;
						MessageDialog.showMessage(d, "chartutilities.m_save_successfully", JOptionPane.INFORMATION_MESSAGE, this.t.getResourceBundle());
					}
				}
			} catch (Exception ex) {
				ChartUtilities_1_0.logger.trace(null, ex);
		...
		protected int saveChartConfiguration(String conf) {
			...
			if (prefs != null) {
						...
						if (index > 0) {
							...
							if (name.equalsIgnoreCase(conf)) {
								...
								int o = JOptionPane.showConfirmDialog(ApplicationManager.getApplication().getFrame(),
										ApplicationManager.getTranslation("chartutilities.overwritechart?", this.t.getResourceBundle()), "", JOptionPane.YES_NO_OPTION);
								found = true;
								if (o != JOptionPane.OK_OPTION) {
									return o;
								}
				...
				prefs.setPreference(((ClientReferenceLocator) ApplicationManager.getApplication().getReferenceLocator()).getUser(), preferenceKey, preferenceConfiguration.toString());
		...
		protected String getCurrentChartConfiguration() {
			...
			// X-axis
			String p = this.comboX.getSelectedItem() != null ? ((TranslatedItem) this.comboX.getSelectedItem()).getText() : null;
			this.add(sb, "~xaxis~" + p);
		...
		private void updateChart() {
			...
			try {
				...
				if (!temporalSeries) {
					this.chartUtilities.configureChart(colXTrad.toString(), descrColsY.toString(), colX.toString(), colsY, colsYTrad, descr, type, operations,
							columnSeriesValuesGen, intervalType);
				} else {
					this.chartUtilities.configureChartXDate(colXTrad.toString(), descrColsY.toString(), colX.toString(), colsY, colsYTrad, descr, type, true, operations,
				...
				if (chart.getPlot() instanceof CategoryPlot) {
					...
					((CategoryPlot) chart.getPlot()).getDomainAxis().setTickLabelPaint(ChartUtilities_1_0.axisTextPaint);
				} else if (chart.getPlot() instanceof XYPlot) {
				...
				if (p instanceof CategoryPlot) {
					...
					if (((CategoryPlot) p).getRenderer() instanceof AbstractRenderer) {
						AbstractRenderer render = (AbstractRenderer) ((CategoryPlot) p).getRenderer();
						if (render instanceof AbstractCategoryItemRenderer) {
							((AbstractCategoryItemRenderer) render).setBaseItemLabelGenerator(this.getCategoryItemLabelGenerator());
						}
						((AbstractRenderer) ((CategoryPlot) p).getRenderer()).setBaseItemLabelsVisible(this.paintLabelValues.isSelected() || this.paintPercentValues.isSelected());
					}
				} else if ((p instanceof PiePlot) || (p instanceof MultiplePiePlot)) {
					this.paintLabelValues.setEnabled(true);
					this.paintPercentValues.setEnabled(true);
		...
		public void setResourceBundle(ResourceBundle res) {
			...
			if (res != null) {
				...
				this.stockedBar3DChart.setText(ApplicationManager.getTranslation(DefaultChartDialog.stockedBar3DKey, res));
				...
				this.timeSeriesChart.setText(ApplicationManager.getTranslation(DefaultChartDialog.timeSerialChartKey, res));
				...
				this.averageOperationMenu.setText(ApplicationManager.getTranslation(DefaultChartDialog.averageOpKey, res));
				...
				this.paintPercentValues.setText(ApplicationManager.getTranslation(DefaultChartDialog.showPercentKey, res));
				...
				if ((this.chartTitlePanel.getBorder() != null) && (this.chartTitlePanel.getBorder() instanceof TitledBorder)) {
					((TitledBorder) this.chartTitlePanel.getBorder()).setTitle(ApplicationManager.getTranslation(DefaultChartDialog.chartTitleKey, res));
				}

				if ((this.chartTypePanel.getBorder() != null) && (this.chartTypePanel.getBorder() instanceof TitledBorder)) {
					((TitledBorder) this.chartTypePanel.getBorder()).setTitle(ApplicationManager.getTranslation(DefaultChartDialog.charTypeKey, res));
				}
				if ((this.periodTypePanel.getBorder() != null) && (this.periodTypePanel.getBorder() instanceof TitledBorder)) {
					((TitledBorder) this.periodTypePanel.getBorder()).setTitle(ApplicationManager.getTranslation(DefaultChartDialog.periodTypeKey, res));
				}

				if ((this.pX.getBorder() != null) && (this.pX.getBorder() instanceof TitledBorder)) {
					((TitledBorder) this.pX.getBorder()).setTitle(ApplicationManager.getTranslation(DefaultChartDialog.axisXKey, res));
				}

				if ((this.pY.getBorder() != null) && (this.pY.getBorder() instanceof TitledBorder)) {
					((TitledBorder) this.pY.getBorder()).setTitle(ApplicationManager.getTranslation(DefaultChartDialog.axisYKey, res));
				}

				this.yList.setToolTipText(ApplicationManager.getTranslation(DefaultChartDialog.SELECTION_OPERATION_TOOLTIP, res));

				this.labelNoChart.setText(ApplicationManager.getTranslation(DefaultChartDialog.noColumnSelectedKey, res));

				this.breakdownMenu.setText(ApplicationManager.getTranslation(DefaultChartDialog.breakdownKey, res));
				this.withoutBreakdownMenu.setText(ApplicationManager.getTranslation(DefaultChartDialog.withoutBreakDownOpKey, res));
			}
		}
	...
	protected CategoryData getCategoryData(String colX, String colY, int operation, String columnSeriesValuesGen, int timeGrouping) {
		...
		if ((columnSeriesValuesGen != null) && !columnSeriesValuesGen.equals(colY) && !columnSeriesValuesGen.equals(colX)) {
		...
		for (int j = 0; j < serieDistinctValues.size(); j++) {
			for (int i = 0; i < xDistintcValues.size(); i++) {
				...
				Object valueY = this.getYValue((Vector) ((Hashtable) serieDistinctValues.get(j)).get(colX), (Vector) ((Hashtable) serieDistinctValues.get(j)).get(colY), dataX,
						colY, operation);
		...
		return new CategoryData(xDistintcValues, yCorrespondentValues, aListDistinctValuesColSeriesGen == null ? null : aListDistinctValuesColSeriesGen.toArray(),
				columnSeriesValuesGen);
	}

	// Now for each distinct value in X axis calculate the Y axis equivalent
	// value
	protected Object getYValue(Vector xDataVector, Vector yDataVector, Object xData, String yColumnName, int operation) {
