 ...
 */
public class RadioMenuItem extends JRadioButtonMenuItem implements FormComponent, IdentifiedElement, SecureElement, StatusComponent, HasPreferenceComponent, IDynamicItem, Freeable {
	 ...
	 *
	 * @param parameters
	 *            the <code>Hashtable</code> with parameters. The next parameters are added:
	 *            <p>
		...
		for (int i = 0; i < menuChildren.length; i++) {
			if (menuChildren[i] instanceof Freeable) {
				try {
					...
					if (com.ontimize.gui.ApplicationManager.DEBUG) {
						RadioMenuItem.logger.debug("Exception in free() method " + menuChildren[i].getClass().toString(), e);
					} else {
						RadioMenuItem.logger.trace("Exception in free() method " + menuChildren[i].getClass().toString(), e);
					}
				}
			}
		}
