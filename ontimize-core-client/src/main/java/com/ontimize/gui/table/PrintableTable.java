...
public class PrintableTable implements Printable {
	...
	public PrintableTable(Table t, boolean scaleToFit) {
		...
		ObjectCellRenderer rend = new ObjectCellRenderer() {
			...
			@Override
			public Component getTableCellRendererComponent(JTable jTable, Object oValue, boolean selected, boolean hasFocus, int row, int column) {
	...
	private boolean fitInPage(PageFormat pf) {
		...
		int[] columnWidths = this.table.setPreferredTableColumnWidths(this.printedColumns, this.printingProgressWindow.progressBar);
	...
	void setTablePageWidth() {
		this.table.getJTable().setBounds(0, 0, (int) this.pf.getImageableWidth() - this.lateralMargin, this.table.getJTable().getPreferredSize().height);
		this.table.getJTable().getTableHeader().setSize((int) this.pf.getImageableWidth() - this.lateralMargin, this.table.getJTable().getTableHeader().getPreferredSize().height);
	...
	private int[] preparePages(PageFormat pf) {
		...
		if (this.printingProgressWindow != null) {
			this.printingProgressWindow.setStateText(ApplicationManager.getTranslation(PrintableTable.PREPARING_PRINTING_PAGES));
		}
		Thread.yield();
		...
		if (Table.DEBUG_MEMORY) {
			PrintableTable.logger.debug(
					"PrintableTable. Setting columns width: Memory used: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024.0) + " kbytes");
		}
		int[] columnWidths = this.table.setPreferredTableColumnWidths(this.printedColumns, this.printingProgressWindow.progressBar);
		if (Table.DEBUG_MEMORY) {
			PrintableTable.logger
			.debug("PrintableTable. Column width set: Memory used: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024.0) + " kbytes");
		}
		...
		if (Table.DEBUG_MEMORY) {
			PrintableTable.logger.debug("PrintableTable. Calculating limit rows per page: Memory used: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime()
					.freeMemory()) / 1024.0) + " kbytes");
		}
		while (lastPageRowIndex < (rowsNumber - 1)) {
			...
			int posPageBottom = (pagesNumber + 1) * ((int) pf.getImageableHeight() - headerHeight - this.verticalMargin - this.pageNumberMargin - this.pageTitleMargin);
			...
			for (int i = lastPageRowIndex; i < rowsNumber; i++) {
				...
				if (lastRowPos > posPageBottom) {
					if (ApplicationManager.DEBUG_TIMES) {
						PrintableTable.logger.debug("Page time: " + pagesNumber + " " + (System.currentTimeMillis() - tIni));
					}
		...
		if (Table.DEBUG_MEMORY) {
			PrintableTable.logger
			.debug("PrintableTable. Limit rows caculated: Memory use: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024.0) + " kbytes");
		}
		...
		if (ApplicationManager.DEBUG_TIMES) {
			PrintableTable.logger.debug("Time to prepare " + index.length + " pages: " + (System.currentTimeMillis() - t));
		}
		if (this.printingProgressWindow != null) {
			this.printingProgressWindow
			.setStateText(ApplicationManager.getTranslation(PrintableTable.PAGES_READY, this.table.getResourceBundle(), new Object[] { "" + index.length }));
		}
	...
	int printInPage(Graphics g, PageFormat pf, int pageIndex, boolean updateProgress, double scale) {
		...
		g2D.translate(pf.getImageableX() + (this.lateralMargin / 2), pf.getImageableY() + (this.verticalMargin / 2) + this.pageTitleMargin);
		// Title
		if (this.pageTitles != null) {
			g2D.setColor(Color.black);
			g2D.setFont(g2D.getFont().deriveFont(Font.BOLD));
			g2D.drawString(this.pageTitles, (int) ((pf.getImageableWidth() - g2D.getFontMetrics().stringWidth(this.pageTitles) - this.lateralMargin) / 2),
		...
		g2D.setColor(Color.black);
		g2D.drawLine(this.lateralMargin, (int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.pageTitleMargin) - 10,
				(int) (pf.getImageableWidth() - this.lateralMargin),
				(int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.pageTitleMargin) - 10);
		g2D.setFont(g2D.getFont().deriveFont((float) 8));
		g2D.drawString(pageNumberText, (int) (pf.getImageableWidth() - g2D.getFontMetrics().stringWidth(pageNumberText) - this.lateralMargin),
				(int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.pageTitleMargin));

		g2D.drawString(this.date, this.lateralMargin, (int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.pageTitleMargin));
		Thread.yield();
		...
		int lastRowBottomPixel = this.table.getJTable().getCellRect(lastPageRowIndex, 0, true).y + this.table.getJTable().getCellRect(lastPageRowIndex, 0, true).height;

		// Adjust the height translation
		Thread.yield();
		int heightTranslation = -firstRowTopPixel;
		g2D.setClip(0, 0, this.table.getJTable().getTableHeader().getWidth(), this.table.getJTable().getTableHeader().getHeight());
		Thread.yield();
