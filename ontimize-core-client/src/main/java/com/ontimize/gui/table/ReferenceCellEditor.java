...
public class ReferenceCellEditor extends StringCellEditor implements ReferenceComponent, AccessForm, ISetReferenceValues {
	 ...
	 *            <td>no</td>
	 *            <td>no</td>
	 *            <td>Indicates when it is needed to add asterisk characters at the beginning and at the end to include in the query.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>search</td>
	 *            <td></td>
	...
	public ReferenceCellEditor(Hashtable parameters) {
		...
		try {
			...
			ReferenceCellEditor.logger.debug("ReferenceDataField: Table Creation Time: {}", System.currentTimeMillis() - tIni);
		} catch (Exception e) {
			ReferenceCellEditor.logger.error(this.getClass().toString() + ": Table cannot be created." + e.getMessage(), e);
		}
	}
	...
	protected void createTableWindow() {
		...
		if ((w instanceof Frame) || (w == null)) {
			this.tableWindow = new EJDialog((Frame) w, ApplicationManager.getTranslation(this.entityName, this.resources), true) {
			...
			this.tableWindow = new EJDialog((Dialog) w, ApplicationManager.getTranslation(this.entityName, this.resources), true) {
				...
				if (selectedRow >= 0) {
					...
					for (int i = 0; i < columnsToSet.size(); i++) {
						if (hData.containsKey(columnsToSet.get(i))) {
							ReferenceCellEditor.this.valuesKeys.put(columnsToSet.get(i), hData.get(columnsToSet.get(i)));
						}
					}
	...
	protected void processingAction(boolean oneResultIsNotShown) {
		...
		if (oValue == null) {
			...
			String translation = ApplicationManager.getTranslation(ReferenceCellEditor.NO_DATA_KEY_MESSAGE, ReferenceCellEditor.this.resources, args);
			this.parentForm.message(SwingUtilities.getWindowAncestor(this.parentForm), translation, Form.WARNING_MESSAGE);
		...
		try {
			...
			if (this.table.getParentKeys() != null) {
				for (int i = 0; i < this.table.getParentKeys().size(); i++) {
					...
					ReferenceCellEditor.logger.debug("Filtering by parent key: {} with value: {}", oParentKey, oParentKeyValue);
			...
			if (res.getCode() == EntityResult.OPERATION_WRONG) {
				this.parentForm.message(SwingUtilities.getWindowAncestor(ReferenceCellEditor.this), res.getMessage(), Form.ERROR_MESSAGE);
			} else {
			...
			this.parentForm.message(SwingUtilities.getWindowAncestor(ReferenceCellEditor.this), ex.getMessage(), Form.ERROR_MESSAGE);
		}
	}
