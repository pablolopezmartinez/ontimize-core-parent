...
/**
 * Class that defines additional buttons for <code>Table</code> when it is pageable. These buttons are placed at the bottom of table and allows move forward/back on table pages, go
 * to the first and last page and download all records.
 *
...
public class PageFetcher extends JPanel implements TableModelListener {
	...
	protected void setResourceBundle(ResourceBundle bundle) {
		...
		this.firstPageButton.setToolTipText(ApplicationManager.getTranslation(PageFetcher.FIRST_BUTTON_TOOLTIP_KEY, bundle));

		this.previousPageButton.setToolTipText(ApplicationManager.getTranslation(PageFetcher.PREVIOUS_BUTTON_TOOLTIP_KEY, bundle));

		this.nextPageButton.setToolTipText(ApplicationManager.getTranslation(PageFetcher.NEXT_BUTTON_TOOLTIP_KEY, bundle));

		this.lastPageButton.setToolTipText(ApplicationManager.getTranslation(PageFetcher.LAST_BUTTON_TOOLTIP_KEY, bundle));

		this.downloadAllButton.setToolTipText(ApplicationManager.getTranslation(PageFetcher.DOWNLOAD_ALL_BUTTON_TOOLTIP_KEY, bundle));

		this.pageSizeField.setToolTipText(ApplicationManager.getTranslation(PageFetcher.CHANGE_PAGE_SIZE_TOOLTIP_KEY, bundle));
	...
	protected void refreshLabel(AdvancedEntityResult aER) {
		...
		if (this.totalSize <= this.pageSize) {
			...
			this.messageLabel.setText(
					ApplicationManager.getTranslation(PageFetcher.PAGINABLE_LABEL_KEY, this.bundle, new String[] { "" + firstindex, "" + lastoffset, "" + this.totalSize }));
	...
	protected void calculatePages() {
		...
		for (int i = initValue; i < this.totalSize; i += this.pageSize) {
			...
			item.setText(
					ApplicationManager.getTranslation(PageFetcher.PAGINABLE_LABEL_KEY, this.bundle, new String[] { "" + firstElement, "" + lastElement, "" + this.totalSize }));
			if (this.pageMenuListener == null) {
	...
	protected void init() {
		...
		this.add(this.pageSizeLabel,
				new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, 0, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 2, 0, 2), 0, 0));
		this.add(this.pageSizeField,
				new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 2, 0, 2), 0, 0));
		this.add(this.firstPageButton,
				new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, 0, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 2, 0, 2), 0, 0));
		...
		this.add(this.lastPageButton,
				new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, 0, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 2, 0, 0), 0, 0));
	...
	protected void queryRecords(int offset, int recordNumber) {
		...
		try {
			...
			AdvancedEntityResult res = ((AdvancedEntity) ent).query(kv, this.table.getAttributeList(), locator.getSessionId(), recordNumber, offset, this.table.getSQLOrderList());
			...
			if (res.getCode() == EntityResult.OPERATION_WRONG) {
				...
				if (threshold > 0) {
					...
					if ((opt != null) && (locator instanceof ClientReferenceLocator)) {
						try {
							...
							PageFetcher.logger.info("Compression threshold set to {} {} in: {}", ((ClientReferenceLocator) locator).getUser(), locator.getSessionId(), threshold);
						} catch (Exception e) {
				...
				if (nReturnedRecords != recordNumber) {
					PageFetcher.logger.debug("Table: The records amount returned by the advanced entity does not match the requested amount: Requested: {}. Returned: {}",
