...
/**
 * This class renders the table header according to the information that must be
 * shown. For example, changes the label and the icon in the header depending on
 * the sorting, grouping, etc., of the information managed by the table.
 */
// HeadCellRenderer
...
public class SortTableCellRenderer extends DefaultTableCellRenderer {
	...
	public SortTableCellRenderer(JTable table) {
		...
		if (ApplicationManager.useOntimizePlaf) {
			this.predFont = UIManager.getFont("TableHeader:\"TableHeader.renderer\".font") != null ? UIManager.getFont("TableHeader:\"TableHeader.renderer\".font")
					: this.getFont();
		} else {
	...
	/**
	 * Adds configurable parameters for class that renders the table header
	 * according to the information that must be shown. This parameters are
	 * specified in .xml definition of {@link Table}.
	 * <p>
	 *
	 * @param parameters
	 ...
	 *            <td><code></code></td>
	 *            <td>no</td>
	 *            <td>Font for header text. It is accepted a string like :
	 *            'Arial-BOLD-18'. Similar to Font.decode</td>
	 *            </tr>
	 *
	 *
	 *            <tr>
	...
	public static class ListMouseListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if ((e.getModifiers() == InputEvent.META_MASK) && sorter.isFilterEnabled()) {
				...
				MessageDialog.showErrorMessage(null, "Column: View index:" + viewColumn + ", Model index: " + column + "Id: " + id + " , Name: " + sName + " , Renderer: "
						+ descrRenderer + " PreferredWidth: " + col.getPreferredWidth() + " Width: " + col.getWidth() + " Max width: " + col.getMaxWidth());
				...
				if (ApplicationManager.DEBUG_TIMES) {
					SortTableCellRenderer.logger.debug("TableSorter: Time to sort the table (Total event): " + (System.currentTimeMillis() - t) + " ms");
				}
			}
		}
		 ...
		 * columnIndex. If the point corresponds to the header, the method
		 * returns the related TableColumn.
		 *
		 * @param header
		 *            the table header
		 * @param point
		 ...
		 *            corresponding to the columnIndex in case the mouse event
		 *            was performed into the column header; null otherwise
		 */
