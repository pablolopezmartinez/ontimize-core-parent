...
/**
 * Class to manage edition of dates in table components. It wraps a <code>DateDataField</code> whose parameters are passed in <code>Hashtable</code> parameter in constructor. It
 * allows to manage some date patterns: including date and hour.
 *
...
public class DateCellEditor extends CellEditor implements OpenDialog {
	 ...
	 *
	 * @param parameters
	 *            the <code>Hashtable</code> with parameters. They are allowed parameters of <code>DateDataField</code>: {@link DateDataField#init(Hashtable)} and additionally:
	 *            <p>
	 ...
	 *            <td>no</td>
	 *            <td>no</td>
	 *            <td>Shows a graphical calendar to choose the date from a calendar. It is also possible fix this parameter for all DateCellEditor objects with static variable
	 *            SHOW_CALENDAR.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>withhour</td>
	 ...
	 *
	 * @param parameters
	 *            <code>Hashtable</code> with parameters
	...
	public static DateDataField createDataField(Hashtable parameters) {
		...
		if (ParseUtils.getBoolean((String) parameters.get(DateCellEditor.WITH_HOUR), false)) {
			dataField.setDocument(new HourDateDocument(ParseUtils.getBoolean((String) parameters.get(DateCellEditor.ONLY_HOUR), false),
					ParseUtils.getBoolean((String) parameters.get(DateCellEditor.HOUR_FIRST), DateCellEditor.hourDefaultValueFirst)));
		}
