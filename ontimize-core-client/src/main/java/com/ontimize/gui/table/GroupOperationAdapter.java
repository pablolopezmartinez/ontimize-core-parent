...
public abstract class GroupOperationAdapter implements GroupOperation {
	...
	public JMenuItem getItem() {
		if (this.item == null) {
			this.item = new JMenuItem(ApplicationManager.getTranslation(this.getOperationText(), this.table.getResourceBundle()));
			this.item.addActionListener(new ActionListener() {

				@Override
				public void actionPerformed(ActionEvent e) {
					int col = GroupOperationAdapter.this.table.getJTable().convertColumnIndexToModel(GroupOperationAdapter.this.table.getColPress());
					((TableSorter) GroupOperationAdapter.this.table.getJTable().getModel()).setGroupedColumnFunction(col, GroupOperationAdapter.this.getOperationId());
				}
			});
		}
