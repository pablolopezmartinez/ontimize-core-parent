...
class ColumnCalculatorWindow extends EJDialog {
	...
	private void init() {
		...
		this.calculedColList.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (index >= 0) {
					...
					ColumnCalculatorWindow.this.textArea.setText(((ColumnCalculatorWindow.Expression) value).getExpression());
				}
		...
		jbButtonsPanel.add(this.bAccept, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		jbButtonsPanel.add(this.bCancel, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		jbButtonsPanel.add(this.bResult, new GridBagConstraints(2, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		...
		this.bResult.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ColumnCalculatorWindow.this.resultTable.setModel(new ResultModel(ColumnCalculatorWindow.this.table.getJTable().getModel(), parser));
			}
		});
		...
		this.bAdd.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				String name = JOptionPane.showInputDialog(ColumnCalculatorWindow.this.bAdd, "Name of the column to insert", null);
				ColumnCalculatorWindow.Expression exp = new Expression(name, ColumnCalculatorWindow.this.textArea.getText());
		...
		this.bEdit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int index = ColumnCalculatorWindow.this.calculedColList.getSelectedIndex();
				if (index >= 0) {
					ColumnCalculatorWindow.Expression exp = (ColumnCalculatorWindow.Expression) ((DefaultListModel) ColumnCalculatorWindow.this.calculedColList.getModel())
							.getElementAt(index);
		...
		buttonsPanel.add(this.bAdd, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.NORTH, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		buttonsPanel.add(this.bDelete, new GridBagConstraints(0, 1, 1, 1, 0, 0, GridBagConstraints.NORTH, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		buttonsPanel.add(this.bEdit, new GridBagConstraints(0, 2, 1, 1, 0, 1, GridBagConstraints.NORTH, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		...
		pCenter.add(this.spWest, new GridBagConstraints(0, 0, 2, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		JScrollPane sP = new JScrollPane(this.resultTable);
		sP.setPreferredSize(new Dimension(125, 500));
		pCenter.add(sP, new GridBagConstraints(2, 0, 1, 1, 0, 1, GridBagConstraints.EAST, GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 0), 0, 0));
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			...
			if (this.parser.hasError()) {
				if (ApplicationManager.DEBUG) {
					ColumnCalculatorWindow.logger.debug(this.getClass().toString() + ". Calculated column Error: " + ". Expression: " + ". Error: " + this.parser.getErrorInfo());
				}
			}
