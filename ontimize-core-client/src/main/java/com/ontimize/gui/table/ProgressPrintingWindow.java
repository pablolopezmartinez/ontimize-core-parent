...
/**
 * The class that display the printing window progress, while the application is printing the table information.
 */
public class ProgressPrintingWindow extends JFrame implements Internationalization {
	...
	public ProgressPrintingWindow(PrintableTable printTable) {
		...
		panelAux.add(this.state, new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		panelAux.add(this.cancelButton, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		...
		this.cancelButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				boolean choice = MessageDialog.showQuestionMessage(SwingUtilities.getWindowAncestor(ProgressPrintingWindow.this.cancelButton), "Printing cancel?");
				if (choice) {
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					...
					public void run() {
						...
						ProgressPrintingWindow.this.progressBar.paintImmediately(0, 0, ProgressPrintingWindow.this.progressBar.getWidth(),
		...
		if (SwingUtilities.isEventDispatchThread()) {
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					...
					@Override
					public void run() {
						ProgressPrintingWindow.this.progressBar.setValue(ProgressPrintingWindow.this.progressBar.getValue() + 1);
						ProgressPrintingWindow.this.progressBar.paintImmediately(0, 0, ProgressPrintingWindow.this.progressBar.getWidth(),
								ProgressPrintingWindow.this.progressBar.getHeight());
					}
				});
			...
			try {
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						ProgressPrintingWindow.this.state.paintImmediately(0, 0, ProgressPrintingWindow.this.state.getWidth(), ProgressPrintingWindow.this.state.getHeight());
					}
				});
			} catch (Exception e) {
				ProgressPrintingWindow.logger.trace(null, e);
