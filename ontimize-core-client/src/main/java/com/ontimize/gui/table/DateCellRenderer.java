...
public class DateCellRenderer extends CellRenderer implements Internationalization {
	...
	public DateCellRenderer(Hashtable params) {
		this(ParseUtils.getBoolean((String) params.get("withhour"), false), ParseUtils.getBoolean((String) params.get("hourfirst"), DateCellRenderer.hourDefaultValueFirst),
	...
	protected void createFormatter(Locale locale) {
		if (!DateFormatCache.containsDateFormat(locale)) {
			...
			this.buildPattern();
			if (!this.withHour && this.dateFormat.toPattern().equals(this.datePattern)) {} else {
