...
public class ChartButton extends TableButtonSelection {
	...
	protected void createChartConfigMenu() {
		...
		if (originalSize != 0) {
			...
			JLabel label = new JLabel(ApplicationManager.getTranslation("table.no_custom_chart_stored", this.table.getResourceBundle()));
	...
	public String getCustomChartPreferenceKey() {
		...
		return f != null ? BasicApplicationPreferences.TABLE_CONF_CHART_CONFIGURATIONS + "_" + f.getArchiveName() + "_" + this.table
				.getEntityName() : BasicApplicationPreferences.TABLE_CONF_CHART_CONFIGURATIONS + "_" + this.table.getEntityName();
	}
	...
	protected List getChartConfiguration() {
		...
		try {
			...
			if ((preferenceKey != null) && (prefs != null)) {
				String p = prefs.getPreference(((ClientReferenceLocator) ap.getReferenceLocator()).getUser(), preferenceKey);
				if (p != null) {
			...
			try {
				...
				if ((preferenceKey != null) && (prefs != null)) {
					...
					if (p != null) {
						...
						prefs.setPreference(((ClientReferenceLocator) ap.getReferenceLocator()).getUser(), preferenceKey, pout);
