...
public class RowHeadCellRenderer extends JLabel implements TableCellRenderer {
	...
	public RowHeadCellRenderer(JTable table, Hashtable parameters) {
		...
		Color foreground = ParseUtils.getColor((String) parameters.get(RowHeadCellRenderer.ROWNUMBERFG), RowHeadCellRenderer.rowNumberForeground);
		if (foreground == null) {
			foreground = header.getForeground();
		}
		this.setForeground(foreground);

		Color background = ParseUtils.getColor((String) parameters.get(RowHeadCellRenderer.ROWNUMBERBG), RowHeadCellRenderer.rowNumberBackground);
		if (background == null) {
			background = header.getBackground();
		}
		this.setBackground(background);

		Font font = ParseUtils.getFont((String) parameters.get(RowHeadCellRenderer.ROWNUMBERFONT), RowHeadCellRenderer.rowNumberFont);
		if (font == null) {
			font = header.getFont();
		}
		this.setFont(font);

		Border border = ParseUtils.getBorder((String) parameters.get(RowHeadCellRenderer.ROWNUMBERBORDER), RowHeadCellRenderer.rowNumberBorder);
		if (border == null) {
