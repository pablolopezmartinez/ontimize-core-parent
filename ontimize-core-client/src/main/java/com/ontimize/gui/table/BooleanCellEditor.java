...
public class BooleanCellEditor extends CellEditor {
	...
	/**
	 * 'autostopediting': Indicates if the edition finishes when the editor value changes. No validation is needed.
	 *
	...
	public BooleanCellEditor(Hashtable parameters) {
		...
		if (parameters.containsKey(BooleanCellEditor.SELECTED_EDITOR)) {
			this.selectedEditor = ParseUtils.getBoolean((String) parameters.get(BooleanCellEditor.SELECTED_EDITOR), BooleanCellEditor.defaultSelectedEditor);
		}
