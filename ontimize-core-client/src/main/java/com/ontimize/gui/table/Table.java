...
/**
 * The <code>Table</code> is used to display regular two-dimensional tables of
 * cells.
 * <p>
 * The <code>Table</code> has many facilities that make it possible to customize
 ...
 * <p>
 * The <code>Table</code> can be added to a {@link com.ontimize.gui#Form Form}
 ...
 * - {@link #init(Hashtable params)} where params contains the xml
 * attributes.</br>
 * -{@link #configureButtons(Hashtable params)} where all table button icons are
 * configured. </br>
 * - {@link SortTableCellRenderer#init(Hashtable params)} where are additional
 * configurations for table header.</br>
 * - {@link #configureQuickFilter(Hashtable params)} where are additional
 * configurations for table quickfilter search box.</br>
 * -When table is autoinsertable (<b>inserttable='yes'</b>) there are additional
 * parameters in {@link #configureInsertTable(Hashtable params)}
 */
...
ReferenceComponent, InsertTableInsertRowChange, ITemplateField, IFilterElement, ListSelectionListener, IRefreshable, InteractionManagerModeListener {
	...
	/**
	 * Complete path to renderer and editor configuration file. It is used in
	...
	public static interface EditorManager {
		...
		/**
		 * Returns the {@link #TableCellEditor} associated to the
		 * <code>JTable</code> specified by the params, in the concrete row and
		 * column.
		 *
		 * @param table
		 * @param row
	...
	public static interface RendererManager {
		...
		/**
		 * Returns the {@link #TableCellRenderer} associated to the
		 * <code>JTable</code> specified by the params, in the concrete row and
		 * column.
		 *
		 * @param table
	...
	/**
	 * The default implementation of the {@link #RendererManager}. Provides
	 * references to the table renderes.
	 *
	 * @author Imatia Innovation
	 */
	...
	/**
	 * Allows the user to show/hide globally the quickfilter warning message on
	 * paged tables.
	 *
	 * @since 5.2072EN-0.1
	 */
	 ...
	 * @deprecated
	 */
	@Deprecated public static boolean DEFAULT_VALUE_SET_HEIGHT_HEAD = false;
	...
	/**
	 * Configures the MEMORY trace, to see how much memory is being used by the
	 * table. This traces are focused on the printing process.
	 */
	 ...
	 * @deprecated
	 */
	@Deprecated public static final String DOWN_10 = "table.download_10+";
	 ...
	 * @deprecated
	 */
	@Deprecated public static final String DOWN_50 = "table.download_50+";
	 ...
	 * @deprecated
	 */
	@Deprecated public static final String DOWN_100 = "table.download_100+";
	 ...
	 * @deprecated
	 */
	@Deprecated public static final String DOWN_ALL = "table.download_all";
	 ...
	 * @deprecated
	 */
	@Deprecated public static final String TOTAL = "table.total";
	 ...
	 * <code>width</code> will set the preferred row height.
	 *
	...
	/**
	 * This object is used to store onsetvalueset attributes and equivalences
	 * (for these fields in entity) when structure of parameter
	...
	/**
	 * This object is used to store onsetvalueset attributes and function when
	 * structure of parameter <code>onsetvalueset</code> is:
	 * "fieldonset1:SUM(fieldentitypk1);fieldonset2:CONCAT(fieldentitypk2);...fieldonsetn:MAX(fieldentitypkn)"
	 */
	...
	protected ActionListener addRecordListener = new ActionListener() {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			if ((Table.this.formName != null) || ((Table.this.formName == null) && (Table.this.insertFormName != null))) {
	 ...
	 * @deprecated
	 */
	@Deprecated protected TableAttribute attributeTable = null;
	...
	protected static class ComponentsPermissionsStore {
		...
		/**
		 * Returns the permission form the component key corresponding to the
		 * permission determined by type.
		 *
		 * @param key
		 *            the component
		 * @param type
		 *            the permission
		 * @return the corresponding permission or null if the permission is not
		 *         set
		 */
		...
		/**
		 * Returns the index corresponding to the key passed as parameter. In
		 * case that the key is not set, the method will return null.
		 *
		 * @param key
		 *            a component key
		 * @return
		...
		/**
		 * Returns the index corresponding to the different permissions set for
		 * this table, for example "visible" or "enabled"
		 *
		 * @param type
		 *            the permission type
		...
		/**
		 * Adds a new type to the types collection, and returns the index set to
		 * the new type.
		 *
		 * @param type
		 *            the type to add
		...
		/**
		 * Adds a new component key to the components key collection, and
		 * returns the index set.
		 *
		 * @param key
		 *            new component key to add
		 * @return the index set
		 */
		...
		/**
		 * Adds a new Permission to the corresponding pair component-permission
		 * type.
		 *
		 * @param key
		 * @param type
	...
	/**
	 * Listener that manager the control buttons visibility. These controls
	 * visibility will be activate when the table is enable and has records in
	 * it.
	 */
		...
		public void run() {
			try {
				...
				try {
					...
					EntityReferenceLocator referenceLocator = this.table.parentForm.getFormManager().getReferenceLocator();
					if (this.stop) {
						this.refreshState = RefreshTableEvent.CANCEL;
						return;
					}
					...
					if (ent == null) {
						Table.logger.info("REFRESH ERROR: Table: The refresh operation can't be executed because the entity can't be found. Entity:{}", this.table.getEntityName());
						return;
					}
					...
					if ((this.table.pageFetcher != null) && this.table.pageFetcher.isPageableEnabled()) {
						...
						res = ((AdvancedEntity) ent).query(kv, this.table.attributes, referenceLocator.getSessionId(), this.table.pageFetcher.getPageSize(),
						...
						synchronized (this.table) {
							...
							if (threshold > 0) {
								...
								if ((opt != null) && (this.table.locator instanceof ClientReferenceLocator)) {
									try {
										opt.setDataCompressionThreshold(((ClientReferenceLocator) this.table.locator).getUser(), this.table.locator.getSessionId(), threshold);
										Table.logger.info("Table: Compression threshold established to {} {} in: {}", ((ClientReferenceLocator) this.table.locator).getUser(),
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							...
							RefreshThread.this.table.fireRefreshTableEvent(new RefreshTableEvent(RefreshThread.this.table, RefreshThread.this.refreshState));
						}
					});
				}
			} catch (Exception e) {
	...
	protected class HTMLFileFilter extends javax.swing.filechooser.FileFilter {
		...
		public boolean accept(File file) {
			if (file.isDirectory()) {
				...
				int dotIndex = file.getPath().lastIndexOf(".");
				if (file.getPath().substring(dotIndex + 1).equalsIgnoreCase("htm") || file.getPath().substring(dotIndex + 1).equalsIgnoreCase("html")) {
					return true;
				} else {
					return false;
				}
	...
	/**
	 * Constructs a <code>Table</code> with the default configuration. This
	 * method creates the layout and calls the methods that configures the
	 * table. The table is a component to represent database information in a
	 * grid, with several functionalities such as sorting, filtering, visible
	 * column configuration, data export, etc.
	 *
	 * @param params
	 * @throws Exception
	 * @see #init
	...
	public Table(Hashtable params) throws Exception {
		...
		this.setPreferredSize(new Dimension((int) this.getPreferredSize().getWidth(), this.preferredSizeInRows * fontMetrics.getHeight()));
		...
		this.tablePanel.setBorder(ParseUtils.getBorder((String) params.get("border"), BorderManager.getBorder(BorderManager.DEFAULT_TABLE_BORDER_KEY)));
		...
		try {
			this.shareRemoteReferenceFilters = (IShareRemoteReference) ((UtilReferenceLocator) loc).getRemoteReference(IShareRemoteReference.REMOTE_NAME,
	...
	protected void initRendererEditors() {
		if (Table.rendererEditorConfigurationFile != null) {
			TableConfigurationManager tableConfigurationManager = TableConfigurationManager.getTableConfigurationManager(Table.rendererEditorConfigurationFile, true);
	 ...
	 * contains the values set in the XML in which the <code>Table</code> is
	 * placed.</br>
	 * </br>
	 * More parameters in: <br>
	 * -{@link #configureButtons(Hashtable params)} where all table button icons
	 * are configured. </br>
	 * -{@link SortTableCellRenderer#init(Hashtable)} where are additional
	 * configurations for table header.<br>
	 * <br>
	 * -{@link #configureQuickFilter(Hashtable)} where it is possible to manage
	 * additional graphical configurations for quickfilter box.
	 * <p>
	 ...
	 * <td>yes</td>
	 * <td></td>
	 * <td>Allows the TableHeader to adjusts its height to the FontText in order
	 * to see the the text when the font size changes.</td>
	 * </tr>
	 *
	 * <tr>
	 * <td>calculedcols</td>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>The value is a string with the name of the new column and the math
	 ...
	 * This will create a new column named total which value will be the
	 * multiplication of number and units. This columns must be too in visible
	 * cols if you want to show them in the table but you don't have to add this
	 * names in cols</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Required columns in the table for the calculed colums. When these
	 * required columns has a null value, the calculed colum involving it will
	 * show a null value as well.</td>
	 * </tr>
	 *
	 * <tr>
	 * <td>cods</td>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Establishes a correspondence between a table column and a field in
	 * detail form.</td>
	 * </tr>
	 *
	 * <tr>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Indicates whether user will be able to configure visible
	 * columns.</td>
	 * </tr>
	 * <tr>
	 * <td>controls</td>
	 ...
	 * buttons, by default, can export to excel the data in the table, show
	 * charts, reports, and so on. If the value is not, the controlsvisible
	 * attribute will not affect.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td>yes</td>
	 * <td></td>
	 * <td>Shows the table controls. See the <code>controls</code>
	 * attribute.</td>
	 * </tr>
	 * <tr>
	 * <td>currency</td>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>This attribute specifies the columns that will be renderer as a
	 * currency, using a renderer to do that. See
	 * {@link com.ontimize.gui.table#CurrencyCellRenderer
	 * CurrencyCellRenderer}.</td>
	 * </tr>
	 * <tr>
	 * <td>customcharts</td>
	 * <td>yes/no</td>
	 * <td>yes</td>
	 * <td></td>
	 * <td>Activates a Table default functionality tha can represent the Table
	 * information using graphical charts.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>If the table has a detail form set, with this then in which the
	 * window will be open can be controled. There are two ways of opening a new
	 * window: as a dialog that blocks the applicacion in backgroung (default),
	 * or as a window itself. With this the default behaviour can be change.<br>
	 * Pressing the 'Shift' while opening the window the non-default behaviour
	 * will be chosen.<br>
	 * If the attribute <code>opennewwindow</code> is set, <defaultnewwindow>
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>Activates the functionallity of deleting records directly from the
	 * <code>Table</code>, instead using the detail form to do that.</td>
	 * </tr>
	 * <tr>
	 * <td>disableinsert</td>
	 * <td>yes/no</td>
	 * <td>no</td>
	 * <td></td>
	 * <td>Activates the functionallity of inserting records directly from the
	 * <code>Table</code> using the detail form to do that. Is is set to
	 * <code>yes</code> no records can be inserted using this table amd the
	 * button to that, a small one in a top-right corner od the
	 * <code>Table</code> will be deactivated.</td>
	 * </tr>
	 * <tr>
	 * <td>dynamicform</td>
	 * <td></td>
	 ...
	 * <td></td>
	 * <td>name of the class which provides the form dynamc names. The
	 * contructor without parameter will be used to do so. No one set by
	 * default.</td>
	 * </tr>
	 * <tr>
	 * <td>dynamic</td>
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>Defines a new type of table: a dynamic table where columns are
	 * inferred from value setted in the {@link #setValue(Object)} method, not
	 * defined in xml.</td>
	 * </tr>
	 * <tr>
	 * <td>translateheader</td>
	 * <td>yes/no</td>
	 * <td>By default, if table is 'dynamic' fixes translateheader='no', in
	 * other case always is translated.</td>
	 * <td>no</td>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Columns that can be edited directly within the <code>Table</code>. By
	 * default, the database will not be updated. This can be used with a Editor
	 * to update the record.</td>
	 * </tr>
	 * <tr>
	 * <td>entity</td>
	 * <td></td>
	 ...
	 * <td></td>
	 * <td>1</td>
	 * <td>If the <code>Table</code> has detail form, with this parameter, when
	 * the detail form is open, the <code>Table</code> will pass the field
	 * values of the parent form (this is, the form in which the table is
	 * placed) to the detail form, to fit them, not allowing them to be
	 * modified.It is accepted to indicate only the parentFormFieldN when it is
	 * equal to detailFormField</td>
	 *
	 * </tr>
	 * <tr>
	 * <td>fontsize</td>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Sets the printing font size. The values can be from 1 up to 7 (more
	 * is too big).</td>
	 * </tr>
	 * <tr>
	 * <td>form</td>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Name of the form that will be open to show the <code>Table</code>
	 * information. By default, this form will be open by double-clicking a
	 * record of the <code>Table</code > or when pressing the <code>Table</code>
	 * insert button if "insertform" is not defined.</td>
	 * </tr>
	 * <tr>
	 * <td>insertform</td>
	 * <td></td>
	 ...
	 * <td>no</td>
	 * <td>Name of the form that will be open to show the <code>Table</code>
	 * insert form.If you don't declare this, the detail form also will be the
	 * insert form. The insert form</td>
	 * </tr>
	 * <tr>
	 * <td>inserttable</td>
	 * <td>yes/no</td>
	 * <td>no</td>
	 * <td>no</td>
	 * <td>Defines the table as a autoinsertable table. The last row in this
	 * table allows to insert records directly into database. See
	 * {@link #configureInsertTable(Hashtable)}</td>
	 * </tr>
	 * <tr>
	 * <td>key</td>
	 * <td></td>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Additional key names, when the key is multiple. The {@link #getKeys}
	 * methid will return the key and the keys values.</td>
	 * </tr>
	 * <tr>
	 * <td>memoryentity</td>
	 * <td></td>
	 * <td></td>
	 * <td></td>
	 * <td>Name of the Entity related to the <code>Table</code>. In this case
	 * the entity is a local one, stored un the client memory, to perform local
	 * operations that do not need a database interaction.</td>
	 * </tr>
	 * <tr>
	 * <td>numrowscolumn</td>
	 * <td>yes/no</td>
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>Activates the functionality to open the detail form in a new window.
	 * See the defaultnewwindow attribute. This option can be set to the whole
	 * application by setting the <code>Table</code> variable
	 * SHOW_OPEN_IN_NEW_WINDOW_MENU.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Name of the other fields that are parentkeys, that are passed to the
	 * detail form when it is open. These values will be get from the parent
	 * form, in which the <code>Table</code> is placed.</td>
	 * </tr>
	 * <tr>
	 ...
	 * :fieldentitypkn (since version 5.2058EN)</i></td>
	 * <td></td>
	 * <td>yes</td>
	 * <td>The field that is parentkey and correspondent associated field in
	 * entity. It is accepted to indicate only the fieldpki when it is equal to
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>Allows the pivot functionality, to work with the information in the
	 * <code>Table</code></td>
	 * </tr>
	 * <tr>
	 * <td>queryrows</td>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>A positive value in this parameter converts the table into a
	 * pageable. This value indicates the number of records to query for each
	 * page.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>The names of the columns use by quickfilter for querying. By default
	 * use the visible columns</td>
	 * </tr>
	 *
	 * <tr>
	 * <td>refreshbutton</td>
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>Add the refreshbutton to the table to allow the user to refresh the
	 * <code>Table</code>.</td>
	 * </tr>
	 * <tr>
	 * <td>rendermemo</td>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>Selects the columns to add a RederMemo, this is, the columns that
	 * stores long texts.</td>
	 * </tr>
	 * <tr>
	 * <td>reportcols</td>
	 * <td><I>colname1;...;colnameN</I></td>
	 ...
	 * <td>no</td>
	 * <td>Selects the columns that will be showed in report dialog to print
	 * custom reports.</td>
	 * </tr>
	 * <tr>
	 * <td>rendertime</td>
	 ...
	 * <td>no</td>
	 * <td></td>
	 * <td>Allows the <code>Table</code> to have a horizontal scroll. This is
	 * useful in <code>Table</code> with several columns to display and there is
	 * no room enough for all of them.</td>
	 * </tr>
	 * <tr>
	 * <td>sumrow</td>
	 * <td></td>
	 * <td></td>
	 * <td></td>
	 * <td>Columns to sum at the botton of the <code>Table</code>, in a new
	 * row.</td>
	 * </tr>
	 * <tr>
	 * <td>title</td>
	 ...
	 * <td></td>
	 * <td>Columns that will update the changes in the database when the user
	 * change the values stored in those.</td>
	 * </tr>
	 * <tr>
	 * <td>visiblecols</td>
	 * <td></td>
	 * <td></td>
	 * <td></td>
	 * <td>The columns that will be show in the <code>Table</code> among all the
	 * table columns.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td></td>
	 * <td></td>
	 * <td>The default columns that will be show in the <code>Table</code> when
	 * user preferences don't exist.</td>
	 * </tr>
	 * <tr>
	 * <td>opaque</td>
	 * <td>yes/no</td>
	 ...
	 * <td></td>
	 * <td>Sets the minimun row height. 'fitrowheight' parameter must be setted
	 * to 'yes'</td>
	 * </tr>
	 *
	 *
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Definition of columns and associated renderers. Renderer identifiers
	 * are defined in external file.</td>
	 * </tr>
	 *
	 * <tr>
	 * <td>editors</td>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Definition of columns and associated editors. Editor identifiers are
	 * defined in external file.</td>
	 * </tr>
	 *
	 * <tr>
	 ...
	 * <td>yes</td>
	 * <td>no</td>
	 * <td>Establishes if the detail form is built in a background thread.(Since
	 * 5.2060EN-0.5)</td>
	 * </tr>
	 *
	 * <tr>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Specifies the max. number of characters in the insert tab title in
	 * TabbedFormManager</td>
	 * </tr>
	 * </tr>
	 * <td>blockedcols</td>
	 * <td>yes/no</td>
	...
	public void init(Hashtable parameters) throws Exception {
		...
		this.translateHeader = ParseUtils.getBoolean((String) parameters.get(Table.TRANSLATE_HEADER), !this.dynamicTable);

		this.configureOpenInNewWindow(parameters);

		this.allowSetupVisibleColumns = ParseUtils.getBoolean((String) parameters.get(Table.CONF_VISIBLE_COLS), Table.DEFAULT_VALUE_SETUP_VISIBLE_COLS);
		...
		this.scrollHorizontal = ParseUtils.getBoolean((String) parameters.get(Table.HORIZONTAL_SCROLL), this.scrollHorizontal);
		this.visibleRowNumberColumn = ParseUtils.getBoolean((String) parameters.get(Table.NUM_ROWS_COLUMN), this.visibleRowNumberColumn);
		...
		this.setPivotTableButton = ParseUtils.getBoolean((String) parameters.get(Table.PIVOT_BUTTON), Table.defaultPivotTableVisibility);
		this.installQuickFilter = ParseUtils.getBoolean((String) parameters.get(Table.QUICK_FILTER), Table.DEFAULT_QUICK_FILTER);
		this.quickFilterVisible = ParseUtils.getBoolean((String) parameters.get(Table.QUICK_FILTER_VISIBLE), Table.defaultQuickFilterVisible);
		this.quickfilterOnFocusSelectAll = ParseUtils.getBoolean((String) parameters.get(Table.QUICK_FILTER_ON_FOCUS_SELECT_ALL), Table.defaultQuickFilterOnFocusSelectAll);
		
		this.sortReportColumn = ParseUtils.getBoolean((String) parameters.get(Table.SORT_REPORT_COLUMS), Table.defaultSortReportColumns);

		this.detailTitleMaxSize = ParseUtils.getInteger((String) parameters.get(Table.DETAIL_TITLE_MAX_SIZE), Table.defaultDetailTitleMaxSize);
		...
		this.tableBackgroundColor = ParseUtils.getColor((String) parameters.get(Table.TABLEBGCOLOR),
				(Table.defaultTableBackgroundColor != null ? Table.defaultTableBackgroundColor : this.tableBackgroundColor));
		...
		this.backgroundDetailFormBuilder = ParseUtils.getBoolean((String) parameters.get(Table.BACKGROUND_FORM_BUILDER), false);
		...
		this.controlButtonLayout = ParseUtils.getString((String) parameters.get(Table.CONTROL_LAYOUT_KEY), Table.defaultControlButtonLayout);
		...
		this.dynamicPivotTableForm = ParseUtils.getString((String) parameters.get(Table.D_PIVOT_TABLE_PREFERENCES_FORM), null);

		this.dynamicPivotTableEntity = ParseUtils.getString((String) parameters.get(Table.D_PIVOT_TABLE_PREFERENCES_ENTITY), null);
	...
	protected void configureOpenInNewWindow(Hashtable parameters) {
		this.allowOpenInNewWindow = ParseUtils.getBoolean((String) parameters.get(Table.OPEN_NEW_WINDOW), Table.SHOW_OPEN_IN_NEW_WINDOW_MENU);
		if (this.allowOpenInNewWindow) {
			this.openInNewWindowByDefault = ParseUtils.getBoolean((String) parameters.get(Table.DEFAULT_NEW_WINDOW), this.openInNewWindowByDefault);
		}
	}
	...
	protected void configureRows(Hashtable parameters) {
		try {
			this.preferredSizeInRows = ParseUtils.getInteger((String) parameters.get(Table.ROWS), this.preferredSizeInRows);
		} catch (Exception e) {
	...
	protected void configureQueryRows(Hashtable parameters) {
		try {
			this.rowsNumberToQuery = ParseUtils.getInteger((String) parameters.get(Table.QUERY_ROWS), this.rowsNumberToQuery);
		} catch (Exception e) {
		...
		if (this.rowsNumberToQuery > 0) {
			this.quickFilterLocal = ParseUtils.getBoolean((String) parameters.get(Table.QUICK_FILTER_PAGEABLE_LOCAL), Table.defaultQuickFilterLocal);
		}
	}
	...
	protected void configureCalculedColumns(Hashtable parameters, boolean hasVisibleColumns) {
		...
		this.modifiableCalculatedColumns = ParseUtils.getBoolean((String) parameters.get(Table.MODIFIABLE_CALCULATED_COLUMNS), Table.defaultModifiableCalculatedColumns);
	...
	protected void configureCurrencyColumns(Hashtable parameters) {
		...
		if (currency != null) {
			...
			while (st.hasMoreTokens()) {
				...
				if (this.attributes.contains(nom) || ((this.calculedColumns != null) && this.calculedColumns.containsKey(nom))) {
	...
	public String getDebugInfo() {
		...
		buffer.append("<br>");
		buffer.append("<B>" + Table.COLS + "</B>" + ":  " + ApplicationManager.vectorToStringSeparateBy(this.attributes, ";"));
		buffer.append("<br>");
		buffer.append("<B>" + Table.VISIBLE_COLS + "</B>" + ":  " + ApplicationManager.vectorToStringSeparateBy(this.visibleColumns, ";"));
		buffer.append("<br>");
		buffer.append("<B>" + Table.EDITABLE_COLUMNS + "</B>" + ":  " + ApplicationManager.vectorToStringSeparateBy(this.editableColumns, ";"));
		buffer.append("<br>");
		buffer.append("<B>" + Table.UPDATE_ENTITY_EDITABLE_COLUMNS + "</B>" + ":  " + ApplicationManager.vectorToStringSeparateBy(this.editableColumnsUpdateEntity, ";"));
		buffer.append("<br>");
	...
	/**
	 * This method configure parameters for insertable table. This method is
	 * called only when 'inserttable' parameter is enabled in .xml definition
	 * file.
	 *
	 *
	 * @param params
	 ...
	 *            <td>yes</td>
	 *            <td>no</td>
	 *            <td>Indicates whether insert operations are executed directly
	 *            in database.</td>
	 *            </tr>
	 *
	 *            <tr>
	 ...
	 *            <td>yes</td>
	 *            <td>no</td>
	 *            <td>Indicates whether remove operations are executed directly
	 *            in database.</td>
	 *            </tr>
	 *
	 *            <tr>
	 ...
	 *            <td>yes</td>
	 *            <td>no</td>
	 *            <td>Indicates whether the row at bottom to show total
	 *            operation results is showed. Note: This parameter is read in
	 *            {@link TableSorter} constructor</td>
	 *            </tr>
	 *            </Table>
	 */
	 ...
	 *
	 * @param description
	 *            the chart to remove description
	 * @see ChartUtilities
	 */
	 ...
	 *
	 * @param xLabel
	 *            the text corresponding to the x axis label
	 * @param yLabel
	 *            the text corresponding to the y axis label
	 * @param xColumn
	 *            the column displayed in the x axis
	 * @param yColumns
	 *            the columns displayed in the y axis
	 * @param series
	 * @param descr
	 *            a chart description
	 * @param type
	 *            the chart type. Values can be be
	 *            <p>
	 *            <ul>
	 ...
	 *            </ul>
	 * @see ChartUtilities#configureChart(String , String , String , String[] ,
	 *      String[] , String , int )
	 */
	public void configureChart(String xLabel, String yLabel, String xColumn, String[] yColumns, String[] series, String descr, int type) {
		if (!Table.CHART_ENABLED) {
			return;
		}
	 ...
	 *            a chart description
	 * @param interval
	 *            the time interval. Values can be be
	 *            <p>
	 *            <ul>
	...
	public void configureChartXDate(String xLabel, String yLabel, String xColumn, String[] yColumns, String[] series, String descr, int interval) {
		if (!Table.CHART_ENABLED) {
			return;
		}
		if (this.chartUtilities == null) {
	 ...
	 * @param fillZeros
	 */
	public void configureChartXDate(String xLabel, String yLabel, String xColumn, String[] yColumns, String[] series, String descr, int interval, boolean fillZeros) {
		if (!Table.CHART_ENABLED) {
			return;
		}
	 ...
	 *
	 * @param e
	 *            event that launched the request to open the chart menu
	 */
	 ...
	 *
	 * @param calculatedRow
	 *            To include or not the calculated row
	 * @param translateHeader
	 *            To translate or not the header titles. If you use this option
	 ...
	 *            To include or not the calculated row
	 * @param translateHeader
	 *            To translate or not the header titles.
	 * @param useNoStringKeys
	 *            When you are translating the header values you need to put an
	 *            Object diferent of a String as key to avoid duplicated keys
	 *            (to string with the same translation)
	 * @return
	 */
	 ...
	 * end of each line. The headers are exported as well.
	 *
	...
	/**
	 * Creates a HTML representation of the information in the table. The
	 * response will contain all the data in the table, formatted as a HTML
	 * table The headers are also exported.
	 *
	...
	public String getHTMLString() {
		...
		int rowsCount = this.table.getRowCount();
		if ((this.getJTable().getModel() instanceof TableSorter) && ((TableSorter) this.getJTable().getModel()).isInsertingRow(rowsCount - 1)) {
			rowsCount = rowsCount - 1;
		}
		for (int j = 0; j < rowsCount; j++) {
	...
	/**
	 * Get the value of the specified cell as a String, using the renderer if
	 * exist
	 *
	 * @param row
	 *            Row number
	...
	/**
	 *
	 * This method returns the value of rendered cell take into account when a
	 * cell contains a combo reference cell renderer.
	 *
	 * @param row
	 *            The current row
	 * @param column
	...
	/**
	 * Sets the initial column width. The value set is the one stored in
	 * {@link #preferredSizeInRows}
	 *
	 ...
	 * @param columnName
	 *            the column name
	 * @return true if the column with the specified name is visible; false
	 *         otherwise
	 */
	public boolean isVisibleColumn(String columnName) {
		...
		if (this.attributes.contains(columnName) || this.calculedColumns.containsKey(columnName)) {
			if ((this.visibleColumns != null) && !this.visibleColumns.isEmpty()) {
				if (this.visibleColumns.contains(columnName) && this.checkColumnTablePermission(columnName, "visible")) {
					return true;
				} else {
					return false;
				}
	...
	public void packTable() {
		...
		total = total + 40;
		Table.logger.debug("Preferred size established: {} , {}", total, this.preferredSizeInRows * fontMetrics.getHeight());
	...
	protected void setPreferredTableWidth() {
		...
		this.setPreferredSize(new Dimension(Math.min(600, preferredWidth), this.preferredSizeInRows * this.table.getRowHeight()));// fontMetrics.getHeight()));
	}
			...
			if ((columnClass != null) && Number.class.isAssignableFrom(columnClass)) {
				if (!vColumns.contains(sKey)) {
					if (vVisibleColumns.contains(sKey)) {
						...
						if ((renderer != null) && ((renderer instanceof BooleanCellRenderer) || (renderer instanceof ComboReferenceCellRenderer))) {
							continue;
						}
						vColumns.add(sKey);
					}
	...
	/**
	 * Sets values for the table. The <code>value</code> must be a Hashtable, an
	...
	/**
	 * Sets the data that the table will handle, this is, the columns
	 * information. This method also forces to fit the column size to its
	 * contents.
	 *
	...
	/**
	 * Sets the information the table will manage. The information must be a
	 * Hashtable instance, where keys are the column names, and values must be
	 * vectors containing the information corresponding to each column. All
	 * vectors must have the same size. Only the information keys that are
	 * configured as table attributes in the table definition will be processed.
	 *
	 * @param value
	 *            new information to be displayed and managed by the table
	 * @param autoSizeColumns
	 *            when true, the columns size will be adjusted to the column
	 *            contents
	 */
		...
		if (this.isOperationInMemory()) {
			try {
				...
				if ((ent instanceof DynamicMemoryEntity) && (value instanceof Hashtable)) {
					EntityResult data = new EntityResult(EntityResult.OPERATION_SUCCESSFUL, EntityResult.BEST_COMPRESSION);
	...
	protected void checkNumberColumnVisibility(Object value) {
		...
		if (value instanceof Hashtable) {
			...
			if ((h == null) || h.isEmpty() || ((h instanceof EntityResult) && (((EntityResult) h).calculateRecordNumber() == 0))) {
				this.setRowNumberColumnVisible(false);
			} else {
				boolean visible = true;
				if ((ApplicationManager.getApplication().getPreferences() != null) && (this.getParentForm() != null) && (this.getParentForm().getFormManager() != null)) {
					String preferenceValue = ApplicationManager.getApplication().getPreferences().getPreference(
							((ClientReferenceLocator) this.getParentForm().getFormManager().getReferenceLocator()).getUser(), BasicApplicationPreferences.SHOW_TABLE_NUM_ROW);
	...
	/**
	 * Returns the table model. In this case returns an instance of
	 * {@link TableSorter}.
	 *
	...
	/**
	 * Returns the information contained in the table. Returns the information
	 * in the TableSorter.
	 *
	...
	/**
	 * Returns the table attribute. The attribute for a table is an object that
	 * is an {@link TableAttribute} instance.
	 */
	@Override
	...
	/**
	 * Sets whether the table is editable. If <code>showHorizontalLines</code>
	...
	/**
	 * Returns all the primary keys. Deprecated, use instead
	 * {@link #getAllPrimaryKeys}
	 *
	 * @deprecated
	...
	/**
	 * Returns all the values for the columns configured as key and keys for the
	 * table. In the returned Hashtable, the keys will be the columns mentioned
	 * previously, and the values, all the values in the table corresponding to
	 * those columns.
	 *
	...
	/**
	 * Returns all the information in the table for all the attributes, this is,
	...
	/**
	 * Provides the value of the 'key' parameter, configured in the xml that
	 * defines the table.
	 *
	...
	/**
	 * Returns all the configured keys for the table, this is, the column names
	 * correponding to the attributes key and keys.
	 *
	...
	/**
	 * Returns all text keys the table stores, and that must be translated in
	 * order to internationalize the component. Text such as tips, menu keys and
	...
	/**
	 * A table is considered empty when it has no records. If the table has
	 * records, the method returns false.
	 *
	...
	/**
	 * Returns the value of one single key, for the specified row. This is,
	 * returns the value of a single cell in the table, when the cell is a key.
	 *
	 * @param rowIndex
	...
	/**
	 * Returns the value stored in a concrete column and row in the column
	 * model.
	 *
	 * @param rowIndex
	...
	/**
	 * Returns the values of the keys for the index specified. The result is a
	 * {@link #Hashtable} containing the keys as the Hashtable keys, and the key
	 * value in the TableModel as value for each entry in the Hashtable.
	 * <p>
	 * In case that the asked key is not present in the model, the key will not
	 * be in the result Hashtable.
	 *
	 * @param rowIndex
	 *            the model row index that specifies the row to query
	 * @return a Hastable with the key values. The value will be null if the
	 *         rowIndex is bigger than the total amount of records in the grid.
	 */
	 ...
	 *
	 * @param rowIndex
	 *            an array with the model indexes to delete
	 */
		...
		if (SwingUtilities.isEventDispatchThread()) {
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					...
					@Override
					public void run() {
						if ((Table.this.table != null) && Table.this.table.isEditing() && (((EJTable) Table.this.table).getCellEditor() != null)) {
	 ...
	 *
	 * @param rowIndex
	 *            the index of the row to delete in the Model
	 */
	 ...
	 *
	 * @param keysValues
	 *            stores pairs key-value that defines the record to delete
	 */
	 ...
	 *
	 * @param keysValues
	 *            the values for the keys that defines the queried row.
				...
				for (int j = 1; j < v.size(); j++) {
					...
					if (oKeyValuej == null) {
						if (Table.logger.isDebugEnabled()) {
							this.parentForm.message(SwingUtilities.getWindowAncestor(this), " Value for the key " + oKeyj + " to search in rows is null : " + keysValues,
									Form.ERROR_MESSAGE);
						}
						allKeysMatch = false;
						break;
	...
	/**
	 * For each Hashtable passed as parameter in the array, the method will
	 * search the row in which the keys have the same value that the ones stored
	 * in the Hashtable.
	 * <p>
	 * Each Hashtable must have, for every element, as key a column name (that
	 * is, a table's key) and as value the concrete value corresponding to that
	 * key.
	 * <p>
	 * The indexes of all these coincidences will be returned in the response
	 * int array.
	 *
	 * @param keysValuesToQuery
	 *            an array of {@link #Hashtable} that stores in each element the
	...
	/**
	 * Maps the index of the row in the view to the index of the row in the
	 * table model. Returns the index of the corresponding row in the model. If
	...
	/**
	 * If the rowIndex is <0 the selection is removed and false is returned. In
	 * case the index is > than the getRowCount, only a false is returned.
	 * Finally, if the index is right, a true is returned.
	 *
	 * @param rowIndex
	 * @return
	 */
	 ...
	 *
	 * @param viewRowIndex
	 *            [] indexes of the rows in the view
	 */
	...
	public void setEnabled(boolean enabled) {
		...
		if (this.isInsertingEnabled() && (this.table.getModel() instanceof TableSorter)) {
			if ((this.getParentForm() != null) && (this.getParentForm().getInteractionManager() != null)
					&& (this.getParentForm().getInteractionManager().getCurrentMode() == InteractionManager.UPDATE) && this.table.isEnabled()
					&& ((TableSorter) this.table.getModel()).isInsertingRow(0)) {
				this.evaluatePreferredRowsHeight();
			} else {
				((TableSorter) this.getJTable().getModel()).fireTableChanged(new TableModelEvent(this.getJTable().getModel()));
			}
		}
	...
	/**
	 * Enables or disables the table button depending on the table state. The
	 * table buttons panel will be enabled only in case the table is enabled and
	 * also has records in it. The buttons will be disabled in other case.
	 */
	 ...
	 *
	 * @param frame
	 *            the frame that will be used as parent frame for the dialogs.
	 */
	@Override
	...
	/**
	 * Sets the {@link FormBuilder} that the table will use to build the detail
	 * form
	 *
	 * @param builder
	 *            the {@link FormBuilder} that will be used to create the detail
	 *            form
	 */
	@Override
	...
	/**
	 * Sets the Form in which the table is placed. This method is call
	 * automatically when the table is created.
	 *
	 * @param form
	 *            the form that contains the table
	 */
	@Override
	public void setParentForm(Form form) {
		...
		if ((this.insertFormName != null) && !this.insertFormName.equals(this.formName) && this.backgroundDetailFormBuilder) {
	...
	/**
	 * Returns the data contained by the asked columns. The response is a
	 * {@link #Hashtable} in which the keys will be the columns asked as a
	 * method parameter, and the values are {@link #Vector} of String containing
	 * the printable data for each column.
	 * <p>
	 * If the column asked is not in the table, the response will not contain
	 * that column.
	 * <p>
	 * The values in the response will be renderized to convert the value stored
	 * in the table to the String suitable to be printed, according to the
			...
			if (askedColumns.contains(sColumnName)) {
				if (this.attributes.contains(sColumnName)) {
					...
					for (int j = 0; j < this.table.getRowCount(); j++) {
						iterations++;
						if ((this.printingWindow != null) && (iterations >= (this.printingWindow.progressBar.getMaximum() / 20))) {
							if (this.printingWindow != null) {
								this.printingWindow.progressBar.setValue(this.printingWindow.progressBar.getValue() + iterations);
							}
							if (this.printingWindow != null) {
								this.printingWindow.progressBar.paintImmediately(0, 0, this.printingWindow.progressBar.getWidth(), this.printingWindow.progressBar.getHeight());
							}
							iterations = 0;
						}
					...
					Table.logger.trace("Table: Render column time - {}: {}", sColumnName, System.currentTimeMillis() - t);
	...
	/**
	 * Returns a String with all the information displayed in the corresponding
	 * row, for each table attributes on that row. The data contained by each
	 * cell is rendered in order to have a legible result.
	 *
	 * @param rowIndex
	...
	/**
	 * Returns the table SQL type. In this case the type is
	...
	/**
	 * Empty method. Default interface implementation. The table cannot be set
	 * modified.
	 *
	 * @param modif
	 */
	...
	/**
	 * Returns always false. The table cannot be modified. Interface
	 * implementation.
	 *
	 * @return false
	 */
	...
	/**
	 * Returns a {@link #Vector} with the column names of all the visible
	 * columns. The column order is the same they have in the ColumnModel
	 *
	 * @return a {@link #Vector} containing the visible column names in the same
	 *         order in which they are been displayed
	 */
	...
	/**
	 * Disables the table insert button, '+', even though the table has a detail
	 * form.
	 */
	...
	/**
	 * Enables the insert button, '+'. The user must have permissions to insert
	 * records.
	 *
	...
	/**
	 * Returns the data contained in the specified row. The result is a
	 * {@link #Hashtable} with pairs key-value, where the keys are the column
	 * names, containing the row data. If the table is filtered, returns the
	 * data as well.
	 *
	 * @param rowIndex
	 *            the model index for the column
	...
	/**
	 * Returns the information corresponding to the row which index is passed as
	 * parameter.
	 *
	 * @param rowIndex
	 *            the model row index
	...
	/**
	 * Returns a Hashtable containing the data associated to the row calculated
	 * columns. The Hashtable keys are the calculated column names. The
	 * Hashtable values are the values in the table corresponding to the row
	 * passed as parameter.
	 *
	 * @param rowIndex
	 * @return
	 ...
	 *
	 * @param enable
	 *            if true sorting is enabled. If false, the functionality will
	 *            be disabled.
	 */
	 ...
	 * </tr>
	 * There are also renderers for the columns specified as currency, hour and
	 * memo.
	 * </Table>
	 *
	 * @see #JTable
	...
	/**
	 * Sets a concrete renderer for the specified table column. See the concrete
	 * render type configuration in order to create the renderer right. For
	 * example, if the renderer is a Reference one, the reference locator must
	 * be sat, as well the locale must be set in the internationalized ones,
	 * etc.
	 *
	 * @param column
	 *            the column that will be configured with the renderer
	 * @param renderer
	 *            the renderer to set
	...
	public void setRendererForColumn(String column, TableCellRenderer renderer) {
		this.checkRefreshThread();
		if ((!this.attributes.contains(column)) && (!this.calculedColumns.containsKey(column)) && !ExtendedTableModel.ROW_NUMBERS_COLUMN.equals(column)) {
			...
			if (renderer instanceof CachedComponent) {
				if (this.parentForm != null) {
					if (this.locator == null) {
						Table.logger.debug("Cannot set CacheManager to the renderer " + renderer.getClass() + ", because locator is NULL");
					} else {
						CacheManager.getDefaultCacheManager(this.locator).addCachedComponent((CachedComponent) renderer);
						((CachedComponent) renderer).setCacheManager(CacheManager.getDefaultCacheManager(this.locator));
					}
				} else {
					Table.logger.debug("Cannot set CacheManager to the renderer " + renderer.getClass() + ", because parent form is NULL");
				}
			}
		}
	...
	/**
	 * Returns the renderers that are being used by the table. The result is an
	 * array of renderers, containing the renderers in the order: <br>
	...
	public TableCellRenderer[] getDefaultRenderers() {
		this.checkRefreshThread();
		return new TableCellRenderer[] { this.rBoolean, this.rDate, this.rReal, this.rObject, this.rImagen, this.rMemo };
	}
	 ...
	 *
	 * @param enable
	 *            if true, the table allows to filter; if false, filtering will
	 *            be disabled
	 */
	...
	/**
	 * Returns the value stored in the field, placed in the table's parent form,
	 * that is configured as the table parent key in the XML.
	 *
	...
	protected void createPopupMenu() {
		...
		if (this.checkOk("ZOZP")) {
			...
			String dateGroupText = ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_DATE_es_ES, this.resourcesFile);
			...
			String monthYearGroupText = ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_YEAR_MONTH, this.resourcesFile);
			...
			String dayMonthYearGroupText = ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_YEAR_MONTH_DAY, this.resourcesFile);
			...
			String quarterYearGroupText = ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_QUARTER_YEAR, this.resourcesFile);
			...
			String weekYearGroupText = ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_WEEK_YEAR, this.resourcesFile);
			...
			String quarterGroupText = ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_QUARTER, this.resourcesFile);
			...
			this.menuGroupFunction = new JMenu(ApplicationManager.getTranslation(Table.GROUP_FUNCTION, this.resourcesFile));
			this.menuGroupFunction.setIcon(ImageManager.getIcon(ImageManager.SUM));
			this.menuSumFunction = new JMenuItem(ApplicationManager.getTranslation(Table.SUM_es_ES, this.resourcesFile));
			this.menuAvgFunction = new JMenuItem(ApplicationManager.getTranslation(Table.AVERAGE_es_ES, this.resourcesFile));
			this.menuMaxFunction = new JMenuItem(ApplicationManager.getTranslation(Table.MAXIMUM_es_ES, this.resourcesFile));
			this.menuMinFunction = new JMenuItem(ApplicationManager.getTranslation(Table.MINIMUM_es_ES, this.resourcesFile));
			this.menuCountFunction = new JMenuItem(ApplicationManager.getTranslation(Table.COUNT_es_ES, this.resourcesFile));
		...
		if (this.allowOpenInNewWindow) {
			...
			this.menuOpenInNewWindow = new JMenuItem(ApplicationManager.getTranslation(Table.openInNewWindowKey, this.resourcesFile));
	...
	public void addGroupedFunction(int id, GroupOperation operation) {
		if (id <= 4) {
			throw new IllegalArgumentException("Lowest grouped function id must be 5 or higher. Previous ones are reserved.");
		}
	...
	/**
	 * Opens the detail form for the rows passed as parameter in the array, in a
	 * new window each. The maximum number of rows that can opened at the same
	 * time is 5.
	 *
	 * @param modelSelectedRows
	 *            contains the model row indexes to open the detail form
	 */
	public void openInNewWindow(int[] modelSelectedRows) {
		...
		if (modelSelectedRows.length > 5) {
			Table.logger.debug(this.getClass().toString() + "openInNewWindow: will be only shown data corresponding to 5 rows");
			...
			if (!this.windowCache[i].isVisible()) {
				if (form instanceof FormExt) {
					...
					try {
						...
						EntityResult res = ent.query(this.getRowKeys(modelSelectedRows[shown]), form.getDataFieldAttributeList(), referenceLocator.getSessionId());
						form.updateDataFields(res);
						form.getInteractionManager().setUpdateMode();
					} catch (Exception ex) {
						Table.logger.error(null, ex);
		...
		for (int i = shown; i < modelSelectedRows.length; i++) {
			...
			TableFrame f = new TableFrame(ApplicationManager.getTranslation(this.detailFormTitleKey, this.resourcesFile), form);
			// Set the parentkeys
			...
			if (form instanceof FormExt) {
				...
				try {
					...
					EntityResult res = ent.query(this.getRowKeys(modelSelectedRows[i]), form.getDataFieldAttributeList(), buscador.getSessionId());
					form.updateDataFields(res);
					form.getInteractionManager().setUpdateMode();
				} catch (Exception ex) {
					Table.logger.error(null, ex);
		...
		if (this.checkOk("ZOZP")) {
			...
			if (column > 0) {
				...
				if ((columnClass == Date.class) || (columnClass == Timestamp.class)) {
					...
					this.menuGroupDate.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_DATE_es_ES, this.resourcesFile));
					this.menuGroupByYear.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_YEAR, this.resourcesFile));
					this.menuGroupByYearMonth.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_YEAR_MONTH, this.resourcesFile));
					this.menuGroupYearMonthDay.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_YEAR_MONTH_DAY, this.resourcesFile));
					this.menuGroupByQuarterYear.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_QUARTER_YEAR, this.resourcesFile));
					this.menuGroupByWeekYear.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_WEEK_YEAR, this.resourcesFile));

					this.menuGroupByQuarter.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_QUARTER, this.resourcesFile));
					this.menuGroupByMonth.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_BY_MONTH, this.resourcesFile));
				} else {
					if (columnClass.getSuperclass() == Number.class) {
						this.menuGroupFunction.setVisible(true);
						if (this.isGrouped() && (this.getTableSorter() != null) && !this.getTableSorter().isGrouped(column)) {
					...
					this.menuGroup.setText(ApplicationManager.getTranslation(Table.GROUP_COLUMN_es_ES, this.resourcesFile));
				}
			} else {
				this.menuGroupDate.setVisible(false);
				this.menuGroup.setVisible(false);
			}
			this.menuDeleteGroup.setText(ApplicationManager.getTranslation(Table.DELETE_GROUP_es_ES, this.resourcesFile));
	...
	protected void showFilterPopupMenu(TableSorter sorter, int column) {
		Object[] columns = new Object[] { ApplicationManager.getTranslation(this.table.getColumnName(this.colPress), this.resourcesFile) };
		this.menuFilter.setText(ApplicationManager.getTranslation(Table.FILTER_COLUMN_es_ES, this.resourcesFile, columns));
		...
		if (this.colPress >= 0) {
			...
			if (this.getJTable().getCellRenderer(0, this.colPress) instanceof ComboReferenceCellRenderer) {
				oValue = ((ComboReferenceCellRenderer) this.getJTable().getCellRenderer(0, this.colPress)).getCodeDescription(oValue);
			}
			...
			if (sorter.isGrouped() || filterOr) {
				...
				this.menuFilterByValue.setText(ApplicationManager.getTranslation(Table.FILTER_BY_VALUE_es_ES, this.resourcesFile, new Object[] { "" }));
				this.menuFilterByValue.setEnabled(false);
			} else {
				this.menuFilterByValue.setText(ApplicationManager.getTranslation(Table.FILTER_BY_VALUE_es_ES, this.resourcesFile, valuesFilter));
		...
		this.menuDeleteColumnFilter.setText(ApplicationManager.getTranslation(Table.DELETE_FILTER_COLUMN, this.resourcesFile));
	...
	protected void showPopupMenu(int x, int y) {
		...
		if (this.enabled && (m != null) && (m instanceof TableSorter)) {
			...
			if (this.formName != null) {
				if ((this.table.getSelectedRow() >= 0) && (!sorter.isSumRow(this.table.getSelectedRow())) && this.isFrame()) {
	...
	protected void createDetailForm() {
		...
		if (formCopy != null) {
			...
			if (w instanceof Frame) {
				this.detailForm = new DetailForm((Frame) w, this.detailFormTitleKey, true, formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
			} else if (w instanceof Dialog) {
				this.detailForm = new DetailForm((Dialog) w, this.detailFormTitleKey, true, formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
			} else {
				this.detailForm = new DetailForm(this.detailFormTitleKey, true, formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
			}
	...
	protected void createInsertDetailForm() {
		...
		if (formCopy != null) {
			...
			Window w = SwingUtilities.getWindowAncestor(this);
			if (w instanceof Frame) {
				this.insertDetailForm = new DetailForm((Frame) w, this.detailFormTitleKey, true, formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
			} else if (w instanceof Dialog) {
				this.insertDetailForm = new DetailForm((Dialog) w, this.detailFormTitleKey, true, formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
			} else {
				this.insertDetailForm = new DetailForm(this.detailFormTitleKey, true, formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
			}
	...
	/**
	 * Creates the detail form for this table. The detail form is used to insert
	 * new records to the table (if "insertform" not exists) and to diplay de
	 * information contained buy the table in a more detailed way using a form
	 * instead a row.
	 */
	...
	protected IDetailForm createTabbedDetailForm(String sFormName) {
		...
		if (formCopy != null) {
			...
			Hashtable hPrimaryKeys = new Hashtable();
			IDetailForm detailForm = new TabbedDetailForm(formCopy, hPrimaryKeys, vKeys, Table.this, filterKeys, this.codValues);
	...
	public String getDetailFormSizePreferenceKey() {
		...
		return f != null ? BasicApplicationPreferences.DETAIL_DIALOG_SIZE_POSITION + "_" + f.getArchiveName() + "_" + this.entity
	...
	protected EntityResult deleteEntityRow(int rowIndex) throws Exception {
		if (this.isInsertingEnabled() && this.getTableSorter().isInsertingRow(rowIndex)) {
			...
			if ((eR.getCode() == EntityResult.OPERATION_SUCCESSFUL) && (this.parentForm.getFormManager() instanceof ITabbedFormManager)) {
	...
	/**
	 * Changes the table representation to the table one. That is, instead using
	 * a form to display the table information, the normal grid format will be
	 * used.
	 */
	 ...
	 * @param rowData
	 *            a {@link #Hashtable} containing the data to add
	 */
	 ...
	 *            a {@link #Hashtable} containing the data to add
	 * @param row
	 *            the position in the model in which the row will be added
	 */
	 ...
	 *
	 * @param rows
	 *            and array with the position of the rows in which the new rows
	 *            will be inserted
	 * @param rowsData
	 *            a {@link #Vector} containing {@link #Hashtable}. Each position
	 *            of the Vector is a Hashtable containing information for a row
	 * @deprecated
	 * @see #addRows(Vector)
		...
		try {
			...
			for (int j = 0; j < viewRowIndexes.length; j++) {
				...
				for (int i = 0; i < vKeys.size(); i++) {
					...
					if (bexpIndex != null) {
						BasicExpression bexpAux = new BasicExpression(new BasicField((String) oKey), BasicOperator.EQUAL_OP, rowKeyValue);
						bexpIndex = new BasicExpression(bexpIndex, BasicOperator.AND_OP, bexpAux);
					} else {
						bexpIndex = new BasicExpression(new BasicField((String) oKey), BasicOperator.EQUAL_OP, rowKeyValue);
					}
				}
	...
	/**
	 * Updates the table content querying the entity specified in the XML, and
	 * filtering the results by the values set in the form to the fields
	 * corresponding to the attributes parentkey and otherkeys.
	 *
	 *
	 * @since 5.3.8
	 *
	 *        Refresh operation in Tables is made in separated thread.
	 *        Sometimes, users refresh manually table and execute code after
	 *        this (coloring rows, computing data...). This listener allows to
	 *        execute after this operation finishes (success/error).E.g.: <br>
	 *
	 *        table.addRefreshTableListener(new RefreshTableListener() {
	 *
	 * @Override public void postIncorrectRefresh(RefreshTableEvent e) {
	 *           managedForm.message("Incorrect refresh",
	 *           MessageDialog.ERROR_MESSAGE); }
	 *
	 * @Override public void postCorrectRefresh(RefreshTableEvent e) {
	 *           managedForm.message("Succesfully refresh",
	 ...
	 *
	 * @param autoSizeColumns
	 *            if true, resizes the column width
	 */
	...
	public void refreshInEDT(boolean autoSizeColumns) {
		try {
			...
			if (ent == null) {
				if (Table.logger.isDebugEnabled()) {
					MessageDialog.showErrorMessage(this.parentFrame,
							"DEBUG: Table: The refresh operation can't be executed because the entity can't be found. Entity: " + this.entity);
				}
				return;
			}
			...
			EntityResult res = null;
			if ((this.pageFetcher != null) && this.pageFetcher.isPageableEnabled()) {
				res = ((AdvancedEntity) ent).query(kv, this.attributes, locator.getSessionId(), this.pageFetcher.getPageSize(), this.pageFetcher.getOffset(),
			...
			if (res.getCode() == EntityResult.OPERATION_WRONG) {
				...
				if (threshold > 0) {
					...
					if ((opt != null) && (locator instanceof ClientReferenceLocator)) {
						try {
							opt.setDataCompressionThreshold(((ClientReferenceLocator) locator).getUser(), locator.getSessionId(), threshold);
							Table.logger.debug("Table: Compression threshold established to " + ((ClientReferenceLocator) locator).getUser() + " " + locator.getSessionId()
									+ " en : " + threshold);
						} catch (Exception e) {
							Table.logger.error("Table: Error establishing compression threshold", e);
						}
	...
	/**
	 * Refreshes the data in the table periodically. The time between table
	 * updates is passed as parameter.
	 *
	 * @param delay
	 ...
	 *
	 * @param lineRemark
	 *            true if odd and even lines must be remarked, false otherwise
	 */
	...
	/**
	 * Sets the font for this component, for all the elements that must have a
	 * configured fort, for example renderers.
	 *
	 * @param newFont
	...
	protected int calculatePreferredRowHeights() {
		...
		for (int i = 1; i < this.table.getColumnCount(); i++) {
			...
			if (cellRenderer instanceof ObjectCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((ObjectCellRenderer) cellRenderer).getPreferredSize().height);
			} else if (cellRenderer instanceof RealCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((RealCellRenderer) cellRenderer).getPreferredSize().height);
			} else if (cellRenderer instanceof ComboReferenceCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((ComboReferenceCellRenderer) cellRenderer).getPreferredSize().height);
			} else if (cellRenderer instanceof CurrencyCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((CurrencyCellRenderer) cellRenderer).getPreferredSize().height);
			} else if (cellRenderer instanceof DateCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((DateCellRenderer) cellRenderer).getPreferredSize().height);
			} else if (cellRenderer instanceof BooleanCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((BooleanCellRenderer) cellRenderer).getPreferredSize().height);
			} else if (cellRenderer instanceof ImageCellRenderer) {
				preferredHeight = Math.max(preferredHeight, ((ImageCellRenderer) cellRenderer).getPreferredSize().height);
	...
	protected ExtendedTableModel createExtendedTableModel() {
		return new ExtendedTableModel(new Hashtable(0), this.attributes, this.calculedColumns, false, this.requiredColumnsCalculedColumns);
	}
	...
	protected JTable createSumRowBlockedTable() {
		...
		sumRowBlockedTable.getTableHeader().setPreferredSize(new Dimension(this.blockedTable.getTableHeader().getPreferredSize().width, 0));
	...
	protected JTable createSumRowTable() {
		...
		sumRowTable.getTableHeader().setPreferredSize(new Dimension(this.table.getTableHeader().getPreferredSize().width, 0));
		return sumRowTable;
	}

	/**
	 * Creates the <code>JScrollpane</code> that contains the Table. This method
	 * should assign field class: <code>scrollpane</code>.
	 *
	 */
	...
	/**
	 * Creates the <code>JScrollpane</code> that contains the BlockedTable. This
	 * method should assign field class: <code>scrollpane</code>.
	 *
	 * @since 5.2080EN
	 */
	...
	protected JScrollPane createSumRowBlockedScrollPane(Hashtable params) {
		...
		BlockedBoundedRangeModel rangeModel = new BlockedBoundedRangeModel(sumRowBlockedScrollPane.getHorizontalScrollBar().getModel());
	...
	protected void createJScrollPane(Hashtable params) {
		...
		this.scrollPane.getVerticalScrollBar()
				.setPreferredSize(new Dimension(ParseUtils.getInteger((String) params.get("vscrollwidth"), this.scrollPane.getVerticalScrollBar().getPreferredSize().width),
	...
	/**
	 * Creates a new TableCellRenderer to use in the table header renderer, by
	 * default creates a SortTableCellRenderer
	 *
	 * @param table
	 * @return
	 */
	...
	/**
	 * Calls the {@link #createTableHeaderRenderer} method and configures the
	...
	/**
	 * Adds a default mouse listener to the table header. Controls all the click
	 * dependent behaviour of the table header.
	 *
	 * @param table
	 ...
	 * @param columnIndex
	 *            the columnIndex to compare return the TableColumn
	 *            corresponding to the columnIndex in case the mouse event was
	 *            performed into the column header; null otherwise
	 */
	...
	protected void setScrollTooltipText() {
		if ((this.scrollPane != null) && (this.scrollPane.getVerticalScrollBar() != null)) {
			...
			if (this.tipScrollEnabled) {
				if (this.scrollPane.getVerticalScrollBar().getVisibleAmount() < this.scrollPane.getVerticalScrollBar().getMaximum()) {
					// Show a window
	...
	/**
	 * Installs a {@link #MouseMotionListener} and a {@link #MouseListener} to
	...
	/**
	 * Returns a reference an object that implements the {@link TableComponent}
	 * interface, stored in the table {link #controlsPanel}, specified by the
	 * key. For example, elements such a {@link TableButton} and
	 * {@link TableButtonSelection} implements this interface.
	 *
	 * @param key
	 *            the key of the TableComponent
	 * @return the TableComponent with the specified key or null if there is not
	 *         matching
	 */
	...
	/**
	 * Sets whether or not the specified component of the table should be
	 * enabled or not. Permissions are checked in order to verify that the user
	 * that tries to enable the component has the rights to do so.
	 *
	 * @param key
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>Font for message showed when user does not type any value
	 *            to search.</td>
	 *            </tr>
	 *
	 *            </Table>
	...
	protected void configureQuickFilter(Hashtable params) {
		if (this.quickFilterText != null) {
			this.quickFilterText.setFont(ParseUtils.getFont((String) params.get("quickfilterfont"), this.quickFilterText.getFont()));
			this.quickFilterText.setForeground(ParseUtils.getColor((String) params.get("quickfilterfontcolor"), this.quickFilterText.getForeground()));
			this.quickFilterText.setBorder(ParseUtils.getBorder((String) params.get("quickfilterborder"), this.quickFilterText.getBorder()));
	 ...
	 * Finally, this method also calls to
	 * {@link #changeButtonIcon(JButton, String, boolean, boolean, MouseListener)}
	 ...
	 *            <td>yes</td>
	 *            <td>no</td>
	 *            <td>Defines whether buttons are highlighted when mouse is
	 *            entered over them.</td>
	 *            </tr>
	 *
	 *            <tr>
	...
	protected void configureButtons(Hashtable params) {
		...
		this.changeButtonIcon(this.groupTableButton, (String) params.get("grouptableicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);

		this.changeButtonIcon(this.buttonChangeView, (String) params.get("changeviewicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonCopy, (String) params.get("copyicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonExcelExport, (String) params.get("excelicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonHTMLExport, (String) params.get("htmlicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonPlus2, (String) params.get("inserticon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonPrint, (String) params.get("printicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonDelete, (String) params.get("deleteicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonRefresh, (String) params.get("refreshicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonChart, (String) params.get("charticon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonVisibleColsSetup, (String) params.get("visiblecolsicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonSumRowSetup, (String) params.get("sumrowicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonSaveFilterOrderSetup, (String) params.get("savefiltericon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonCalculatedColumns, (String) params.get("calculatedcolsicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		this.changeButtonIcon(this.buttonPivotTable, (String) params.get("pivottableicon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		...
		if (this.buttonDefaultChart != null) {
			...
			this.changeButtonIcon(this.buttonDefaultChart.getButton(), (String) params.get("defaultcharticon"), this.borderbuttons, this.opaquebuttons,
					this.listenerHighlightButtons);
			this.changeButtonIcon(this.buttonDefaultChart.getMenuButton(), null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		...
		if (this.buttonReports != null) {
			...
			this.changeButtonIcon(this.buttonReports.getButton(), (String) params.get("reporticon"), this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
			this.changeButtonIcon(this.buttonReports.getMenuButton(), null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		}

		if (this.pageFetcher != null) {
			this.changeButtonIcon(this.pageFetcher.downloadAllButton, null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
			this.changeButtonIcon(this.pageFetcher.firstPageButton, null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
			this.changeButtonIcon(this.pageFetcher.lastPageButton, null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
			this.changeButtonIcon(this.pageFetcher.nextPageButton, null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
			this.changeButtonIcon(this.pageFetcher.previousPageButton, null, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
		}
	}

	protected void changeButtonIcon(JButton button, String iconpath, boolean borderButtons, boolean opaqueButtons, MouseListener listenerHighlightButtons) {
		if (button != null) {
			...
			if (button.getIcon() != null) {
				button.setPreferredSize(new Dimension(button.getIcon().getIconWidth() + 6, button.getIcon().getIconHeight() + 6));
			}
	...
	protected void installQuickFilter(JPanel pTopControls) {
		if (this.installQuickFilter) {
			pTopControls.add(this.lInfoFilter, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

			this.quickFilterText = this.createQuickFilter();
			this.quickFilterText.setSelectAll(this.quickfilterOnFocusSelectAll);

			pTopControls.add(this.quickFilterText, new GridBagConstraints(2, 0, 1, 1, 0, 0, GridBagConstraints.SOUTHEAST, GridBagConstraints.NONE, new Insets(0, 0, 6, 0), 0, 0));
		} else {
			pTopControls.add(this.lInfoFilter, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		}
	}
			...
			this.buttonDelete.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent actionEvent) {
					...
					if (selectedRows.length > 0) {
						int option = Table.this.parentForm.message(Table.M_WOULD_YOU_LIKE_TO_DELETE_ROWS, Form.QUESTION_MESSAGE);
						if (option == Form.YES) {
										...
										for (int i = 0; i < selectedRows.length; i++) {
											...
											try {
												...
												if (res.getCode() == EntityResult.OPERATION_SUCCESSFUL) {
													...
													this.status = sErrorDeletingText + " " + iSelectedRow + " " + res.getMessage();

												}
											} catch (Exception ex) {
												Table.logger.trace(null, ex);
												this.status = sErrorDeletingText + " " + iSelectedRow + " " + ex.getMessage();
											}
											this.currentPosition++;
											long t2 = System.currentTimeMillis();
											this.estimatedTimeLeft = ((int) (t2 - t) / (i + 1)) * (selectedRows.length - (i + 1));
										}
										// Update the table
			...
			public void actionPerformed(ActionEvent evento) {
				...
				if (option == JFileChooser.APPROVE_OPTION) {
					try {
						...
						if (!selectedFileString.substring(selectedFileString.length() - 4, selectedFileString.length()).equalsIgnoreCase(".htm")) {
						...
						if (e instanceof SecurityException) {
							Table.this.parentForm.message("table.operation_cannot_be_performed", Form.WARNING_MESSAGE, "table.security_error_message");
						}
					}
				}
			}
							...
							if (selectedFileString.endsWith(".xlsx")) {
								if (XLSExporterFactory.isAvailableXLSX()) {
									...
									MessageDialog.showMessage(ApplicationManager.getApplication().getFrame(), "table.xlsx_extension_not_supported",
											XLSExporterFactory.getErrorMessage(), JOptionPane.WARNING_MESSAGE, Table.this.resourcesFile);
									...
									public void run() {
										...
										try {
											if (Table.this.resourcesFile != null) {
												this.status = Table.this.resourcesFile.getString("table.generating_xls_file");
											} else {
											...
											for (Object current : orderColumns) {
												if (current instanceof Table.KeyObject) {
													...
													if (renderers.containsKey(currentKO.getKey())) {
														renderers.put(currentKO.toString(), renderers.get(currentKO.getKey()));
													}
												}
											}
											exporter.createXLS(res, finalFile, null, renderers, res.getOrderColumns(), true, xlsx, true);
										} catch (Exception e) {
											Table.logger.error(null, e);
											MessageDialog.showErrorMessage(SwingUtilities.getWindowAncestor(Table.this), "table.error_generating_xls_file");
										} finally {
	...
	protected void installGroupTableButton() {
		if (this.groupTableButton == null) {
			...
			this.groupTableButton.addPropertyChangeListener(ControlPanel.CHANGE_BUTTON_PROPERTY, this.buttonChangeListener);
	...
	/**
	 * Installs the control panel buttons, and the listeners associated to them,
	 * when necessary.
	 */
	protected void installButtonsListener() {
		...
		if (this.showControls) {
			if (this.defaultButtons) {
				this.controlsPanel.addPropertyChangeListener(ControlPanel.CHANGE_BUTTON_PROPERTY, this.buttonChangeListener);
				...
				if ((!this.dynamicTable || (this.dynamicPivotable)) && (this.buttonSaveFilterOrderSetup == null) && Table.PERMIT_SAVE_FILTER_ORDER_CONFIGURATION) {
			...
			pTopControls.add(this.controlsPanel, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			...
			// 10 lines
			// FontMetrics fontMetrics = this.getFontMetrics(this.getFont());
			this.setPreferredSize(new Dimension((int) this.getPreferredSize().getWidth(), this.preferredSizeInRows * this.table.getRowHeight()));// fontMetrics.getHeight()));

		}
	}

	/**
	 * If there is a detail form set, installs the listeners that manager the
	 * interaction between the <code>Table</code> and the detail form. Two
	 * listeners will be installed, one for insertion related to the table
	 * insert button, placed at the table top right corner, and another
	 * listening double clicks events over the data records. one for the
	 *
	 * @see #detail
		...
		if (this.formName != null) {
			...
			MouseAdapter detailFormMouseAdapter = new MouseAdapter() {
				...
				public void mouseEntered(MouseEvent event) {
					...
					if ((Table.this.enabled) && (Table.this.formName != null) && Table.this.enabledDetail && query && !Table.this.isGrouped()) {
	...
	/**
	 * Determines whether the window in which the table is placed is a Frame or
	 * not
	 *
	...
	protected boolean isTabbedFormManagerMainForm() {
		return (this.parentForm.getFormManager() instanceof ITabbedFormManager) && this.parentForm.equals(((ITabbedFormManager) this.parentForm.getFormManager()).getMainForm());
	}
	...
	protected void detail(InputEvent event) {
		...
		if ((event instanceof MouseEvent) && event.isControlDown() && (((MouseEvent) event).getClickCount() == 2) && this.isTabbedFormManagerMainForm()) {
			...
			if (this.formName != null) {
				if (this.table.getSelectedRowCount() != 1) {
					if ((this.table.getSelectedRow() > 0) && this.isTabbedFormManagerMainForm()) {
						...
						this.parentForm.message(Table.M_SELECTION_ONLY_ONE_ROW_TO_OPEN_DETAIL_FORM, Form.WARNING_MESSAGE);
					}
					return;
				...
				if (this.parentForm != null) {
					if (this.table.getListeners(MouseListener.class).length <= 1) {
						Table.this.parentForm.message("table.table_has_no_associated_detail_form", Form.INFORMATION_MESSAGE);
					}
				}
			}
		}
	...
	/**
	 * Checks the refresh thread to update the GUI while the table update is
	 * being performed.
	 */
	...
	/**
	 * Opens the table {@link DetailForm} with the data related to the row
	 * passed as parameter. The form will be open in the UPDATE mode.
	 * <p>
	 * In case that the row is a sum row, the form will not be open. If the
	 * table is grouped or the user has not permission to see the form, it will
	 * not be open neither.
	 * <p>
	 *
	 * @param rowIndex
	 *            the row which data will be shown in the form
	 */
	public void openDetailForm(int rowIndex) {
		...
		if ((this.formName != null) && bPemission && !this.isGrouped()) {
			...
			if ((this.parentForm.getFormManager() instanceof ITabbedFormManager) && this.parentForm.equals(((ITabbedFormManager) this.parentForm.getFormManager()).getMainForm())) {
			...
			Table.logger.debug("Form is NULL. Or 'form' tag was not especified, or the user has not permission to open the form to query, or the table is grouped.");
		}
	}

	/**
	 * Opens the table {@link DetailForm} from in Insert mode. The user must
	 * have permission to do that and the form must be specified.
	 */
	...
	/**
	 * Sets a column editor to the specified column. configures the editor if
	 * necessary.
	 *
	 * @param col
	 *            the column name
		...
		if (this.locator != null) {
			...
			if (editor instanceof CachedComponent) {
				if (this.parentForm != null) {
					...
					Table.logger.info("Cannot set CacheManager to the editor {}, because for is NULL", editor.getClass());
				}
			}
		}
	...
	/**
	 * Allows the specified column to be editable from the <code>Table</code>.
	...
	/**
	 * Allows the specified column to be editable from the <code>Table</code>.
	 * To update the chages produced to this columns can be configuired.
	 *
	 * @param col
	 *            the column name
	 * @param updateEntity
	 *            if true, the column will be updated
	 */
	...
	/**
	 * Activates or deactivates the editable mode for the specified column, in
	 * order to be the column editable or not from the <code>Table</code>.
	 ...
	 *            if true, the column will be updated
	 * @param editable
	 *            if true, sets the column editable, if false, will avoid the
	...
	/**
	 * Returns the sorting column names. This is, returns the column names of
	 * the columns that are being used to sort the information in the table. In
	 * this case the some columns could not be visible, although they are used
	 * to sort the table.
	 *
	...
	/**
	 * Returns an array with the sorting type of all the sorting columns. The
	 * array size will be the same of the number of sorting columns present in
	 * the table. The value of each position will be true, if the sorting is
	 * ascendant, or false, is the column is ordered in a descendant way. The
	 * index in the array corresponds to the application sorting order, this is,
	 * the index 0 will correspond with the first sorting applied, etc.
	 *
	 * @return
	 */
	...
	/**
	 * Returns a vector with the names of the columns that have filters applied
	 * in the table view.
	 *
	 ...
	 *
	 * @return true if the first sorted column is ascending. If no ordenation
	 *         applied or in case that there is a sorting applied and this is
	 *         descendant, the return will be false.
	 */
	 ...
	 * @param ascendant
	 *            true if the sorting will start with the smallest values, false
	 *            otherwise
	 */
	...
	/**
	 * Sorts the table based on the column values. Previous sorting is
	 * mantained.
	 *
	 * @param column
	 *            the column that will be used for sorting
	 ...
	 *
	 * @return the width set for each column, in pixels, order by the column
	 *         order in the table
	 */
	...
	/**
	 * Sets the preferred width for the columns in the table, up to the maximum
	 * number of rows.
	 *
	 * @param maxRows
	 ...
	 *
	 * @param visible
	 *            true if the row number column should be visible, false
	 *            otherwise
	 */
	...
	/**
	 * Sets the current column width to the preferred column width for the
	 * columns past as parameter
	 *
	...
	/**
	 * Returns the grouping for the column over which was pressed the mouse to
	 * select the deletion.
	 */
	...
	/**
	 * Groups the selected column by the defined type. Types are defined in the
	 * TableSorter class, and can be at least:
	 * <ul>
	 * <li>YEAR</li>
	 * <li>YEAR_MONTH</li>
	...
	/**
	 * Popup menu method that gets the value in the cell over which the menu was
	 * displayed, gets that value, and filters the table applying that value as
	 * filter value. The column that will be filtered in the corresponding the
	 * cell.
	 */
	...
	/**
	 * Popup menu method. Deletes all the filters that are being applied to the
	 * column over which the right click menu was invoked.
	 */
	...
	/**
	 * Opens the printing dialog to print the information contained in the
	 * selected rows.
	 */
	...
	/**
	 * Copies to the clipboard the information contained by the cell over which
	 * the mouse right click opened the popup menu.
	 */
	...
	/**
	 * Returns the total amount of records shown in the table. In case that the
	 * table is filtered or grouped, returns the total amount of records after
	...
	/**
	 * Returns the total amount of records in the table. Sum row is not
	 * included. In case that the table is filtered or grouped, the result will
	 * not be modified.
	 *
	...
	/**
	 * Returns a {@link #Hashtable} in which the keys are the field names
	 ...
	 *            If this parameter is true then the names used as keys in the
	 *            result are the names of the parent keys in the table entity.
	 *            If false, then the names are the names of the fields in the
	 *            parent form
	 * @return
	 */
			...
			for (int i = 0; i < this.parentkeys.size(); i++) {
				...
				if (v != null) {
					...
					if (Table.logger.isTraceEnabled()) {
						MessageDialog.showErrorMessage(this.parentFrame, "DEBUG: Table: Parentkey " + this.parentkeys.get(i) + " is null. It won't be included in the query. "
	...
	/**
	 * Returns the DetailForm configured for this table. The detail form is the
	 * form in which the records can be shown in detail by double clicking the
	 * rows in the grid. It is used as well to perform insert operations from
	 * the tables, by clicking the insert button in the top right corner of the
	 * table.
	 *
	...
	public DetailForm getDetailForm() {
		if (this.formName != null) {
			if (this.detailForm == null) {
				...
				for (int i = 0; i < this.parentkeys.size(); i++) {
					Object vParentKey = Table.this.parentForm.getDataFieldValueFromFormCache(this.parentkeys.get(i).toString());
					if (Table.logger.isDebugEnabled() && (vParentKey == null)) {
						MessageDialog.showErrorMessage(this.parentFrame, "DEBUG: Table: parentkey " + this.parentkeys.get(i)
	...
	/**
	 * Returns the information contained in the selected rows in a
	 * {@link #Hashtable}. The {@link #Hashtable} keys are the table attributes,
	 * and the values are {@link #Vector} with the row values.
	 *
	 * @return the information contained by the selected rows, and null when
	...
	/**
	 * Checks the component visibility. Depending on the client permissions, the
	 * table can be shown or not. This method checks that permission to hide the
	...
	/**
	 * Checks if the table must be disabled. Depending on the client
	 * permissions, the table can be enabled or not. This method checks that
	 * permission to disable or enable the table when necessary.
	 *
	...
	/**
	 * Checks if the table can perform insertions. Depending on the client
	 * permissions, the table can have the insertions restricted. This method
	 * checks that permission to disable or enable the inserting from the table
	 * feature.
	 *
	...
	/**
	 * Checks if the table can perform queries. Depending on the client
	 * permissions, the table can have the queries restricted. This method
	 * checks that permission to disable or enable querying.
	 *
	...
	/**
	 * Checks whether the component with this <code>key</code> has been
	 * restricted by the permission <code>type</code> . If this method returns
	 * true the table component will not be restricted by this permission
	 * <code>type</code>. <br>
	...
	/**
	 * Checks whether the column with this <code>columnName</code> has been
	 * restricted by the permission <code>type</code> . If this method returns
	 * true the table column will not be restricted by this permission
	 * <code>type</code>. <br>
	 * For example, if the a call to this method with the type "visible" returns
	 * true, then column is able to be visible according to permission (another
	 * program condition could be hide it).
	 *
	 * @param key
	 *            the key of the table component to be restricted
	 * @param type
	...
	/**
	 * Some table functionalities and behaviours can be controled using client
	 * security, so the same table can be different depending on the user
	 * profile.
	 * <p>
	 * This method sets the client permission for the table, according to the
	 * client permissions XML profile. This is, checks whether the table can be
	 * visible or not, as well as whether the table is enabled or not.
	 * <p>
	 * The method checks some other permissions such as the query permission and
	 * the insert permission, which can allow insertions from the table.
	 *
	 ...
	 * <p>
	 * The table functionalities, accessible through the controlsPanel buttons,
	 * can be controlled using client permission. This implies that those
	 * buttons can be disabled or hidden using the client permission XML.
	 *
	 * @see #checkComponentTablePermission
	 */
	 ...
	 * <p>
	 * The table columns can be controlled using client permission. This implies
	 * those columns can be hidden using the client permission XML. e.g. for
	 * hiding a column named BALANCE: <br>
	 * <b>
	 * <code> &lt;column attr="BALANCE" restricted="yes" type="visible"/&gt</code>
	 * </b>
	 *
	 * @see #checkComponentTablePermission
	 *
	...
	/**
	 * Sets whether a tip informing about the record number when pressing the
	 * table scroll must be shown or not.
	 *
	 * @param enable
	 ...
	 *            if true, the visibility set will be stored as application
	 *            preference
	 */
	...
	/**
	 * Is call when a preference changes, because the table is registered as a
	 ...
	 * <p>
	 * Tables can remember their state between application execution. For
	 * instance things like visible columns, column size and order, and, in
	 * general, all the parameters that can be modified and changed bu the user
	 * can be reset by calling this method.
	 */
	@Override
	...
	/**
	 * Establishes as visible columns the columns passed as parameter and shows
	 * the columns.
	 *
	 * @param visibleColumns
	 *            a Vector containing the column names to show.
	 */
	...
	protected String getControlPanelPreferenceKey() {
		Form f = this.parentForm;
		return f != null ? BasicApplicationPreferences.TABLE_CONTROL_PANEL + "_" + f.getArchiveName() + "_" + this.entity
				: BasicApplicationPreferences.TABLE_VISIBLE_COLS + "_" + this.entity;
	}

	/**
	 * Returns the client preferences key to store the visibility of the
	 * controls.
	 *
	 * @return the client preferences key to store the visibility of the
	 *         controls
	 */
	protected String getVisibleControlsPreferenceKey() {
		Form f = this.parentForm;
		return f != null ? BasicApplicationPreferences.SHOW_TABLE_CONTROLS + "_" + f.getArchiveName() + "_" + this.entity
				: BasicApplicationPreferences.SHOW_TABLE_CONTROLS + "_" + this.entity;
	}

	/**
	 * Returns the client preferences key to store the filters for a determined
	 * column.
	 *
	 * @param columnName
	 *            the column name
	 * @return the client preferences key to store the filters for a determined
	 *         column.
	 */
	protected String getFilterOrderConfPreferenceKey(String columnName) {
		if (!this.dynamicPivotable) {
			if ((columnName == null) || (columnName.length() == 0)) {
				Form f = this.parentForm;
				return f != null ? BasicApplicationPreferences.TABLE_CONF_SORT_FILTER + "_" + f.getArchiveName() + "_" + this.entity
						: BasicApplicationPreferences.TABLE_CONF_SORT_FILTER + "_" + this.entity;
			} else {
				Form f = this.parentForm;
				return f != null ? BasicApplicationPreferences.TABLE_CONF_SORT_FILTER + "_" + f.getArchiveName() + "_" + this.entity + "_" + columnName
	...
	/**
	 * Returns the client preferences key to store the configuration of the
	 * sorting columns
	 *
	 * @return the client preferences key to store the configuration of the
	 *         sorting columns
	 */
	protected String getFilterOrderConfigurationPreferenceKey() {
		if (!this.dynamicPivotable) {
			Form f = this.parentForm;
			return f != null ? BasicApplicationPreferences.TABLE_CONF_SORT_FILTER_CONFIGURATIONS + "_" + f.getArchiveName() + "_" + this.entity
	...
	/**
	 * Returns the client preferences key to store the calculated columns
	 * configuration.
	 *
	 * @return the client preferences key to store the calculated columns
	 *         configuration
	 */
	protected String getCalculatedColumnsConfPreferenceKey() {
		Form f = this.parentForm;
		return f != null ? BasicApplicationPreferences.TABLE_CALCULATED_COLUMNS_CONFIGURATION + "_" + f.getArchiveName() + "_" + this.entity
	...
	/**
	 * Returns a {@link #Vector} with the column names that have been set as
	 * visible columns in the XML, and that are visible in the table as well.
	 *
	 * @return he column names set as visible columns in the XML, and that are
	 *         visible in the table as well
	 */
	...
	/**
	 * Check whether or not the charting functionality is enabled for this
	 * table.
	 *
	...
	protected QuickFieldText createQuickFilter() {
		...
		quickFilterText.getDocument().addDocumentListener(new DocumentListener() {
			...
			protected void update(DocumentEvent e) {
				try {
					if ((Table.this.pageFetcher != null) && Table.this.pageFetcher.isPageableEnabled() && (Table.this.pageFetcher.totalSize > Table.this.pageFetcher.pageSize)
	...
	/**
	 * Updates row values in the table. The {@link #Hashtable} with the
	 * parameters contains both the keys that select the row and the values to
	 * be updated.
	 *
	 * @param rowData
	...
	/**
	 * Determines if the table is restricted, this is, has client permission
	 * restrictions.
	 *
	...
	/**
	 * Configures the table to perform the operations in memory. In that case,
	 * the entity that will be use to perform the operations will be the one
	 * defined in the table configuration
	 *
	 * @param inMemory
	 *            true if this component should perform operations in memory.
	 *            false otherwise
	 * @see Table#init
	 */
	...
	/**
	 * Adds a {@link #TableRowHeader} to the table. The {@link #TableRowHeader}
	 * will be actually set in the {@link #scrollPane}.
	 *
	...
	/**
	 * Applies a filter to the specified column. When the filter depends on a
	 * range, the array size must be at least 2. If values is null, a
	 * {@link SimpleFilter} will be set. In case that values has a String or a
	 * Boolean, a {@link SimpleFilter} will be applied, and in other case a
	 * {@link Filter} will be set.
	 *
	 * @param columnName
	 ...
	 * @param columnName
	 * @param filter
	 *            must be an instance of the classes {@link MultipleFilter},
	 *            {@link Filter} , {@link SimpleFilter} or {@link DateFilter}
	 */
	 ...
	 * @see Filter
	 * @param filters
	 *            is a Hashtable containing the filters. The key is the columns
	 *            name and the value must be an instance of the classes
	 *            {@link MultipleFilter}, {@link Filter} , {@link SimpleFilter}
	 *            , {@link DateFilter}
	 */
	...
	/**
	 * Sets the fixed attributes to the detail form. Thas is, make the detail
	 * form to have, in the fields configured as fixattr, the same values that
	 * are placed in the form in which the talbe is placed.
	 */
	...
	/**
	 * Sets an horizontal scroll to the <code>Table</code>. This allows the
	 * <code>Table</code> to contain a big number of columns and display them.
	 *
	 * @param scrollH
	 ...
	 * in the XML to the attribute 'parentkeys'.
	 *
	 * @see #init
	...
	/**
	 * Get the name of the table column associated with the parent key field
	 * name
	 *
	 * @param parentkey
	 * @return
	...
	/**
	 * Updates the value determined by the specified column and row in the
	 * Entity. If other data related to the row must be updates, can be passed
	 * as param.
	 *
	 * @param rowIndex
	 *            the row indes
	...
	/**
	 * Sets whether the default charting functionality is enabled or not. If
	 * <code>enabled</code> is true it is; if it is false it is not.
	 *
	 * @param enabled
	...
	/**
	 * Creates, adds to the control panel, and creates the action listener
	 * related to the button that manages the configuration of the sum rows.
	 */
	...
	/**
	 * Creates, adds to the control panel, and creates the action listener
	 * related to the button that manages the configuration of the visible
	 * columns.
	 */
	...
	/**
	 * Creates, adds to the control panel, and creates the action listener
	 * related to the button that open the charting window.
	 */
	...
	/**
	 * Sets whether or not the <code>DetailForm</code> of this component is
	 * enabled.
	 *
	 * @param enabled
	 *            true if this component should be enabled, false otherwise
	...
	/**
	 * If the table has a detail form, the method returns true. False, in other
	 * case.
	 *
	 ...
	 * @return the view index of the first column that is sorted, null when the
	 *         table is not sorted
	 */
	...
	/**
	 * Determines whether the first sorting applied to this table is ascending
	 * or not.
	 *
	 * @return true if the first column sorted has an ascending sorting; false
	 *         otherwise
	 */
	...
	/**
	 * Sets all the editable columns determined by the <code>Table</code>
	 * definition.
	 *
	 ...
	 *            true if the visible columns configuration should be enabled,
	 *            false otherwise
	 */
	...
	/**
	 * Sets the color manager for all the {@link CellRenderer} contained by the
	 * table.
	 *
	 * @param colorManager
	...
	/**
	 * Sets the font manager for all the {@link CellRenderer} contained by the
	 * table.
	 *
	 * @param fontManager
	 */
	...
	/**
	 * Saves the table filter configuration with a name, and optionally the
	...
	/**
	 * Get the value of the table preference, optionally with the position and
	 * with of the columns
	 *
	 * @param savePositionAndWidth
	...
	/**
	 * Method call by the listener of the TableButton that manages the saving of
	 * the filter configuration. Creates the menu corresponding to the stored
	 * configurations and displays it.
	 *
	 * @param e
	 *            the event
	 */
	...
	/**
	 * Loads the configuration which name is passed as parameter. If the filter
	 * configuration cannot be found, loads the default one, if exists.
	 *
	 * @param filterName
	 */
	...
	/**
	 * Sets the RendererManager, which provides the renderes depending on the
	 * object type that must be renderer.
	 *
	 * @param manager
	 * @see RendererManager
	 */
	...
	/**
	 * Establishes the initial parameters for the {@link TableCellEditor}
	 * components
	 * <p>
	...
	/**
	 * Establishes the initial parameters for the {@link TableCellRenderer}
	 * components
	 * <p>
	...
	/**
	 * Sets the default filter to the table. The default filter can be
	...
	/**
	 * Installs the button that manages the calculated columns configuration in
	 * the table.
	 */
	...
	/**
	 * Returns the configured reports for this table stored in the application
	 * preferences.
	 *
	 * @return
	 */
	...
	/**
	 * Class that implement an action listener that shows the custom report
	 * window with the selected report. The selected report corresponds to the
	 * comand of the action.
	 */
	...
	/**
	 * Creates the report setup menu, and sets it to the menuReportSetup
	 * variable.
	 */
	...
	/**
	 * Saves the operations passed as parameter in the preferences. The
	 * parameter keys must be column names and the values, the operations
	 * described in the <code>Table</code>.
	 *
	 * @param operations
	...
	/**
	 * Ensures that the {@link #setWidthAndPositionColumns()} method only is
	 * called once.
	 *
	...
	/**
	 * Sets the column width and position to the values stored in the
	 * preferences.
	 *
	 * @return true if all was ok, false otherwise
	 */
	...
	/**
	 * Returns the key that will be used to identify the preference that will
	 * save the operations applied to the different columns.
	 *
	...
	/**
	 * Returns the key that will be used to identify the preference that will
	 * save the columns position and order.
	 *
	 * @return the preference key
	 */
	protected String getColumnsPosAndOrderPreferenceKey() {
		Form f = this.parentForm;
		return f != null ? BasicApplicationPreferences.TABLE_COLS_POSITION_SIZE + "_" + f.getArchiveName() + "_" + this.entity
	...
	/**
	 * Returns the key that will be used to identify the preference that will
	 * save the custom reports associated to this table.
	 *
	 * @return the preference key
	 */
	protected String getCustomReportPreferenceKey() {
		if (!this.dynamicPivotable) {
			Form f = this.parentForm;
			return f != null ? BasicApplicationPreferences.TABLE_CONF_REPORT_CONFIGURATIONS + "_" + f.getArchiveName() + "_" + this.entity
	...
	/**
	 * Returns a String with the information of all the filters that are being
	 * applied to the table. This String will be stored in the preferences in
	 * order to recover the filter configuration later.
	 *
	...
	/**
	 * Updates the text in the table that shows the current filter status,which
	 * shows the columns that are being filtered and the values that are being
	...
	/**
	 * Installs the button that provides pivot functionallity in the control
	 * panel.
	 */
	...
	/**
	 * Returns the key that will be used to identify the preference that will
	 * save the pivot table configuration.
	 *
	...
	protected String getPivotTablePreferenceKey(String configurationName) {
		if ((configurationName == null) || (configurationName.length() == 0)) {
			Form f = this.parentForm;
			return f != null ? BasicApplicationPreferences.TABLE_CONF_PIVOT_TABLE_CONFIGURATIONS + "_" + f.getArchiveName() + "_" + this.entity
					: BasicApplicationPreferences.TABLE_CONF_PIVOT_TABLE_CONFIGURATIONS + "_" + this.entity;
		} else {
			Form f = this.parentForm;
			return f != null ? BasicApplicationPreferences.TABLE_CONF_PIVOT_TABLE_CONFIGURATIONS + "_" + f.getArchiveName() + "_" + this.entity + "_" + configurationName
					: BasicApplicationPreferences.TABLE_CONF_PIVOT_TABLE_CONFIGURATIONS + "_" + this.entity + "_" + configurationName;
		}
	}

	/**
	 * Returns the key that will be used to identify the preference that will
	 * save the pivot table configurations.
	 *
	...
	/**
	 * Method called by the table pivot button listener that shows the pivot
	 * table dialog.
	 *
	 * @param e
	 */
	...
	/**
	 * The listener that loads a concrete pivot table configuration. Called by
	 * the pivot table setup window.
	 */
	...
	/**
	 * The listener that deletes a concrete pivot table configuration. Called by
	 * the pivo table setup window.
	 */
			...
			if ((pref != null) && (pref.length() > 0)) {
				...
				while (tokens.hasMoreTokens()) {
					...
					if (ind > 0) {
						...
						if (PivotTableUtils.PIVOTTABLE_ROWFIELD.equals(key) || PivotTableUtils.PIVOTTABLE_COLUMNFIELD.equals(key)
						...
						} else if (PivotTableUtils.PIVOTTABLE_OPERATION.equals(key) || PivotTableUtils.PIVOTTABLE_DATEGROUPOPTIONS.equals(key)
	...
	/**
	 * Get the configuration parameters. If this is a dynamic table this method
	 * updates the parameters with the new column configuration. When the value
	 * changes, the columns configuration changes too, and so, there is a
	 * difference between the configuration in the xml (that says that there are
	 * no columns) and the information that is being displayed in the GUI (which
	 * has the value in the hashtable). Some Table tools, such and the
	 * PrintTable button, creates a new Table from the parameters configuration,
	 * and then adds the values in the previous Table to the new one, provoking
	...
	/**
	 * Get the required columns to insert. This method must be used only if
	 * inserting row exist
	 *
	 * @return
	 */
	...
	/**
	 * Enabled/disabled updating the changes in the database when the user
	 * change the values stored in those.
	 *
	 * @param update
	 */
	 ...
	 * parameter is missing. Also wrappers the content of the
