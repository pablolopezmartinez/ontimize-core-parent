...
/**
 * Basic Thread implementation that performs an operation and stores the result into an {@link EntityResult} class.<br>
 * The operation must be performed in the {@link #run()} method, and the result must be stored in an OperationThread variable called {@link #res}.<br>
 * The run method must set the right operation status, such as {@link #hasFinished}, {@link #hasStarted}, {@link #cancelled} , etc. An implementation example could be <BR>
...
public class OperationThread extends Thread {
	...
	/**
	 * Sets the thread status as canceled. If stopping safely the thread can be done, it must be done in this method. The {@link EntityResult} configured for this thread is set as
	 * a new {@link EntityResult} with operation wrong and the message OPERATION_CANCELLED.
	 */
