...
/**
 * This method implements the calendar window for the DateDataField component. It works independently of the date component, but it must be associated at one data component.
 */
public class CalendarWindow extends EJDialog implements Internationalization {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean selected, boolean hasFocus, int row, int column) {
			...
			if (row == 0) {
				...
				if (component instanceof JComponent) {
					if (this.border == null) {
						this.border = new BevelBorder(BevelBorder.RAISED, component.getBackground(), Color.white, component.getBackground(), Color.darkGray);
					}
		...
		public CalendarTable(int rows, int columns) {
			...
			this.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
				...
				public void valueChanged(ListSelectionEvent evento) {
					...
					CalendarWindow.this.dayTable.setDefaultRenderer(Object.class, new CalendarCellRenderer());
					Object oDayValue = CalendarWindow.this.dayTable.getValueAt(selectedRow, CalendarWindow.this.dayTable.getSelectedColumn());
					if (oDayValue != null) {
	...
	protected void init(Locale l, DataComponent field, Date date) {
		...
		this.setLocation(((int) ((Container) field).getLocationOnScreen().getX() + ((Container) field).getWidth()) - this.getWidth(),
		...
		for (int i = 0; i < this.dayTable.getColumnCount(); i++) {
			for (int j = 0; j < this.dayTable.getRowCount(); j++) {
				...
				try {
					...
					Component componenteRender = renderer.getTableCellRendererComponent(null, oValue, false, false, i, j);
