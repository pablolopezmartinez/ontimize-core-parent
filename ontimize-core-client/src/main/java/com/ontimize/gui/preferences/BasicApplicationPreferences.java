...
public class BasicApplicationPreferences extends AbstractApplicationPreferences {
	...
	/**
	 * Creates new BasicApplicationPreferences object with the preferences stored in the file <code>file</code>.
	 *
	 * @param file
	 *            Name of the file (without path) that contains the application preferences. This file must be in the user directory
	 */
	...
	/**
	 * Creates a BasicApplicationPreferences object with the preferences stored in the file <code>file</code>.
	 *
	 * @param file
	 ...
	 *
	 * @param file
	 *            Name of the file (without path). This file must be in the user directory
	 * @param rp
	 *            Remote preferences
	 */
			...
			if (this.rprefs != null) {
				...
				if (BasicApplicationPreferences.remoteUserPreferences) {
					...
					if ((user != null) && (user.length() > 0)) {
						String pref = this.rprefs.getRemotePreference(sessionId, user, RemoteApplicationPreferences.KEY_USER_PREFERENCE);
						if (pref == null) {
	...
	protected void loadDefaults() {
		if (this.loadDeaults) {
			...
			this.setPreference(null, BasicApplicationPreferences.APP_WINDOW_SIZE, ApplicationManager.parseDimensionValue(new Dimension(d.width - 50, d.height - 50)));
			this.setPreference(null, BasicApplicationPreferences.APP_STATUS_BAR_VISIBLE, ApplicationManager.parseBooleanValue(true));
			this.setPreference(null, BasicApplicationPreferences.APP_TOOL_BAR_VISIBLE, ApplicationManager.parseBooleanValue(true));

			// Application window position
			this.setPreference(null, BasicApplicationPreferences.APP_WINDOW_POSITION, ApplicationManager.parseDimensionValue(new Dimension(25, 25)));

			// Table preference
			this.setPreference(null, BasicApplicationPreferences.SHOW_TABLE_CONTROLS, ApplicationManager.parseBooleanValue(true));
			this.setPreference(null, BasicApplicationPreferences.SHOW_TABLE_NUM_ROW, ApplicationManager.parseBooleanValue(false));

			// Show tips
			this.setPreference(null, BasicApplicationPreferences.SHOW_TIPS, ApplicationManager.parseBooleanValue(true));

			this.setPreference(null, BasicApplicationPreferences.ADJUST_TREE_SPACE, ApplicationManager.parseBooleanValue(BasicApplicationPreferences.ADJUST_TREE_SPACE_VALUE));

			this.setPreference(null, BasicApplicationPreferences.APP_REMEMBER_LAST_LOGIN, ApplicationManager.parseBooleanValue(false));
	...
	public String getPreference(String user, String preferenceName) {
		...
		if (BasicApplicationPreferences.DEBUG) {
			BasicApplicationPreferences.logger.debug("Requeste preference " + preferenceName + " for the user: " + user + ", value: " + prop);
		}
		if (prop == null) {
			if (BasicApplicationPreferences.DEBUG) {
				BasicApplicationPreferences.logger.debug("Preference " + preferenceName + " not found for the user: " + user + ". Returns the default preference value");
			}
			// Get the default value if it exists
			String sDefaultProperty = this.props.getProperty(preferenceName);
			if (sDefaultProperty == null) {
				if (BasicApplicationPreferences.DEBUG) {
					BasicApplicationPreferences.logger.debug("Default preference " + preferenceName + " not found: request the default value");
				}
		...
		if (BasicApplicationPreferences.remoteUserPreferences) {
			try {
				if (this.rprefs != null) {
					...
					if ((user != null) && (user.length() > 0)) {
						if (this.userProps == null) {
							this.rprefs.setRemotePreference(sessionId, user, RemoteApplicationPreferences.KEY_USER_PREFERENCE, null);
						} else {
							String pref = BasicApplicationPreferences.parserPropertiesToStringBase64(this.userProps);
							this.rprefs.setRemotePreference(sessionId, user, RemoteApplicationPreferences.KEY_USER_PREFERENCE, pref);
						}
	...
	protected void setResourceBundleColorChooser() {
		if (this.colorDialog != null) {
			...
			this.labelTableColor.setText(ApplicationManager.getTranslation(BasicApplicationPreferences.L_EVEN_COLOR_TABLE_ROW));
			this.labelRequiredBackgroungColor.setText(ApplicationManager.getTranslation(BasicApplicationPreferences.L_REQUIRED_BACKGROUND_COLOR_FIELD));
			this.backgroundColorFieldLabel.setText(ApplicationManager.getTranslation(BasicApplicationPreferences.L_FOCUS_COLOR_FIELD));
	...
	protected void createColorDialog(Window parent) {
		if (this.colorDialog == null) {
			...
			this.labelRequiredBackgroungColor = new JLabel("applicationpreferences.background_color_of_required_fields");
			this.backgroundColorFieldLabel = new JLabel("applicationpreferences.background_colour_of_field_with_focus");

			this.colorDialog.getContentPane().add(this.labelTableColor,
					new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 10), 0, 0));
			this.colorDialog.getContentPane().add(this.labelRequiredBackgroungColor,
					new GridBagConstraints(0, 1, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 10), 0, 0));
			this.colorDialog.getContentPane().add(this.backgroundColorFieldLabel,
					new GridBagConstraints(0, 2, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 5, 5, 10), 0, 0));

			this.colorDialog.getContentPane().add(this.panelTableColor,
					new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.EAST, new Insets(5, 5, 5, 5), 0, 0));
			this.colorDialog.getContentPane().add(this.panelRequiredBackgroundColor,
					new GridBagConstraints(1, 1, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.EAST, new Insets(5, 5, 5, 5), 0, 0));

			this.colorDialog.getContentPane().add(this.panelFieldBackgroundColor,
					new GridBagConstraints(1, 2, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.EAST, new Insets(5, 5, 5, 5), 0, 0));
			...
			this.colorDialog.getContentPane().add(jbChooseTableColorButton,
					new GridBagConstraints(2, 0, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.WEST, new Insets(0, 0, 0, 0), 0, 0));

			this.colorDialog.getContentPane().add(jbChooseRequiredFieldsColorButton,
					new GridBagConstraints(2, 1, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.WEST, new Insets(0, 0, 0, 0), 0, 0));

			this.colorDialog.getContentPane().add(jbChooseFocusColorButton,
					new GridBagConstraints(2, 2, 1, 1, 0, 0, GridBagConstraints.EAST, GridBagConstraints.WEST, new Insets(0, 0, 0, 0), 0, 0));
			...
			jbChooseTableColorButton.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					Color c = JColorChooser.showDialog(BasicApplicationPreferences.this.colorDialog, "Color", BasicApplicationPreferences.this.panelTableColor.getBackground());
					if (c != null) {
			...
			this.colorDialog.getContentPane().add(jbButtonsPanel,
					new GridBagConstraints(0, 3, 3, 1, 1, 0, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(10, 5, 5, 5), 0, 0));
			...
			this.okButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					BasicApplicationPreferences.this.setPreference(null, BasicApplicationPreferences.TABLE_EVEN_ROWS_COLOR,
							ColorConstants.colorToRGB(BasicApplicationPreferences.this.panelTableColor.getBackground()));
					BasicApplicationPreferences.this.setPreference(null, BasicApplicationPreferences.REQUIRED_FIELDS_BG_COLOR,
							ColorConstants.colorToRGB(BasicApplicationPreferences.this.panelRequiredBackgroundColor.getBackground()));
					BasicApplicationPreferences.this.setPreference(null, BasicApplicationPreferences.FOCUSED_FIELD_BG_COLOR,
							ColorConstants.colorToRGB(BasicApplicationPreferences.this.panelFieldBackgroundColor.getBackground()));
			...
			this.restoreButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					BasicApplicationPreferences.this.panelRequiredBackgroundColor.setBackground(DataComponent.VERY_LIGHT_SKYBLUE);
					BasicApplicationPreferences.this.panelFieldBackgroundColor.setBackground(DataComponent.COMP_FOCUS_YELLOW);
				}
			});
