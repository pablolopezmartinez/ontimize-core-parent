...
public class ShortcutDialogConfiguration extends JDialog implements Internationalization {
	...
	protected class ChangeDialog extends EJDialog {
		...
		protected JTextField textKeys = new JTextField() {
			...
			protected boolean processKeyBinding(KeyStroke ks, KeyEvent e, int c, boolean press) {
				if (press) {
					...
					if (inUse) {
						ChangeDialog.this.repetLabel.setText(ApplicationManager.getTranslation(ShortcutDialogConfiguration.usedKeyStrokeKey,
		...
		public KeyStroke show(KeyStroke ks) {
			...
			this.setTitle(ApplicationManager.getTranslation(ShortcutDialogConfiguration.shortcutKeyChangeKey, ShortcutDialogConfiguration.this.resources));
			this.infoLabel.setText(ApplicationManager.getTranslation(ShortcutDialogConfiguration.keyStrokeKey, ShortcutDialogConfiguration.this.resources));
			this.acceptButton.setText(ApplicationManager.getTranslation("application.accept", ShortcutDialogConfiguration.this.resources));
			this.cancelButton.setText(ApplicationManager.getTranslation("application.cancel", ShortcutDialogConfiguration.this.resources));
			boolean inUse = this.used(ks);
			if (inUse) {
				this.repetLabel.setText(ApplicationManager.getTranslation(ShortcutDialogConfiguration.usedKeyStrokeKey, ShortcutDialogConfiguration.this.resources));
				this.repetLabel.setForeground(Color.red);
			} else {
				this.repetLabel.setText(ApplicationManager.getTranslation(ShortcutDialogConfiguration.notUsedKeyStrokeKey, ShortcutDialogConfiguration.this.resources));
	...
	protected void init(ApplicationMenuBar bm) {
		...
		// Table and model
		DefaultTableModel model = new DefaultTableModel(new String[] { ShortcutDialogConfiguration.actionColumnKey, ShortcutDialogConfiguration.shortcutColumnKey }, 0);
		...
		this.table.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (selectedRow >= 0) {
					...
					if ((valueAt instanceof JMenuItem) && (ShortcutDialogConfiguration.this.table.getValueAt(selectedRow, 1) != null)) {
	...
	public void setResourceBundle(ResourceBundle res) {
		...
		this.table.getColumnModel().getColumn(0).setHeaderValue(ApplicationManager.getTranslation("application.action", this.resources));
		this.table.getColumnModel().getColumn(1).setHeaderValue(ApplicationManager.getTranslation("Shortcut", this.resources));
	}
	...
	protected void changeComponentKeyStroke(ComponentKeyStroke component, KeyStroke newKeyStroke) {
		...
		if (prefs != null) {
			...
			prefs.setPreference(user, ShortcutDialogConfiguration.getAcceleratorPreferencesKey(component.getGroupName(), component.getKeyName()),
