...
public abstract class AbstractApplicationPreferences implements ApplicationPreferences {
	 ...
	 * changes in preference values. The fire event include information about
	 * the user, the preference key and the new value for this preference.
	 *
	 * @param user
	 *            Name of the user
	 * @param pref
	...
	protected void fireApplicationPreferencesChanged(String user, String pref, String value) {
		for (int i = 0; i < this.listeners.size(); i++) {
			((ApplicationPreferencesListener) this.listeners.get(i)).preferenceChanged(new PreferenceEvent(this, user, pref, value));
		}
	}
