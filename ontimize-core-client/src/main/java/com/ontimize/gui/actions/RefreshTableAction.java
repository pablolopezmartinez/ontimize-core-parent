			...
			if (c instanceof Table) {
				...
				if (t != null) {
					...
					for (int i = 0; i < v.size(); i++) {
						if (f.isEmpty(v.get(i).toString()) && (f.getDataFieldReference(v.get(i).toString()) != null) && f.getDataFieldReference(v.get(i).toString()).isRequired()) {
					...
					if (someEmpty) {
						String translation = ApplicationManager.getTranslation(RefreshTableAction.M_INSERT_VALUE_FIELD, f.getResourceBundle());
						String fieldAttribute = ApplicationManager.getTranslation(emptyAttribute, f.getResourceBundle());
						...
						t.addRefreshTableListener(new RefreshTableListener() {
							...
							public void postCorrectRefresh(RefreshTableEvent e) {
								if (t.isEmpty() && (t.getValue() != null) && (t.getValue() instanceof EntityResult) && (((EntityResult) t.getValue())
