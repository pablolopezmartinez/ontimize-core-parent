...
public class PreferenceYesNoSelectionDialog {
	...
	public PreferenceYesNoSelectionDialog(Component parent, String message, int messageType, String checkMessage, String title, Icon icon) {
		this.questionPanel = new JOptionPane(ApplicationManager.getTranslation(message), messageType, JOptionPane.YES_NO_OPTION, icon);
	...
	/**
	 * Creates and shows a new <code>PreferenceYesNoSelectionDialog</code> when is necessary. First of all checks the preferences, if this preference exists the dialog is not
	 * created, and returns preference value.
	 *
	 * @param parent
	 *            Parent component
	 * @param message
	 ...
	 *            message type
	 * @param checkMessage
	 *            message to show as label in the check to save the preference
	 * @param title
	 *            Dialog title
	 * @param preference
	 *            Preference name
	 * @param icon
	 *            Icon, if icon is null it uses a default one
	 * @return
	 */
	public static int showYesSelectionDialog(Component parent, String message, int messageType, String checkMessage, String title, String preference, Icon icon) {
		...
		if (result == -1) {
			PreferenceYesNoSelectionDialog prefDialog = new PreferenceYesNoSelectionDialog(parent, message, messageType, checkMessage, title, icon);
			result = prefDialog.showDialog();
			if ((result == PreferenceYesNoSelectionDialog.SAVE_NO_VALUE) || (result == PreferenceYesNoSelectionDialog.SAVE_YES_VALUE)) {
				// Save the preference
				ApplicationManager.getApplication().getPreferences().setPreference(user, preference, "" + result);
			}
		}
		if ((result == PreferenceYesNoSelectionDialog.SAVE_NO_VALUE) || (result == PreferenceYesNoSelectionDialog.NO_SAVE_NO_VALUE)) {
			result = JOptionPane.NO_OPTION;

		} else if ((result == PreferenceYesNoSelectionDialog.SAVE_YES_VALUE) || (result == PreferenceYesNoSelectionDialog.NO_SAVE_YES_VALUE)) {
