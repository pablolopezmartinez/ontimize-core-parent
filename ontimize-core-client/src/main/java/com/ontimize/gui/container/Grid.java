...
public class Grid extends JPanel implements FormComponent, IdentifiedElement, AccessForm, Freeable {
	 ...
	 *            <td>raised</td>
	 *            <td>no</td>
	 *            <td>The border definition. Border is set only when parameter 'title' is present (in other case border will be raised).</td>
	 *            </tr>
	 *
	 *            <tr>
	 *            <td>title</td>
		...
		if (bgcolor != null) {
			...
			if (bg.indexOf(";") > 0) {
				try {
					...
					Grid.logger.error(this.getClass().toString() + ": Error in parameter 'bgcolor': " + e.getMessage(), e);
				}
			} else {
				try {
