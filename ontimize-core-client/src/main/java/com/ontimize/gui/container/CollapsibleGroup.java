...
/**
 * This class implements a deployable panel into a <code>CollapsibleGroupPanel</code> container. Each CollapsibleGroup is composed by a Header and a Body. The Header is the visible
 * part when the component is collapsed and the one that contains the title of the group and the responsible of the motion of the component. The body is the motion part of the
 * component and the one that functions as a container of elements.
 * <p>
 *
 * @author Imatia Innovation
 */
public class CollapsibleGroup extends JCollapsibleGroup implements FormComponent, IdentifiedElement, AccessForm, Freeable {
		...
		this.addPropertyChangeListener("collapsed", new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if (CollapsibleGroup.this.header != null) {
					((CollapsibleGroupHeader) CollapsibleGroup.this.header).setCollapsed(CollapsibleGroup.this.collapsed);
				}
			}
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>The icon for Collapsible Group Header when is collapsed. If 'openicon' and 'rollovericon' are not specified this icon is the default icon.</td>
	 * </tr>
	 * <tr>
	 * <td>openicon</td>
	 * <td></td>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Specifies the path to background image of Collapsible Group Header background when is on mouse rollover event.</td>
	 * </tr>
	 * <tr>
	 ...
	 * <td></td>
	 * <td>no</td>
	 * <td>Specifies the path to background image of Collapsible Group Header background when is deployed.</td>
	 * </tr>
	 * <tr>
	 * <td>font</td>
	...
	protected void setAttributeAndTitle(Hashtable parameters) {
		...
		if (attr == null) {
			if (ApplicationManager.DEBUG) {
				CollapsibleGroup.logger.debug(this.getClass().toString() + DataField.ATTR + " parameter hasn't been found");
			}
		} else {
			this.attribute = attr;
	 ...
	 * @param parameterName
	 *            Parameter name for which is the specified color. In case of bad parsing, an error is printed with the parameter name.
	...
	protected Color getColor(String strColor, String parameterName) {
		if (strColor.indexOf(";") > 0) {
			try {
				...
				CollapsibleGroup.logger.error(this.getClass().toString() + ": Error in parameter '" + parameterName + "': " + e.getMessage(), e);
			}
		} else {
			try {
	...
	public static class CollapsibleGroupHeader extends JCollapsibleGroupHeader implements Internationalization {
		...
		protected ImageIcon rolloverIcon = ImageManager.getIcon("com/ontimize/designer/gui/images/component/folder_close.png");
		 ...
		 *
		 * @param g
		 *            The Graphics of the CollapsibleGroup.
		 */
		protected void paintOptions(Graphics g) {
			...
			if ((this.getParent() != null) && (this != null)) {
				...
				Rectangle iconBound = new Rectangle(CollapsibleGroupHeader.LEFT_TO_ICON_MARGIN, (this.getHeight() / 2) - this.icon.getIconHeight(), this.icon.getIconWidth(),
				...
				markWidth += CollapsibleGroupHeader.LEFT_TO_ICON_MARGIN + this.icon.getIconWidth() + CollapsibleGroupHeader.ICON_TO_TEXT_MARGIN + 8;
		 ...
		 *            Graphics
		 * @param x
		 *            X position where starts the text.
		 * @param y
		 *            Y position where starts the text.
		 * @param width
		 *            Width of the text.
		 * @param height
		 *            Height of the text.
		...
		protected void drawTitle(Graphics g, int x, int y, int width, int height) {
			int textLocx = CollapsibleGroupHeader.LEFT_TO_ICON_MARGIN + this.icon.getIconWidth() + CollapsibleGroupHeader.ICON_TO_TEXT_MARGIN;
