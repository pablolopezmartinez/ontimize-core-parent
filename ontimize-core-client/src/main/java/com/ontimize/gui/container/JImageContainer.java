...
/**
 * Implements a container with a background image. It is possible to add new components in a specified pixel position
 */
	...
	protected class PointLayout implements LayoutManager2 {
		...
		public void addLayoutComponent(Component component, Object constraints) {
			...
			if (constraints instanceof Point) {
				...
				component.setBounds(((Point) constraints).x, ((Point) constraints).y, component.getPreferredSize().width, component.getPreferredSize().height);
				if (JImage.DEBUG) {
					...
					c.addMouseMotionListener(new MouseMotionAdapter() {
						...
						public void mouseDragged(MouseEvent e) {
							...
							c.setBounds(e.getX() + (int) r.getX(), e.getY() + (int) r.getY(), c.getPreferredSize().width, c.getPreferredSize().height);
							...
							if (c instanceof ToggleImage) {
								...
								((JComponent) c).setToolTipText(Integer.toString((int) c.getBounds().getX()) + "," + Integer.toString((int) c.getBounds().getY()));
								this.pos.setText(Integer.toString((int) (c.getBounds().x + ((ToggleImage) c).getImageOffset().getX())) + "," + Integer
										.toString((int) (c.getBounds().getY() + ((ToggleImage) c).getImageOffset().getY())));
								this.tip.pack();
		...
		public void layoutContainer(Container containter) {
			...
			for (int i = 0; i < this.components.size(); i++) {
				...
				c.setBounds(((Point) constraints).x, ((Point) constraints).y, c.getPreferredSize().width, c.getPreferredSize().height);
			}
			containter.repaint();
		}
	}
