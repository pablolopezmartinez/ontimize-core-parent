...
/**
 * This class implements a scrollable separator. For a good performance, only two components are allowed.<br>
 * If orientation is vertical(horizontal), first component will be placed on top(on the left).
 * <p>
 *
 * @author Imatia Innovation
 */
public class SplitPane extends JSplitPane implements FormComponent, IdentifiedElement, AccessForm, HasPreferenceComponent, Freeable {
	...
	/**
	 * The class constructor. Calls to JSplitPane <code>constructor</code>, inits parameters and sets orientation.
	 * <p>
	 *
	 * @param parameters
				...
				if (this.parentForm.getFormManager() != null) {
					...
					try {
						if (ap.getPreferences() != null) {
							ap.getPreferences().setPreference(this.getUser(), preferenceKey, this.getDividerLocation() + "");
	...
	protected String getPositionPreferenceKey() {
		...
		return f != null ? BasicApplicationPreferences.SPLIT_POSITION + "_" + f.getArchiveName() + "_" + at : BasicApplicationPreferences.SPLIT_POSITION + "_" + at;
	}
