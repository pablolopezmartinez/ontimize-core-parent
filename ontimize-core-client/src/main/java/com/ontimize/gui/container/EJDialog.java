...
public class EJDialog extends JDialog implements Freeable {
	...
	static {
		...
		class EAction extends AbstractAction {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (SwingUtilities.getWindowAncestor((Component) e.getSource()) instanceof EJDialog) {
					...
					if (close) {
						ejDialog.processWindowEvent(new WindowEvent(SwingUtilities.getWindowAncestor((Component) e.getSource()), WindowEvent.WINDOW_CLOSING));
					}
				}
			}
		}
		...
		EJDialog.setActionForKey(KeyEvent.VK_I, InputEvent.CTRL_MASK | InputEvent.SHIFT_MASK, new ECreateIncidence(), "Create incidence");
	}

	protected boolean askCloseQuestion() {
		int result = MessageDialog.showMessage(this, EJDialog.closeQuestion, JOptionPane.QUESTION_MESSAGE, ApplicationManager.getApplicationBundle());
	...
	protected void processKeyEvent(KeyEvent e) {
		/*
		 * if(e.getKeyCode()==KeyEvent.VK_ESCAPE && e.getID()==KeyEvent.KEY_RELEASED) { e.consume(); processWindowEvent(new WindowEvent(this,WindowEvent.WINDOW_CLOSING)); } else
		...
		try {
			if (this.sizePositionPreference != null) {
				...
				if (prefs != null) {
					...
					prefs.setPreference(user, this.sizePositionPreference, this.getWidth() + ";" + this.getHeight() + ";" + this.getX() + ";" + this.getY());
				}
			}
		} catch (Exception ex) {
			EJDialog.logger.trace(null, ex);
