...
public class Tab extends JPanel implements FormComponent, IdentifiedElement, AccessForm, Freeable {
		...
		if (bgcolor != null) {
			...
			if (bg.indexOf(";") > 0) {
				try {
					...
					Tab.logger.error(this.getClass().toString() + ": Error in parameter 'bgcolor': " + e.getMessage(), e);
				}
			} else {
				try {
	 ...
	 *
	 * @param c
	 *            the container where component is present
	 * @param v
	 *            the vector to insert the field attributes
	 */
	 ...
	 *            the container
	 * @param v
	 *            the vector to get the not required field attributes
	 */
