...
/**
 * Utility class that implements a JDialog to show client application messages. It presents several methods to customize the dialog.
 */
...
public class MessageDialog extends JOptionPane implements Internationalization {
	...
	private MessageDialog(Dialog parent, String message, String details, int type) {
		...
		if (details != null) {
			...
			this.dialog.getContentPane().add(this,
					new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
			...
			bDetailsButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent evento) {
					...
					MessageDialog.this.dialog.setLocation((d.width - MessageDialog.this.dialog.getSize().width) / 2, (d.height - MessageDialog.this.dialog.getSize().height) / 2);
				}
			});
			...
			this.dialog.getContentPane().add(jpDetailsPanel,
					new GridBagConstraints(0, 1, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		} else {
		...
		Dimension d = Toolkit.getDefaultToolkit().getScreenSize();
		this.dialog.setLocation((d.width - this.dialog.getSize().width) / 2, (d.height - this.dialog.getSize().height) / 2);
		this.addPropertyChangeListener(new PropertyChangeListener() {

			@Override
			public void propertyChange(PropertyChangeEvent event) {
				if (MessageDialog.this.dialog.isVisible() && (event.getSource() == MessageDialog.this) && (event.getPropertyName().equals(JOptionPane.VALUE_PROPERTY) || event
		...
		if (details != null) {
			...
			try {
				if (((UtilReferenceLocator) ApplicationManager.getApplication().getReferenceLocator())
						.supportIncidenceService() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
	 ...
	 *
	 * @param parent
	 *            the window that will hold the message, in this case a {@link Frame} instance
	 * @param message
	 *            the key of the message to display; this key will be translated
	 * @param type
	 *            the message type. The message type can be selected among the following values from the {@link JOptionPane} class. Values can be: <code>
	 ...
	 * @param args
	 *            an object array with the values that will configure the message, in case the message can be configurable, that is, uses {0}, {1}, etc. to mark the spots where the
	 *            args params will be set
	 */
	public static int showMessage(Frame parent, String message, int type, ResourceBundle resourceBundle, Object[] args) {
	 ...
	 * @param resourceBundle
	 *            the resource bundle that will be used to look up the message key
	 * @return the selected value in the dialog, that is, the index of the selected option when the chosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if no
	 ...
	 *           {@link JOptionPane#PLAIN_MESSAGE}
	 * &#64;param newType
	 *            an integer specifying the options the L&F is to display: <code> {@link JOptionPane#DEFAULT_OPTION} </code>, <code>{@link JOptionPane#YES_NO_OPTION}</code>, <code>
	 *            {@link JOptionPane#YES_NO_CANCEL_OPTION}</code>, or <code>{@link JOptionPane#OK_CANCEL_OPTION}</code>
	 * @param resourceBundle
	 *            the resource bundle that will be used to look up the message key
	 * @return the selected value in the dialog, that is, the index of the selected option when the chosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if no
	 *         options selected / available
	 ...
	 *            the key of the message to display; this key will be translated
	 * @param details
	 *            the content of details will be displayed into an auxiliary text box of the dialog and can be shown or hidden by the user
	 * @param type
	 *            the message type. The message type can be selected among the following values from the {@link JOptionPane} class. Values can be: <code>
	 *            {@link JOptionPane#ERROR_MESSAGE}, {@link JOptionPane#INFORMATION_MESSAGE}, {@link JOptionPane#WARNING_MESSAGE}, {@link JOptionPane#QUESTION_MESSAGE},
	 *            {@link JOptionPane#PLAIN_MESSAGE}
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, String details, int type, ResourceBundle resourceBundle, Object[] args) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, String details, int type, int newType, ResourceBundle resourceBundle) {
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, String details, int type, int newType, ResourceBundle resourceBundle, Object[] args) {
	 ...
	 *            {@link JOptionPane#YES_NO_CANCEL_OPTION}</code>, or <code>{@link JOptionPane#OK_CANCEL_OPTION}</code>
	 * @param newValues
	 *            an array of <code>Objects</code> the user to be displayed (usually in a list or combo-box) from which the user can make a selection
	 * @return the selected value in the dialog, that is, the index of the selected option when the chosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if no
	 *         options selected / available
	 */
	public static int showOptionMessage(Frame parent, String message, String details, int type, ResourceBundle resourceBundle, int newType, Object[] newValues) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
	 ...
	 *
	 * @param parent
	 *            the window that will hold the message, in this case a {@link Dialog} instance
	 * @param message
	 *            the key of the message to display; this key will be translated
	 * @param type
	 *            the message type. The message type can be selected among the following values from the {@link JOptionPane} class. Values can be: <code>
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, int type, ResourceBundle resourceBundle, Object[] args) {
		return MessageDialog.showMessage(parent, message, null, type, resourceBundle, args);
	}
	 ...
	 * @param resourceBundle
	 *            the resource bundle that will be used to look up the message key
	 * @return the selected value in the dialog, that is, the index of the selected option when the choosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if
	 *         no options selected / available
	 */
	...
	/**
	 * Shows a message. The message type can be selected among the following values from the {@link JOptionPane} class. <br/>
	 * <ul>
	 ...
	 *
	 * @param parent
	 *            the Dialog that will hold the message
	 * @param message
	 *            the message to show
	 * @param type
	 *            the message type
	 * @param newType
	 *            an integer specifying the options the L&F is to display: <code> {@link JOptionPane#DEFAULT_OPTION} </code> , <code>{@link JOptionPane#YES_NO_OPTION}</code>,
	 *            <code>{@link JOptionPane#YES_NO_CANCEL_OPTION}</code>, or <code>{@link JOptionPane#OK_CANCEL_OPTION}</code>
	 * @param resourceBundle
	 *            the resource bundle that will be used to look up the message key
	 * @return the selected value in the dialog, that is, the index of the selected option when the choosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if
	 *         no options selected / available
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, ResourceBundle resourceBundle) {
		return MessageDialog.showMessage(parent, message, details, type, resourceBundle, (Object[]) null);
	}
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, ResourceBundle resourceBundle, Object[] args) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
		final MessageDialog messageDialog = new MessageDialog(parent, message, details, type);
	 ...
	 * @param details
	 * @param type
	 *            type the message type. The message type can be selected among the following values from the {@link JOptionPane} class. Values can be: <code>
	 *            {@link JOptionPane#ERROR_MESSAGE}, {@link JOptionPane#INFORMATION_MESSAGE}, {@link JOptionPane#WARNING_MESSAGE}, {@link JOptionPane#QUESTION_MESSAGE},
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, int newType, ResourceBundle resourceBundle) {
		return MessageDialog.showMessage(parent, message, details, type, newType, resourceBundle, null, null);
	}
	 ...
	 * @param args
	 *            an object array with the values that will configure the message, in case the message can be configurable, that is, uses {0}, {1}, etc. to mark the spots where the
	 *            arguments parameters will be sets
	 * @return the selected value in the dialog, that is, the index of the selected option when the chosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if no
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, int newType, ResourceBundle resourceBundle, Object[] args) {
		return MessageDialog.showMessage(parent, message, details, type, newType, resourceBundle, null, args);
	}
	 ...
	 *         options selected / available
	 */
	public static int showOptionMessage(Dialog parent, String message, String details, int type, ResourceBundle resourceBundle, int newType, Object[] newValues) {
	...
	/**
	 * All the messages that can be translated, apart from those translated in the getTextsToTranslate method.
	 *
	 * @return
	 */
	 ...
	 *            the resource bundle that will be used to look up the message key
	 * @param title
	 *            the key of the dialog title; the key will be translated.
	 * @return the selected value in the dialog, that is, the index of the selected option when the chosen type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if no
	 *         options selected / available
	 */
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, int type, int newType, ResourceBundle resourceBundle, String title) {
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, int type, int newType, ResourceBundle resourceBundle, String title, Object[] args) {
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, String details, int type, ResourceBundle resourceBundle, String title) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
		final MessageDialog messageDialog = new MessageDialog(parent, message, details, type);
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, String details, int type, int newType, ResourceBundle resourceBundle, String title) {
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Frame parent, String message, String details, int type, int newType, ResourceBundle resourceBundle, String title, Object[] args) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
	 ...
	 *         options selected / available
	 */
	public static int showOptionMessage(Frame parent, String message, String details, int type, ResourceBundle resourceBundle, int newType, Object[] newValues, String title) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
		final MessageDialog messageDialog = new MessageDialog(parent, message, details, type, newType, newValues);
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, int type, ResourceBundle resourceBundle, String title) {
		return MessageDialog.showMessage(parent, message, null, type, resourceBundle, title);
	}
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, int type, int newType, ResourceBundle resourceBundle, String title) {
		return MessageDialog.showMessage(parent, message, null, type, newType, resourceBundle, title);
	}
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, int type, int newType, ResourceBundle resourceBundle, String title, Object[] args) {
		return MessageDialog.showMessage(parent, message, null, type, newType, resourceBundle, title, args);
	}
	 ...
	 *            the resource bundle that will be used to look up the message key
	 * @param title
	 *            the key of the dialog title; the key will be translated. * @return the selected value in the dialog, that is, the index of the selected option when the chosen
	 *            type of the dialog enables it, or {@link JOptionPane#CLOSED_OPTION} if no options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, ResourceBundle resourceBundle, String title) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
		final MessageDialog messageDialog = new MessageDialog(parent, message, details, type);
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, int newType, ResourceBundle resourceBundle, String title) {
		return MessageDialog.showMessage(parent, message, details, type, newType, resourceBundle, title, null);
	}
	 ...
	 *         options selected / available
	 */
	public static int showMessage(Dialog parent, String message, String details, int type, int newType, ResourceBundle resourceBundle, String title, Object[] args) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
	 ...
	 * @return
	 */
	public static int showOptionMessage(Dialog parent, String message, String details, int type, ResourceBundle resourceBundle, int newType, Object[] newValues, String title) {
		if (MessageDialog.logger.isDebugEnabled() && ((type == JOptionPane.ERROR_MESSAGE) || (type == JOptionPane.WARNING_MESSAGE))) {
			ApplicationManager.printCurrentThreadMethods(10);
		}
		final MessageDialog messageDialog = new MessageDialog(parent, message, details, type, newType, newValues);
	 ...
	 *
	 * @param window
	 *            the window that will hold the message. It can be either a {@link #Dialog} or a {@link Frame}
	 * @param message
	 *            the key of the message to display
	 */
	 ...
	 *
	 * @param window
	 *            the window that will hold the message. It can be either a
	 *            {@link #Dialog} or a {@link Frame}
	 * @param message
	 *            the key of the message to display
	 * @param resourceBundle
	 *            the resourceBundle to traslate
	 */
	...
	/**
	 * Shows a yes/no question message. These kind of messages has two buttons, to get the client response to the query
	 *
	 * @param window
	 *            the window that will hold the message. It can be either a {@link #Dialog} or a {@link Frame}
	 * @param message
	...
	public static boolean showQuestionMessage(Window window, String message) {
		if (window instanceof Dialog) {
			return MessageDialog.showMessage((Dialog) window, message, JOptionPane.QUESTION_MESSAGE, null) == JOptionPane.YES_OPTION;
		} else {
			return MessageDialog.showMessage((Frame) window, message, JOptionPane.QUESTION_MESSAGE, null) == JOptionPane.YES_OPTION;
		}
	}
	 ...
	 *
	 * @param component
	 *            the component that will invoke the message.
	 * @param message
	 *            the key of the message to display
	 * @param resourceBundle
	 *            the resource bundle that will be used to look up the message key
	...
	public static boolean showQuestionMessage(Window w, String message, ResourceBundle resourceBundle) {
		if (w instanceof Dialog) {
			return MessageDialog.showMessage((Dialog) w, message, JOptionPane.QUESTION_MESSAGE, resourceBundle) == JOptionPane.YES_OPTION;
		} else {
			return MessageDialog.showMessage((Frame) w, message, JOptionPane.QUESTION_MESSAGE, resourceBundle) == JOptionPane.YES_OPTION;
		}
	}
	 ...
	 *            the window that will hold the message. It can be either a {@link #Dialog} or a {@link Frame}
	 * @param message
	 *            message the key of the message to display
	 * @param resourceBundle
	 *            the resource bundle that will be used to look up the message key
	 ...
	 * @return the client response
	 */
	public static Object showInputMessage(Window window, String message, ResourceBundle resourceBundle, Object initialValue) {
		if (window instanceof Dialog) {
			final MessageDialog messageDialog = new MessageDialog((Dialog) window, message, null, JOptionPane.QUESTION_MESSAGE);
			...
			final MessageDialog messageDialog = new MessageDialog((Frame) window, message, null, JOptionPane.QUESTION_MESSAGE);
