...
public class Chat extends JPanel {
	...
	protected Vector getUserList() {
		...
		if (this.locator != null) {
			try {
				Entity entity = this.locator.getEntityReference(((UtilReferenceLocator) this.locator).getLoginEntityName(this.locator.getSessionId()));
				...
				if (index >= 0) {
					...
					if ((user instanceof User) && ((User) user).isConnected()) {
						...
						for (int i = 0; i < indexSelected.length; i++) {
							...
							if ((oCurrentUser instanceof User) && ((User) oCurrentUser).isConnected() && (oCurrentUser != user)) {
	...
	protected class UserChatWindow extends JDialog {
				...
				if (notify && this.initCommunication) {
					for (int i = 0; i < this.userList.size(); i++) {
						try {
							...
							SMessage message = new SMessage(currentUser, SMessage.REMOVE_USER, user.toString(), this.sessionCommunicationId, list);
							((UtilReferenceLocator) Chat.this.locator).sendMessage(message, this.userList.get(i).toString(), Chat.this.locator.getSessionId());
						} catch (Exception e) {
							Chat.logger.error(null, e);
						}
					}
			...
			Runnable r = new Runnable() {
				...
				public void run() {
					...
					try {
						...
						if (c != null) {
							((SimpleAttributeSet) UserChatWindow.this.atMessage).addAttribute(StyleConstants.Foreground, c);
						...
						UserChatWindow.this.sReceiver.getVerticalScrollBar().setValue(UserChatWindow.this.sReceiver.getVerticalScrollBar().getMaximum());
			...
			if (msg instanceof BMessage) {
				...
				if (SMessage.REMOVE_USER == smsg.getType()) {
					...
					if (user.equals(currentUser) && UserChatWindow.this.isVisible()) {
						String sMessage = ApplicationManager.getTranslation("Chat.someone_put_you_out_conversation", ApplicationManager.getApplicationBundle(),
					...
					String sMessage = ApplicationManager.getTranslation("Chat.someone_left_conversation", ApplicationManager.getApplicationBundle(), new Object[] { userFrom });
					JOptionPane.showMessageDialog(UserChatWindow.this, sMessage);
		...
		protected void init() {
			...
			((SimpleAttributeSet) this.atName).addAttribute(StyleConstants.Size, new Integer(this.getFont().getSize() - 2));
			...
			this.sendText.addKeyListener(new KeyListener() {
				...
				public void keyPressed(KeyEvent e) {
					if ((e.getKeyCode() == KeyEvent.VK_ENTER) && e.isControlDown()) {
						try {
							UserChatWindow.this.sendText.getDocument().insertString(UserChatWindow.this.sendText.getDocument().getLength(), "\n", null);
			...
			bottomPanel.add(new JScrollPane(this.sendText),
					new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(1, 1, 1, 1), 0, 0));
			bottomPanel.add(this.bSend, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
			...
			bAdd.addActionListener(new ActionListener() {
				...
				@Override
				public void actionPerformed(ActionEvent e) {
					List lAddList = UserSelectionList.showAddList((Component) e.getSource(), Chat.this.locator, UserChatWindow.this.userList.toStringArray());
					if (lAddList == null) {
						return;
					}
			...
			bDelete.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					List listDelete = UserSelectionList.showRemoveList((Component) e.getSource(), Chat.this.locator, UserChatWindow.this.userList);
					if (listDelete == null) {
						return;
					}
			...
			JButton bClose = new JButton(ApplicationManager.getTranslation(Chat.CLOSE_CONVERSATION, ApplicationManager.getApplication().getResourceBundle()), this.closeIcon);
						...
						while (enu.hasMoreElements()) {
							...
							if (UserChatWindow.this.equals(actual)) {
								...
								for (int i = 0; i < list.size(); i++) {
									...
									SMessage message = new SMessage(currentUser, SMessage.SIGN_DOWN, destino, UserChatWindow.this.sessionCommunicationId, null);
									try {
										((UtilReferenceLocator) Chat.this.locator).sendMessage(message, destino, Chat.this.locator.getSessionId());
									} catch (Exception ex) {
										Chat.logger.error(null, ex);
									}
								}
			...
			jbButtonsPanel.add(this.toolBar, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 0), 0, 0));
			jbButtonsPanel.add(bClose, new GridBagConstraints(1, 0, 1, 1, 0, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
	...
	protected static class UserSelectionList extends JList {
		...
		protected static void showUserList(Component comp) {
			...
			panel.add(bAccept, new GridBagConstraints(0, 0, 1, 1, 0, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
			panel.add(bCancel, new GridBagConstraints(1, 0, 1, 1, 0, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0));
	...
	protected void init() {
		...
		this.headerUser = new JLabel(ApplicationManager.getTranslation(Chat.USER, ApplicationManager.getApplication().getResourceBundle()));
		...
		this.headerConnection = new JLabel(ApplicationManager.getTranslation(Chat.CONVERSATION, ApplicationManager.getApplication().getResourceBundle()));
		...
		this.add(sPane, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		this.add(new JSeparator(), new GridBagConstraints(0, 1, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
		this.add(connectionPanel, new GridBagConstraints(0, 2, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
	}
		...
		public void run() {
			...
			while (true) {
				try {
					...
					Vector v = ((UtilReferenceLocator) Chat.this.locator).getMessages(Chat.this.locator.getSessionId(), Chat.this.locator.getSessionId());
					if (v != null) {
						for (int i = 0; i < v.size(); i++) {
	...
	public static void showChat(EntityReferenceLocator locator, Vector messages, int messagesCheckTime, int chatCheckTime) {
		try {
			if (Chat.fChat == null) {
