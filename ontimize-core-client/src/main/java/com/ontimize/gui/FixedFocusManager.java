...
public class FixedFocusManager extends FocusManager {
	...
	/**
	 * This method is called by JComponents when a key event occurs. JComponent gives key events to the focus manager first, then to key listeners, then to the keyboard UI
	 * dispatcher. This method should look at the key event and change the focused component if the key event matches the receiver's focus manager hot keys. For example the default
	 * focus manager will change the focus if the key event matches TAB or Shift + TAB. The focus manager should call consume() on <b>anEvent</b> if <code>anEvent</code> has been
	 * processed. <code>focusedComponent</code> is the component that currently has the focus. Note: FocusManager will receive KEY_PRESSED, KEY_RELEASED and KEY_TYPED key events.
	 * If one event is consumed, all other events type should be consumed.
	 */
					...
					if (FixedFocusManager.DEBUG) {
						...
						if (cont instanceof IdentifiedElement) {
							FixedFocusManager.logger.debug(
									((IdentifiedElement) cont).getAttribute() + " is an identified element detected as father of component focused, visible " + cont.isVisible());
						}
	...
	private Component getFocusableComponentAfter(Component focusedComponent, Container rootContainer, boolean moveForward) {
		...
		do {
			nextComponent = moveForward ? this.getNextComponent(nextComponent, rootContainer, true) : this.getPreviousComponent(nextComponent, rootContainer);
	...
	private Component getNextComponent(Component component, Container root, boolean canGoDown) {
		...
		if (canGoDown && component.isVisible() && (!(component instanceof JComponent) || !(((JComponent) component).isManagingFocus()))
	...
	private Component getDeepestLastComponent(Component component) {
		if (component.isVisible() && (((component instanceof JComponent) && !((JComponent) component).isManagingFocus()) || !(component instanceof JComponent))
				&& ((component instanceof Container) && (((Container) component).getComponentCount() > 0))) {
	...
	public Component getComponentAfter(Container aContainer, Component aComponent) {
		Component nc;
		if ((aComponent instanceof JComponent) && ((nc = ((JComponent) aComponent).getNextFocusableComponent()) != null)) {
			return nc;
		}
	 ...
	 * "tab" order. The default implementation will order tab to give a left to
	 * right, top down order. Override this method if another order is required.
	 */
	...
	/**
	 * Return the component after comp according to compareTabOrder. If comp is
	 * the last component according to that order, return null.
	 */
	...
	/**
	 * Return the component before comp according to compareTabOrder. If comp is
	 * the first component according to that order, return null.
	 */
	...
	/**
	 * If there is a child c of Cont such that c.getNextFocusableComponent ==
	 * comp, return that component. Otherwise, return null.
	 */
	...
	private Component getPreviousFocusable(Container cont, Component comp) {
		...
		for (i = 0, c = children.length; i < c; i++) {
			if ((children[i] instanceof JComponent) && ((((JComponent) children[i]).getNextFocusableComponent()) == comp)) {
				return children[i];
			}
		}
		return null;
