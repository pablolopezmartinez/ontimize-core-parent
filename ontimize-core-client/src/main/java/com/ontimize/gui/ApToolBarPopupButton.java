...
public class ApToolBarPopupButton extends ApToolBarToggleButton {
	...
	/**
	 * The class constructor. Calls to <code>super</code> with parameters and makes visible the pop-up menu.
	 * <p>
	 *
	 * @param parameters
	...
	public ApToolBarPopupButton(Hashtable parameters) {
		...
		if (ApToolBarPopupButton.arrow == null) {
			...
			if (ApToolBarPopupButton.arrow != null) {
				ApToolBarPopupButton.deactivateArrow = new ImageIcon(GrayFilter.createDisabledImage(ApToolBarPopupButton.arrow.getImage()));
			} else {
				ApToolBarPopupButton.logger.debug(this.getClass().toString() + ": " + ImageManager.POPUP_ARROW + " hasn't been found");
			}
		}
	...
	public Dimension getPreferredSize() {
		if (ApplicationManager.useOntimizePlaf) {
			...
			if (this.dimension == null) {
				this.dimension = new Dimension(super.getSwingPreferredSize().width + (ApToolBarPopupButton.arrow == null ? 0 : ApToolBarPopupButton.arrow
						.getIconWidth() + ApToolBarPopupButton.arrowMargin), super.getSwingPreferredSize().height);
			}
		}
		if (this.dimension == null) {
			// Create the dimension
			this.dimension = new Dimension(
					super.getPreferredSize().width + (ApToolBarPopupButton.arrow == null ? 0 : ApToolBarPopupButton.arrow.getIconWidth() + ApToolBarPopupButton.arrowMargin),
	...
	protected void paintComponent(Graphics g) {
		...
		// Paint the arrow
		if (this.isEnabled()) {
			ApToolBarPopupButton.arrow.paintIcon(this, g, this.getWidth() - ApToolBarPopupButton.arrow.getIconWidth() - 3, (this.getHeight() / 2) - 1);
		} else {
			ApToolBarPopupButton.deactivateArrow.paintIcon(this, g, this.getWidth() - ApToolBarPopupButton.arrow.getIconWidth() - 3, (this.getHeight() / 2) - 1);
		}
	}
	...
	public DataFlavor[] getTransferDataFlavors() {
		try {
			return new DataFlavor[] { new DataFlavor(DataFlavor.javaJVMLocalObjectMimeType + ";class=\"" + ApToolBarPopupButton.class.getName() + "\"") };
