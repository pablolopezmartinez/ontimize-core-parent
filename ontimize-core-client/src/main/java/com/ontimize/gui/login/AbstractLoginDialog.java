...
public abstract class AbstractLoginDialog extends JDialog implements ILoginDialog {
	...
	protected void registerKeyBindings() {
		...
		KeyStroke ks1 = KeyStroke.getKeyStroke(KeyEvent.VK_A, InputEvent.ALT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK, false);
		Act act1 = new Act() {

			@Override
			public void actionPerformed(ActionEvent e) {
				IRemoteAdministrationWindow window = ApplicationManager.getRemoteAdminWindow((RemotelyManageable) AbstractLoginDialog.this.locator);
	...
	public void setKeyBinding(String key, KeyStroke keyStroke, Action action, InputMap inMap, ActionMap actMap) {
		...
		ComponentKeyStroke compKeyStroke = new ComponentKeyStroke((JComponent) this.getContentPane(), "Remote Administration");
	...
	protected void init(Hashtable parameters) {
		if (parameters.containsKey(ILoginDialog.REMEMBER_PASSWORD)) {
			boolean allowRememberPassword = ApplicationManager.parseStringValue((String) parameters.get(ILoginDialog.REMEMBER_PASSWORD));
		...
		if (parameters.containsKey(ILoginDialog.REMEMBER_LAST_LOGIN)) {
			boolean rememberLastLogin = ApplicationManager.parseStringValue((String) parameters.get(ILoginDialog.REMEMBER_LAST_LOGIN));
	...
	protected void savePreferences() {
		...
		if (preferences != null) {
			...
			if (this.isRememberLogin() && (((ClientReferenceLocator) this.locator).getUser() != null)) {
				preferences.setPreference(null, MainApplication.APP_REMEMBER_LAST_LOGIN, ApplicationManager.parseBooleanValue(true));
				preferences.setPreference(null, MainApplication.APP_LAST_LOGIN, ((ClientReferenceLocator) this.locator).getUser());
				preferences.savePreferences();
				if (this.isRememberPassword() && (this.getPasswordValue() != null)) {
					preferences.setPreference(null, MainApplication.APP_REMEMBER_LAST_PASSWORD, ApplicationManager.parseBooleanValue(true));
					preferences.setPreference(null, MainApplication.APP_LAST_PASSWORD, EncryptHelper.encrypt(this.getPasswordValue()));
	...
	protected void changePassword() {
		try {
			if (((UtilReferenceLocator) this.getEntityReferenceLocator()).supportChangePassword(this.getUserValue(), this.getEntityReferenceLocator().getSessionId())) {
				...
				if (this.params.containsKey(IChangePasswordDialog.CHANGE_PASSWORD_DIALOG_CLASS)) {
					...
					try {
						Class cpClass = Class.forName(this.params.get(IChangePasswordDialog.CHANGE_PASSWORD_DIALOG_CLASS).toString());
						...
						cpdialog = (IChangePasswordDialog) constructor.newInstance(new Object[] { this.getApplication(), this.params, this.getEntityReferenceLocator(), this.user
								.getValue().toString(), this.password.getValue().toString() });
					} catch (Exception e) {
						AbstractLoginDialog.logger.error("Cannot instanciate the class: {}. Using default class. Error: {}",
								this.params.get(IChangePasswordDialog.CHANGE_PASSWORD_DIALOG_CLASS).toString(), e.getMessage(), e);
						cpdialog = new DefaultChangePasswordDialog(this.getApplication(), this.params, this.getEntityReferenceLocator(), this.user.getValue().toString(),
	...
	public void certificateButtonPressed() {
		...
		if (!this.isInstalledDNIeProvider()) {
			if ((ApplicationManager.getApplication() != null) && (ApplicationManager.getApplication().getPreferences() != null) && (ApplicationManager.getApplication()
					.getPreferences().getPreference(null, SunPKCS11Wrapper.PREFERENCE_PKCS11_MODULE) != null)) {
				SunPKCS11Wrapper.pkcsConfigFile = ApplicationManager.getApplication().getPreferences().getPreference(null, SunPKCS11Wrapper.PREFERENCE_PKCS11_MODULE);
			...
			if (!this.isInstalledDNIeProvider()) {
				...
				if (accepted) {
					...
					if (!this.isInstalledDNIeProvider()) {
						MessageDialog.showMessage(this.getApplication().getFrame(), "abstractlogindialog.noproviderinstalled", JOptionPane.ERROR_MESSAGE, this.bundle);
						return;
					}
				}
			}
		...
		try {
			...
			MessageDialog.showMessage(this.getApplication().getFrame(), ex.getMessage(), JOptionPane.ERROR_MESSAGE, this.bundle);
			this.setCursor(c);
		...
		// select Certificate
		this.setCursor(Cursor.getDefaultCursor());
		this.aliasCertPair = CertificateChooserDialog.selectCertificate(l, this.getApplication().getFrame(), this.bundle);

		if (this.aliasCertPair == null) {
			this.setCursor(Cursor.getDefaultCursor());
			MessageDialog.showMessage(this.getApplication().getFrame(), "abstractlogindialog.nocertificateselected", JOptionPane.ERROR_MESSAGE, this.bundle);
			this.setCursor(c);
			return;
		}
		...
		PrivateKey sk = null;
		try {
			sk = (PrivateKey) CertificateUtils.getKeystoreInstance().getKey(this.aliasCertPair.getAlias(), pin.toCharArray());
		} catch (Exception ex) {
			this.setCursor(Cursor.getDefaultCursor());
			MessageDialog.showMessage(this.getApplication().getFrame(), ex.getMessage(), JOptionPane.ERROR_MESSAGE, this.bundle);
			this.setCursor(c);
		...
		if (!(this.aliasCertPair.getCert() instanceof X509Certificate)) {
			this.setCursor(Cursor.getDefaultCursor());
			MessageDialog.showMessage(this.getApplication().getFrame(), "abstractlogindialog.certificateinvalidformat", JOptionPane.ERROR_MESSAGE, this.bundle);
			this.setCursor(c);
			return;
		}
	...
	protected void fillDialogFields(Certificate cert, String token, String signedToken) {
		...
		if (AbstractLoginDialog.useDNIeCN) {
			if (cert instanceof X509Certificate) {
				userName = CertificateUtils.getX509CertificateSubjectDNFields((X509Certificate) cert, AbstractLoginDialog.userDNIeCNColumn);
			}
			if (userName != null) {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				try {
					...
					if (AbstractLoginDialog.this.getEntityReferenceLocator() != null) {
						...
						try {
							if (AbstractLoginDialog.this.bundle != null) {
								sConnectiongText = AbstractLoginDialog.this.bundle.getString(ILoginDialog.CONNECTING_KEY);
							}
						} catch (Exception ex) {
							if (ApplicationManager.DEBUG) {
						...
						AbstractLoginDialog.this.status.paintImmediately(0, 0, AbstractLoginDialog.this.status.getWidth(), AbstractLoginDialog.this.status.getHeight());
					...
					AbstractLoginDialog.this.status.setText(ApplicationManager.getTranslation(ILoginDialog.ERROR_LOGIN_KEY));

					MessageDialog.showMessage(AbstractLoginDialog.this.getApplication().getFrame(), e2.getMessage(), JOptionPane.ERROR_MESSAGE, AbstractLoginDialog.this.bundle);
					if (ApplicationManager.DEBUG) {
	...
	public String getUserValue() {
		...
		if (this.loggedIn && (this.password.getValue() != null) && this.password.getValue().toString().startsWith(CertificateUtils.USER_CERTIFICATE)) {
			try {
				...
				try {
					...
					String userValue = ((TokenLogin) this.getEntityReferenceLocator()).getUserFromCert(stringCertificate);
					return userValue;
				} catch (Exception ex) {
					AbstractLoginDialog.logger.error(null, ex);
					MessageDialog.showMessage(this.getApplication().getFrame(), ex.getMessage(), JOptionPane.ERROR_MESSAGE, this.bundle);
	...
	public String getPasswordValue() {
		...
		if (this.loggedIn && ((ClientReferenceLocator)this.locator).isAllowCertificateLogin() && obj.toString().startsWith(CertificateUtils.USER_CERTIFICATE)) {
			try {
				Hashtable h = CertificateUtils.parseTokenReceived(obj.toString());
				Certificate cert = (Certificate) h.get(CertificateUtils.USER_CERTIFICATE);
				try {
					String stringCertificate = CertificateUtils.encodeCertificate(cert);
					String passwordValue = ((TokenLogin) this.getEntityReferenceLocator()).getPasswordFromCert(stringCertificate);
