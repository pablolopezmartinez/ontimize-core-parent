...
public class LoginDialog extends AbstractLoginDialog implements MouseInputListener {
	...
	protected void layoutComponents(Hashtable parameters) {
		...
		this.getContentPane().add(panel, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(LoginDialog.topMargin, LoginDialog.leftMargin, LoginDialog.bottomMargin, LoginDialog.rightMargin), 0, 0));
	...
	protected JPanel createCenterPanel(Hashtable parameters) {
		...
		centerPanel.add(this.status,
				new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 1, GridBagConstraints.SOUTH, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));

		this.user = this.createLogin(parameters);
		centerPanel.add(this.user, new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
				new Insets(LoginDialog.statusUserGap, 0, 0, 0), 0, 0));

		this.password = this.createPassword(parameters);
		centerPanel.add(this.password, new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
		...
		checkRow.add(this.rememberLogin, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		checkRow.add(this.rememberPassword, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));

		centerPanel.add(checkRow, new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE,
		...
		if (this.serverCombo != null) {
			...
			connectToRow.add(this.connectToLabel,
					new GridBagConstraints(0, 0, 1, 1, 0.25, 1, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
			connectToRow.add(this.serverCombo, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0));
			centerPanel.add(connectToRow, new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
		...
		if (!((ClientReferenceLocator)locator).isAllowCertificateLogin()) {
			buttonRow.add(this.acceptButton, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 0, 2, 10), 30, 0));
			buttonRow.add(this.cancelButton, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(2, 10, 2, 0), 30, 0));
		} else {
			buttonRow.add(this.acceptButton, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(2, 0, 2, 0), 0, 0));
			buttonRow.add(this.certificateButton, new GridBagConstraints(1, 0, 1, 1, 0, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 0, 2, 0), 0, 0));
			buttonRow.add(this.cancelButton, new GridBagConstraints(2, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(2, 0, 2, 0), 0, 0));
		}

		centerPanel.add(buttonRow, new GridBagConstraints(0, GridBagConstraints.RELATIVE, 1, 1, 1, 0, GridBagConstraints.SOUTH, GridBagConstraints.HORIZONTAL,
	...
	protected TextDataField createLogin(Hashtable parameters) {
		...
		try {
			...
			text.setLabelFontColor(ParseUtils.getColor((String) p.get("labelfontcolor"), ColorConstants.parseColor("#333333")));
		} catch (Exception e) {
			LoginDialog.logger.error(null, e);
		}
		return text;
	...
	protected PasswordDataField createPassword(Hashtable parameters) {
		...
		try {
			...
			text.setLabelFontColor(ParseUtils.getColor((String) p2.get("labelfontcolor"), ColorConstants.parseColor("#333333")));
		} catch (Exception e) {
			LoginDialog.logger.error(null, e);
		}
		return text;
