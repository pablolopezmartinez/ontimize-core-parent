...
public class TopWindow extends JWindow implements Internationalization, ISplash {
	...
	public TopWindow(ResourceBundle res) {
		...
		this.getContentPane().add(this.label, new GridBagConstraints(0, 1, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(10, 10, 10, 10), 0, 0));
	...
	public TopWindow(String message) {
		...
		this.getContentPane().setLayout(new GridBagLayout());
		this.getContentPane().add(this.label, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(10, 10, 10, 10), 0, 0));
	...
	public TopWindow(Frame owner, String message, ResourceBundle res, ImageIcon image, ImageIcon image2, Border border) {
		this((Window) owner, message, res, image, image2, border);
	}

	public TopWindow(Window owner, String message, ResourceBundle res, ImageIcon image, ImageIcon image2, Border border) {
		super(owner);
		this.resources = res;
		if (message != null) {
			try {
		...
		this.getContentPane().setLayout(new GridBagLayout());
		this.getContentPane().add(this.labelIm, new GridBagConstraints(0, 0, 2, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		if (this.label != null) {
			this.getContentPane().add(this.label, new GridBagConstraints(0, 1, 1, 1, 0, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(10, 10, 10, 5), 0, 0));
		}
		if (this.labelIm2 != null) {
			this.getContentPane().add(this.labelIm2, new GridBagConstraints(1, 1, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		}
	...
	/**
	 * repaint specifies that the window must be painted each <code>repaintTime</code> milliseconds using a high priority thread, but this painting does not happen if the event
	 * dispatcher thread is busy
	 */
	@Override
	...
	/**
	 * Show the window during the specified time. If this time is 0, then shows the window until the next call to the hide method. If the window is already visible then does
	 * nothing
	 *
	 * @param milliseconds
	 */
	 ...
	 */
	public void update() {
		TopWindow.this.getRootPane().paintImmediately(0, 0, TopWindow.this.getRootPane().getWidth(), TopWindow.this.getRootPane().getHeight());
	}
