...
public class MouseHandler extends MouseAdapter implements MouseMotionListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		case AttachmentListCellRenderer.SAVE:
			...
			try {
				...
				com.ontimize.gui.actions.DownloadThread eop = new com.ontimize.gui.actions.DownloadThread(this.form.getResourceBundle(), selectedFile, kv,
						(FileManagementEntity) this.getAttachmentEntity(), this.form.getFormManager().getReferenceLocator());
		...
		case AttachmentListCellRenderer.DELETE:
			...
			try {
				...
				EntityResult res = this.getAttachmentEntity().delete(kv, this.form.getFormManager().getReferenceLocator().getSessionId());
				...
				if (hRecord.containsKey(Form.DESCRIPTION_FILE)) {
					...
					if (o instanceof String) {
						if (!((String) o).equals(this.descriptionPopupEditor.getDescriptionField().getText())) {
							kv.put(Form.ATTACHMENT_ID, hRecord.get(Form.ATTACHMENT_ID));
							av.put(Form.DESCRIPTION_FILE, this.descriptionPopupEditor.getDescriptionField().getText());
							try {
		...
		case AttachmentListCellRenderer.OPEN:
			...
			try {
				...
				Hashtable kv = new Hashtable();
				kv.put(Form.ATTACHMENT_ID, this.attachmentListPopup.getRecord(sel).get(Form.ATTACHMENT_ID));
				com.ontimize.gui.actions.DownloadThread eop = new com.ontimize.gui.actions.DownloadThread(this.form.getResourceBundle(), fSelec, kv,
						(FileManagementEntity) this.getAttachmentEntity(), this.form.getFormManager().getReferenceLocator());
	...
	public void configureDescriptionDialog(int sel) {
		...
		this.descriptionPopupEditor.getDescriptionField().setText((String) this.attachmentListPopup.getRecord(sel).get(Form.DESCRIPTION_FILE));
		this.descriptionPopupEditor.setSelectedRecord(sel);
		this.descriptionPopupEditor.gePrivateField()
		.setSelected(Integer.parseInt(this.attachmentListPopup.getRecord(sel).get(Form.PRIVATE_ATTACHMENT).toString()) == 1 ? true : false);
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (renderer instanceof AttachmentListCellRenderer) {
			AttachmentComponent component = ((AttachmentListCellRenderer) this.attachmentListPopup.getCellRenderer()).component;
	...
	protected class JDescriptionPanelExtended extends JDescriptionPanel {
		...
		public JDescriptionPanelExtended(ResourceBundle bundle) {
			...
			this.add(this.panelButtons, new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
		...
		public void setToolTips() {
			...
			.setToolTipText(ApplicationManager.getTranslation(JDescriptionPanelExtended.SAVE_DESCRIPTION_TOOLTIP_KEY, ApplicationManager.getApplicationBundle()));
			this.buCancelSaveDescription
			.setToolTipText(ApplicationManager.getTranslation(JDescriptionPanelExtended.CANCEL_DESCRIPTION_TOOLTIP_KEY, ApplicationManager.getApplicationBundle()));
		}
