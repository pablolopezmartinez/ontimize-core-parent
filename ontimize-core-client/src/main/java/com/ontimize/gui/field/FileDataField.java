...
/**
 * This class implements a specific text data field for files. This data field permits insert files into database table, but the Entity must have established the
 * {@link TableEntity#FILE_COLUMNS} property.
 * <p>
 *
...
public class FileDataField extends TextFieldDataField implements OpenDialog {
	 ...
	 *            the file
	 * @throws Exception
	 *             when a <CODE>Thread</CODE> Exception occurs
	 */
	...
	public void setResourceBundle(ResourceBundle resource) {
		...
		if (resource != null) {
			this.selectionButton.setToolTipText(ApplicationManager.getTranslation(FileDataField.SELECTION_FILE_TIP, resource));
			this.openButton.setToolTipText(ApplicationManager.getTranslation(FileDataField.OPEN_FILE_TIP, resource));
			if (this.saveButton != null) {
				this.saveButton.setToolTipText(ApplicationManager.getTranslation(FileDataField.SAVE_FILE_TIP, resource));
			}
		}
	}
	...
	public void saveFile() {
		...
		if (this.chooserSave == null) {
			...
			this.chooserSave.addPropertyChangeListener(JFileChooser.DIRECTORY_CHANGED_PROPERTY, new PropertyChangeListener() {

				@Override
				public void propertyChange(PropertyChangeEvent e) {
					if (e.getPropertyName().equals(JFileChooser.DIRECTORY_CHANGED_PROPERTY)) {
						FileDataField.this.chooserSave.setSelectedFile(new File(FileDataField.this.chooserSave.getCurrentDirectory(), FileDataField.this.file.getFileName()));
					}
				}
			});
		}
