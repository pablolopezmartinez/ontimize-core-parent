...
public class XMLDataField extends MemoDataField {
	...
	/**
	 * The class constructor. Calls to <code>super()</code> with parameters. This field has not additional XML parameters.
	 * <p>
	 *
	 * @param parameters
	...
	public void init(Hashtable params) {
		...
		this.indentItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				((JTextComponent) XMLDataField.this.dataField).setText(XMLDataField.this.doFormat((String) XMLDataField.this.getValue()));
			}
		});
	}
		...
		if (this.isEmpty()) {
			...
			try {
				...
				if (w instanceof Dialog) {
					MessageDialog.showMessage((Dialog) w, message, e.getMessage(), JOptionPane.ERROR_MESSAGE, this.resources);
				} else {
					MessageDialog.showMessage((Frame) w, message, e.getMessage(), JOptionPane.ERROR_MESSAGE, this.resources);
				}
				return true;
			} finally {
				try {
	 ...
	 *
	 * @param line
	 *            the line to get start line offset
	 ...
	 *
	 * @param line
	 *            the line to get end line offset
	 * @return the line offset value
	 */
	...
	protected void setErrorAttributes(int offsetIni, int length) {
		try {
			...
			doc.setCharacterAttributes(offsetIni, Math.min(doc.getLength() - offsetIni, length), this.errorTextAttribute, true);
		} catch (Exception e) {
			XMLDataField.logger.error(null, e);
		}
	}
	...
	protected void setOkAttributes(int offsetIni, int lenght) {
		try {
			StyledDocument doc = ((JTextPane) this.dataField).getStyledDocument();
			doc.setCharacterAttributes(offsetIni, Math.min(doc.getLength() - offsetIni, lenght), this.oKTextAttribute, true);
		} catch (Exception e) {
			XMLDataField.logger.error(null, e);
		}
	}
			...
			protected void processKeyEvent(KeyEvent e) {
				if ((e.getKeyCode() == KeyEvent.VK_F3) && (e.getID() == KeyEvent.KEY_RELEASED)) {
					if (XMLDataField.this.dQuery == null) {
						XMLDataField.this.dQuery = new FindDialog(XMLDataField.this.parentFrame, (JTextPane) XMLDataField.this.dataField);
						...
						XMLDataField.this.dQuery.show(((JTextComponent) XMLDataField.this.dataField).getCaretPosition());
					} else {
						if (XMLDataField.this.dQuery != null) {
							XMLDataField.this.dQuery.find(((JTextComponent) XMLDataField.this.dataField).getCaretPosition());
						}
					}
					e.consume();
					return;
