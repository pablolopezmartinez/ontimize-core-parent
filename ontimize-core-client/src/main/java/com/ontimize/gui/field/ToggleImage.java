...
/**
 * This class implements a component with two states. For each state an image is associated. Size for both images must be the same. Moreover, component allows resizing (setSize(),
 * setBounds(), resize()) and internal status ('0'->OFF or '1'->ON) could be obtained from getStatus().
 *
...
public class ToggleImage extends JPanel implements FormComponent, AccessForm, IdentifiedElement, Freeable {
	...
	public ToggleImage(Hashtable parameters) {
		...
		case TOP:
			this.add(this.panelImage, new GridBagConstraints(0, 1, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			this.add(this.labelPanel, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			break;
		case LEFT:
			this.add(this.panelImage, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			this.add(this.labelPanel, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			break;
		case RIGHT:
			...
			this.add(this.labelPanel, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			break;
		default:
			this.add(this.panelImage, new GridBagConstraints(0, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			this.add(this.labelPanel, new GridBagConstraints(0, 1, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
			break;
		}
	 ...
	 * <td>no</td>
	 * <td>no</td>
	 * <td>Rotation of field. When this feature is enabled, height and width for image are interchanged.</td>
	 * </tr>
	 * <tr>
	 * <td>status</td>
	 * <td><i>on/off</td>
	...
	public Object getConstraints(LayoutManager layout) {
		...
		if (layout instanceof GridBagLayout) {
			if (this.dim == ToggleImage.NO) {
				return new GridBagConstraints(0, 0, 1, 1, 0.0001, 0, totalAlignment, GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0);
			} else {
				if ((this.dim == ToggleImage.YES) || (this.dim == ToggleImage.TEXT)) {
					return new GridBagConstraints(0, 0, 1, 1, 1, 0, totalAlignment, GridBagConstraints.HORIZONTAL, new Insets(1, 1, 1, 1), 0, 0);
				} else {
					return new GridBagConstraints(0, 0, 1, 1, 1, 0, totalAlignment, GridBagConstraints.NONE, new Insets(1, 1, 1, 1), 0, 0);
				}
			}
		} else {
			return null;
	...
	/*
	 * public String getToolTipText() { return this.getConstraints(null).toString(); } public String getToolTipText(MouseEvent e) { return this.getConstraints(null).toString(); }
	 */
	...
	public void setStatus(int status) {
		...
		if (this.panelImage != null) {
			...
			if (status == ToggleImage.ALARM) {
				if ((this.alarmThread != null) && this.alarmThread.isAlive()) {} else {
