...
/**
 * This class implements a checkable image. This one provides two level states corresponding with two integer values in database (Number and Short is allowed). So, "0" will be the
 * value obtained from getValue() while the state is disabled. When a click is registered, changes the image and state for component. A new click over the image returns the
 * component to "0" state. By default, "0" is the initial state.
 *
