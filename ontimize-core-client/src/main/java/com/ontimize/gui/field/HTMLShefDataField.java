...
public class HTMLShefDataField extends MemoDataField implements AdvancedDataComponent, ITemplateField, ActionPerformedListener {
	...
	/**
	 * Initializes parameters. Configure buttons and adds key events. XML definition could be contains an optional parameter: 'plaintextcolumn' specifying the identifier to save
		...
		this.dataField.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if ((e.getKeyCode() == KeyEvent.VK_H) && e.isControlDown() && e.isAltDown()) {
					HTMLShefDataField.this.htmlViewerAction.actionPerformed(new ActionEvent(e.getSource(), 0, "text_mode"));
				}
			}
		});
	...
	protected void createPopupMenu() {
		...
		if (this.popupMenu == null) {
			...
			this.searchMenu.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (HTMLShefDataField.this.dQuery == null) {
						HTMLShefDataField.this.dQuery = new FindDialog(HTMLShefDataField.this.parentFrame, (JTextComponent) HTMLShefDataField.this.dataField);
					...
					HTMLShefDataField.this.dQuery.show(((JTextComponent) HTMLShefDataField.this.dataField).getCaretPosition());
				}
			});
			...
			this.stylesMenu = HTMLEditorActionFactory.createMenu(lst, ApplicationManager.getTranslation("HTMLShef.styles", this.resources));
			...
			this.tableMenu.add(HTMLEditorActionFactory.createMenu(lst, ApplicationManager.getTranslation("HTMLShef.insert", this.resources)));

			lst = HTMLEditorActionFactory.createDeleteTableElementActionList();
			if (this.actionList != null) {
				this.actionList.addAll(lst);
			}
			this.tableMenu.add(HTMLEditorActionFactory.createMenu(lst, ApplicationManager.getTranslation("HTMLShef.delete", this.resources)));
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if (evt.getPropertyName().equals("selected")) {
					if (evt.getNewValue().equals(Boolean.TRUE)) {
						HTMLShefDataField.this.paragraphCombo.removeActionListener(HTMLShefDataField.this.paragraphComboHandler);
						HTMLShefDataField.this.paragraphCombo.setSelectedItem(evt.getSource());
						HTMLShefDataField.this.paragraphCombo.addActionListener(HTMLShefDataField.this.paragraphComboHandler);
					}
				}
			}
		};
	...
	*/
	...
	protected void changePanelButtons() {
		if ((this.buttonsPanel != null) && (this.buttonsPanel.getComponentCount() > 0)) {
			for (int i = 0; i < this.buttonsPanel.getComponentCount(); i++) {
				Component c = this.buttonsPanel.getComponent(i);
				if (c instanceof AbstractButton) {
					this.changeButton((AbstractButton) c, this.borderbuttons, this.opaquebuttons, this.listenerHighlightButtons);
				}
			}
		}
	}
			...
			protected void processKeyEvent(KeyEvent e) {
				if ((e.getKeyCode() == KeyEvent.VK_F3) && (e.getID() == KeyEvent.KEY_RELEASED)) {
					if (HTMLShefDataField.this.dQuery == null) {
						...
						if (HTMLShefDataField.this.dQuery != null) {
							HTMLShefDataField.this.dQuery.find(((JTextComponent) HTMLShefDataField.this.dataField).getCaretPosition());
						}
					}
					e.consume();
					return;
	...
	public boolean isEmpty() {
		try {
			if ((((JTextComponent) this.dataField).getDocument().getLength() == 0) || ((JTextComponent) this.dataField).getDocument()
					.getText(0, ((JTextComponent) this.dataField).getDocument().getLength()).equalsIgnoreCase("\n") || (((JTextComponent) this.dataField).getDocument()
							.getText(0, ((JTextComponent) this.dataField).getDocument().getLength()).trim().length() == 0)) {
				return true;
			}
			return false;
	...
	protected class FontChangeHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == HTMLShefDataField.this.fontFamilyCombo) {
				HTMLDocument document = (HTMLDocument) ((JTextComponent) HTMLShefDataField.this.dataField).getDocument();
				CompoundUndoManager.beginCompoundEdit(document);

				if (HTMLShefDataField.this.fontFamilyCombo.getSelectedIndex() != 0) {
					HTMLUtils.setFontFamily((JEditorPane) HTMLShefDataField.this.dataField, HTMLShefDataField.this.fontFamilyCombo.getSelectedItem().toString());
				} else {
