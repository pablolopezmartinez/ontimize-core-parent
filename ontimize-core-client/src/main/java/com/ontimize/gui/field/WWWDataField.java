...
public class WWWDataField extends TextDataField {
	...
	public static void processURL(String url) {
		...
		if (url.indexOf('@') != -1) {
			...
			// htm
			// then add /
			if (url.substring(url.length() - 3, url.length()).equalsIgnoreCase("htm") || url.substring(url.length() - 4, url.length()).equalsIgnoreCase("html")) {
				url = url + "/";
			}
			...
			if (urlEncoded.substring(0, 7).equalsIgnoreCase("http://") || urlEncoded.substring(0, 8).equalsIgnoreCase("https://")) {
