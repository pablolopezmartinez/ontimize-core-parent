 ...
 * <p>
 *
 * @see @see <a href= "http://java.sun.com/products/java-media/jai/forDevelopers/jai-apidocs/index.html" ><code>Java[tm] Advanced Imaging API Documentation</code></a>
...
public class JAIImageDataField extends DataField {
			...
			public void mouseClicked(MouseEvent e) {
				if ((e.getClickCount() == 2) && (!JAIImageDataField.this.isEmpty())) {
					if (JAIImageDataField.this.dZoom == null) {
						...
						JAIImageDataField.this.dZoom.getContentPane().add(new JScrollPane(JAIImageDataField.this.displayZoom));
					}
					JAIImageDataField.this.displayZoom.set(((JImageDisplayJAI) JAIImageDataField.this.dataField).getSource());
	 ...
	 *
	 * @param parameters
	 *            the <code>Hashtable</code> with parameters. The next parameter are added:
	 *
	 *            <p>
	 *
	...
	public void init(Hashtable parameters) {
		...
		this.add(this.scroll, new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, this.weightDataFieldH, 0.0, GridBagConstraints.EAST, this.redimensJTextField,
	...
	public Object getValue() {
		...
		if ((this.value != null) && ApplicationManager.DEBUG) {
			JAIImageDataField.logger.debug(this.getClass().toString() + " : " + this.getAttribute() + " -> Image size in bytes: " + this.value.getBytes().length);
		}
		return this.value;
	}
	...
	public void setImagen(RenderedImage im) throws Exception {
		...
		try {
			...
			if (ApplicationManager.DEBUG) {
				JAIImageDataField.logger.debug(this.getClass().toString() + " : " + this.getAttribute() + " -> setValue : Image size in bytes: " + this.value.getBytes().length);
			}
			in = new com.sun.media.jai.codec.ByteArraySeekableStream(this.value.getBytes());
			((JImageDisplayJAI) this.dataField).set(JAIUtils.loadImage((com.sun.media.jai.codec.ByteArraySeekableStream) in));
	...
	public void setValue(Object value) {
		if (ApplicationManager.DEBUG) {
			JAIImageDataField.logger.debug(this.getClass().toString() + " " + this.getAttribute() + " Set Value with : " + value);
		}
		if (value == null) {
			this.deleteData();
			return;
			...
			try {
				...
				if (ApplicationManager.DEBUG) {
					JAIImageDataField.logger
							.debug(this.getClass().toString() + " : " + this.getAttribute() + " -> Set Value : Image size (bytes): " + this.value.getBytes().length);
				}
				in.close();
