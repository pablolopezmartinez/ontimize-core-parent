...
public class OImageView extends View {
	 ...
	 * <p>
	 * This is set to false as swing does not offer a repaint that takes a delay. If this were true, a bunch of immediate repaints would get generated that end up significantly
	 * delaying the loading of the image (or anything else going on for that matter).
	 */
	...
	/**
	 * Bitmask containing some of the above bitmask values. Because the image loading notification can happen on another thread access to this is synchronized (at least for
	 * modifying it).
	 */
	protected int state;
	...
	/**
	 * Sets how the image is loaded. If <code>newValue</code> is true, the image we be loaded when first asked for, otherwise it will be loaded asynchronously. The default is to
	...
	public void paint(Graphics g, Shape a) {
		...
		if (image != null) {
			...
			if ((view != null) && (((this.state & OImageView.WIDTH_FLAG) == 0) || (this.width > OImageView.DEFAULT_WIDTH))) {
				// Assume layout along the y direction
				Rectangle altRect = new Rectangle(rect.x + this.leftInset + OImageView.DEFAULT_WIDTH, rect.y + this.topInset,
						rect.width - this.leftInset - this.rightInset - OImageView.DEFAULT_WIDTH, rect.height - this.topInset - this.bottomInset);
	...
	protected void paintBorder(Graphics g, Rectangle rect) {
		...
		if (((this.borderSize > 0) || (this.image == null)) && (color != null)) {
			...
			for (int counter = 0; counter < n; counter++) {
				g.drawRect(rect.x + xOffset + counter, rect.y + yOffset + counter, rect.width - counter - counter - xOffset - xOffset - 1,
	 ...
	 * @return the span the view would like to be rendered into; typically the view is told to render into the span that is returned, although there is no guarantee; the parent may
	...
	/**
	 * Determines the desired alignment for this view along an axis. This is implemented to give the alignment to the bottom of the icon along the y axis, and the default along the
	 * x axis.
	 *
	 * @param axis
	 *            may be either X_AXIS or Y_AXIS
	 * @return the desired alignment; this should be a value between 0.0 and 1.0 where 0 indicates alignment at the origin and 1.0 indicates alignment to the full span away from
	...
	public void setSize(float width, float height) {
		...
		if (this.getImage() == null) {
			...
			if (view != null) {
				view.setSize(Math.max(0f, width - (OImageView.DEFAULT_WIDTH + this.leftInset + this.rightInset)), Math.max(0f, height - (this.topInset + this.bottomInset)));
			}
		}
	}
	...
	protected void refreshImage() {
		synchronized (this) {
			...
			this.state = (this.state | OImageView.LOADING_FLAG | OImageView.RELOAD_IMAGE_FLAG | OImageView.WIDTH_FLAG | OImageView.HEIGHT_FLAG) ^ (OImageView.WIDTH_FLAG | OImageView.HEIGHT_FLAG | OImageView.RELOAD_IMAGE_FLAG);
			this.image = null;
	...
	/**
	 * Loads the image from the URL <code>getImageURL</code>. This should only be invoked from <code>refreshImage</code>.
	 */
	...
	protected class ImageHandler implements ImageObserver {
		...
		public boolean imageUpdate(Image img, int flags, int x, int y, int newWidth, int newHeight) {
			if ((OImageView.this.image == null) || (OImageView.this.image != img) || (OImageView.this.getParent() == null)) {
				return false;
			}
			...
			short changed = 0;
			if (((flags & ImageObserver.HEIGHT) != 0) && !OImageView.this.getElement().getAttributes().isDefined(HTML.Attribute.HEIGHT)) {
				changed |= 1;
			}
			if (((flags & ImageObserver.WIDTH) != 0) && !OImageView.this.getElement().getAttributes().isDefined(HTML.Attribute.WIDTH)) {
				changed |= 2;
			}
	...
	/**
	 * ImageLabelView is used if the image can't be loaded, and the attribute specified an alt attribute. It overriden a handle of methods as the text is hardcoded and does not
