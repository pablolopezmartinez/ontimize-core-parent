...
/**
 * An HTML editor kit which can properly draw borderless tables and allows for resizing of tables and images.
 *
 * @author Imatia S.L.
 *
 */
public class OHTMLEditorKit extends HTMLEditorKit {
		...
		public View create(Element elem) {
			...
			if (o instanceof HTML.Tag) {
				...
				if (kind == HTML.Tag.TABLE) {
					...
					return ov;
				} else if ((kind instanceof HTML.UnknownTag) || (kind == HTML.Tag.TITLE) || (kind == HTML.Tag.META) || (kind == HTML.Tag.LINK) || (kind == HTML.Tag.STYLE) || (kind == HTML.Tag.SCRIPT) || (kind == HTML.Tag.AREA) || (kind == HTML.Tag.MAP) || (kind == HTML.Tag.PARAM) || (kind == HTML.Tag.APPLET)) {
					return new UnknownElementView(elem);
				}
			}
	...
	protected class ResizeHandler extends MouseInputAdapter {
		...
		/**
		 * Updates the list of monitored ResizeableViews. If they don't exist in the document, they're removed from the list.
		 *
		 * @param doc
		 */
	 ...
	 * <p>
	 * The default view for such {@link Element}s are big ugly blocky JTextFields, which doesn't really work well in practice and tends to confuse users. This view replaces the
	 * default and simply draws the elements as blocks that indicates the presence of an uneditable {@link Element}.
	 * <p>
	 *
	 * @author Imatia S.L.
	 *
