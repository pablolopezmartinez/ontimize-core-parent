...
public class Label extends JPanel implements FormComponent, IdentifiedElement, AccessForm, Freeable {
	...
	protected class EJLabel extends JLabel {
		 ...
		 *
		 * @param u
		 *            the condition to set underlined.
		 */
	 ...
	 *
	 * @param parameters
	 *            a <code>Hashtable</code> with label parameters
	 */
	...
	/**
	 * This method gets the <code>Hashtable</code> and initializes the label with parameters specified, also aligns the label.
	 * <p>
	 *
	 * @param parameters
	 ...
	 *            <td>no</td>
	 *            <td>no</td>
	 *            <td>The resize possibilities (no resize or resize, values yes and text are the same but both of them exist to maintain the same possibilities that other fields
	 *            accept).</td>
	 *            </tr>
	 *            </Table>
	 */
	...
	public void init(Hashtable parameters) {
		...
		if (margin != null) {
			try {
				...
				this.label.setBorder(new EmptyBorder(Label.DEFAULT_TOP_MARGIN, Label.DEFAULT_LABEL_LEFT_MARGIN, Label.DEFAULT_BOTTOM_MARGIN, Label.DEFAULT_LABEL_RIGHT_MARGIN));
			}
		} else {
			this.label.setBorder(new EmptyBorder(Label.DEFAULT_TOP_MARGIN, Label.DEFAULT_LABEL_LEFT_MARGIN, Label.DEFAULT_BOTTOM_MARGIN, Label.DEFAULT_LABEL_RIGHT_MARGIN));
		}
	...
	public Object getConstraints(LayoutManager parentLayout) {
		if (parentLayout instanceof GridBagLayout) {
			...
			return new GridBagConstraints(0, 0, 1, 1, this.dim, 0, totalAlignment, GridBagConstraints.NONE,
					new Insets(Label.DEFAULT_PARENT_MARGIN, Label.DEFAULT_PARENT_MARGIN, Label.DEFAULT_PARENT_MARGIN, Label.DEFAULT_PARENT_MARGIN), 0, 0);
		} else {
			return null;
		}
	}
	 ...
	 *
	 * @param visible
	 *            the condition of visualization
	 */
	@Override
	...
	protected boolean checkEnabledPermission() {
		...
		if (manager != null) {
			if (this.permissionsActived == null) {
				if ((this.attribute != null) && (this.parentForm != null)) {
					this.permissionsActived = new FormPermission(this.parentForm.getArchiveName(), "enabled", this.attribute.toString(), true);
				}
			}
			try {
				// Check to show
