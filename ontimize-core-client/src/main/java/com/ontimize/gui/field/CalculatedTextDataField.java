...
/**
 * This class supports a concatenation of data fields obtained with {@link DataField}{@link #getText()}.
 * <p>
 *
...
public class CalculatedTextDataField extends TextDataField implements ValueChangeListener {
	 ...
	 *
	 * @param parameters
	 *            the hashtable with parameters. The next parameters are added:
	 *
	 *            <p>
	 *
	...
	public void valueChanged(ValueEvent e) {
						...
						if (c != null) {
							if (((DataField) c).isEmpty()) {
								if (ApplicationManager.DEBUG) {
									CalculatedTextDataField.logger.debug(this.getClass().toString() + " : Field " + c.getAttribute() + " is empty");
								}
								continue;
							} else {
		...
		if (ApplicationManager.DEBUG) {
			CalculatedTextDataField.logger.debug(this.getClass().toString() + " : ValueChanged: " + e.getNewValue() + " before: " + e.getOldValue());
		}
