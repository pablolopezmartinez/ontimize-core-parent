...
/**
 * Class to implement a component for signature introduction. It is formed by a label (which shows the title), a SignLabel (JLabel which stores the sign) and a JButton to clean the
 * signature.
 *
...
public class SignDataField extends JPanel implements DataComponent, AccessForm, ValueChangeDataComponent, Freeable {
	 ...
	 *
	 * @param params
	 *            A Hashtable which contains the initialize parameters
	 * @throws Exception
	 */
	 ...
	 *
	 * @param res
	 *            This resource bundle contains all application translations
	 */
	@Override
	...
	public void init(Hashtable params) throws Exception {
		...
		if (this.labelView) {
			if (this.labelPosition != BorderLayout.EAST) {
				...
				panel.add(this.dataField, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 10, 0, 10), 0, 0));
		...
		this.labelText = ParseUtils.getString((String) params.get(SignDataField.TEXT_STR), (String) this.getAttribute());
		...
		this.dataClean.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				SignDataField.this.fireValueChanged(SignDataField.this.getValue(), oPreviousValue, ValueEvent.USER_CHANGE);
			}
		});
		...
		this.dataField.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				if (SignDataField.this.dataField.isEnabled()) {
					...
					try {
						SignDataField.this.dataField.bufferedSignLabel = SignLabel.jcomponentToImage(SignDataField.this.dataField);
	...
	/**
	 * Initialize the component's permissions, setting the interaction and visibility, checking with the users permission
	 */
	@Override
	 ...
	 *
	 * @return <code>true</code> The user has permission to interact with the component <code>false</code> Otherwise
	 */
		...
		if (manager != null) {
			if (this.permissionEnabled == null) {
				if ((this.attribute != null) && (this.parentForm != null)) {
					this.permissionEnabled = new FormPermission(this.parentForm.getArchiveName(), "enabled", this.attribute.toString(), true);
				}
			}
			try {
				if (this.permissionEnabled != null) {
	 ...
	 *
	 * @return <code>true</code> The user has permission to view the component <code>false</code> Otherwise
	 */
	 ...
	 *
	 * @param parentLayout
	 *            The LayoutManager of the parentLayout
	...
	public Object getConstraints(LayoutManager parentLayout) {
		return new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.BOTH,
				new Insets(SignDataField.DEFAULT_PARENT_MARGIN, SignDataField.DEFAULT_PARENT_MARGIN, SignDataField.DEFAULT_PARENT_MARGIN, SignDataField.DEFAULT_PARENT_MARGIN), 0,
				0);
	}
	 ...
	 *
	 * @return <code>true</code> The component is restricted <code>false</code> The component is not restricted
	 */
	@Override
	...
	public static class SignLabel extends JLabel {
		 ...
		 *
		 * @param ba
		 *            The bytes array of the signature
		 ...
		 *
		 * @param component
		 *            The component of signature field
	 ...
	 *
	 * @param newValue
	 *            the new value of SignLabel
	 * @param oldValue
	 *            the old value of SignLabel
	 * @param type
	 *            Integer indicating who made the changes , if the user or application
	 */
	 ...
	 *
	 * @return <code>true</code>The component was modified <code>false</code>The component was not modified
	 */
	@Override
	public boolean isModified() {
		...
		if ((oValue == null) && (this.valueSave != null)) {
			SignDataField.logger.debug("Component: {} Modified. Previous value = {} New value = {}", this.attribute, this.valueSave, oValue);
			return true;
		}
		if ((oValue != null) && (this.valueSave == null)) {
			SignDataField.logger.debug("Component: {} Modified. Previous value = {} New value = {}", this.attribute, this.valueSave, oValue);
