...
public class CalculatedExpressionDataField extends JCalculatedExpression implements FormComponent, AccessForm, IdentifiedElement, DataComponent, ValueChangeDataComponent, Freeable {
	...
	public Object getConstraints(LayoutManager parentLayout) {
		if (parentLayout instanceof GridBagLayout) {
			return new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0);
		} else {
			return null;
		}
	}
	...
	public void init(Hashtable parameters) throws Exception {
		...
		if (bgcolor != null) {
			try {
				...
				CalculatedExpressionDataField.logger.error(this.getClass().toString() + " Error 'bgcolor' parameter:" + e.getMessage(), e);
			}
		}
	...
	/**
	 * This method returns a list of <code>{@link SelectableItem}</code> objects from a specified list of field names. Each {@link SelectableItem} includes the name of the
	 * available field and a check field used to configure the component.
	 *
	 * @param fields
	 *            List of available fields names.
	 ...
	 *            List with the fields to set required.
	 */
	...
	/**
	 * This method check if the current expression of the editor is well defined. If it is well defined the method returns <i>true</i>, if not, the method returns <i>false</i>.
	 *
	 * @return
	 */
	...
	public boolean isModified() {
		...
		if ((oValue == null) && (this.valueSave != null)) {
			if (ApplicationManager.DEBUG) {
				CalculatedExpressionDataField.logger.debug("Component: " + this.attribute + " Modified: Previous value = " + this.valueSave + " New value = " + oValue);
			}
			return true;
		}
		if ((oValue != null) && (this.valueSave == null)) {
	...
	protected class InnerDocumentListener implements DocumentListener {
		...
		public void insertUpdate(DocumentEvent e) {
			if (this.innerListenerEnabled) {
				CalculatedExpressionDataField.this.fireValueChanged(CalculatedExpressionDataField.this.getValue(), CalculatedExpressionDataField.this.valueSave,
