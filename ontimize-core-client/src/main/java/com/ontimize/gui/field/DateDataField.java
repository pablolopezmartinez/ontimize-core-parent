...
/**
 * The main class for creating a field to introduce a date. It adds the parameter <code>calendar</code>. that indicates if a dialog box with the calendar to set the date.
 * <p>
 *
...
public class DateDataField extends TextFieldDataField implements OpenDialog, Freeable, AdvancedDataComponent {
	 ...
	 *
	 * @param parameters
	 *            a <code>Hashtable</code> with <CODE>calendar</CODE> and <CODE>calendarchange</CODE> parameters
	 */
	...
	/**
	 * This method gets the <code>Hashtable</code> and creates the calendar, the access button and implements the listeners.
	 * <p>
	 *
	 * @param parameters
	 ...
	 *            <td>no</td>
	 *            <td>no</td>
	 *            <td>Indicates the form to notify the change event in calendar values. By default, its value is 'no' and so, the changes are only showed when you abandon the
	 *            calendar.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>calendaricon</td>
	...
	public void init(Hashtable parameters) {
		...
		if (this.mainCalendar != null) {
			ImageIcon calendarIcon = ParseUtils.getImageIcon((String) parameters.get("calendaricon"), ImageManager.getIcon(ImageManager.CALENDAR));
	 ...
	 *
	 * @param c
	 *            Component to create graphical representation and user interaction
	 */
	...
	/**
	 * Sets the field value by user when <CODE>auxValue</CODE> is a instance of java.sql.timestamp or java.util. During the method execution the inner Listener is disabled.
	 * <p>
	 *
	...
	/**
	 * Sets the field value by program when <CODE>auxValue</CODE> is a instance of java.sql.timestamp or java.util. During the method execution the inner Listener is disabled.
	 * <p>
	 *
			...
			if (auxValue instanceof Date) {
				...
				if (auxValue instanceof SearchValue) {
					if (((JTextField) this.dataField).getDocument() instanceof AdvancedDateDocument) {
						AdvancedDateDocument document = (AdvancedDateDocument) ((JTextField) this.dataField).getDocument();
			...
			for (int i = 0; i < list.length; i++) {
				if (list[i].equals(loc)) {
					...
					if (list[i].getCountry().equals(loc.getCountry())) {
						...
						if ((sameCountryLocale == null) || ((list[i].getLanguage() != null) && list[i].getCountry().toUpperCase().equals(list[i].getLanguage().toUpperCase()))) {
	...
	public boolean isModified() {
		...
		if ((oValue == null) && (this.valueSave != null)) {
			if (ApplicationManager.DEBUG) {
				DateDataField.logger.debug("Component: " + this.attribute + " Modified: Previous value = " + this.valueSave + " New value = " + oValue);
			}
			return true;
		}
		if ((oValue != null) && (this.valueSave == null)) {
	...
	protected void createPopupMenu() {
		if (this.popupMenu == null) {
			...
			this.advancedHelpBMenu.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						HelpUtilities.showDefaultAdvancedHelpDialog(SwingUtilities.getWindowAncestor(DateDataField.this), DateDataField.this.advancedHelpBMenu.getText(),
								DateDataField.this.locale);
					} catch (Exception ex) {
						DateDataField.logger.trace(null, ex);
						DateDataField.this.parentForm.message("datafield.help_files_cannot be displayed", Form.ERROR_MESSAGE, ex);
					}
				}
			});
		}
	...
	/**
	 * @param highlight
	 *            whether or not the emphasizeInvalidDate is enabled
	 */
	...
	/**
	 * Fill the field with the the specified day and the actual month and year. If the given day is higher than the last day of actual month, the field will be cleared.
	 *
	 * @param day
	 *            The specific day.
	 */
	...
	/**
	 * Fill the field with the the specified day and month and the actual year. If the given month is higher than 12, the field will be cleared.
	 *
	 * @param day
	 *            The specific day.
	 * @param month
