...
public class PasswordDataField extends TextFieldDataField implements Freeable {
	...
	protected class SecurityKeyListener extends KeyAdapter {
		...
		public void keyReleased(KeyEvent e) {
			if (!((PasswordDocument) ((JTextField) PasswordDataField.this.dataField).getDocument()).isPatternMatches()) {
				...
				PasswordDataField.this.popupTipSecurity.show(PasswordDataField.this.dataField,
						PasswordDataField.this.dataField.getWidth() - PasswordDataField.this.popupTipSecurity.getWidth(), PasswordDataField.this.dataField.getHeight());
	...
	protected class SecurityFocusListener extends FocusAdapter {
		...
		public void focusGained(FocusEvent e) {
			if ((((JTextField) PasswordDataField.this.dataField).getDocument()
					.getLength() > 0) && !((PasswordDocument) ((JTextField) PasswordDataField.this.dataField).getDocument()).isPatternMatches()) {
				PasswordDataField.this.popupTipSecurity.pack();
				PasswordDataField.this.popupTipSecurity.show(PasswordDataField.this.dataField,
						PasswordDataField.this.dataField.getWidth() - PasswordDataField.this.popupTipSecurity.getWidth(), PasswordDataField.this.dataField.getHeight());
				PasswordDataField.this.popupTipSecurity.setAlwaysOnTop(true);
	...
	protected String setTextSecurityTooltip() {
		...
		String tooltip = ApplicationManager.getTranslation(security_tooltip_lv, bundle,
				new String[] { ApplicationManager.getTranslation("security_tooltip.character", bundle, new String[] { "" + (level + 4) + "" }), ApplicationManager
						.getTranslation("security_tooltip.uppercase", bundle), ApplicationManager.getTranslation("security_tooltip.lowercase", bundle), ApplicationManager
						.getTranslation("security_tooltip.number", bundle), ApplicationManager.getTranslation("security_tooltip.symbol", bundle), });
		return tooltip;
	}
	 ...
	 * <td><i>yes</td>
	 * <td>no</td>
	 * <td>Indicates if the field has a button to check the security of the password. Only available if 'securitylevel' is enable or greater than 0</td>
	 * </tr>
	 * </table>
	 */
	@Override
		...
		if (oSecurityLevel != null) {
			...
			if ((oSecurityButton != null) && (!DataField.parseBoolean(oSecurityButton.toString(), true))) {
				...
				if (this.secureLevel > 0) {
					...
					PasswordDataField.logger.debug("Security level is {}. Security button is not enabled", this.secureLevel);
				}
			}
		} else {
	...
	protected void createSecurityButton() {
		...
		this.securityButton.setToolTipText(ApplicationManager.getTranslation("passworddatafield.security_field_no", ApplicationManager.getApplicationBundle()));
		...
		this.securityButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (PasswordDataField.this.validPassword) {
					message = ApplicationManager.getTranslation("passworddatafield.security_field_yes", ApplicationManager.getApplicationBundle());
				} else {
					message = ApplicationManager.getTranslation("passworddatafield.security_field_no", ApplicationManager.getApplicationBundle());
				}
				Toast.showMessage(((FieldButton) e.getSource()).getParent(), message, ApplicationManager.getApplicationBundle(), 1000);
			}
		});
	...
	protected void checkPasswordSecurity(DocumentEvent e) {
		...
		this.validPassword = ((PasswordDocument) ((JTextField) this.dataField).getDocument()).checkPattern(password, this.patternLevels.get(this.getSecureLevel()));
		if (this.validPassword) {
			...
			if (this.securityButton != null) {
				...
				this.securityButton.setToolTipText(ApplicationManager.getTranslation("passworddatafield.security_field_yes", ApplicationManager.getApplicationBundle()));
			}
		} else {
	...
	public Object getValue() {
		...
		if (!this.encrypt) {
			...
			return PasswordDataField.encryptPassword(((PasswordDocument) ((JTextField) this.dataField).getDocument()).getContain());
		}
	}

	@Override
	public boolean isEmpty() {
		if (this.isPasswordSecurity() && !((PasswordDocument) ((JTextField) this.dataField).getDocument()).isPatternMatches()) {
			return true;
		} else {
