...
public class TextArea extends JTextArea implements FormComponent, Freeable {
	...
	public Object getConstraints(LayoutManager parentLayout) {
		if (parentLayout instanceof GridBagLayout) {
			return new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(5, 5, 5, 5), 0, 0);
		} else {
			return null;
		}
	}
	...
	public void init(Hashtable parameters) {
		...
		if (fontcolor != null) {
			try {
				...
				TextArea.logger.error(this.getClass().toString() + " Error in parameter 'fontcolor':" + e.getMessage(), e);
			}
		}
	}
