...
/**
 * This class creates a component with linkeable components. Links are created with <code>html</code> code.
 * <p>
 *
...
public class LinkComponent extends IdentifiedAbstractFormComponent {
	...
	/**
	 * Creates the component. Creates the {@link JEditorPane}, {@link LinkComponent}{@link #setLayout(LayoutManager)} and adds scroll.
	 */
	 ...
	 *
	 * @param parameters
	 *            the <code>Hashtable</code> with parameters. Possible values for <code>Hashtable</code> are:
	 *            <p>
	 *
	 *            <TABLE BORDER=1 CELLPADDING=3 CELLSPACING=1 RULES=ROWS FRAME=BOX>
	 *
		...
		if (stLabels != null) {
			while (stLabels.hasMoreTokens()) {
				...
				if ((stLinks != null) && stLinks.hasMoreTokens()) {
					...
					sContent = sContent.concat("<A HREF=\"" + sLinkText + "\">" + ApplicationManager.getTranslation(sLinkText, this.bundle) + "</A>");
		...
		if (stLinks != null) {
			while (stLinks.hasMoreTokens()) {
				...
				sContent = sContent.concat(
						"<A HREF=\"" + ApplicationManager.getTranslation(sLinkText, this.bundle) + "\">" + ApplicationManager.getTranslation(sLinkText, this.bundle) + "</A>");
				if ((stTables != null) && stTables.hasMoreTokens()) {
					String k = stTables.nextToken();
					this.tableVector.add(k.toString());
				}
				...
				public void hyperlinkUpdate(HyperlinkEvent hEvent) {
					if (LinkComponent.this.enabled) {
						if (hEvent.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
							...
							if ((pos < LinkComponent.this.tableVector.size()) && (pos < LinkComponent.this.insertModeVector.size())) {
								Table tSource = (Table) LinkComponent.this.parentForm.getElementReference((String) LinkComponent.this.tableVector.get(pos));
								if (tSource != null) {

									if (((String) LinkComponent.this.insertModeVector.get(pos)).equalsIgnoreCase("yes")) {
										...
										if (tSource.getSelectedRows().length == 1) {
											...
											LinkComponent.this.parentForm.message("LinkComponent.SelectTableRecord", Form.INFORMATION_MESSAGE);
										}
									}
								} else {
									LinkComponent.this.parentForm.message("LinkComponent.TableNotFound", Form.INFORMATION_MESSAGE);
								}
							}
						}
					}
		...
		while (i < this.labelVector.size()) {
			...
			if (j < this.linkVector.size()) {
				...
				if (this.enabled) {
					...
					sContent = sContent.concat("<A COLOR=\"000000\" HREF=\"" + sLinkText + "\">" + ApplicationManager.getTranslation(sLinkText, this.bundle) + "</A>");
				}
				j += 1;
			}
			i += 1;
	...
	public Object getConstraints(LayoutManager parentLayout) {
		if (parentLayout instanceof GridBagLayout) {
			return new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(1, 1, 1, 1), 0, 0);
		} else {
			return null;
		}
	}
