...
/**
 * This class implements a sensitive hyperlink label. It allows developers to open a detail form from a normal field (in this case a label) which is inserted in the source form.
 * Several parameters can be set to show different layouts, such as, icon, background color, underlined option, and so on.
 * <p>
 * s
 *
...
public class HyperlinkLabel extends Label implements HyperlinkComponent {
	...
	protected void activateLink() {
		...
		if (this.action == null) {
			this.action = new CreateFormInDialog(this.formsManagerName, this.formName, this.keyTitle, this.equivalences) {
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>The title of form. Do not confuse with the text parameter, which is the text for this label</td>
	 *            </tr>
	 *
	 *            <tr>
	 *            <td>underlined</td>
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>The equivalences between navigation form fields and the form where component is placed. For instance, it can be useful when alias are used<br>
