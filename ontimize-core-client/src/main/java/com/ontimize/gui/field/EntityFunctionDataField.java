...
/**
 * This class implements a data field that can make a query against a entity different to the form entity <br>
 * The query is like:<br>
 * <b>select fuction(functioncolum) as attr from (select cols from entity.table where parentkeys=parentkeyvalues) as vTemp</b><br>
 * It is useful to get an operation result against related data in other entity, for example:<br>
 * In a form with information about a person and his account number it is possible to use this data field to calculate the current balance using all movements information.
 */
public class EntityFunctionDataField extends TextDataField {
	 ...
	 *            If type=number it is possible to use RealDataField parameters to configure the number format<br>
	 *            These parameters are: minintegerdigits,maxintegerdigits,mindecimaldigits ,maxdecimaldigits,grouping</td>
	 *            </tr>
	 *            <tr>
	 *            <td>pattern</td>
	 *            <td></td>
	 ...
	 *            <td>no</td>
	 *            <td>no</td>
	 *            <td>Indicates if it is necessary to use NullValue in the query filter if some of the filter column values are null</td>
	 *            </tr>
	 *            <tr>
	 ...
	 *            <td>yes</td>
	 *            <td>no</td>
	 *            <td>Indicates if it is necessary to use the form entity key as a filter column in the query</td>
	 *            </tr>
	 *            </table>
	 */
	@Override
	public void init(Hashtable parameters) {
		...
		if (entity == null) {
			if (com.ontimize.gui.ApplicationManager.DEBUG) {
				EntityFunctionDataField.logger.debug("Parameter 'entity' not found in EntityFunctionDataField. Check parameters.");
			}
		} else {
		...
		if (function == null) {
			if (com.ontimize.gui.ApplicationManager.DEBUG) {
				EntityFunctionDataField.logger.debug("Parameter 'function' not found in EntityFunctionDataField. Check parameters.");
			}
		} else {
		...
		this.entityAttribute = new EntityFunctionAttribute((String) parameters.get("attr"), entityName, columnName, filterColumns, functionName);
	...
	protected void configureRealField(Hashtable parameters) {
		...
		if (minintegerdigits != null) {
			try {
				...
				EntityFunctionDataField.logger.error(this.getClass().toString() + " Error in parameter 'minintegerdigits' " + e.getMessage(), e);
			}
		}
		...
		if (maxintegerdigits != null) {
			try {
				...
				EntityFunctionDataField.logger.error(this.getClass().toString() + " Error in parameter 'maxintegerdigits' " + e.getMessage(), e);
			}
		}
		...
		if (mindecimaldigits != null) {
			try {
				...
				EntityFunctionDataField.logger.error(this.getClass().toString() + "Error in parameter 'mindecimaldigits' " + e.getMessage(), e);
			}
		}
		...
		if (maxdecimaldigits != null) {
			try {
				...
				EntityFunctionDataField.logger.error(this.getClass().toString() + "Error in parameter 'maxdecimaldigits' " + e.getMessage(), e);
			}
		}
