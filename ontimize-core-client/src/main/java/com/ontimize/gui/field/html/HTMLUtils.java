...
public class HTMLUtils {
	 ...
	 *
	 * @param enclTag
	 *            the tag to enclose the HTML in
	 * @param innerHTML
	...
	/**
	 * Incloses a chunk of HTML text in the specified tag with the specified
	 * attribs
	 *
	 * @param enclTag
	 * @param set
	...
	/**
	 * Searches for a list {@link Element} that is the parent of the specified
	 * {@link Element}.
	 *
	 * @param elem
	 * @return A list element (UL, OL, DIR, MENU, or DL) if found, null
	 *         otherwise
	 */
	public static Element getListParent(Element elem) {
		Element parent = elem;
		while (parent != null) {
			if (parent.getName().toUpperCase().equals("UL") || parent.getName().toUpperCase().equals("OL") || parent.getName().equals("dl") 
	...
	/**
	 * Gets the element one position less than the start of the specified
	 * element
	 *
	 * @param doc
	 * @param el
	...
	/**
	 * Gets the element one position greater than the end of the specified
	 * element
	 *
	 * @param doc
	 * @param el
	...
	/**
	 * Inserts a string of html into the {@link JEditorPane}'s
	 * {@link HTMLDocument} at the current caret position.
	 *
	 * @param html
	 * @param tag
	 * @param editor
	...
	public static void insertHTML(String html, HTML.Tag tag, JEditorPane editor) {
		...
		try {
			if (breakParagraph && beginParagraph) {
				...
				/*
				 * Trick: insert a non-breaking space after start, so that we're inserting into the middle of a line. Then, remove the space. This works around a bug when using
				 * insertHTML near the beginning of a paragraph.
				 */
	 ...
	 * <p>
	 * If there is no selection, the character attributes at caretPos - 1 are
	 * retuned. If there is a slection, the attributes at selectionEnd - 1 are
	 * returned
	 * </p>
	 *
	 * @param editor
	 * @return An {@link AttributeSet} or null, if the editor doesn't have a
	 *         {@link StyledDocument}
	 */
	...
	/**
	 * Gets the font family name at the {@link JEditorPane}'s current caret
	 * position
	 *
	 * @param editor
	...
	/**
	 * Set's the font family at the {@link JEditorPane}'s current caret positon,
	 ...
	 * {@link JEditorPane}'s current caret position or selection.
	 * <p>
	 * The val parameter is a {@link String} even though the actual attribute
	 * value is not. This is because the actual attribute values are not public.
	 * Thus, this method checks the value via the toString() method
	 * </p>
	 *
	 * @param editor
	 * @param atr
	...
	/**
	 * Removes a single character attribute from the editor's current
	 * position/selection.
	 *
	 * <p>
	 * Removes from the editor kit's input attribtues and/or document at the
	 * caret position. If there is a selction the attribute is removed from the
	 * selected text
	 * </p>
	 *
	 * @param editor
	 ...
	 * @param attrs
	 * @param replace
	 *            if true, replaces the attrubutes
	 */
	...
	/**
	 * Converts an html tag attribute list to a {@link Map}. For example, the
	 * String 'href="http://blah.com" target="_self"' becomes name-value
	 * pairs:<br>
	...
	/**
	 * Removes self-closing tags from xhtml for the benifit of
	 * {@link JEditorPane}
	 *
	 * <p>
	 * JEditorpane can't handle empty xhtml containers like &lt;br /&gt; or
	 * &lt;img /&gt;, so this method replaces them without the "/" as in
	 * &lt;br&gt;
	 * </p>
	 *
	 * @param html
	...
	/**
	 * Helper method that prints out the contents of an {@link AttributeSet} to
	...
	public static void printAttribs(AttributeSet attr) {
		...
		while (ee.hasMoreElements()) {
			...
			HTMLUtils.logger.debug(name + " " + name.getClass().getName() + " | " + atr + " " + atr.getClass().getName());
		}
		HTMLUtils.logger.debug("----------------------------------------------------------------");
	}
	...
	public static class Base64 {
		...
		static {
			byte[] __bytes;
			try {
				__bytes = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".getBytes(Base64.PREFERRED_ENCODING);
			} // end try
		 ...
		 * as big as <var>numSigBytes</var>. Code can reuse a byte array by
		...
		private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset) {
			...
			if (source[srcOffset + 2] == Base64.EQUALS_SIGN) {
				...
				int outBuff = ((Base64.DECODABET[source[srcOffset]] & 0xFF) << 18) | ((Base64.DECODABET[source[srcOffset + 1]] & 0xFF) << 12);
			...
			// Example: DkL=
				...
				int outBuff = ((Base64.DECODABET[source[srcOffset]] & 0xFF) << 18) | ((Base64.DECODABET[source[srcOffset + 1]] & 0xFF) << 12) | ((Base64.DECODABET[source[srcOffset + 2]] & 0xFF) << 6);
			...
			// Example: DkLE
				...
				try {
					...
					// );
					int outBuff = ((Base64.DECODABET[source[srcOffset]] & 0xFF) << 18) | ((Base64.DECODABET[source[srcOffset + 1]] & 0xFF) << 12) | ((Base64.DECODABET[source[srcOffset + 2]] & 0xFF) << 6) | (Base64.DECODABET[source[srcOffset + 3]] & 0xFF);
		...
		public static byte[] decodeFromFile(String filename) {
			...
			try {
				...
				if (file.length() > Integer.MAX_VALUE) {
					HTMLUtils.logger.debug("File is too big for this convenience method (" + file.length() + " bytes).");
					return null;
		...
		public static class InputStream extends java.io.FilterInputStream {
			 ...
			 * reached or <var>len</var> bytes are read. Returns number of bytes
		...
		public static class OutputStream extends java.io.FilterOutputStream {
			...
			/**
			 * Calls {@link #write(int)} repeatedly until <var>len</var> bytes
			 * are written.
			 *
			 * @param theBytes
