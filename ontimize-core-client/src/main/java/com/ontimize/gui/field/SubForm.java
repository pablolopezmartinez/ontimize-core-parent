...
/**
 * This class allows to put a form into another form like a typical component. This subform is wrapped in a TableAttribute structure.
 * <p>
 *
...
public class SubForm extends IdentifiedAbstractFormComponent
		implements CreateForms, OpenDialog, AccessForm, Freeable, ReferenceComponent, DataComponent, SubFormComponent, IFormComponentManager {
	 ...
	 *            <td></td>
	 *            <td>yes</td>
	 *            <td>The complete path to the subform. For example, <i>"com/ontimize/form/form.xml"</i></td>
	 *            </tr>
	 *            </TABLE>
	 */
	@Override
				...
				for (int i = 0, j = this.parentKeys.size(); i < j; i++) {
					...
					if (parentkeyDataComponent != null) {
						...
						if (ApplicationManager.DEBUG) {
							SubForm.logger.debug(this.getClass().getName() + ": component with attr: " + parentk + " used like parentkey not included in parent form");
						}
					}
				}
			}
