...
public class FilterComponent extends IdentifiedAbstractFormComponent implements HasPreferenceComponent {
	...
	public void init(Hashtable h) {
		...
		JMenuItem menuItem = new JMenuItem(ApplicationManager.getTranslation("filtercomponent.delete_filter", ApplicationManager.getApplication().getResourceBundle()));
		...
		this.arrowButton.addMouseListener(new MouseAdapter() {
			...
			public void mousePressed(MouseEvent e) {
						...
						public void actionPerformed(ActionEvent e) {
							...
							if (n != null) {
								...
								if (FilterComponent.this.entityName != null) {
									ok = QueryBuilder.showParameterValuesTable(ApplicationManager.getApplication().getFrame(),
											ApplicationManager.getApplication().getResourceBundle(), qexp.getExpression(), FilterComponent.this.entityName);
								} else {
									ok = QueryBuilder.showParameterValuesTable(ApplicationManager.getApplication().getFrame(),
								...
								if (ok) {
									...
									FilterComponent.this.filterText.setText(ContainsSQLConditionValuesProcessorHelper.renderQueryConditionsExpressBundle(
											FilterComponent.this.queryExpression.getExpression(), ApplicationManager.getApplication().getResourceBundle()));
								}
				...
				if (sList.length != 0) {
					FilterComponent.this.pList.show((Component) e.getSource(), (int) (0 - FilterComponent.this.filterButton.getPreferredSize().getWidth()),
	 ...
	 * <ul>
	 * <li><code>cols</code> and <code>typecols</code> parameters no defined: All columns of this entity are used. Column types are infered from entity columns.
	 * <li><code>cols</code> and <code>typecols</code> parameters defined: Columns and their types are obtained from these parameters.
	 ...
	 * <ul>
	 * <li><code>cols</code> and <code>typecols</code> parameters defined: Columns and their types are obtained from these parameters.
	 * <li>Only <code>cols</code> parameter defined: Columns are obtained from this parameter and they are associated with a <code>String</code> type.
	 * </ul>
	 *
	 * @param h
	 *            the <code>Hashtable</code> with <code>XML</code> parameters:
	 *            <p>
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>Data types for columns. Only <code>String</code>, <code>Date</code> or <code>Number</code> are allowed./td>
	 *            </tr>
	 *            </TABLE>
	 */
	public FilterComponent(Hashtable h) {
		...
		if (h.get("text") != null) {
			this.filterButton.setText(ApplicationManager.getTranslation((String) h.get("text"), ApplicationManager.getApplication().getResourceBundle()));
		}

		// Initializes the label
		if (h.get("labeltext") != null) {
			this.label.setText(ApplicationManager.getTranslation((String) h.get("labeltext"), ApplicationManager.getApplication().getResourceBundle()));
		}
		...
		if (visibleFilter) {
			this.add(this.filterText, new GridBagConstraints(i, 0, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
			i++;
		}

		this.add(this.filterButton, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 0), 0, 0));
		i++;

		this.add(this.arrowButton, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.VERTICAL, new Insets(2, 0, 2, 2), 0, 0));
		i++;

		this.add(this.clearButton, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0));

	}

	/**
	 * Returns the standard expression. If the expression contains the <code>typecols</code> parameter, it will be returned a OR operation between <code>String</code> column types.
	 *
	...
	/**
	 * Returns the query expression for field. Filter expression is obtained from <code>getFilter()</code> method.
	 * <p>
	 *
	...
	public void initPreferences(ApplicationPreferences ap, java.lang.String user) {
		...
		if (sName != null) {
			...
			if (((aux.getEntity() == null) && (this.entityName == null)) || ((aux.getEntity() != null) && (this.entityName != null) && aux.getEntity().equals(this.entityName))) {
				this.queryExpression = aux;
				this.filterText.setText(ContainsSQLConditionValuesProcessorHelper.renderQueryConditionsExpressBundle(this.queryExpression.getExpression(),
	...
	public void filterAction(ActionEvent e) {
		...
		if (this.entityName != null) {
			aux = com.ontimize.db.query.QueryBuilder.showQueryBuilder((Component) e.getSource(), this.entityName, ApplicationManager.getApplication().getResourceBundle(),
					ApplicationManager.getApplication().getReferenceLocator(), this.queryExpression, true, true, true, this.tCols, lastName);
		} else if (this.tCols != null) {
			aux = com.ontimize.db.query.QueryBuilder.showQueryBuilder((Component) e.getSource(), ApplicationManager.getApplication().getResourceBundle(), this.lCols, this.tCols,
					this.queryExpression.getExpression(), lastName);
		}
		if (aux != null) {
			this.filterText.setText(
					ContainsSQLConditionValuesProcessorHelper.renderQueryConditionsExpressBundle(aux.getExpression(), ApplicationManager.getApplication().getResourceBundle()));
			this.queryExpression = aux;
