...
public class RealDataField extends TextFieldDataField implements OpenDialog, Freeable, AdvancedDataComponent {
	...
	/**
	 * The class constructor. It initializes the parameters, adds focus listener, sets alignment and optionally replaces the decimal separator.
	 * <p>
	 *
	 * @param parameters
		...
		if (this.popupMenu == null) {
			...
			this.advancedHelpBMenu.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						HelpUtilities.showDefaultAdvancedHelpDialog(SwingUtilities.getWindowAncestor(RealDataField.this), RealDataField.this.advancedHelpBMenu.getText(),
								RealDataField.this.locale);
					} catch (Exception ex) {
						RealDataField.logger.error(null, ex);
						RealDataField.this.parentForm.message("datafield.help_files_cannot be displayed", Form.ERROR_MESSAGE, ex);
					}
				}
			});
		}
	 ...
	 *
	 * @param parameters
	 *            <code>Hashtable</code> for initialization parameters.
	 */
	@Override
		...
		if ((calcul != null) && calcul.toString().equalsIgnoreCase("yes")) {
			...
			this.calcButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent evento) {
					...
					double result = RealDataField.this.calc.showCalculator(firstValue, RealDataField.this.getLocationOnScreen().x,
	 ...
	 *
	 * @param enabled
	 *            the condition to enable component
	 */
	@Override
	 ...
	 *
	 * @param enable
	 *            the condition to enable the advanced query mode
	 */
	@Override
	...
	public boolean isModified() {
		...
		if ((oValue == null) && (this.valueSave != null)) {
			if (ApplicationManager.DEBUG) {
				RealDataField.logger.debug("Component: " + this.attribute + " Modified: Previous value = " + this.valueSave + " New value = " + oValue);
			}
			return true;
		}
		if ((oValue != null) && (this.valueSave == null)) {
