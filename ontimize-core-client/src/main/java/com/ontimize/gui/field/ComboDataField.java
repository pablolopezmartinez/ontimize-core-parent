...
/**
 * This class implements a scrollable list. It is an abstract class to use for
 * all data components showed in a scrollable list.
 * <p>
 *
...
public abstract class ComboDataField extends DataField implements Freeable {
	...
	/**
	 * The interface that defines a <code>KeySelectionManager</code> to
	...
	protected static class CustomComboBox extends JComboBox implements Freeable {
				...
				public void keyPressed(KeyEvent e) {
					if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
						...
						if (CustomComboBox.this.queryText.length() > 0) {
							CustomComboBox.this.queryText.delete(CustomComboBox.this.queryText.length() - 1, CustomComboBox.this.queryText.length());
							CustomComboBox.this.getTipScroll().show(CustomComboBox.this, 0,
									(CustomComboBox.this.getTipScroll().getFontMetrics(CustomComboBox.this.getTipScroll().getFont()).getHeight() / 2)
		...
		private Component getRendererComponent() {
			Component comp = this.getRenderer().getListCellRendererComponent(CustomComboBox.auxList, this.getSelectedItem(), this.getSelectedIndex(), true, false);
			return comp;
		}
		 ...
		 *
		 * @param c
		 *            the combo box to calculate
		 * @return the preferred text width
		 */
		protected static int calculatePreferredTextWidth(JComboBox c) {
			try {
				Component comp = c.getRenderer().getListCellRendererComponent(CustomComboBox.auxList, c.getSelectedItem(), c.getSelectedIndex(), true, false);
		...
		public void setSelectedIndex(int i) {
			if ((this.getModel() instanceof CustomComboBoxModel) && ((CustomComboBoxModel) this.getModel()).isNullSelection() && (i < 0)) {
		...
		protected boolean checkKeySelectionInstanceExtKeySelectionManager(char keyChar) {
			int index;
			if (Character.isLetterOrDigit(keyChar) || (Character.isSpaceChar(keyChar) && (this.queryText.length() > 0)) || this.isValidChar(keyChar)) {
				this.queryText.append(keyChar);
				this.getTipScroll().show(this, 0, (this.getTipScroll().getFontMetrics(this.getTipScroll().getFont()).getHeight() / 2) + this.getHeight() + 2,
						this.queryText.toString());
				index = ((ExtKeySelectionManager) this.keySelectionManager).getComboIndex(this.queryText.toString(), this.getModel());
				if (index != -1) {
		...
		protected boolean isValidChar(char keyChar) {
			if ((keyChar == '.') || (keyChar == '-') || (keyChar == '+') || (keyChar == '*') || (keyChar == '_') || (keyChar == '<') || (keyChar == '>') || (keyChar == '¡')
	...
	protected class InnerListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (this.innerListenerEnabled) {
				ComboDataField.this.fireValueChanged(ComboDataField.this.getValue(), this.storeInnerValue, ValueEvent.USER_CHANGE);
	...
	/**
	 * Class constructor. Creates the component, sets the selected index and
	 * adds a renderer.
	 */
	public ComboDataField() {
		this.dataField = new JComboBox() {
			...
			public Dimension getPreferredSize() {
				Dimension d = super.getPreferredSize();
				return new Dimension(ComboDataField.this.fieldSize * this.getFontMetrics(this.getFont()).charWidth('A'), d.height);
			}
		};
	...
	/**
	 * Class constructor with combo parameter. Only sets the renderer and fix
	 * the <code>dataField</code> variable.
	 * <p>
	 *
	 * @param combo
	 *            the combo
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>Java Date pattern to use in <code>format</code> parameter.
	 *            </td>
	 *            </tr>
	 *
	 *            <tr>
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>The field shows the translation with the given message and
	 *            columns.</td>
	 *            </tr>
	 *
	 *            <tr>
	 ...
	 *            <td>yes</td>
	 *            <td>no</td>
	 *            <td><i>Since 5.2057EN-0.3</i>. By default, combo contains an
	 *            empty value in first position of deployed list. With this
	 *            parameter is possible to delete this value and show only field
	 *            values.</td>
	 *            </tr>
	 *            </Table>
	 *
	...
	/**
	 * Establishes the combo values. If value parameter is a string array or a
	 * vector, values are added to combo one by one. If value is null, data are
	 * deleted.
	 * <p>
	 *
	 * @param value
	...
	/**
	 * This method allows to enable/disable the ValueEvent events notifier. So,
	 * inner events will be not triggered when inner listener is disabled. It is
	 * advisable disabling the listener only when content is inserted by
	 * program.
	 * <p>
	 *
	 * @param enabled
	 *            the condition to enable/disable the inner listener.
	 */
	...
	public boolean isModified() {
		...
		if ((oValue == null) && (this.valueSave != null)) {
			if (ApplicationManager.DEBUG) {
				ComboDataField.logger.debug("Component: " + this.attribute + " modified: Previous value = " + this.valueSave + " New value = " + oValue);
			}
			return true;
		}
		if ((oValue != null) && (this.valueSave == null)) {
