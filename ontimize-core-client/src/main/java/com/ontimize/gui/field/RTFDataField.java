...
public class RTFDataField extends MemoDataField implements AdvancedDataComponent, ITemplateField {
	...
	public static class ExtForegroundAction extends StyledEditorKit.StyledTextAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (editor != null) {
				...
				Color c = JColorChooser.showDialog(editor, ApplicationManager.getTranslation("ColorDataField.chooseColor"), cIni);
				if (c != null) {
	...
	protected class ActionWrapper implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			RTFDataField.this.fireValueChanged(RTFDataField.this.getValue(), RTFDataField.this.valueSave, ValueEvent.USER_CHANGE);
		}
	}
	...
	/**
	 * Initializes parameters. Configure buttons and adds key events. XML
	 * definition could be contains an optional parameter: 'plaintextcolumn'
	 * specifying the identifier to save the text in plain format.
	 * <p>
	 *
	 * @param parameters
		...
		this.dataField.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if ((e.getKeyCode() == KeyEvent.VK_SPACE) && e.isControlDown() && e.isAltDown()) {
					MessageDialog.showMessage(RTFDataField.this.parentFrame, (String) RTFDataField.this.getValue(), JOptionPane.INFORMATION_MESSAGE, null);
				}
			}
		});
		this.plainTextColumn = (String) parameters.get("plaintextcolumn");

		this.templateDataType = ParseUtils.getTemplateDataType((String) parameters.get(ITemplateField.TEMPLATE_DATA_TYPE), RTFDataField.defaultTemplateDataType);
	...
	/**
	 * Creates a combo with height fonts <code>minFontSize</code> to
	...
	protected void installButtonsHandler() {
		...
		((JTextComponent) this.dataField).addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent e) {
				...
				Element element = ((StyledDocument) ((JTextComponent) RTFDataField.this.dataField).getDocument()).getCharacterElement(dot);
				if (element != null) {
				...
				Element pElement = ((StyledDocument) ((JTextComponent) RTFDataField.this.dataField).getDocument()).getParagraphElement(dot);
				if (pElement != null) {
	...
	public void configureButtons() {
		...
		try {
			...
			this.leftAlignBt.addActionListener(new ActionWrapper(new StyledEditorKit.AlignmentAction("leftAlign", StyleConstants.ALIGN_LEFT)));
			this.centerAlignBt.addActionListener(new ActionWrapper(new StyledEditorKit.AlignmentAction("centerAlign", StyleConstants.ALIGN_CENTER)));
			this.rightAlignBt.addActionListener(new ActionWrapper(new StyledEditorKit.AlignmentAction("rightAlign", StyleConstants.ALIGN_RIGHT)));
	...
	protected void configureButtonsPanelAndEditor() {
		...
		GridBagConstraints buttonsPanelConstraints = new GridBagConstraints(2, 0, 1, 1, 0, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(0, 0, 0, 0), 0, 0);
		...
		this.scroll.setPreferredSize(new Dimension(this.dataField.getPreferredSize().width, this.dataField.getFontMetrics(this.dataField.getFont()).getHeight() * this.rows));
	...
	public void deleteData() {
		...
		try {
			this.enableInnerListener(false);
			((JTextComponent) this.dataField).getDocument().remove(0, ((JTextComponent) this.dataField).getDocument().getLength());
		} catch (Exception e) {
			RTFDataField.logger.error(null, e);
		} finally {
			this.enableInnerListener(true);
	...
	public Object getValue() {
		...
		try {
			...
			if (this.plainTextColumn != null) {
				FormatText vm = new FormatText(this.plainTextColumn, (String) this.attribute, doc.getText(0, doc.getLength()), res);
				return vm;
			}
			return res;
		} catch (Exception e) {
			...
			protected void processKeyEvent(KeyEvent e) {
				if ((e.getKeyCode() == KeyEvent.VK_F3) && (e.getID() == KeyEvent.KEY_RELEASED)) {
					if (RTFDataField.this.dQuery == null) {
						RTFDataField.this.dQuery = new FindDialog(RTFDataField.this.parentFrame, (JTextComponent) RTFDataField.this.dataField);
						...
						RTFDataField.this.dQuery.show(((JTextComponent) RTFDataField.this.dataField).getCaretPosition());
					} else {
						if (RTFDataField.this.dQuery != null) {
							RTFDataField.this.dQuery.find(((JTextComponent) RTFDataField.this.dataField).getCaretPosition());
						}
					}
					e.consume();
					return;
	...
	public boolean isEmpty() {
		try {
			if ((((JTextComponent) this.dataField).getDocument().getLength() == 0)
					|| ((JTextComponent) this.dataField).getDocument().getText(0, ((JTextComponent) this.dataField).getDocument().getLength()).equalsIgnoreCase("\n")
					|| (((JTextComponent) this.dataField).getDocument().getText(0, ((JTextComponent) this.dataField).getDocument().getLength()).trim().length() == 0)) {
				return true;
			}
			return false;
	 ...
	 *
	 * @param v
	 *            the condition about visibility panel
	 */
	...
	public Object getTemplateDataValue() {
		if (ITemplateField.DATA_TYPE_IMAGE == this.templateDataType) {
			...
			this.height = editorPane.getPreferredSize().height - editorPane.getInsets().top - editorPane.getInsets().bottom;
