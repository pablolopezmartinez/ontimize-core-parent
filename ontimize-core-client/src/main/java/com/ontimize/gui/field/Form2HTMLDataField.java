...
public class Form2HTMLDataField extends IdentifiedAbstractFormComponent implements DataComponent, ActionListener, Internationalization, ValueChangeDataComponent {
	 ...
	 * It contains basically a HTML Editor, a HTML Viewer and a combo with the transform style-sheets.<BR>
	 * This field takes the source from another field, refenreced by the <I>xmlFieldAttr</I> attribute,<BR>
	 * then applies the <code>xsl</code> selected to the content, and stores the result in the component or in the <BR>
	...
	public Form2HTMLDataField(Hashtable parameters) {
		...
		this.add(this.menuComponent, new GridBagConstraints(0, 0, 1, 1, 1, 0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 2, 2));

		this.add(viewer, new GridBagConstraints(0, 1, 1, 1, 1, 1, GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 2, 2));
	}
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td><I>attr</I> of the field where is stored the XML Form and which will be used for the transformations</td>
	 *            </tr>
	 *
	 *            <tr>
	 *            <td>htmlTitle</td>
	...
	public void init(Hashtable parameters) {
		...
		if (o != null) {
			this.cssContentFile = Form2HTMLDataField.fileToString(Form2HTMLDataField.class.getClassLoader().getResourceAsStream(o.toString()));
		}

		o = parameters.get(Form2HTMLDataField.PATTERN_TAG);
		if (o != null) {
			this.patternContentFile = Form2HTMLDataField.fileToString(Form2HTMLDataField.class.getClassLoader().getResourceAsStream(o.toString()));
		} else {
	...
	private void setText() {
		this.reload.setToolTipText(ApplicationManager.getTranslation(Form2HTMLDataField.RELOAD_TEXT, this.resourceBundle));
		this.apply.setToolTipText(ApplicationManager.getTranslation(Form2HTMLDataField.APPLY_TEXT, this.resourceBundle));
		this.navigator.setToolTipText(ApplicationManager.getTranslation(Form2HTMLDataField.NAVIGATOR_TEXT, this.resourceBundle));
		this.zoomIn.setToolTipText(ApplicationManager.getTranslation(Form2HTMLDataField.ZOOM_IN_TEXT, this.resourceBundle));
		this.zoomOut.setToolTipText(ApplicationManager.getTranslation(Form2HTMLDataField.ZOOM_OUT_TEXT, this.resourceBundle));
		this.right.setToolTipText(ApplicationManager.getTranslation(Form2HTMLDataField.SHOW_RIGHT_TEXT, this.resourceBundle));

		if (this.htmlTitle != null) {
			this.htmlEditorPanel.setBorder(new TitledBorder(ApplicationManager.getTranslation(this.htmlTitle, this.resourceBundle)));
		}
		if ((this.htmlViewerTitle != null) && (this.viewer != null)) {
			this.viewer.setBorder(new TitledBorder(ApplicationManager.getTranslation(this.htmlViewerTitle, this.resourceBundle)));
		}
	...
	private void createMenuComponent() {
		...
		if (this.xslLabel != null) {
			this.menuComponent.add(this.jlXSLLabel, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
			i += 1;
		}

		this.menuComponent.add(this.xslList, new GridBagConstraints(i, 0, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, new Insets(2, 2, 2, 2), 2, 2));
		i += 1;
		this.menuComponent.add(this.apply, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
		i += 1;
		this.menuComponent.add(this.navigator, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
		i += 1;
		this.menuComponent.add(this.reload, new GridBagConstraints(i, 0, 1, 1, 0, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
	}
	...
	private void createHTMLEditorPanel() {
		...
		this.htmlEditorPanel.add(js, new GridBagConstraints(0, 0, 1, 3, 1, 1, GridBagConstraints.NORTHWEST, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 2, 2));
		this.htmlEditorPanel.add(this.zoomIn, new GridBagConstraints(1, 0, 1, 1, 0, 0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
		this.htmlEditorPanel.add(this.zoomOut, new GridBagConstraints(1, 1, 1, 1, 0, 0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
		this.htmlEditorPanel.add(this.right, new GridBagConstraints(1, 2, 1, 1, 0, 0, GridBagConstraints.NORTHWEST, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 2, 2));
	...
	public boolean isModified() {
		...
		if ((oValue == null) && (this.valueSave != null)) {
			if (ApplicationManager.DEBUG) {
				Form2HTMLDataField.logger.debug("Component: " + this.attribute + " Modified: Previous value = " + this.valueSave + " New value = " + oValue);
			}
			return true;
		}
		if ((oValue != null) && (this.valueSave == null)) {
	...
	protected void applyXSL() {
		...
		if (this.xmlFieldAttr != null) {
			if ((this.getParentForm().getElementReference(this.xmlFieldAttr) != null) && (this.getParentForm().getElementReference(this.xmlFieldAttr) instanceof DataComponent)) {
		...
		try {
			...
			this.transformer = this.tFactory.newTransformer(new StreamSource(Form2HTMLDataField.class.getClassLoader().getResourceAsStream(selectedItem.toString())));
		} catch (Exception ex) {
			Form2HTMLDataField.logger.error(null, ex);
