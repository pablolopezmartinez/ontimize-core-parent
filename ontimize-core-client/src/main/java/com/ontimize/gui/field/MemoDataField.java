...
public class MemoDataField extends DataField implements OpenDialog, Freeable, InteractionManagerModeListener {
	...
	protected class InnerDocumentListener implements DocumentListener {
		...
		public void insertUpdate(DocumentEvent e) {
			if (this.innerListenerEnabled) {
				MemoDataField.this.fireValueChanged(MemoDataField.this.getValue(), MemoDataField.this.valueSave, ValueEvent.USER_CHANGE);
			}
		}
	...
	/**
	 * The class constructor. Calls to super constructor, initializes parameters
				...
				if (e.getKeyCode() != KeyEvent.VK_TAB) {
					if ((e.getKeyCode() == KeyEvent.VK_F3) && (e.getID() == KeyEvent.KEY_RELEASED)) {
						if (MemoDataField.this.dQuery == null) {
							MemoDataField.this.dQuery = new FindDialog(MemoDataField.this.parentFrame, (JTextArea) MemoDataField.this.dataField);
							...
							if (MemoDataField.this.dQuery != null) {
								MemoDataField.this.dQuery.find(((JTextComponent) MemoDataField.this.dataField).getCaretPosition());
							}
						}
						e.consume();
						return;
				...
				public void mouseDragged(MouseEvent e) {
					if (MemoDataField.this.activationScrollTip) {
						if (scrollBarVertical.getVisibleAmount() < scrollBarVertical.getMaximum()) {
							...
							int topRow = (p.y / MemoDataField.this.dataField.getFontMetrics(MemoDataField.this.dataField.getFont()).getHeight()) + 1;
	...
	public void init(Hashtable params) {
		...
		if (rows != null) {
			try {
				...
				MemoDataField.logger.error(this.getClass().toString() + " Error in parameter 'rows': " + e.getMessage(), e);
			}
		}
		...
		if (maxlength != null) {
			try {
				...
				MemoDataField.logger.error(this.getClass().toString() + "Error in parameter 'maxlength': " + e.getMessage(), e);
			}
		}
		...
		if (this.labelPosition == SwingConstants.TOP) {
			this.add(this.scroll,
					new GridBagConstraints(GridBagConstraints.RELATIVE, 2, 1, 1, this.weightDataFieldH, verticalCoeficient, GridBagConstraints.EAST, fill,
			...
			this.add(this.scroll,
					new GridBagConstraints(GridBagConstraints.RELATIVE, 0, 1, 1, this.weightDataFieldH, verticalCoeficient, GridBagConstraints.EAST, fill,
							new Insets(DataField.DEFAULT_PARENT_MARGIN_FOR_SCROLL, DataField.DEFAULT_PARENT_MARGIN_FOR_SCROLL, DataField.DEFAULT_PARENT_MARGIN_FOR_SCROLL,
									DataField.DEFAULT_PARENT_MARGIN_FOR_SCROLL),
							0, 0));
		}
	...
	public void setValue(Object value) {
		...
		try {
			...
			if (value != null) {
				...
				((JTextComponent) this.dataField).getDocument().putProperty(DefaultEditorKit.EndOfLineStringProperty, "\n");
	...
	/**
	 * This method allows to enable/disable the ValueEvent events notifier. So, inner events will be not triggered when inner listener is disabled. It is advisable disabling the
	 * listener only when content is inserted by program.
	 * <p>
	 *
	 * @param enable
	...
	protected void createPopupMenu() {
		...
		this.searchMenu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (MemoDataField.this.dQuery == null) {
					MemoDataField.this.dQuery = new FindDialog(MemoDataField.this.parentFrame, (JTextComponent) MemoDataField.this.dataField);
					MemoDataField.this.dQuery.setResourceBundle(MemoDataField.this.resources);
					MemoDataField.this.dQuery.setComponentLocale(MemoDataField.this.locale);
				}
				MemoDataField.this.dQuery.show(((JTextComponent) MemoDataField.this.dataField).getCaretPosition());
	...
	protected void updateBackgroundColor() {
		if (this.requiredBorder != null) {
			...
			if (!this.enabled) {
				...
				this.scroll.setBorder(this.required ? BorderManager.getBorder(this.requiredBorder) : this.noRequiredBorder);
