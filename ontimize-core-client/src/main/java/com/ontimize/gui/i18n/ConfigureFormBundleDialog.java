...
public class ConfigureFormBundleDialog extends EJDialog implements IConfigureFormBundleGUI {
	...
	protected Vector			textsToTranslate							= new Vector(Arrays.asList(
			new String[] { ConfigureFormBundleDialog.MESSAGE_BUNDLE_ALL_TRANSLATIONS_REQUIRED, ConfigureFormBundleDialog.MESSAGE_BUNDLE_BUNDLE_CLASS_REQUIRED, ConfigureFormBundleDialog.MESSAGE_BUNDLE_SAVE_CHANGES }));
	...
	public ConfigureFormBundleDialog(Frame owner, String title, boolean modal, String remoteObjectName) throws Exception {
		super(owner, title, modal);
		this.remoteObjectName = remoteObjectName;
		this.originalTitle = title;
	}

	public ConfigureFormBundleDialog(Dialog owner, String title, boolean modal, String remoteObjectName) throws Exception {
		super(owner, title, modal);
		this.remoteObjectName = remoteObjectName;
		this.originalTitle = title;
	}
	...
	protected Table buildTable() throws Exception {
		...
		deletebutton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int option = tableBundle.getParentForm().message(ConfigureFormBundleDialog.M_WOULD_YOU_LIKE_TO_DELETE_TRANSLATION_ROWS, Form.QUESTION_MESSAGE);
				if (option == Form.YES) {

					for (int i = 0; i < tableBundle.getSelectedRowsNumber(); i++) {
						String key = (String) ((Vector) tableBundle.getSelectedRowData().get(ConfigureFormBundleDialog.textKeyColumn)).get(i);
						String bundleClassName = (String) ((Vector) tableBundle.getSelectedRowData().get(ConfigureFormBundleDialog.bundleClassColumn)).get(i);
						...
						try {
							...
							MessageDialog.showErrorMessage(SwingUtilities.getWindowAncestor(tableBundle),
									ApplicationManager.getTranslation(ConfigureFormBundleDialog.MESSAGE_BUNDLE_NOT_REMOVE_TRANSLATION));
						}
					}
		...
		String applicationResourceFileName = ((MainApplication) this.form.getFormManager().getApplication()).getResourcesFileName();
	...
	public void setForm(Form form) throws Exception {
		...
		this.setSizePositionPreference(("bundledialog." + form.getArchiveName()) != null ? form.getArchiveName() : form.getFormTitle());
		...
		this.confTable.addTableEditorListener(new TableEditorListener() {
			...
			public void editingStopped(TableEditionEvent e) {
				...
				if (rowValues != null) {
					...
					keys.put(ConfigureFormBundleDialog.textKeyColumn, rowValues.get(ConfigureFormBundleDialog.textKeyColumn));
					keys.put(ConfigureFormBundleDialog.bundleClassColumn, rowValues.get(ConfigureFormBundleDialog.bundleClassColumn));
	...
	protected boolean deleteTranslationForKey(String key, String bundleClassColumn) throws Exception {
		Object remoteReference = ((UtilReferenceLocator) this.form.getFormManager().getReferenceLocator()).getRemoteReference(this.remoteObjectName,
		...
		if ((remoteReference != null) && (remoteReference instanceof IDatabaseBundleManager)) {
			...
			((IDatabaseBundleManager) remoteReference).deleteBundleValues(dbbv, this.form.getFormManager().getReferenceLocator().getSessionId());
		}
		if (this.acceptButton != null) {
			this.acceptButton.setEnabled(false);
		}
				...
				for (int i = 0; i < this.tableChanges.calculateRecordNumber(); i++) {
					...
					for (int k = 0; k < this.availableLocales.length; k++) {
						if (h.get(this.availableLocales[k]) != null) {
							...
							this.form.message(ConfigureFormBundleDialog.MESSAGE_BUNDLE_ALL_TRANSLATIONS_REQUIRED, Form.WARNING_MESSAGE);
							return false;
						}
					}

					if (h.get(ConfigureFormBundleDialog.bundleClassColumn) == null) {
						this.form.message(ConfigureFormBundleDialog.MESSAGE_BUNDLE_BUNDLE_CLASS_REQUIRED, Form.WARNING_MESSAGE);
						return false;
					}

					dbbv.addBundleValue((String) h.get(ConfigureFormBundleDialog.textKeyColumn), (String) h.get(ConfigureFormBundleDialog.bundleClassColumn), trans);
				}

				((IDatabaseBundleManager) remoteReference).updateBundleValues(dbbv, this.form.getFormManager().getReferenceLocator().getSessionId());
			}
			if (this.acceptButton != null) {
				this.acceptButton.setEnabled(false);
			}
			...
			if (this.availableLocales == null) {
				try {
					Object remoteReference = ((UtilReferenceLocator) this.form.getFormManager().getReferenceLocator()).getRemoteReference(this.remoteObjectName,
							this.form.getFormManager().getReferenceLocator().getSessionId());
					if ((remoteReference != null) && (remoteReference instanceof IDatabaseBundleManager)) {
						this.availableLocales = ((IDatabaseBundleManager) remoteReference).getAvailableLocales(this.form.getFormManager().getReferenceLocator().getSessionId());
					}
				} catch (Exception e) {
					ConfigureFormBundleDialog.logger.error(null, e);
				}
	...
	protected void configureValues() {
		...
		LanguageResourcesManager lrm = new LanguageResourcesManager(locales, formResourceFileName, formManagerResourceFileName, applicationResourceFileName);
		...
		for (int i = 0; i < textsToTranslate.size(); i++) {
			if (textsToTranslate.get(i) instanceof String) {
				...
				ConfigureFormBundleDialog.logger
						.debug(this.getClass().getName() + " there is an object " + textsToTranslate.get(i).getClass().getName() + " in the list of texts to translate");
			}
		}
		this.confTable.setValue(this.originalValues);
	}
	...
	protected boolean compareRecordBundleKeys(Hashtable record1, Hashtable record2) {
		if ((record1 != null) && (record2 != null)) {
			...
			if ((textKey1 != null) && (textKey2 != null) && textKey1.equals(textKey2)) {
				...
				if (((bundleClass1 == null) && (bundleClass2 == null)) || ((bundleClass1 != null) && (bundleClass2 != null) && bundleClass1.equals(bundleClass2))) {
					return true;
				}
			}
		}
	...
	public Vector getTextsToTranslate() {
		if (this.internationalizationComponents != null) {
			...
			for (int i = 0; i < this.internationalizationComponents.size(); i++) {
				Vector textsToTranslate = ((Internationalization) this.internationalizationComponents.get(i)).getTextsToTranslate();
	...
	public static class LanguageResourcesManager {
		...
		public LanguageResourcesManager(String[] locales, String formResources, String managerResources, String applicationResources) {
	...
	protected static class LanguageResources {
		...
		protected void build() {
			...
			for (int i = 0; i < this.locales.length; i++) {
				this.bundles[i] = ExtendedPropertiesBundle.getExtendedBundle(this.resourcesFileName, LanguageResources.getLocale(this.locales[i]));
			}
		}
