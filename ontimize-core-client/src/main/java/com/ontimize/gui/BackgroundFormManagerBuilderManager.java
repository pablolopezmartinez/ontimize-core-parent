...
public class BackgroundFormManagerBuilderManager {
	...
	protected static class FormManagerBuilderThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				BackgroundFormManagerBuilderManager.logger.error(this.getClass().getName() + ": Thread error building form manager -> " + this.backgroundFormManager.getId(), e);
			} finally {
	...
	public static void finishThread(Thread th) {
		synchronized (BackgroundFormManagerBuilderManager.runningList) {
			...
			if (BackgroundFormManagerBuilderManager.waitingList.size() > 0) {
				FormManagerBuilderThread thread = (FormManagerBuilderThread) BackgroundFormManagerBuilderManager.waitingList.remove(0);
	...
	public static void buildFormManager(IFormManager formManager) {
		synchronized (BackgroundFormManagerBuilderManager.runningList) {
			...
			if (BackgroundFormManagerBuilderManager.runningList.size() < BackgroundFormManagerBuilderManager.runningThreadNumber) {
				BackgroundFormManagerBuilderManager.runningList.add(thread);
				thread.start();
			} else {
