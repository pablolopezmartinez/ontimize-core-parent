 ...
 * @version 1.0
 */
public class ApplicationMenuBar extends JMenuBar implements FormComponent, Freeable, HasHelpIdComponent, HasPreferenceComponent {
		...
		if (element instanceof IdentifiedElement) {
			if (this.menuItemList.containsKey(((IdentifiedElement) element).getAttribute())) {
				ApplicationMenuBar.logger
						.debug("ApplicationMenuBar: '" + ((IdentifiedElement) element).getAttribute() + "' attribute has multiple instances. The xml file must be checked.");
			}
			...
			if (children[i] instanceof Freeable) {
				try {
					...
					if (com.ontimize.gui.ApplicationManager.DEBUG) {
						ApplicationMenuBar.logger.debug(this.getClass().toString() + ": " + "Exception in free() " + children[i].getClass().toString() + " : " + e.getMessage(), e);
					}
				}
			}
		}
	...
	protected void createMenuShortcutsConfigurationDialog() {
		if (this.menuShortcutConfiguration == null) {
			this.menuShortcutConfiguration = new ShortcutDialogConfiguration(ApplicationManager.getApplication().getFrame(), this);
	...
	/**
	 * This method convert a keystroke in a String with this structure: i.e. <b>'Ctrl + Alt + a'</b>. Modifiers (Ctrl + Alt ) are also internazionalized for avoiding problems with
	 * jvm locale-dependent method: <code>getKeyStroke</code>.
	 *
	 * @param accelerator
	 *            keystroke to convert
	 * @return A <code>String</code> with text for keystroke or directly the codes for keystroke when it does not exist in bundle.
	 */
	public static String acceleratorMessageFromKeystroke(KeyStroke accelerator) {
		...
		int keyCode = accelerator.getKeyCode();
		if ((keyCode != 0) && (KeyEvent.VK_CONTROL != keyCode) && (KeyEvent.VK_ALT != keyCode) && (KeyEvent.VK_SHIFT != keyCode)) {
						...
						for (int i = 0; i < allItems.size(); i++) {
							...
							if ((item instanceof IDynamicItem) && ((IDynamicItem) item).isDynamic()) {
								if (item instanceof MenuLocale) {
									((MenuLocale) item).addLocaleListener((LocaleListener) ApplicationManager.getApplication().getMenuListener());
								} else {
									((JMenuItem) item).addActionListener((ActionListener) ApplicationManager.getApplication().getMenuListener());
								}
				...
				for (int i = 0; i < allItems.size(); i++) {
					...
					if ((currenComponent instanceof IDynamicItem) && ((IDynamicItem) currenComponent).isDynamic()) {
						...
						if (currenComponent instanceof MenuLocale) {
							((MenuLocale) currenComponent).removeLocaleListener((LocaleListener) ApplicationManager.getApplication().getMenuListener());
						} else {
							((JMenuItem) currenComponent).removeActionListener((ActionListener) ApplicationManager.getApplication().getMenuListener());
						}
					}
				}
