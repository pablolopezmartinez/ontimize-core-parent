...
public class FormBundleButton extends FormHeaderButton implements InteractionManagerModeListener {
	...
	/**
	 * This method gets the <code>Hashtable</code> and configure the button. This class use all the parameters of {@link Button}
	 * <p>
	 *
	 * @param parameters
	 ...
	 *            <td>no</td>
	 *            <td>Name of the class to show to configure the resource bundle values. This class must implements IConfigureFormBundleGUI.</td>
	 *            </tr>
	 *            <tr>
	 *            <td>enabledmodes</td>
	...
	public void init(Hashtable parameters) {
		...
		this.dialogTitle = ParseUtils.getString((String) parameters.get(FormBundleButton.CONFIGURATION_DIALOG_TITLE), FormBundleButton.defaultConfigutationDialogTitle);
		this.remoteObjectName = ParseUtils.getString((String) parameters.get(FormBundleButton.REMOTE_OBJECT_NAME), ExtendedPropertiesBundle.getDbBundleManagerName());
		this.configurationDialogClassName = ParseUtils.getString((String) parameters.get(FormBundleButton.CONFIGURATION_DIALOG_CLASS), null);
		...
		if (this.configurationDialog == null) {
			...
			if (this.configurationDialogClassName != null) {
				...
				try {
					...
					try {
						constructor = dialogClassObject.getConstructor(new Class[] { Frame.class, String.class, boolean.class, String.class });
						this.configurationDialog = (IConfigureFormBundleGUI) constructor
								.newInstance(new Object[] { (Frame) w, this.dialogTitle, Boolean.TRUE, this.remoteObjectName });
					} catch (Exception e) {
						FormBundleButton.logger.trace(null, e);
						try {
							constructor = dialogClassObject.getConstructor(new Class[] { Dialog.class, String.class, boolean.class, String.class });
							this.configurationDialog = (IConfigureFormBundleGUI) constructor
									.newInstance(new Object[] { (Dialog) w, this.dialogTitle, Boolean.TRUE, this.remoteObjectName });
						} catch (Exception e2) {
			...
			if (this.configurationDialog == null) {
				if (w instanceof Frame) {
					this.configurationDialog = new ConfigureFormBundleDialog((Frame) w, this.dialogTitle, true, this.remoteObjectName);
				} else if (w instanceof Dialog) {
					this.configurationDialog = new ConfigureFormBundleDialog((Dialog) w, this.dialogTitle, true, this.remoteObjectName);
				}
			}
