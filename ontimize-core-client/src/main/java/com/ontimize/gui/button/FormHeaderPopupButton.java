...
public class FormHeaderPopupButton extends ApToolBarPopupButton implements AccessForm {
	 ...
	 *            <td></td>
	 *            <td>no</td>
	 *            <td>The icon showed for each text. If <code>texts</code> and <code>icons</code> are both presents should have the same size and it is possible to complete with
	 *            <code>Null</code> values to indicate empty value. For example:<br>
		...
		if ((texts != null) || (icons != null)) {
			...
			if ((vTexts != null) && (vIcons != null) && (vTexts.size() != vIcons.size())) {
				throw new IllegalArgumentException(
						this.getClass().toString() + " texts and icons must have identical number of elements: " + vTexts.size() + " != " + vIcons.size());
			}
			...
			if (((nKeys != nelementsi) && (nelementsi > 0)) || (nKeys != nelementst)) {
				throw new IllegalArgumentException(this.getClass().toString() + "keys, texts and icons must have identical number of elements: " + vKeys.size() + " != " + vTexts
						.size() + " != " + vIcons.size());
			}
