...
public class JOrgTree extends JPanel implements Scrollable {
	...
	public void paintNode(JOrgTreeNode node, Graphics g) {
		...
		if (this.model == this.innerModel) {
			...
			component = this.cellRenderer.getJOrgTreeCellRendererComponent(this, node.getTreeNode(), bNodeSelected, 1, false);
		}
	...
	public JOrgTreeNode getNodeAtCoordinates(int x, int y) {
		if (this.innerModel != null) {
			...
			while (enumNodes.hasMoreElements()) {
				...
				if ((x >= jtNode.getX()) && (x <= (jtNode.getX() + this.rendererWidth)) && (y >= jtNode.getY()) && (y <= (jtNode.getY() + this.rendererHeight))) {
					return jtNode;
				}
			}
		}
	...
	public static void addChildren(JOrgTreeModel model, JOrgTreeNode node, int level, int maxLevel, int minChildren, int maxChildren) {
		if (level < maxLevel) {}
	...
	public static void addTreeChildren(TreeModel model, DefaultMutableTreeNode node, int level, int maxLevel, int minChildren, int maxChildren) {
		if (level < maxLevel) {}
