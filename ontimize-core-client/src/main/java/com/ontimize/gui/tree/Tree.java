...
/**
 * Class that implements a tree where nodes are objects of class. This class
 * divides nodes in two categories: organizational and data nodes.
 * <ul>
 * <li><b>organizational node:</b> It is similar to a folder in a system
 * directory. It could contain any type of node (organizational or data).
 * <li><b>data node:</b> It corresponds with a database record specified by the
 * organizational node. It is similar to a directory structure in operative
 * system.
 * </ul>
 * For example, it is possible that a organizational node contains clients. This
 * node will show a empty form with the client fields disabled. When an user
 * expands that node, a group of client nodes are showed. These clients are data
 * nodes. Organizational nodes could contain another organizational nodes
 * inside, for client accounts for example.
 * <p>
 *
 ...
 */
public class Tree extends JTree implements Internationalization, Freeable, OpenDialog, SecureElement, HasHelpIdComponent, HasPreferenceComponent, ApplicationPreferencesListener {
	...
	class MTreeModelListener extends TreeModelHandler {
		...
		/**
		 * Event is captured. Before calling to parent class, TreeWillCollapse
		 * event is triggered, in all child nodes of source event node. So, when
		 * internally tree status is updated, FormManager will receive the
		 * notification.
		 */
		@Override
		public void treeStructureChanged(TreeModelEvent e) {
			/*
			 * Parent class method marks like expanded the child nodes of this
			 * node. It exists a problem because expand/collapse change is not
			 * notified to the listeners, so these ones will not able to detect
			 * a collapse event. For avoiding this circumstance, child nodes
			 * will never be expanded when methods of TreeModelHandler (JTree
			 * class) are executed.
			 */
	...
	class QueryPopup extends JPopupMenu implements Internationalization, Freeable {
		...
		class QueryDocumentListener implements DocumentListener {
			...
			protected void update() {
				...
				try {
					int maxMatches = -1;
					String searchText = QueryPopup.this.textField.getDocument().getText(0, QueryPopup.this.textField.getDocument().getLength());
			...
			if (event.getSource() instanceof JMenuItem) {
				if (((JMenuItem) event.getSource()).getActionCommand().equals(Tree.updateTextKey)) {
					...
					if (selectedRowsCount > 0) {
						int option = MessageDialog.showMessage(Tree.this.parentFrame, "tree.do_you_really_want_deleted_the_selected_nodes", JOptionPane.QUESTION_MESSAGE,
								Tree.this.resource);
				...
				} else if (((JMenuItem) event.getSource()).getActionCommand().equalsIgnoreCase(Tree.fitSpaceToTreeKey)) {
					boolean selec = ((JCheckBoxMenuItem) event.getSource()).isSelected();
					Tree.this.fitSpaceToTree = selec;
					ApplicationManager.getApplication().getPreferences().setPreference(((ClientReferenceLocator) Tree.this.locator).getUser(), Tree.ADJUST_TREE_SPACE, "" + selec);
					...
					long tf = System.currentTimeMillis();
					if (ApplicationManager.DEBUG_TIMES) {
						System.out.println("Sorting time : " + ((JMenuItem) event.getSource()).getActionCommand() + " " + ((tf - t) / 1000.0) + " seconds");
					}
				}
	...
	/**
	 * Class constructor. Moreover to call to <code>super()</code>, registers
	 * the tooltip manager, creates the contextual pop-up menu options for right
	 * mouse button and fixes the tree model.
	 *
	 * @param treeURI
	 *            the path to the file with tree definition
	 * @param constructor
	 *            reference to the tree constructor that must process tree
	...
	public Tree(String treeURI, TreeBuilder constructor) {
		...
		/*
		 * It exists a problem with original tree approach due to JTree listener
		 * model. It occurs when nodeStructureChanged() (Class DefaultTreeModel)
		 * is called. This method notifies to the model listeners that process
		 * the update operation. This listener changes the collapse-expand state
		 * for any nodes. But this change is realized in a HashMap of JTree
		 * class and it is not notified to our listeners, for example
		 * FormManager. So, it has been modified the class TreeModelListener .
		 */
	...
	public void setEditable(boolean editable) {
		if (editable == true) {
			if (this.locator != null) {
				...
				if ((this.getCellEditor() == null) || ((this.getCellEditor() instanceof DefaultDataTreeCellEditor) == false)) {
	...
	protected void showPopupMenu(MouseEvent event) {
		...
		if (this.getSelectionCount() > 0) {
			...
			for (int i = 0; i < selectedRows.length; i++) {
				...
				if ((oNode instanceof OTreeNode) && ((!((OTreeNode) oNode).canDelete()) || ((OTreeNode) oNode).isOrganizational())) {
	...
	protected class DataNodeOperationThread extends OperationThread {
		...
		protected void createDataNodes(EntityResult result, Vector resultVector, Vector organizationNodes, Vector keys, String[] attributes) {
			...
			for (int j = 0; j < resultVector.size(); j++) {
						...
						if (keyValues.get(j) == null) {
							...
							if (bShowErrorMessage) {
								MessageDialog.showErrorMessage((Frame) null,
										"Error in tree data. There is a null value for the key: " + keys.get(k) + ". Some results have been removed.");
								bShowErrorMessage = MessageDialog.showQuestionMessage(null, "Do you want to show the error messages for each node?");
							}
							break;
						}
				...
				if (this.parentNode instanceof PageFetchTreeNode) {
					...
					if (Tree.DEBUG) {
						Tree.logger.debug("Tree (pageable tree): Adding node: " + oDataNode + " to -> " + this.parentNode.getParent() + " total Child count: "
		...
		protected void executeQuery(Entity entity, Vector vectorAttributes, Hashtable keysValuesSearch, int records, Vector organizationNodes, Vector keys, String[] attributes,
			...
			if (entity instanceof CancellableQueryEntity) {
				...
				if (this.parentNode.isPageableEnabled()) {
					result = ((AdvancedEntity) entity).query(keysValuesSearch, vectorAttributes, this.locator.getSessionId(), records, offset,
			...
			if (this.cancelled) {
				this.res = new EntityResult(EntityResult.OPERATION_SUCCESSFUL_SHOW_MESSAGE, EntityResult.NODATA_RESULT, "entity.operation_cancelled");
				this.hasFinished = true;
				return;
			}
			...
			if (!(this.parentNode instanceof PageFetchTreeNode)) {
				...
				if (resultVector == null) {
					...
					auxResult.setMessage("Error in tree configuration. Node key not found in the results: " + keys.get(0) + " Is in the data : " + result.keySet());
					this.res = auxResult;
					this.hasFinished = true;
					return;
				}
		...
		protected void enableSortingPageFetchTreeNode(final OTreeNode parentNode, Vector organizationNodes, EntityResult result) {
			((OTreeNode) parentNode.getParent()).enableNodeSorting();
			((OTreeNode) parentNode.getParent()).sortNow();
			if (result instanceof AdvancedEntityResult) {
				((OTreeNode) parentNode.getParent()).setTotalCount(((AdvancedEntityResult) result).getTotalRecordCount());
				// si no están todos añadimos un nodo 'aux'
				final OTreeNode nodepru = (OTreeNode) parentNode.clone();
				if (((OTreeNode) parentNode.getParent()).getChildCount() <= ((OTreeNode) parentNode.getParent()).getTotalCount()) {
					if (!((((OTreeNode) parentNode.getParent()).getChildCount() == (((OTreeNode) parentNode.getParent()).getTotalCount() - 1))
		...
		public void run() {
			...
			try {
				...
				try {
					this.executeQuery(entity, vectorAttributes, keysValuesSearch, records, organizationNodes, keys, attributes, t);
					...
					if (this.locator instanceof UtilReferenceLocator) {
						((UtilReferenceLocator) this.locator).removeEntity(this.parentNode.getEntityName(), this.locator.getSessionId());
					}
					return;
				} catch (Exception e) {
					this.res = new EntityResult(EntityResult.OPERATION_WRONG, EntityResult.NODATA_RESULT, e.getMessage());
	...
	/**
	 * This function creates the child nodes of parent node. For organizational
	 * nodes,child nodes are created with data obtained of node entity. Model
	 * for tree only contains organizational nodes, so data nodes are created
	 * dynamically. For each organizational node, this method creates the child
	 * nodes.
	 *
	 * @param parentNode
	...
	/**
	 * This function deletes the child data nodes in an organizational node, for
	 * all child nodes. Organizational nodes are ignored.
	 *
	 * @param oTreeNode
	 *            the node
	 */
	...
	protected boolean processAltUpDownLeftKeyEvent(KeyEvent event, TreePath selectedPath) {
		if ((event.getKeyCode() == KeyEvent.VK_DOWN) && event.isAltDown()) {
			...
			for (int i = selectedRow + 1; i < this.getRowCount(); i++) {
				...
				if ((n.isOrganizational() == selectedNode.isOrganizational()) && n.getText().equals(selectedNode.getText())) {
	...
	protected void processKeyEvent(KeyEvent event) {
		...
		// be expanded.
		if (((this.getSelectionCount() > 0) && ((OTreeNode) this.getSelectionPath().getLastPathComponent()).isOrganizational() && this.isExpanded(this.getSelectionPath()))
				|| ((this.getSelectionCount() > 0) && !((OTreeNode) this.getSelectionPath().getLastPathComponent()).isOrganizational())) {
	...
	/**
	 * Insertion notification. This path will be the associated path to the form
	 * where insertion has been produced. Collapsed nodes has not effect about
	 * tree aspect. If node is expanded, a new node is created with a icon that
	 * indicates new record. When it is not possible, organizational node is
	 * changed indicating that tree is not updated. Moreover, font color in node
	 * will be green.
	 *
	 * @param path
	 *            the path
	 * @param keysValues
	 *            the keysValues of inserted record
	 * @param locator
	 *            the locator
	 * @param selectedChild
	 *            the condition to allow selection children
	 */
	public void insertedNode(TreePath path, Hashtable keysValues, EntityReferenceLocator locator, boolean selectedChild) {
	...
	/**
	 * Notification for deleded node. It is deleted the indicated node and
	 * selected the next node if condition selected is true.
	 *
	 * @param p
	 *            the path
	 * @param keysValues
	 *            the key values that determine the node
	 * @param selected
	 *            the condition for allowing select a node after deleting
	 */
					...
					for (int i = 0; i < ((OTreeNode) node).getChildCount(); i++) {
						...
						if (bKeysMatch) {
							...
							if (((OTreeNode) node).getChildCount() == 1) {
								...
								if (selected) {
									this.setSelectionPath(path.pathByAddingChild(((OTreeNode) node).getChildAt(childToSelect)));
								}
								return;
							}
						}
	...
	/**
	 * Method called when a node is updated. When data changes, node is updated
	 * and tree is put in order. If node dissappears, organizational parent node
	 * is selected.
	 *
	 * @param treePath
	 *            the path
	 * @param attributesValues
	 *            the keys and values before updating node process.
	 * @param filter
	 *            keyvalues to obtain the node to update
	 * @param locator
	 *            the reference to locator
	 */
	public void updatedNode(TreePath treePath, Hashtable attributesValues, Hashtable filter, EntityReferenceLocator locator) {
		if ((treePath == null) || (attributesValues == null) || (filter == null) || attributesValues.isEmpty() || filter.isEmpty()) {
			return;
		}
	...
	private void updateNodeData(OTreeNode node, Hashtable newData, TreePath treePath) {
		...
		for (int k = 0; k < keyNames.size(); k++) {
			...
			if (oNodeValue == null) {
				if (ApplicationManager.DEBUG) {
					Tree.logger.debug(this.getClass().toString() + ": Key " + keyNames.get(k) + " is NULL in node " + node.toString());
				}
				continue;
			}
	...
	protected void updatedNodeAux(TreePath treePath, Hashtable attributesValues, Hashtable keysValues, EntityReferenceLocator locator) {
					...
					for (int j = 0; j < vParentKeys.size(); j++) {
						...
						if ((oNodeValue != null) && (oAttributeValues != null)) {
							if (!oNodeValue.equals(oAttributeValues)) {
								if (ApplicationManager.DEBUG) {
									Tree.logger.debug(this.getClass().toString() + " : Detected change in parentkey: " + parentKeyName + " : Value in node: " + oNodeValue
							...
							if (parentKeyNameInParentNode != null) {
								Tree.logger.debug("TREE: Parentkey " + parentKeyNameInParentNode + " is null in the parent node");
							}
						}
					}
	...
	protected void deleteNodes(final int[] rows) {
		...
		final BufferedMessageDialog bMessages = BufferedMessageDialog.createBufferedMessageDialog(this.parentFrame, null);
			...
			public void run() {
				...
				try {
					...
					for (int i = 0; i < rows.length; i++) {
						...
						Object node = path.getLastPathComponent();
						if ((node instanceof OTreeNode) && (!((OTreeNode) node).isOrganizational()) && ((OTreeNode) node).canDelete()) {
							...
							try {
								...
								if (result.getCode() == EntityResult.OPERATION_WRONG) {
									bMessages.addErrorMessage(sErrorMessage + " Node: " + ((OTreeNode) node).toString() + " " + result.getMessage());
									bErrors = true;
								} else {
					...
					for (int i = 0; i < parent.getChildCount(); i++) {
						...
						try {
							...
							if (!result.isEmpty()) {
								...
								Tree.logger.debug(this.getClass().toString() + "-> Node results is empty:  " + parent.getEntityName());
							}
