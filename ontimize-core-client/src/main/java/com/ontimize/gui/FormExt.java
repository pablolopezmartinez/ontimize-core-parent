...
public class FormExt extends Form {
			...
			for (int i = 0; i < FormExt.this.getDataFieldAttributeList().size(); i++) {
				...
				if (attribute instanceof String) {
					if ((attribute != null) && (FormExt.this.getDataFieldReference(
							attribute.toString()) instanceof DataField) && !((FormExt.this.getDataFieldReference(attribute.toString()) instanceof ImageDataField))) {
						if (!((DataField) FormExt.this.getDataFieldReference(attribute.toString())).isHidden() && FormExt.this.checkAccessPermission(attribute.toString())) {
			...
			this.acceptButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					PrintingSetupWindow.this.printingThread = new Thread() {
						...
						@Override
						public void run() {
							TopWindow w = new TopWindow(FormExt.this.parentFrame, "performing_query", FormExt.this.resourcesFile, ImageManager.getIcon(ImageManager.SEARCHING),
									null);
							try {
								w.show();
								...
								EntityResult res = ent.query(PrintingSetupWindow.this.kvQuery, queryAttributes, locator.getSessionId());
								if (res.getCode() == EntityResult.OPERATION_WRONG) {
									FormExt.this.message(PrintingSetupWindow.this, "form.error_ocurred_while_printing", Form.ERROR_MESSAGE, res.getMessage());
									return;
								} else {
									w.hide();
									w.dispose();
								...
								PrintingSetupWindow.this.setVisible(false);
								FormExt.this.message(PrintingSetupWindow.this, "form.error_ocurred_while_printing", Form.ERROR_MESSAGE);
								return;
							} finally {
								w.hide();
								w.dispose();
	...
	public FormExt(Hashtable parameters) throws Exception {
		...
		if ((this.keys == null) || this.keys.isEmpty()) {
			throw new IllegalArgumentException(this.getClass().toString() + " : Parameter 'keys' is required in FormExt.");
		}
	...
	protected void installButtonListeners() {
		/*
		 * The FormExt only stores the record keys and the current record data. The data list contains the keys and the current record values
		 */
	...
	/**
	 * Gets the attribute list that be used to perform the query. This list is obtained from column list established in the 'column' xml parameter and the columns established by
	 * the user in table view.
	 *
	 ...
	 *
	 * @param keysValues
	 *            a <code>Hashtable</code> with the record keys to be printed.
	 */
		...
		if (!data.isEmpty()) {
			...
			for (int i = 0; i < this.keys.size(); i++) {
				...
				if (!(v instanceof Vector)) {
					FormExt.logger.warn("The Hashtable used by method updateDataFields() does not contain a vector for the key: {}.", oKey);
					this.vectorSize = 1;
					continue;
				}
			...
			if ((this.vectorSize > currentIndex) && (currentIndex >= 0)) {
				...
				if (currentIndex >= this.vectorSize) {
					FormExt.logger.warn("TThe index to update by updateDataFields() method is {} but data size is: {}.", currentIndex, this.vectorSize);
				}
				if (currentIndex >= 0) {
