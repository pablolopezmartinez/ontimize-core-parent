...
public class JEPMathExpressionParser implements MathExpressionParser {
	...
	public JEPMathExpressionParser() {
		try {
			...
			if (ApplicationManager.DEBUG) {
				JEPMathExpressionParser.logger.debug("Error creating JEP parser. Check whether jep library is included in build path.", e);
			}
		}
	}
	...
	public void addFunction(String key, Object function) {
		try {
			if ((this.parser != null) && (function != null) && (function instanceof org.nfunk.jep.function.PostfixMathCommandI)) {
				((org.nfunk.jep.JEP) this.parser).addFunction(key, (org.nfunk.jep.function.PostfixMathCommandI) function);
			}
		} catch (Exception e) {
			JEPMathExpressionParser.logger.error(null, e);
		}
