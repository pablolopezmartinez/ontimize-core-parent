...
/**
 * Extension of <code>PDFFiller</code>, that allows to specify input fields that will remain in output PDF.
 *
...
public abstract class ExtPDFFiller extends PdfFiller {
	...
	/**
	 * Fills PDF fields that parameters included in <code>params</code>. If some of these ones are included in <code>imagesFields</code> will be considered images and will be
	 * filled differently.
	 *
	 ...
	 * @param imagesFields
	 *            fields that are images
	 * @param flatFields
	 *            fields to delete or null when all fields must be filled
	 * @throws Exception
	 *             when an <code>Exception</code> occurs
	 */
	public static void fillTextImagesFields(java.io.InputStream pdfInputStream, java.io.OutputStream pdfOutputStream, Hashtable params, Vector imagesFields, String[] flatFields)
			throws Exception {
		...
		try {
			...
			ExtPDFFiller.fillImageFields(new ByteArrayInputStream(inter.toByteArray()), pdfOutputStream, himagefields, flatFields);
			inter.close();
	 ...
	 *             when an <code>Exception</code> occurs
	 */
	public static void fillFields(InputStream pdfInputStream, OutputStream pdfOutputStream, Hashtable params, String[] flatFields) throws Exception {
		try {
	 ...
	 *             when an <code>Exception</code> occurs
	 */
	public static void fillImageFields(InputStream pdfInputStream, OutputStream pdfOutputStream, Hashtable params, String[] flatFields) throws Exception {
		try {
			ByteArrayOutputStream baOut = new ByteArrayOutputStream();
			BufferedInputStream bInput = new BufferedInputStream(pdfInputStream);
			int a = 0;
					...
					if (keyvalue.page == pageNumber) {
						if (params.containsKey(keyvalue.name)) {
							...
							if (img != null) {
								...
								img.setAbsolutePosition(keyvalue.x1 + ((keyvalue.width() - img.width()) / 2), keyvalue.y1 + ((keyvalue.height() - img.height()) / 2));
								cb.addImage(img);
							}
						}
					}
