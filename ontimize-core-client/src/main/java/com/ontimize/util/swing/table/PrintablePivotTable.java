...
public class PrintablePivotTable implements Printable {
    ...
    private void adjustToPage() {
        // TODO
        this.fixedColumnTable.setBounds(0, 0, this.totalFixedColumnWidth,
                this.fixedColumnTable.getPreferredSize().height);
        this.fixedColumnTable.getTableHeader()
            .setSize(this.totalFixedColumnWidth, this.fixedColumnTable.getTableHeader().getPreferredSize().height);
        this.fixedColumnTable.doLayout();

        this.contentTable.setBounds(0, 0, this.totalContentColumnWidth,
                this.fixedColumnTable.getPreferredSize().height);
        this.contentTable.getTableHeader()
            .setSize(this.totalContentColumnWidth, this.fixedColumnTable.getTableHeader().getPreferredSize().height);
    ...
    private int[] preparePages(PageFormat pf) throws Exception {
        ...
        while (iLastPageRowIndex < (iRowsCount - 1)) {
            int iButtonPagePosition = (iPageCount + 1) * ((int) (this.getImageableHeight() - iHeaderHeight
                    - this.verticalMargin - this.getFooterHeight() - this.getHeaderHeight()));
            ...
            for (int i = iLastPageRowIndex; i < iRowsCount; i++) {
                ...
                if (lastRowPosition > iButtonPagePosition) {
                    PrintablePivotTable.logger.trace("Page time: {} ",
                            iPageCount + " " + (System.currentTimeMillis() - tIni));
    ...
    protected void printContent(Graphics2D g2D, int firstPageRowIndex, int lastPageRowIndex, int firstPageColumnIndex,
            int lastPageColumnIndex) {
        ...
        int lastRowLastPixel = this.fixedColumnTable.getCellRect(lastPageRowIndex, 0, true).y
                + this.fixedColumnTable.getCellRect(lastPageRowIndex, 0, true).height;
        ...
        int lastColumnLastPixel = this.contentTable.getCellRect(0, lastPageColumnIndex, true).x
                + this.contentTable.getCellRect(0, lastPageColumnIndex, true).width;
    ...
    protected void printFooter(Graphics2D g2D, int pageIndex) {
        ...
        footerG2D.fillRect(0, (int) ((int) this.getImageableHeight() - this.getFooterHeight()),
                (int) this.getImageableWidth(), (int) this.getFooterHeight());
        ...
        footerG2D.drawLine(0, (int) (this.getImageableHeight() - this.getFooterHeight()),
                (int) (this.getImageableWidth()),
                (int) (this.getImageableHeight() - this.getFooterHeight()));
        Font font = g2D.getFont();
        footerG2D.setFont(font.deriveFont(font.getSize() - 2l));

        footerG2D.drawString(textoNP,
                (int) (this.getImageableWidth() + ((footerG2D.getFontMetrics().stringWidth(textoNP)))) / 2,
                (int) (this.getImageableHeight() - (footerG2D.getFontMetrics().getHeight())));

        footerG2D.drawString(this.date, (int) (10 / this.scale),
                (int) (this.getImageableHeight() - (footerG2D.getFontMetrics().getHeight())));
        footerG2D.dispose();
        Thread.yield();
    ...
    protected void printHeader(Graphics2D g2D) {
        ...
        if (this.pageSubtitle != null) {
            ...
            Rectangle2D bounds = fontMetrics.getStringBounds(this.pageSubtitle, 0, this.pageSubtitle.length(),
                    headerG2D);
    ...
    protected int getPreferredColumnWidth(JTable table, int i) {
        ...
        if (table.getTableHeader().getDefaultRenderer() != null) {
            ...
            Component cHeaderRenderer = table.getTableHeader()
                .getDefaultRenderer()
                .getTableCellRendererComponent(table, sName, false, false, -1, -1);
            if (cHeaderRenderer instanceof JTextField) {
                FontMetrics metrics = ((JTextField) cHeaderRenderer)
                    .getFontMetrics(((JTextField) cHeaderRenderer).getFont());
