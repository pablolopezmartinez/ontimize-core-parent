...
public class ImageTransferHandler extends TransferHandler {
	...
	protected boolean isReadableByImageIO(DataFlavor flavor) {
		...
		if (readers.hasNext()) {
			...
			return InputStream.class.isAssignableFrom(cls) || URL.class.isAssignableFrom(cls) || File.class.isAssignableFrom(cls);
		}
		return false;
	}
	...
	public boolean canImport(TransferSupport support) {
		...
		for (DataFlavor flavor : support.getDataFlavors()) {
			if (flavor.equals(DataFlavor.imageFlavor) || flavor.equals(DataFlavor.javaFileListFlavor) || this.isReadableByImageIO(flavor)) {
				return true;
			}
		}
		return false;
