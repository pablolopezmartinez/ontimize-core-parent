                    ...
                    try {
                        if (text.charAt(start) != this.NEW_LINE) {
                            while ((end < text.length()) && Character.isWhitespace(text.charAt(end))
                                    && (text.charAt(end) != this.NEW_LINE)) {
                                end++;
                            }
                            if (end > start) {
                                XMLTextPane.this.getDocument()
                                    .insertString(XMLTextPane.this.getCaretPosition(),
                                            this.NEW_LINE + text.substring(start, end), null);
                            } else {
                                XMLTextPane.this.getDocument()
                                    .insertString(XMLTextPane.this.getCaretPosition(),
                                            new String(new char[] { this.NEW_LINE }), null);
                            }
                        } else {
                            XMLTextPane.this.getDocument()
                                .insertString(XMLTextPane.this.getCaretPosition(),
