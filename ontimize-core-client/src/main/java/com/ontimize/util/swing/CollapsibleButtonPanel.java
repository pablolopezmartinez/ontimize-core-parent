...
public class CollapsibleButtonPanel extends JPanel {
	...
	public CollapsibleButtonPanel(boolean right2left) {
		...
		if (right2left) {
			super.addImpl(this.collapsibleLabel, new GridBagConstraints(0, 0, 1, 1, 0, 1, GridBagConstraints.WEST, GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 0), 0, 0), -1);
			super.addImpl(this.viewPort, new GridBagConstraints(1, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0), -1);
		} else {
			super.addImpl(this.collapsibleLabel, new GridBagConstraints(1, 0, 1, 1, 0, 1, GridBagConstraints.WEST, GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 0), 0, 0), -1);
			super.addImpl(this.viewPort, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0), -1);
		}
	...
	protected void calculatedCustomWidth(float fraction, boolean animated) {
		if (animated) {
			if (this.deployedState) {
				this.customWidth = (int) (this.viewPort.getView().getPreferredSize().width * fraction) + this.collapsibleLabel.getPreferredSize().width + this
						.getInsets().left + this.getInsets().right;
			} else {
				this.customWidth = (int) (this.viewPort.getView().getPreferredSize().width * (1 - fraction)) + this.getInsets().left + this.collapsibleLabel
			...
			if (this.deployedState) {
				this.customWidth = this.viewPort.getView().getPreferredSize().width + this.getInsets().left + this.getInsets().right + this.collapsibleLabel
						.getPreferredSize().width;
			} else {
				this.customWidth = this.getInsets().left + this.collapsibleLabel
						.getPreferredSize().width + (this.orientation == CollapsibleButtonPanel.VERTICAL_ORIENTATION ? 10 : 0);
				this.minWidth = this.customWidth;
			}
		}
		this.customHeight = Math.max(this.viewPort.getView().getPreferredSize().height, this.collapsibleLabel.getPreferredSize().height);
	...
	public Dimension getPreferredSize() {
		...
		Dimension dim = new Dimension(viewPortDim.width + labelDim.width, Math.max(labelDim.height, viewPortDim.height));
