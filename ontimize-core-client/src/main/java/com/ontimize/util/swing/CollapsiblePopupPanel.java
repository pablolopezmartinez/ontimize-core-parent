...
public class CollapsiblePopupPanel extends JPanel {
	...
	private Popup getPopup() {
		...
		this.popup = CollapsiblePopupPanel.popupFactory.getPopup(this.getInvoker(), this, this.desiredLocationX, this.desiredLocationY);
		// try {
			...
			java.security.AccessController.doPrivileged(new java.security.PrivilegedAction() {
				...
				public Object run() {
					tk.addAWTEventListener(MouseGrabber.this,
							AWTEvent.MOUSE_EVENT_MASK | AWTEvent.MOUSE_MOTION_EVENT_MASK | AWTEvent.MOUSE_WHEEL_EVENT_MASK | AWTEvent.WINDOW_EVENT_MASK | sun.awt.SunToolkit.GRAB_EVENT_MASK);
					return null;
				}
			});

			this.grabbedWindow = invoker instanceof Window ? (Window) invoker : SwingUtilities.getWindowAncestor(invoker);
	...
	public static void main(String[] args) {
		...
		buttonExcelExport.setIcon(new ImageIcon(copyIcon.getImage().getScaledInstance(16, 16, java.awt.Image.SCALE_DEFAULT)));
		panel.add(buttonExcelExport);

		JButton buttonHTMLExport = new TableButton();
		buttonHTMLExport.setMargin(new Insets(0, 0, 0, 0));
		buttonHTMLExport.setIcon(new ImageIcon(copyIcon.getImage().getScaledInstance(16, 16, java.awt.Image.SCALE_DEFAULT)));
