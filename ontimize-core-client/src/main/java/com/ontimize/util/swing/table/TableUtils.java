...
public abstract class TableUtils {
	...
	public static class PrintableTable implements Printable {
		...
		private void adjustToPage() {
			this.table.setBounds(0, 0, (int) this.pf.getImageableWidth() - this.sideMargin, this.table.getPreferredSize().height);
			this.table.getTableHeader().setSize((int) this.pf.getImageableWidth() - this.sideMargin, this.table.getTableHeader().getPreferredSize().height);
			this.table.doLayout();
		}
		...
		private int[] preparePages(PageFormat pf) {
			...
			if (TableUtils.DEBUG_MEMORY) {
				TableUtils.logger.debug("PrintableTable Establishing columns width: Used memory: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime()
						.freeMemory()) / 1024.0) + " kbytes");
			}
			int[] iColumnsWidth = TableUtils.getPreferredColumnsWidth(this.table);
			if (TableUtils.DEBUG_MEMORY) {
				TableUtils.logger.debug(
						"PrintableTable established column width: Used memory: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024.0) + " kbytes");
			}
			...
			if (TableUtils.DEBUG_MEMORY) {
				TableUtils.logger.debug(
						"PrintableTable searching last rows: Used memory: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1024.0) + " kbytes");
			}

			while (iLastPageRowIndex < (iRowsCount - 1)) {
				int iButtonPagePosition = (iPageCount + 1) * ((int) pf.getImageableHeight() - iHeaderHeight - this.verticalMargin - this.pageNumberMargin - this.titleMargin);
				...
				for (int i = iLastPageRowIndex; i < iRowsCount; i++) {
					...
					if (lastRowPosition > iButtonPagePosition) {
						if (TableUtils.DEBUG_TIMES) {
							TableUtils.logger.debug("Page time: " + iPageCount + " " + (System.currentTimeMillis() - tIni));
						}
			...
			if (TableUtils.DEBUG_MEMORY) {
				TableUtils.logger.debug("PrintableTable has search the maximum number of rows: Memory used: " + ((Runtime.getRuntime().totalMemory() - Runtime.getRuntime()
						.freeMemory()) / 1024.0) + " kbytes");
			}
			...
			if (TableUtils.DEBUG_TIMES) {
				TableUtils.logger.debug("Time to " + index.length + " print pages: " + (System.currentTimeMillis() - t));
			}
		...
		private int printInPage(Graphics g, PageFormat pf, int pageIndex, double scale) {
			...
			g2D.translate(pf.getImageableX() + (this.sideMargin / 2), pf.getImageableY() + (this.verticalMargin / 2) + this.titleMargin);
			// Title
			if (this.pageTitle != null) {
				g2D.setColor(Color.black);
				g2D.setFont(g2D.getFont().deriveFont(Font.BOLD));
				g2D.drawString(this.pageTitle, (int) ((pf.getImageableWidth() - g2D.getFontMetrics().stringWidth(this.pageTitle) - this.sideMargin) / 2),
			...
			g2D.setColor(Color.black);
			g2D.drawLine(this.sideMargin, (int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.titleMargin) - 10,
					(int) (pf.getImageableWidth() - this.sideMargin),
					(int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.titleMargin) - 10);
			g2D.setFont(g2D.getFont().deriveFont((float) 8));
			g2D.drawString(textoNP, (int) (pf.getImageableWidth() - g2D.getFontMetrics().stringWidth(textoNP) - this.sideMargin),
					(int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.titleMargin));

			g2D.drawString(this.date, this.sideMargin, (int) (pf.getImageableHeight() - g2D.getFontMetrics().getDescent() - (this.verticalMargin / 2) - this.titleMargin));
			Thread.yield();

			int firstRowFirstPixel = this.table.getCellRect(firstPageRowIndex, 0, true).y;
			int lastRowLastIndex = this.table.getCellRect(lastPageRowIndex, 0, true).y + this.table.getCellRect(lastPageRowIndex, 0, true).height;
