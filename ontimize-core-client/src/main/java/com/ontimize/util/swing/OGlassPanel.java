...
public class OGlassPanel extends JPanel {
	...
	private void paintContent(Graphics g) {
		Container contentPane = ((RootPaneContainer) SwingUtilities.getAncestorOfClass(RootPaneContainer.class, this)).getContentPane();
		if ((this.contentBuffer == null) || (this.contentBuffer.getWidth() != contentPane.getWidth()) || (this.contentBuffer.getHeight() != contentPane.getHeight())) {
			if (this.contentBuffer != null) {
				this.contentBuffer.flush();
				this.contentGraphics.dispose();
			}
			this.contentBuffer = new BufferedImage(contentPane.getWidth(), contentPane.getHeight(), BufferedImage.TYPE_INT_ARGB);
