...
public class DocxTemplateGenerator extends AbstractTemplateGenerator {
     ...
     * @throws Exception
     */
    protected void findAndReplaceDocument(File templateFilled, Hashtable fieldValues, Hashtable valuesTable,
            ...
            for (Object persistentRow : persistentRows) {
                ...
                for (Tc cell : cells) {
                    ...
                    for (Text oText : cellTexts) {
                        ...
                        if (indexB > -1) {
                            String placeholder = oText.getValue()
                                .substring(indexB,
        ...
        if (!valuesImages.isEmpty()) {
            ...
            for (ArrayList l : replaceImages) {
                ...
                for (R o : rs) {
                    R run = o;
                    if (imageBytes != null) {
                        R newRun = this.createImageRun(mlp, imageBytes, tString, part,
    ...
    protected void findAndReplaceImages(Object element, Hashtable valuesImages, WordprocessingMLPackage mlp)
            throws Exception {
                ...
                for (Tc col : cols) {
                    ...
                    for (Text oText : texts) {
                        ...
                        if (indexB > -1) {
                            ...
                            if (oValue instanceof BytesBlock) {
                                P paragraphWithImage = this.paragraphContentImage(
    ...
    public R createImageRun(WordprocessingMLPackage wordMLPackage, byte[] imageBytes, String tString, Part part,
            long imageWidth) throws Exception {
        BinaryPartAbstractImage imagePart = BinaryPartAbstractImage.createImagePart(wordMLPackage.getPackage(), part,
                imageBytes);
     ...
     * @throws Exception
     */
    protected void findAndReplaceTextBox(WordprocessingMLPackage doc, Hashtable valuesTable, Hashtable valuesImages,
            Hashtable fieldValues) throws Exception {
                    ...
                    if (!filterCTShape.isEmpty()) {
                        for (CTShape ctShape : filterCTShape) {
                            ...
                            for (Object oJaxbActual : ctShapeContent) {
                                List<CTTextbox> ctTextBox = this.getElements(oJaxbActual,
    ...
    public File createTemplate(Hashtable fieldValues, Hashtable valuesTable, Hashtable valuesImages) throws Exception {
        ...
        File template = new File(userDirectory,
            ...
            while (valuesit.hasNext()) {
                ...
                if ((o != null) && (o instanceof String)) {
                    if (!((o.toString().indexOf("${") > -1) && (o.toString().lastIndexOf("}") > -1))) {
                        ...
                        buffer.append(DocxTemplateGenerator.OPEN_PLACEHOLDER + str.toUpperCase()
     ...
     *        must have the table entity name as key and the value must be other Hashtable with the
     ...
     * @throws Exception
     */
    protected void generateDocxTemplate(File template, Hashtable fieldValues, Hashtable valuesTable,
    ...
    /**
     * Build a Vector of single{@link String} with all of the fields data in the Hahstable
     ...
     * @return A Vector of {@link String} with all of the field data stored in the
     ...
     * @param valuesSingleTable : A hashtable which has the name of the columns ad its respective
     *        attributes.
    ...
    /**
     * Create a Vector of single {@link Tbl} table, which the data stored in <code>valuesTable</code>.
    ...
    protected Vector<Tbl> createTablesObject(Hashtable valuesTable, MainDocumentPart mainDocument) {
        ...
        while (keys.hasMoreElements()) {
            ...
            Tbl tableForActualTableKey = this.createSingleTable(tableObjectKey.toString(), valuesTableObjectKey,
                    mainDocument);
