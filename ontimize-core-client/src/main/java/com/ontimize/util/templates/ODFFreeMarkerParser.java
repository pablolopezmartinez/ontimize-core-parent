...
/**
 * Check if a ODF file contains FreeMarker directives. FreeMarker directives are necessary to visualize data tables. http://jooreports.sourceforge.net
...
public class ODFFreeMarkerParser extends ODFParser {
	...
	private static final String XML_TAG_SCRIPT = "text:script", XML_TAG_LANG = "script:language", XML_TAG_LANG_V = "JOOScript", XML_TAG_SCRIPT1 = "@table:table-row\n[#list ",
	...
	public File parse(Hashtable fieldValues, Hashtable valuesTable, Hashtable valuesImages, Hashtable valuesPivotTable) throws Exception {
		...
		for (int i = 0, lTables = nlTables.getLength(); i < lTables; i++) {
			...
			if ((fieldValues == null) && (valuesTable == null) && (valuesPivotTable == null) && (valuesImages == null)) {
				pivotTable = false;
				normalTable = true;
			} else {
			...
			if ((attributes != null) && (attributes.getNamedItem(ODFParser.XML_TAG_DRAW_FRAME_NAME) != null)) {
				try {
					...
					if ((imageName != null) && (valuesImages.get(imageName) != null)) {
						...
						attributes.getNamedItem(ODFParser.XML_TAG_DRAW_FRAME_WIDTH).setNodeValue(String.valueOf(im.getIconWidth() / pixelsPerCM) + "cm");
						attributes.getNamedItem(ODFParser.XML_TAG_DRAW_FRAME_HEIGHT).setNodeValue(String.valueOf(im.getIconHeight() / pixelsPerCM) + "cm");
					}
				} catch (Exception ex) {
	...
	protected void configureNormalTable(Node nTable, int tableIndex, Document document, Hashtable valuesTable) {
		...
		if ((keyStr == null) || (keyStr.length() == 0)) {
			ODFParser.log("ODFFreeMarkerParser -> Table number " + tableIndex + " without description key in first cell.");
			return;
		}
