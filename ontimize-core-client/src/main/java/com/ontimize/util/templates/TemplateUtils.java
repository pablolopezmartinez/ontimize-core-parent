...
public class TemplateUtils {
	...
	static {
		...
		try {
			TemplateUtils.wordTemplateGenerator = TemplateGeneratorFactory.templateGeneratorInstance(TemplateGeneratorFactory.WORD);
		...
		try {
			TemplateUtils.ooTemplateGenerator = TemplateGeneratorFactory.templateGeneratorInstance(TemplateGeneratorFactory.OPEN_OFFICE);
		...
		try {
			TemplateUtils.pdfTemplateGenerator = TemplateGeneratorFactory.templateGeneratorInstance(TemplateGeneratorFactory.PDF);
		...
		try {
			TemplateUtils.docxTemplateGenerator = TemplateGeneratorFactory.templateGeneratorInstance(TemplateGeneratorFactory.DOCX);
	 ...
	 * @param form
	 * @param templateType
	 *            (TEMPLATE_TYPE_DOC,TEMPLATE_TYPE_PDF, TEMPLATE_TYPE_ODT, TEMPLATE_TYPE_DOCX)
	 * @return
	 */
	public static File createTemplate(Form form, int templateType) {
		...
		try {
			...
			template = TemplateUtils.getTemplateGenerator(templateType).createTemplate(im.getTemplateFields(form), im.getTemplateTables(form), im.getTemplateImages(form));
	...
	public static Hashtable getTemplateFields(Form form) {
		...
		for (int i = 0; i < fields.size(); i++) {
			if ((fields.get(i) instanceof DataField) && !(fields.get(i) instanceof ImageDataField) && ((DataField) fields.get(i)).isVisible()) {

				if ((fields.get(i) instanceof ITemplateField) && (((ITemplateField) fields.get(i)).getTemplateDataType() != ITemplateField.DATA_TYPE_FIELD)) {
					continue;
				}
	...
	public static Hashtable getTemplateTables(Form form) {
		...
		for (int i = 0; i < fields.size(); i++) {
			if ((fields.get(i) instanceof Table) && ((Table) fields.get(i)).isVisible()) {
				...
				result.put(entityName, TemplateUtils.getTemplateHashtableTranslation(((Table) fields.get(i)).getVisibleColumns(), form.getResourceBundle()));
			} else if ((fields.get(i) instanceof ITemplateField) && (((ITemplateField) fields.get(i)).getTemplateDataType() == ITemplateField.DATA_TYPE_TABLE)) {
	...
	public static Hashtable getTemplateImages(Form form) {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if ((fields.get(i) instanceof ITemplateField) && (((ITemplateField) fields.get(i)).getTemplateDataType() == ITemplateField.DATA_TYPE_IMAGE)) {
				Object attribute = ((IdentifiedElement) fields.get(i)).getAttribute();
				if (attribute instanceof String) {
					String translate = ApplicationManager.getTranslation((String) attribute, form.getResourceBundle());
					result.put(attribute, translate);
				...
				if (attribute instanceof TableAttribute) {
					String translate = ApplicationManager.getTranslation(((TableAttribute) attribute).getEntity(), form.getResourceBundle());
	...
	public static void fillTemplate(File template, Form form) {
		...
		if (template != null) {
			...
			try {
				...
				TemplateUtils.getTemplateGenerator(templateType).fillDocument(inputStream, "template_" + System.currentTimeMillis() + suffix, im.getFieldValues(form),
	...
	public static Hashtable getFieldsValues(Form form) {
		...
		for (int i = 0; i < components.size(); i++) {
			...
			if ((components.get(i) instanceof ITemplateField) && (((ITemplateField) components.get(i)).getTemplateDataType() == ITemplateField.DATA_TYPE_FIELD)) {
	...
	public static Hashtable getTablesValues(Form form) {
		...
		for (int i = 0; i < components.size(); i++) {
			if ((components.get(i) instanceof ITemplateField) && (((ITemplateField) components.get(i)).getTemplateDataType() == ITemplateField.DATA_TYPE_TABLE)) {
				Object attribute = ((IdentifiedElement) components.get(i)).getAttribute();
				String attr = null;
				if (attribute instanceof String) {
					attr = (String) attribute;
	...
	public static Hashtable getImagesValues(Form form, boolean insertEmptyImages) {
		...
		Hashtable result = new Hashtable();
		for (int i = 0; i < components.size(); i++) {
			if ((components.get(i) instanceof ITemplateField) && (((ITemplateField) components.get(i)).getTemplateDataType() == ITemplateField.DATA_TYPE_IMAGE)) {
