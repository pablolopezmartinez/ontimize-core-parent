...
public abstract class TwainManager {
	...
	public static Image preview(boolean showSourceGUI) throws Exception {
		return TwainManager.acquire(TwainManager.DEFAULT_PREVIEW_RES, TwainManager.DEFAULT_PREVIEW_RES, null, showSourceGUI);
	}
	...
	public static Image acquire(int resX, int resY, Rectangle2D.Double rExt, boolean showSourceGUI) throws Exception {
		...
		try {
			...
			TwainManager.logger.debug("TwainUtilities -> Memory use = " + (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()));
			...
			try {
				if (this.adjustSize > 0) {
					...
					if (maximum == width) {
						...
						Dimension dimension = new Dimension((int) (this.adjustSize * currentZoomValue) + insets.left + insets.right,
						...
						Dimension dimension = new Dimension((int) (width * scale * actualZoom) + insets.left + insets.right,
					...
					int height = rect.height;
					Insets insets = this.getInsets();
					Dimension dimension = new Dimension((int) (width * this.zoom) + insets.left + insets.right, (int) (height * this.zoom) + insets.top + insets.bottom);
					return dimension;
				} else {
					Rectangle rect = this.source.getData().getBounds();
					int width = rect.width;
			...
			this.scan.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						...
						dScale = 1 / dScale;
						rSelection = new Rectangle((int) (rSelection.x * dScale), (int) (rSelection.y * dScale), (int) (rSelection.width * dScale),
								(int) (rSelection.height * dScale));
						Rectangle2D.Double rExp = null;
						if (rSelection != null) {
							rExp = TwainUtilities.convertPixelsToMM(TwainManager.DEFAULT_PREVIEW_RES, TwainManager.DEFAULT_PREVIEW_RES,
									new Rectangle.Double(rSelection.x, rSelection.y, rSelection.width, rSelection.height));
						...
						try {
							...
							JOptionPane.showMessageDialog(PreviewDialog.this.scan, PreviewDialog.this.tfResolution + " It is not a valid resolution", "Error",
