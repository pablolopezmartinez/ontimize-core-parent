...
/**
 * This class is notified when any event occurs in form (value change, click, form changes current mode ...). For this reason it implements the most common component interfaces:
 * {@link ValueChangeListener}, {@link ActionListener}, {@link ListSelectionListener} ... to execute these methods. <br>
		...
		if (ValueEvent.USER_CHANGE == valueEvent.getType()) {
			...
			if (form != null) {
				...
				if (rules != null) {
					...
					List lActions = RuleUtils.findActionsByTypeAndField(rules, RuleParser.Attributes.VALUE_TYPE_EVENT, filterEvents, comp.getAttribute().toString(), null);
					ActionDispatcher.execute(lActions, form);
				}
			} else {
				if (ApplicationManager.DEBUG) {
					RuleEngine.logger.debug(this.getClass().getName() + ": rules cannot be set to component:" + comp.getAttribute() + " due to form parent not detected");
				}
			}
		}
	}
