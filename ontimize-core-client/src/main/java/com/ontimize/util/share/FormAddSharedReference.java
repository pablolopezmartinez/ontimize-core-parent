...
/**
 * This form allows to convert any content into a shared element, allowing to add receiver users, add a custom message or custom name to the element
 *
...
public class FormAddSharedReference extends EJDialog implements Internationalization {
	...
	public class AddUserTargetActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			FormAddUserSharedReference sharedReference = new FormAddUserSharedReference(SwingUtilities.getWindowAncestor((Button) e.getSource()), true,
	 ...
	 * @param content
	 *            -> Content to share using the sharing system
	 * @param componentKey
	 *            -> Key to identify type of element
	 * @param sourceUser
	 *            -> Name of the user who shares the element
	 * @param defaultMessage
	 *            -> A default message to add at form
	 * @param name
	 *            -> Default name of the shared element
	 * @param modifyname
	 *            -> Allow or deny modification of name of the element
	 * @param p
	 *            -> Point where the dialog opens (top-left corner)
	 * @throws Exception
	 */
	public FormAddSharedReference(Window owner, boolean modal, EntityReferenceLocator locator, Object content, String componentKey, String sourceUser, String defaultMessage,
		...
		try {
			this.remoteReference = (IShareRemoteReference) ((UtilReferenceLocator) locator).getRemoteReference(IShareRemoteReference.REMOTE_NAME, locator.getSessionId());
			...
			MessageDialog.showErrorMessage(owner, ApplicationManager.getTranslation("M_SHARE_REMOTE_REFERENCE_NOT_EXIST"));
			throw ex;
		}
	}
	 ...
	 *            -> To specify if the dialog will be opened in a model form, blocking the other parts of application
	 * @param locator
	 *            -> Locator, needer to obtain the remote references to the sharing system
	 * @param content
	 *            -> Content to share using the sharing system
	 * @param componentKey
	 *            -> Key to identify type of the element
	 * @param sourceUser
	 *            -> Name of the user who shares the element
	 * @param defaultMessage
	 *            -> Default message of the shared element
	 * @param p
	 *            -> Point where the dialog opens (top-left corner)
	 * @throws Exception
	 */
	...
	/**
	 * Create and configure a button, associated with the action listener {@link AddUserTargetActionListener}, to add receivers of shared elements
	 *
				...
				if (FormAddSharedReference.this.checkFormShareData()) {
					...
					try {
						EntityResult eR = FormAddSharedReference.this.remoteReference.addSharedItem(
								new SharedElement(FormAddSharedReference.this.memoDataField.getText(), FormAddSharedReference.this.componentShareKey,
										FormAddSharedReference.this.contentShare, FormAddSharedReference.this.sourceUser, FormAddSharedReference.this.nameDataField.getText()),
								v, FormAddSharedReference.this.locator.getSessionId());
						if (eR.getCode() == EntityResult.OPERATION_WRONG) {
							MessageDialog.showErrorMessage(SwingUtilities.getWindowAncestor(FormAddSharedReference.this.getContentPane()),
	...
	/**
	 * Return a boolean if the form has data on all required components, like the receivers, content and key
	 *
	 * @return <code>true</code>, <code>false</code> otherwise.
	 */
	protected boolean checkFormShareData() {
		if (!this.listDataField.isEmpty() && (this.contentShare != null) && (this.componentShareKey != null) && !this.nameDataField.isEmpty()) {
			return true;
		}
		return false;
