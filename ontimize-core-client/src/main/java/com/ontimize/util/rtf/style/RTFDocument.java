...
public class RTFDocument extends DefaultStyledDocument {
	...
	public Element insertTable(int offset, int rowCount, int colCount, AttributeSet attr, int[] colWidths, int[] rowHeights) {
		...
		try {
			...
			AbstractDocument.DefaultDocumentEvent dde = new AbstractDocument.DefaultDocumentEvent(insertOffset, rowCount * colCount, DocumentEvent.EventType.INSERT);
			dde.addEdit(u);
			...
			AbstractDocument.DefaultDocumentEvent e = new AbstractDocument.DefaultDocumentEvent(insertOffset, rowCount * colCount, DocumentEvent.EventType.INSERT);
			e.addEdit(u);
	...
	public void deleteTable(int offset) {
		...
		if (table != null) {
			...
			try {
				AbstractDocument.DefaultDocumentEvent e = new AbstractDocument.DefaultDocumentEvent(start, end - start, DocumentEvent.EventType.REMOVE);
				int index = root.getElementIndex(offset);
				AbstractDocument.ElementEdit ee = new AbstractDocument.ElementEdit(root, index, new Element[] { table }, new Element[0]);
	...
	public void deleteRow(int offset) {
		...
		if (row != null) {
			...
			try {
				...
				AbstractDocument.ElementEdit ee = new AbstractDocument.ElementEdit(table, rowNum, new Element[] { row }, new Element[0]);
				this.getContent().remove(start, end - start);
	...
	public void deleteColumn(int offset) {
		...
		if (cell != null) {
			...
			for (int i = 0; i < table.getElementCount(); i++) {
				...
				AbstractDocument.DefaultDocumentEvent e = new AbstractDocument.DefaultDocumentEvent(editableCell.getStartOffset(),
						editableCell.getEndOffset() - editableCell.getStartOffset(), DocumentEvent.EventType.REMOVE);
				AbstractDocument.ElementEdit ee = new AbstractDocument.ElementEdit(editableRow, colNum, new Element[] { editableCell }, new Element[0]);
				try {
	...
	public void insertRow(int offset, boolean insertAbove) {
		...
		if (row != null) {
			...
			try {
				UndoableEdit u = c.insertString(insertOffset, ins);
				AbstractDocument.DefaultDocumentEvent dde = new AbstractDocument.DefaultDocumentEvent(insertOffset, cellCount, DocumentEvent.EventType.INSERT);
				dde.addEdit(u);
			...
			AbstractDocument.DefaultDocumentEvent e = new AbstractDocument.DefaultDocumentEvent(insertOffset, cellCount, DocumentEvent.EventType.INSERT);
		...
		if (row != null) {
			...
			for (int i = 0; i < table.getElementCount(); i++) {
				...
				try {
					...
					AbstractDocument.DefaultDocumentEvent dde = new AbstractDocument.DefaultDocumentEvent(insertOffset, 1, DocumentEvent.EventType.INSERT);
					dde.addEdit(u);
					MutableAttributeSet attr = new SimpleAttributeSet();
				...
				AbstractDocument.DefaultDocumentEvent e = new AbstractDocument.DefaultDocumentEvent(insertOffset, 1, DocumentEvent.EventType.INSERT);
				CellElement cell;
				if (colNum < editableRow.getElementCount()) {
				...
				if (colNum < editableRow.getElementCount()) {
					...
					AbstractDocument.BranchElement parent = (AbstractDocument.BranchElement) editableRow.getParentElement();
	...
	public void deleteLastParagraph(CellElement cell) {
		...
		AbstractDocument.DefaultDocumentEvent de = new AbstractDocument.DefaultDocumentEvent(start, end - start, DocumentEvent.EventType.REMOVE);
		AbstractDocument.ElementEdit ee = new AbstractDocument.ElementEdit(cell, cnt - 1, new Element[] { par }, new Element[0]);
	...
	public void refresh() {
		AbstractDocument.DefaultDocumentEvent e = new AbstractDocument.DefaultDocumentEvent(0, this.getLength() - 1, DocumentEvent.EventType.CHANGE);
		e.end();
	...
	public class CellElement extends AbstractDocument.BranchElement {
		...
		public CellElement(Element parent, AttributeSet attr, int startOffset, int length, int width, int height) {
			...
			AbstractDocument.LeafElement brk = new AbstractDocument.LeafElement(paragraph, null, startOffset, startOffset + length);
		...
		public void setBorders(BorderAttributes ba) {
			...
			AbstractDocument.DefaultDocumentEvent dde = new AbstractDocument.DefaultDocumentEvent(Math.max(this.getStartOffset() - 1, 0), this.getEndOffset(),
	...
	public class RowElement extends AbstractDocument.BranchElement {
		...
		public RowElement(Element parent, AttributeSet attr, int cellCount, int[] cellOffsets, int[] cellLengths, int[] widths, int height) {
			...
			for (int i = 0; i < cellCount; i++) {
				...
				cells[i] = new RTFDocument.CellElement(this, cellAttr, cellOffsets[i], cellLengths[i], widths[i], height);
			}
	...
	public class TableElement extends AbstractDocument.BranchElement {
		...
		public TableElement(int[] rowOffsets, int[] rowLengths, Element parent, AttributeSet attr, int rowCount, int colCount, int[] widths, int[] heights) {
			...
			for (int i = 0; i < rowCount; i++) {
				...
				rows[i] = new RTFDocument.RowElement(this, rowAttr, colCount, cellOffsets, cellLengths, widths, heights[i]);
			}
