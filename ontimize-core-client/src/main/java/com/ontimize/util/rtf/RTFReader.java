...
public class RTFReader {
	...
	public void read(Reader in, int offset) throws IOException, BadLocationException {
		try {
			...
			for (int i = 0; i < cnt; i++) {
				...
				if (nodeName.equals("\\pard") || nodeName.equals("\\li") || nodeName.equals("\\ri") || nodeName.equals("\\fi") || nodeName.equals("\\f")
						|| nodeName.equals("\\fs")) {
					this.currentOffset = offset;
	...
	private void processDocumentBody(RTFElement root, int indexStartBody, com.ontimize.util.rtf.style.RTFDocument document) throws BadLocationException, IOException {
	...
	private void processBodyGroup(RTFElement bodyGroup, MutableAttributeSet parentAttr, com.ontimize.util.rtf.style.RTFDocument document, String nextNodeName)
			throws BadLocationException, IOException {
		String groupName = this.getGroupName(bodyGroup);
		if (groupName.equals("\\datafield") || groupName.equals("\\fldinst") || groupName.equals("\\object") || groupName.equals("\\themedata") || groupName.equals("\\datastore")
				|| groupName.equals("\\latentstyles") || groupName.equals("\\colorschememapping") || groupName.equals("\\objdata")) {
			return;
		}
	...
	protected void processPage(com.ontimize.util.rtf.style.RTFDocument document, MutableAttributeSet attr) throws BadLocationException {
	...
	protected void processApostropheBodyGroup(com.ontimize.util.rtf.style.RTFDocument document, MutableAttributeSet attr, RTFElement node, String nodeName)
	...
	protected Object processParBodyGroup(com.ontimize.util.rtf.style.RTFDocument document, MutableAttributeSet attr) throws BadLocationException {
		Object nodeContent;
		document.insertString(this.currentOffset, "\n", attr);
	...
	private void processBodyGroupNodeContent(com.ontimize.util.rtf.style.RTFDocument document, String nextNodeName, MutableAttributeSet attr, String previousName, int i,
	...
	private int processTableDefinition(RTFElement tableNode, MutableAttributeSet attr, int index, int level, com.ontimize.util.rtf.style.RTFDocument document) {
		int rowCount = 0;
		int colCount = 0;
		int cellCount = 0;
		// int deep = 0;
		...
		for (int i = index + 1; i < cnt; i++) {
			...
			if (nodeName.equals("\\pard")) {
				...
				if (rowsAttributes.isEmpty()) {
					...
					colCount = Integer.parseInt(((SimpleAttributeSet) rowsAttributes.get(0)).getAttribute("cellCount").toString());
				}
					...
					for (int j = 0; j < rowCount; j++) {
						...
						for (int k = 0; k < colCount; k++) {
							if (cellAttributes.size() >= colCount) {
								...
								com.ontimize.util.rtf.style.RTFDocument.CellElement cell = (com.ontimize.util.rtf.style.RTFDocument.CellElement) table.getElement(j).getElement(k);
	...
	private void processTableContent(RTFElement tableNode, MutableAttributeSet attributes, int startIndex, int endIndex, int level,
	...
	protected void processImage(RTFElement pictureGroup, com.ontimize.util.rtf.style.RTFDocument document) throws IOException, BadLocationException {
	...
	protected void insertImage(String content, com.ontimize.util.rtf.style.RTFDocument document) throws IOException, BadLocationException {
