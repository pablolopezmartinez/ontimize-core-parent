...
public class AttachmentScanWIAFileAction extends AbstractButtonAction {
		...
		public void run() {
			...
			try {
				...
				for (File selectedFile : this.multiFile) {
					...
					synchronized (entF) {
						rId = entF.prepareToReceive(kv, selectedFile.getName(), selectedFile.getPath(), this.description, loc.getSessionId());
					}
					try {
						Thread.sleep(100);
					} catch (Exception ex) {
	...
	public AttachmentScanWIAFileAction(String entity, boolean scanAddDescription, boolean scanRefreshForm, boolean synchronize) {
		this(entity, scanAddDescription, scanRefreshForm, synchronize, null);
	}

	public AttachmentScanWIAFileAction(String entity, boolean scanAddDescription, boolean scanRefreshForm, boolean synchronize, String uriSound) {
	...
	protected ExtendedOperationThread createSendThread(final Form f, final List listOfFiles, final String description, final String uriSound) {
	...
	public void actionPerformed(ActionEvent e) {
		if (WIAUtilities.isWIAEnabled()) {
					...
					if (parentForm == null) {
						...
						if (this.addDescription) {
							oDescription = MessageDialog.showInputMessage(SwingUtilities.getWindowAncestor((Component) e.getSource()),
									AttachmentScanWIAFileAction.M_INSERT_DESCRIPTION_SCAN_ATTACHMENT_FILE, parentForm.getResourceBundle());
						}
						...
						if (device != null) {
							...
							if (device instanceof Scanner) {
								...
								AttachmentScanWIAFileAction.logger.debug("Feeder unit : {}", feederUnit >= 0 ? feederUnit : "none found - trying 0");
								if (feederUnit < 0) {
									feederUnit = 0;
								...
								try {
									...
									if (!session.isFeederUsed()) {
										...
										this.transformAndUploadImage(this.feederFiles, parentForm, bOut, oDescription, w, e);
									}
								} catch (Exception ex) {
									// check if error is related to empty ADF
									if (session.isEmptyFeeder()) {
										AttachmentScanWIAFileAction.logger.debug("No more sheets in the document feeder", ex);
									} else {
										AttachmentScanWIAFileAction.logger.error(null, ex);
									}
								}
			...
			AttachmentScanWIAFileAction.logger.error("The Twain libraries are not present in this application. This button not perform any action");
		}
	}

	private void transformAndUploadImage(List<File> files, Form parentForm, ByteArrayOutputStream bOut, Object oDescription, Window w, ActionEvent e) throws IOException {
		...
		if (files.isEmpty()) {
			...
			if (this.synchronize) {
				ExtendedOperationThread eop = this.createSendThread(parentForm, jpegImages, (String) oDescription, this.uriSound);
