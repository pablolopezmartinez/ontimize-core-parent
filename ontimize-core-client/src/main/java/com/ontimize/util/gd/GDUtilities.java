...
public class GDUtilities {
	...
	public static class Result implements Serializable {
		...
		public String toString() {
			return this.getCategory() + " " + this.getId() + " " + this.getTitle() + " " + this.getURL() + " " + this.getTime();
		}
	}
	...
	public static String getURL(String queryString, String start, String num, String flags) throws Exception {
		...
		StringBuilder.append(MessageFormat.format(GDUtilities.getDesktopURL(), new String[] { URLEncoder.encode(StringBuilder1.toString()) }));
		if (num != null) {
	...
	public static InputStream getResultInputStream(String queryString, String start, String num, String flags) throws Exception {
	...
	public static Result[] search(String query, int start, int items, File rootDir, String category, String name) throws Exception {
		InputStream in = GDUtilities.getResultInputStream(query, start >= 0 ? "" + start : null, items > 0 ? "" + items : null, null);
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if ((n.getNodeType() == Node.ELEMENT_NODE) && n.getNodeName().equalsIgnoreCase("result")) {
					...
					for (int j = 0; j < l2.getLength(); j++) {
						...
						} else if (n2.getNodeName().equalsIgnoreCase("id") || n2.getNodeName().equalsIgnoreCase("doc_id")) {
	 ...
	 *
	 * @param root
	 *            File. Directory in which the results must be stored. The results returned will be only of type 'file'. If null no filter is applied to location of the result.
	 * @param category
	 *            String. Type of the result. If null no filter is applied to the type of the result.
	 * @param name
	 *            String. String that must be contained in the name of the resource (case-insensitive). If null no filter is applied.
	 * @param res
	 *            Result[]
	 * @return Result[]
	 */
