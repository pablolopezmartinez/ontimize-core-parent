...
public class ImageRepository {
	...
	/**
	 * Gets an image loading it from the specified URL. If the image is loaded yet returns it, other case start the image load.<br>
	...
	public synchronized static java.awt.Image getImage(URL imageURL, ImageObserver observer) {
		...
		if (ImageRepository.images.containsKey(imageURL)) {
			...
			if (ImageRepository.getLoadingState(imageURL) == MediaTracker.COMPLETE) {
				observer.imageUpdate(image, ImageObserver.ALLBITS, 0, 0, image.getWidth(observer), image.getHeight(observer));
			} else {
				try {
			...
			if (id == null) {
				...
				if (ImageRepository.tracker.isErrorID(id.intValue())) {
					...
					try {
						...
						ImageRepository.logger.trace("Total loading time {} : {} millis", imageURL, System.currentTimeMillis() - t);
					} catch (Exception e) {
						ImageRepository.logger.error("Loading process interrupted: {}", imageURL, e.getMessage(), e);
					}
				}
