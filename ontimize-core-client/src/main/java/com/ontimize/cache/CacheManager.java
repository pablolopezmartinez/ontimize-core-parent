...
public class CacheManager {
	...
	protected synchronized DataCacheId existsCache(String entity, Vector attributes, Hashtable keysValues, long cacheTime) {
	...
	public synchronized Pair<DataCacheId, EntityResult> retrieveDataCache(String entity, Vector attributes, Hashtable keysValues, long cacheTime) {
	...
	public synchronized EntityResult getDataCache(String entity, Vector attributes, Hashtable keysValues) {
		...
		try {
			...
			try {
				res = ent.query(keysValues == null ? new Hashtable() : keysValues, dId.attributes, this.locator.getSessionId());
			...
			if (CacheManager.logger.isDebugEnabled()) {
				...
				for (int i = 0; i < dId.attributes.size(); i++) {
					if (!at.contains(dId.attributes.get(i))) {
						CacheManager.logger.warn("CacheManager: The entity was asked for: {} but the answer does not contain those values", dId.attributes.get(i));
					}
				}
			}
		...
		protected void setHeight() {
			for (int i = 0; i < this.cacheTable.getRowCount(); i++) {
				...
				for (int j = 0; j < 3; j++) {
					...
					Component c = this.memoRender.getTableCellRendererComponent(this.cacheTable, this.cacheTable.getValueAt(i, this.cacheTable.convertColumnIndexToView(j)), false,
