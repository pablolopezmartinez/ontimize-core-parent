...
public class ScriptUtilities {
	...
	/**
	 * Prints and fills a word document replacing <code>Hashtable</code> keys by correspondent descriptions. Max. length for description is 255 characters.
	 *
	 * @param archive
	 *            The name of file
	 * @param valuesDescriptions
	 *            <code>Hashtable</code> with texts and descriptions
	 * @return true Whether document has been filled and printed correctly
	 * @throws Exception
	 *             When a <code>Exception</code> occurs
	 */
	public static boolean fillAndPrintWordDocument(String archive, Hashtable valuesDescriptions) throws Exception {

		File f = ScriptUtilities.createTemporalFileForScript("com/ontimize/windows/office/scripts/fillandprintworddocument.vbs");
	...
	public boolean report2Snapshot(String bdArchive, String report, String dest, String conditions) throws Exception {
		...
		ExecutionResult res = ScriptUtilities.executeScript(f.toString(), bdArchive + " " + report + " " + dest + " " + conditions);
		ScriptUtilities.logger.debug(res.getOuput());
		if (res.getResult() != 0) {
			return false;
		} else {
