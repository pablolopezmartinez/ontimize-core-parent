...
public abstract class WindowsUtils {
	...
	public static final void openFile_Script(File f) throws Exception {
		...
		if (script.startsWith("file:")) {
			...
			File fScript = ScriptUtilities.createTemporalFileForScript("com/ontimize/windows/office/scripts/openfile.vbs");
		...
		ExecutionResult res = ScriptUtilities.executeScript(FileUtils.decode(script), fileName, ScriptUtilities.WSCRIPT);
		if (res.getResult() != 0) {
	...
	public static final void printFile_Script(File f) throws Exception {
		...
		if (script.startsWith("file:")) {
			...
			File fScript = ScriptUtilities.createTemporalFileForScript("com/ontimize/windows/office/scripts/printfile.vbs");
			script = fScript.getAbsolutePath();
		} else {
	...
	public static final void openFileTemplate(File f, File fData) throws Exception {
		...
		if (script.startsWith("file:")) {
			...
			File fScript = ScriptUtilities.createTemporalFileForScript("com/ontimize/windows/office/scripts/openwordtemplate.vbs");
			script = fScript.getAbsolutePath();
		} else {
			throw new Exception("Invalid script URL: " + url);
		}
