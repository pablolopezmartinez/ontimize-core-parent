...
public abstract class SystrayUtils {
	...
	/**
	 * @param icon
	 *            Relative path to the resource without extension to ensure the portability
	 * @param tip
	 * @param menu
	 * @param sListener
	 *            Listener with interest in menu item and icon events
	 * @return
	 */
	public static Object addSystemTrayIcon(String icon, String tip, JMenu menu, final SystrayListener sListener) {
		...
		URL url = SystrayUtils.class.getClassLoader().getResource(icon + snoozesoft.systray4j.SysTrayMenuIcon.getExtension());
		if (url == null) {
	...
	private static void createSubItems(snoozesoft.systray4j.SubMenu tMenu, JMenu menu, snoozesoft.systray4j.SysTrayMenuListener l) {
		for (int i = 0; i < menu.getItemCount(); i++) {
			JMenuItem item = menu.getItem(i);
			if (item == null) {
				tMenu.addSeparator();
			...
			if (item instanceof JMenu) {
				...
				snoozesoft.systray4j.SysTrayMenuItem it = new snoozesoft.systray4j.SysTrayMenuItem(item.getActionCommand());
				it.addSysTrayMenuListener(l);
				it.setLabel(item.getText());
				tMenu.addItem(it);
