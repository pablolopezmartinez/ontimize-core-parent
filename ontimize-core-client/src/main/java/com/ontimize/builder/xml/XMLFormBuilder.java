...
public class XMLFormBuilder extends XMLInterpreter implements FormBuilder {
	...
	public Form buildForm(Container parentContainer, String fileURI) {
		...
		try {
			...
			XMLFormBuilder.logger.trace("Total time building UI: {}  seconds.", new Double(tiempoTranscurrido).toString());
	...
	public Form buildForm(Container parentContainer, InputStream input) {
		try {
			...
			XMLFormBuilder.logger.trace("Total time building UI from InputStream: {} seconds.", new Double(totalTime).toString());
			return (Form) parentContainer.getComponent(0);
		} catch (Exception e) {
	...
	public Form buildForm(Container parentContainer, StringBuffer fileContent) throws Exception {
		try {
			...
			long endTime = System.currentTimeMillis();
			double totalTime = (endTime - initTime) / 1000.0;
			XMLFormBuilder.logger.trace("Total time building UI from StringBuilder: {} seconds.", new Double(totalTime).toString());
			return (Form) parentContainer.getComponent(0);
		} catch (Exception e) {
	...
	protected void processChildren(CustomNode node, Container parentContainer) {
		Container container = this.interpreterTag(node, parentContainer, this.defaultPackage, this.equivalenceLabelList);
					...
					if (attributes.containsKey(ModuleManager.MODULE_ATTR)) {
						...
						while (tokens.hasMoreTokens()) {
							...
							if (!this.moduleManager.hasModule(moduleName)) {
								hasModules = false;
								XMLFormBuilder.logger.info("{} attr has not been created by {} module dependencies", attr, moduleName);
								break;
							}
						}
