...
public class LocalRemoteReportStore implements ReportStore {
	...
	private final String storePath = LocalRemoteReportStore.DEFAULT_DIRECTORY + File.separator + LocalRemoteReportStore.SUBDIR_NAME;
	...
	public LocalRemoteReportStore(ReportStore rs, ResourceBundle bundle) {
		...
		this.bundle = bundle;
		try {
			this.description = this.rs.getDescription(ApplicationManager.getApplication().getReferenceLocator().getSessionId());
		} catch (Exception e) {
	...
	private URL saveTemp(String reportId, ReportStoreDefinition rDef) throws IOException {
		...
		File f2 = new File(this.storePath + File.separator + this.generateDirectoryName(reportId), this.generatePropertiesFileName(reportId));
		f2.deleteOnExit();
		FileOutputStream fOutProp = new FileOutputStream(f2);
		FileOutputStream fOutRes = null;
		File fObject = new File(this.storePath + File.separator + this.generateDirectoryName(reportId), this.generateExpressionFileName(reportId));
		...
		try {
			...
			if ((resources != null) && (resources.length > 0)) {
				...
				for (int i = 0; i < resources.length; i++) {
					File resName = new File(this.storePath + File.separator + this.generateDirectoryName(reportId), resources[i].getName());
	...
	/**
	 * Deletes directory and files associated with report (.xml,.qry and .properties file) in local report store.
	 *
	 * @param path
