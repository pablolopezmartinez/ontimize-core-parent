...
public class PopupGroupDateListener extends MouseAdapter {
	...
	public void mouseClicked(MouseEvent e) {
		if ((e.getClickCount() == 1) && (e.getModifiers() == InputEvent.META_MASK) && (this.reportDialog.getReportEngine() instanceof IGroupByDate)) {
			// Check the selected item
			int x = e.getX();
			int y = e.getY();
			int index = this.reportDialog.getGroupList().locationToIndex(e.getPoint());
			if (index >= 0) {
				if (!(this.reportDialog.getGroupList().getModel().getElementAt(index) instanceof SelectableDateGroupItem)) {
					return;
				}
				this.reportDialog.setCurrentDateGroupItem((SelectableDateGroupItem) this.reportDialog.getGroupList().getModel().getElementAt(index));
				...
				this.reportDialog.getGroupByDatePopup().setLabel(this.reportDialog.getCurrentDateGroupItem().toString());
