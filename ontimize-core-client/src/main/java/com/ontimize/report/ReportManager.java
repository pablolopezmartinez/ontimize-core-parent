...
public class ReportManager {
	...
	public static boolean createReportEngine() {
		try {
			...
			try {
				...
				ReportManager.logger.error("ReportManager: Neither DynamicJasper nor JFreeReport engines registered", e1);
				return false;
			}
			...
			if (ReportManager.reportEngine.checkLibraries()) {
				ReportManager.logger.info("Report engine: {} succesfully registered", ReportManager.reportEngine.getReportEngineName());
				return true;
			} else {
				ReportManager.logger.warn("Report engine found: {} but missing some required libraries.", ReportManager.reportEngine.getReportEngineName());
				return false;
			}
		}
		try {
	...
	public static void registerNewReportEngine(ReportEngine reportEngine) {
		...
		if (reportEngine.checkLibraries()) {
			...
			ReportManager.logger.warn("Report engine found: {} but missing some required libraries.", reportEngine.getReportEngineName());
		}
	}
