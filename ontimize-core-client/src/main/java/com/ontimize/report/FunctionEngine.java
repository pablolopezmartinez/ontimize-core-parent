				...
				for (int i = 0; i < this.colsFunc.size(); i++) {
					...
					if ("ReportDesigner.NumeroOcurrencias".equalsIgnoreCase(sCurrentElement)) {
						...
						if (operationCode == ReportUtils.SUM) {
							FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemSumFunction for the external group");
							function.setAttribute("class", "org.jfree.report.function.ItemSumFunction");
						}
						if (operationCode == ReportUtils.MIN) {
							FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemMinFunction for the external group");
							function.setAttribute("class", "com.ontimize.report.ItemMinFunction");
						}
						if (operationCode == ReportUtils.MAX) {
							FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemMaxFunction for the external group");
							function.setAttribute("class", "org.jfree.report.function.ItemMaxFunction");
						}
						if (operationCode == ReportUtils.AVG) {
							FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemAvgFunction for the external group");
					...
					for (int j = 0; j < this.groupNames.size(); j++) {
						function = this.doc.createElement("function");
						function.setAttribute("name", this.colsFunc.elementAt(i).toString() + "FunctionParaGrupo" + this.groupNames.elementAt(j));
						...
						if ("ReportDesigner.NumeroOcurrencias".equalsIgnoreCase(sCurrentElement)) {
							...
							if (operationCode == ReportUtils.SUM) {
								FunctionEngine.logger.debug("JFreeReport -- INFO: Generating TotalGroupSumFunction for the group {}", this.groupNames.elementAt(j));
								function.setAttribute("class", "org.jfree.report.function.TotalGroupSumFunction");
							}
							if (operationCode == ReportUtils.MIN) {
								FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemMinFunction for the group {}", this.groupNames.elementAt(j));
								function.setAttribute("class", "com.ontimize.report.ItemMinFunction");
							}
							if (operationCode == ReportUtils.MAX) {
								FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemMaxFunction for the group {}", this.groupNames.elementAt(j));
								function.setAttribute("class", "org.jfree.report.function.ItemMaxFunction");
							}
							if (operationCode == ReportUtils.AVG) {
								FunctionEngine.logger.debug("JFreeReport -- INFO: Generating ItemAvgFunction for the group {}", this.groupNames.elementAt(j));
								function.setAttribute("class", "org.jfree.report.function.ItemAvgFunction");
							}
						}
