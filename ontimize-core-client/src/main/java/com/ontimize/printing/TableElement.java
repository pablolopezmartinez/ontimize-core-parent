...
public class TableElement extends RectangleElement {
	...
	public TableElement(Hashtable parameters) {
		...
		if (cols == null) {
			...
			try {
				...
				if (painthlines != null) {
					if (painthlines.equals("no")) {
						...
						this.table.getJTable().setIntercellSpacing(new Dimension(this.table.getJTable().getIntercellSpacing().width, 0));
					}
				}
				...
				if (paintvlines != null) {
					if (paintvlines.equals("no")) {
						...
						this.table.getJTable().setIntercellSpacing(new Dimension(0, this.table.getJTable().getIntercellSpacing().height));
					}
				}
			} catch (Exception e) {
			...
			if ((this.width != -1) && (this.high != -1)) {
				this.table.getJTable().getTableHeader().setSize(AbstractPrintingElement.millimeterToPagePixels(this.width) * 1,
						this.table.getJTable().getTableHeader().getPreferredSize().height);
				this.table.getJTable().setSize(AbstractPrintingElement.millimeterToPagePixels(this.width) * 1, AbstractPrintingElement.millimeterToPagePixels(this.high) * 1);
				this.table.doLayout();
			} else {
				int iTableWidth = Math.max(this.table.getJTable().getPreferredSize().width, this.table.getJTable().getTableHeader().getPreferredSize().width);
				this.table.getJTable().setSize(iTableWidth, this.table.getJTable().getPreferredSize().height);
				this.table.getJTable().getTableHeader().setSize(iTableWidth, this.table.getJTable().getTableHeader().getPreferredSize().height);
				this.table.doLayout();
			}
		}
	}
	...
	public void setContent(Object content) {
		if (this.table != null) {
			...
			if ((this.width != -1) && (this.high != -1)) {
				this.table.setSize(AbstractPrintingElement.millimeterToPagePixels(this.width) * 1, AbstractPrintingElement.millimeterToPagePixels(this.high) * 1);
				this.table.getJTable().getTableHeader().setSize(AbstractPrintingElement.millimeterToPagePixels(this.width) * 1,
						this.table.getJTable().getTableHeader().getPreferredSize().height);

				this.table.getJTable().setBounds(0, 0, AbstractPrintingElement.millimeterToPagePixels(this.width) * 1,
				...
				this.table.getJTable().getTableHeader().setSize(AbstractPrintingElement.millimeterToPagePixels(this.width) * 1,
						this.table.getJTable().getTableHeader().getPreferredSize().height);
				this.table.getJTable().setBounds(0, 0, AbstractPrintingElement.millimeterToPagePixels(this.width) * 1, this.table.getJTable().getPreferredSize().height);
				this.table.initColumnsWidth();
				this.table.getJTable().doLayout();
			...
			this.bi = new BufferedImage(iCellwidth - this.table.getJTable().getCellRect(0, 0, false).width,
					this.table.getJTable().getHeight() + this.table.getJTable().getTableHeader().getHeight(), BufferedImage.TYPE_3BYTE_BGR);
			Graphics g = this.bi.getGraphics();
			g.translate(-this.table.getJTable().getCellRect(0, 0, false).width, 0);
			g.setClip(this.table.getJTable().getCellRect(0, 0, false).width, 0, this.bi.getWidth(), this.bi.getHeight());
	...
	public void paint(Graphics g, double scale) {
		if (this.table != null) {
			...
			if (Double.compare(scale, 1.0) != 0) {
				if (this.bi != null) {
					g.drawImage(this.bi, 0, 0, (int) (this.bi.getWidth() * scale), (int) (this.bi.getHeight() * scale), this.table);
				}
			} else {
				if (this.table != null) {
					Shape r = g.getClip();
					...
					g.translate(-x, 0);
					g.setClip(this.table.getJTable().getCellRect(0, 0, false).width, 0, this.bi == null ? 0 : this.bi.getWidth(),
