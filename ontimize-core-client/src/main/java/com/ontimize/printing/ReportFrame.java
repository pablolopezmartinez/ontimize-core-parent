...
public class ReportFrame extends JFrame implements com.ontimize.gui.Freeable {
	...
	public ReportFrame(URL templateURL, PageFormat formatoPag, boolean multipage, PageHeader pageHeader, PageFooter pageFooter) {
			...
			public void actionPerformed(ActionEvent event) {
				...
				if (iOption == JFileChooser.APPROVE_OPTION) {
					try {
						...
						Integer iSelectedPage = (Integer) JOptionPane.showInputDialog(ReportFrame.this, "Select page", "Report", JOptionPane.QUESTION_MESSAGE, null, oPageList,
								oPageList[0]);

						FileWriter fWriter = new FileWriter(fileChooser.getSelectedFile());
						HTMLDocument htmlDocument = (HTMLDocument) ((Page) ReportFrame.this.pages.get(iSelectedPage.intValue() - 1)).getDocument();
						((Page) ReportFrame.this.pages.get(iSelectedPage.intValue() - 1)).getHTMLEditor().write(fWriter, htmlDocument, 0, htmlDocument.getLength());
						fWriter.flush();
						fWriter.close();
					} catch (Exception e) {
		...
		this.scrollPanel.setPreferredSize(new Dimension(this.scrollPanel.getPreferredSize().width, (int) this.pageFormat.getImageableHeight()));
		this.pack();
		// Center
		...
		try {
			...
			try {
				if (reportElement instanceof TableReportElement) {
					...
					Element tableElement = this.currentPage.getElementById(((TableReportElement) reportElement).getId()).getElement(0);
	 ...
	 * @param reportElement
	 * @param identifier
	 *            Attribute of the place in the template to insert the new element
	 */
	...
	public void insertImage(String identifier, String name, int width, int height) {
		...
		if (element != null) {
			try {
				...
				if (width == 0) {
					sHTMLString = "<IMG src='" + name + "' width='auto' height='" + Integer.toString(height) + "'></IMG>";
				} else {
					sHTMLString = "<IMG src='" + name + "' width='" + Integer.toString(width) + "' height='" + Integer.toString(height) + "'></IMG>";
				}
				this.currentPage.getHTMLDocument().insertBeforeEnd(element, sHTMLString);
			} catch (Exception e) {
				ReportFrame.logger.trace(null, e);
