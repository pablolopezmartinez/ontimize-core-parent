...
public class Image implements ReportElement {
	...
	@Override
	public void insert(ReportFrame reportFrame, String templateElementIdentifier, boolean multiplePage) throws Exception {
		...
		if (element != null) {
			...
			FOSout.flush();
			htmlDocument.insertAfterStart(element, "<IMAGE src='" + fTemp.toURL() + "' height='" + Integer.toString(height) + "' width='" + Integer.toString(width) + "'></IMAGE>");
			FOSout.close();
