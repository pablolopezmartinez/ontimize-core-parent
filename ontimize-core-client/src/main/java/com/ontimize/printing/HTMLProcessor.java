...
public class HTMLProcessor {
		...
		try {
			...
			for (int i = 0; i < (sHTMLContent.length() - iTextLenght); i++) {
				...
				if (sHTMLContent.regionMatches(i, text, 0,
						iTextLenght) && !Character.isLetterOrDigit(sHTMLContent.charAt(i - 1)) && !Character.isLetterOrDigit(sHTMLContent.charAt(i + iTextLenght + 1))) {
	...
	public void replaceTextInHTML(String text, byte[] imageBytes, int width, int height) {
		...
		try {
			...
			for (int i = 0; i < (sHTMLContent.length() - iTextLength); i++) {
				if (sHTMLContent.regionMatches(i, text, 0,
						iTextLength) && !Character.isLetterOrDigit(sHTMLContent.charAt(i - 1)) && !Character.isLetterOrDigit(sHTMLContent.charAt(i + iTextLength + 1))) {
					...
					String sNewText = "<IMG src='" + temporal.toURL() + "' height='" + Integer.toString(height) + "' width='" + Integer.toString(width) + "'></IMG>";
	...
	/**
	 * Insert a table using its values, replacing in the html code the key word <code>text</code> with the table HTML equivalence.<br>
	 *
	 * @param text
	 * @param tableValues
	 *            The keys of this hashtable are the names of the table columns and the values are vectors whit the column values. If some of these values are hashtables then they
	 *            are new tables in the appropriate cell.
	 * @param sortColumns
	 *            This vector contains the name of the table columns in the order to put them in the table. These columns must be the keys of the <code>tableValues</code> hashtable
	 * @return
	 */
	public String insertTable(String text, Hashtable tableValues, Vector sortColumns) {
		...
		for (int j = 0; j < sortColumns.size(); j++) {
			if (tableValues.containsKey(sortColumns.get(j))) {
				sbTableHeader.append(HTMLProcessor.CELL_HEAD_TAG + "<FONT size='1'>" + sortColumns.get(j) + "</Font>" + HTMLProcessor.END_CELL_HEAD_TAG);
			}
		}
			...
			for (int j = 0; j < sortColumns.size(); j++) {
				...
				if (oValue == null) {
					...
					if (oValue instanceof Hashtable) {
						...
						// hashtable
						sbRowString.append(HTMLProcessor.CELL_TAG + this.insertTable(null, (Hashtable) oValue, sortColumns) + HTMLProcessor.END_CELL_TAG);
					} else {
						sbRowString.append(HTMLProcessor.CELL_TAG + "<FONT size='1'>" + oValue.toString() + "</Font>" + HTMLProcessor.END_CELL_TAG);
					}
				}
			}
	...
	public String insertTable(String text, Hashtable tableValues) {
		...
		for (int j = 0; j < vSortColumns.size(); j++) {
			if (tableValues.containsKey(vSortColumns.get(j))) {
				sbTableHeader.append(HTMLProcessor.CELL_HEAD_TAG + "<FONT size='1'>" + vSortColumns.get(j) + "</Font>" + HTMLProcessor.END_CELL_HEAD_TAG);
			}
		}
		sbTableHeader.append(HTMLProcessor.END_ROW_TAG);
		sbHTMLTableCode.append(sbTableHeader.toString());
			...
			for (int j = 0; j < vSortColumns.size(); j++) {
				...
				if (oValue == null) {
					...
					if (oValue instanceof Hashtable) {
						...
						// cell
						sbRowString.append(HTMLProcessor.CELL_TAG + this.insertTable(null, (Hashtable) oValue) + HTMLProcessor.END_CELL_TAG);
					} else {
						sbRowString.append(HTMLProcessor.CELL_TAG + "<FONT size='1'>" + oValue.toString() + "</Font>" + HTMLProcessor.END_CELL_TAG);
					}
				}
	...
	/**
	 * Gets a vector of JTextPane with the document separated by pages with the specified page format.<br>
	 *
	 * @param pageFormat
	 * @return
	 */
	public Vector toPages(PageFormat pageFormat) {
		...
		try {
			...
			while (index < sHTMLContent.length()) {
				...
				textPaneAux.paintImmediately(0, 0, (int) pageFormat.getImageableWidth(), (int) pageFormat.getImageableHeight());
				...
				pj.setPrintable(new Printable() {
					...
					public int print(Graphics g, PageFormat pf, int iPageIndex) {
						if (iPageIndex < iPageCount) {
							...
							g2.drawString(Integer.toString(iPageIndex + 1), (int) pf.getImageableWidth() - 15, (int) pf.getImageableHeight() - 15);
	...
	/**
	 * Fixes the element with attribute <code>id</code> as header in the template for all the pages. If the element has children the children will be in the header too.
	 *
	 * @param id
	 */
	...
	/**
	 * Fixes the element with attribute <code>id</code> as footer in the template for all the pages.If the element has children the children will be in the footer too.
	 *
	 * @param id
	 */
	 ...
	 *
	 * @param pf
	 *            Page format to use in the printing method
	 * @param height
	 *            Height in pixels from the text pane of this processor
	 */
				...
				pj.setPrintable(new Printable() {
					...
					public int print(Graphics g, PageFormat pf, int pageIndex) {
						if (pageIndex < iNumberOfPages) {
							...
							HTMLProcessor.this.textPane.paint(g2);
							g2.drawString(Integer.toString(pageIndex + 1), (int) pf.getImageableWidth(), (int) pf.getImageableHeight());
		...
		return new Printable() {
			...
			public int print(Graphics g, PageFormat pf, int pageIndex) {
				if (pageIndex < iNumberOfPages) {
					...
					g2.translate(pf.getImageableX(), pf.getImageableY());
					g2.translate(0f, -(pageIndex * pf.getImageableHeight()));
					g2.setClip(0, (int) (pageIndex * pf.getImageableHeight()), (int) pf.getImageableWidth(), (int) (pageIndex * pf.getImageableHeight()) + fIHeaderHeight);
					HTMLProcessor.this.textPane.paint(g2);
					if (HTMLProcessor.DEBUG) {
						HTMLProcessor.logger.debug("Header height = " + fIHeaderHeight + " , Footer height = " + fIFootHeight);
					}
					g2.drawString(Integer.toString(pageIndex + 1), (int) pf.getImageableWidth() - 12, (int) pf.getImageableHeight() - 12);
					return Printable.PAGE_EXISTS;
				} else {
					return Printable.NO_SUCH_PAGE;
				}
