...
public class TemplateElement implements PrintingElement {
	...
	class PagePanel extends JPanel implements Printable {
		...
		protected void paintMargins(Graphics g, double zoom) {
			...
			double dRightMargin = (TemplateElement.this.pf.getImageableWidth() * zoom) + (TemplateElement.this.pf.getImageableX() * zoom);
			double dTopMargin = TemplateElement.this.pf.getImageableY() * zoom;
			double dBottomMargin = (TemplateElement.this.pf.getImageableHeight() * zoom) + (TemplateElement.this.pf.getImageableY() * zoom);
		...
		protected synchronized void paintComponent(Graphics g, double zoom, boolean paintMargins) {
			...
			((Graphics2D) g).translate(zoom * TemplateElement.this.pf.getImageableX(), zoom * TemplateElement.this.pf.getImageableY());
	...
	public TemplateElement(Hashtable parameters) {
		...
		if ((iBottomMargin != -1) || (iTopMargin != -1) || (iLeftMargin != -1) || (iRightMargin != -1)) {
			...
			if (ApplicationManager.DEBUG) {
				TemplateElement.logger.debug("Template margins: " + mi + " , " + ms + " , " + (dWidth - mder - mi) + " , " + (dHeight - minf - ms));
			}
	...
	protected JPanel initComponent() {
		...
		this.centralPanel.add(this.view, new GridBagConstraints(0, 0, 1, 1, 1, 1, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(0, 0, 0, 0), 0, 0));
	...
	public void preview(Frame f, int width, int height) {
		...
		this.view.setPreferredSize(
				new Dimension((int) (dLeftMargin + dRigthMargin + this.pf.getImageableWidth()), (int) (dTopMargin + dBottomMargin + this.pf.getImageableHeight())));
	...
	private void changeZoom(double z) {
		...
		double dTopMargin = this.pf.getImageableY();
		double dBottomMargin = this.pf.getHeight() - this.pf.getImageableHeight() - this.pf.getImageableY();
		this.view.setPreferredSize(new Dimension((int) (this.zoom * (dLeftMargin + dRightMargin + this.pf.getImageableWidth())),
		...
		this.view.setSize(new Dimension((int) (this.zoom * (dLeftMargin + dRightMargin + this.pf.getImageableWidth())),
				(int) (this.zoom * (dTopMargin + dBottomMargin + this.pf.getImageableHeight()))));
		this.centralPanel.setSize(new Dimension((int) (this.zoom * (dLeftMargin + dRightMargin + this.pf.getImageableWidth())),
				(int) (this.zoom * (dTopMargin + dBottomMargin + this.pf.getImageableHeight()))));
	...
	public void paint(Graphics g, double scale) {
		if (this.backgroundImage != null) {
			g.drawImage(this.backgroundImage.getImage(), 0, 0, (int) (scale * this.backgroundImage.getIconWidth()) - 1, (int) (scale * this.backgroundImage.getIconHeight()) - 1,
					this.view);
		}
	}
	...
	public void paintStretch(Graphics g, PageFormat pf) {
		if (this.backgroundImage != null) {
			g.drawImage(this.backgroundImage.getImage(), 0, 0, (int) pf.getImageableWidth() - 1, (int) pf.getImageableHeight() - 1, this.view);
		}
	}
