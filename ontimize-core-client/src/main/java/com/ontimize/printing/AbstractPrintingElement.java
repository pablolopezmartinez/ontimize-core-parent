...
public abstract class AbstractPrintingElement implements PrintingElement {
	...
	public static Dimension millimeterToPagePixels(Dimension d) {
		...
		return new Dimension(AbstractPrintingElement.millimeterToPagePixels((int) dWidth), AbstractPrintingElement.millimeterToPagePixels((int) dHeight));
	}
	...
	protected int getInteger(Object v) {
		try {
			...
			if (ApplicationManager.DEBUG) {
				AbstractPrintingElement.logger.debug(this.getClass().toString() + " : Parameter error. " + e.getMessage(), e);
			}
			return 0;
		}
	}
	...
	protected void setBasicParameters(Hashtable parameters) {
		...
		if (fontsize != null) {
			try {
				...
				if (ApplicationManager.DEBUG) {
					AbstractPrintingElement.logger.debug(this.getClass().toString() + " : Error in parameter 'fontsize'", e);
				}
			}
		}
		...
		if (color != null) {
			if (color.equals("none")) {
				...
				try {
					...
					AbstractPrintingElement.logger.debug(this.getClass().toString() + " Error in parameter 'color':" + e.getMessage(), e);
				}
			}
		}
		...
		if (bgcolor != null) {
			try {
				...
				AbstractPrintingElement.logger.debug(this.getClass().toString() + " Error in parameter 'bgcolor':" + e.getMessage(), e);
			}
