...
public class LineElement extends AbstractPrintingElement {
	...
	public void paint(Graphics g, double scale) {
		...
		for (int i = 0; i < this.pixelWidth; i++) {
			g.drawLine((int) (AbstractPrintingElement.millimeterToPagePixels(this.xi) * scale), (int) (AbstractPrintingElement.millimeterToPagePixels(this.yi) * scale) + i,
					(int) (AbstractPrintingElement.millimeterToPagePixels(this.xf) * scale), (int) (AbstractPrintingElement.millimeterToPagePixels(this.yf) * scale) + i);
		}
		g.setColor(c);
	}
