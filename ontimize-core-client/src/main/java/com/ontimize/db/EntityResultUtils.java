...
public abstract class EntityResultUtils extends EntityResultTools {
	...
	public static TableModel createTableModel(Hashtable res, Vector cols, boolean returnEmptyStrings, boolean convertBB2Im) {
		EntityResult resN = new EntityResult();
		for (int i = 0; i < cols.size(); i++) {
			if (res.containsKey(cols.get(i))) {
				resN.put(cols.get(i), res.get(cols.get(i)));
	...
	/**
	 * Combines all data in the Hashtable h and the EntityResult r2. The combination is done in the next way:<BR>
