...
public class QueryBuilderShowColsAndTypes extends JPanel {
	...
	public QueryBuilderShowColsAndTypes(ResourceBundle bundle, String[] cols, int[] types, String entity) {
		...
		for (int i = 0, a = types.length - 1; i < a; i++) {
			middle += "<TR><TD>&nbsp;&nbsp;</TD><TD>" + ApplicationManager.getTranslation(cols[i], bundle) + "</TD><TD>&nbsp;&nbsp;</TD><TD>" + QueryBuilder
					.getStringType(types[i]) + "</TD><TD>&nbsp;&nbsp;</TD></TR>";
		}

		this.helpText.setText("<html><body><BR>&nbsp;&nbsp;" + ApplicationManager.getTranslation("QueryBuilderShowColsAndTypesEntity",
				bundle) + "&nbsp;&nbsp;<B>" + entity + "</B><BR><BR><BR>&nbsp;" + ApplicationManager.getTranslation("QueryBuilderShowColsAndTypesList",
						bundle) + "<BR><table><TR><TD>&nbsp;&nbsp;</TD><TD align=center><B>" + ApplicationManager.getTranslation("QueryBuilderShowColsAndTypesColunm",
								bundle) + "</B></TD><TD>&nbsp;&nbsp;</TD><TD align=center><B>" + ApplicationManager.getTranslation("QueryBuilderShowColsAndTypesType",
										bundle) + "</B></TD><TD>&nbsp;&nbsp;</TD></TR>" + middle + "</table><BR></body></html>");
	...
	public static void show(Component c, ResourceBundle bundle, String[] cols, int[] types, String entity) {
		...
		if (!(c instanceof Frame) && !(c instanceof Dialog)) {
			...
			if (c instanceof Frame) {
				QueryBuilderShowColsAndTypes.dialog = new EJDialog((Frame) c, ApplicationManager.getTranslation("QueryBuilderShowColsAndTypesTitle", bundle), true);
			}
			if (c instanceof Dialog) {
				QueryBuilderShowColsAndTypes.dialog = new EJDialog((Dialog) c, ApplicationManager.getTranslation("QueryBuilderShowColsAndTypesTitle", bundle), true);
			}
		}
