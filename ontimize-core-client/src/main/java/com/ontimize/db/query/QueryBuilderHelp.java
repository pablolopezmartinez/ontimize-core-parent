    ...
    public static void show(Component c, ResourceBundle bundle, boolean okCancel, boolean showCols, int t) {
        ...
        if (!(c instanceof Frame) && !(c instanceof Dialog)) {
            ...
            if (QueryBuilderHelp.w instanceof Frame) {
                QueryBuilderHelp.dialog = new EJDialog((Frame) QueryBuilderHelp.w,
                        ApplicationManager.getTranslation("QueryBuilderHelpTitulo", bundle), true);
            } else if (QueryBuilderHelp.w instanceof Dialog) {
                QueryBuilderHelp.dialog = new EJDialog((Dialog) QueryBuilderHelp.w,
                        ApplicationManager.getTranslation("QueryBuilderHelpTitulo", bundle), true);
            }
        } else {
            if (c instanceof Frame) {
                QueryBuilderHelp.dialog = new EJDialog((Frame) c,
                        ApplicationManager.getTranslation("QueryBuilderHelpTitulo", bundle), true);
            }
            if (c instanceof Dialog) {
                QueryBuilderHelp.dialog = new EJDialog((Dialog) c,
                        ApplicationManager.getTranslation("QueryBuilderHelpTitulo", bundle), true);
            }
        }
