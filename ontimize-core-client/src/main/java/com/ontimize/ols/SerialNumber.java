...
class CharacterConstrainer extends PlainDocument {
	 ...
	 *
	 * @param editor
	 *            Editor to limit the number of characters in.
	...
	/**
	 * Metodo al que llama el editor cada vez que se intenta insertar caracteres. El metodo comprueba que no se sobrepasa el límite. Si es así, llama al metodo de la clase padre
	 * para que se inserten los caracteres. Si se sobrepasa el límite, retorna sin hacer nada.
	 */
	@Override
...
/**
 * Escuchador para que haga el movimiento contrario, cuando se pulse VK_BACK_SPACE salte al JTextField anterior.
 */
...
public class SerialNumber extends JPanel {
	...
	private void init(ResourceBundle b, boolean showButtons, boolean readOnly, boolean uppercase) {
		...
		if (showButtons) {
			this.add(title, new GridBagConstraints(0, i, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
			i++;
		}

		this.add(serial, new GridBagConstraints(0, i, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(2, 2, 2, 2), 0, 0));
		i++;

		if (showButtons) {
			this.add(buttons, new GridBagConstraints(0, i, 1, 1, 1, 0, GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0));
		...
		this.bOK.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if ((SerialNumber.this.t1.getText().length() == 4) && (SerialNumber.this.t2.getText().length() == 4) && (SerialNumber.this.t4.getText().length() == 4)) {
					Window w = SwingUtilities.getWindowAncestor((Component) e.getSource());
					w.setVisible(false);
				} else {
					JOptionPane.showMessageDialog((Component) e.getSource(),
							ApplicationManager.getTranslation("SerialNumber.SERIAL_NUMBER_ERROR_MESSAGE", SerialNumber.this.bundle),
							ApplicationManager.getTranslation("SerialNumber.SERIAL_NUMBER_ERROR_TITLE", SerialNumber.this.bundle), JOptionPane.OK_OPTION);
				}
			}
		});
	...
	public String getSerialNumber() {
		if ((this.t1.getText() == null) || (this.t2.getText() == null) || (this.t3.getText() == null) || (this.t4.getText() == null)) {
			return null;
		}
	...
	public static String showSerialNumber(Object parent, ResourceBundle b, boolean showButtons, boolean readonly, boolean uppercase) {
		EJDialog ejd = null;
	...
	public static String showSerialNumber(Dialog parent, ResourceBundle b, boolean showButtons, boolean readonly, boolean uppercase) {
		EJDialog ejd = new EJDialog(parent, true);
		return SerialNumber.construct(ejd, b, showButtons, readonly, uppercase);
	}

	private static String construct(EJDialog ejd, ResourceBundle b, boolean showButtons, boolean readonly, boolean uppercase) {
		...
		if ((sn.getSerialNumber() == null) || sn.getSerialNumber().equals("") || (sn.getSerialNumber().length() != SerialNumber.CorrectSerialNumberSize)) {
			return null;
		}
