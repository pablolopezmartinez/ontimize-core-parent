...
public class ExtendedRemoteReferencesXmlParser extends ExtendedXmlParser {
	...
	protected void modifyGroupOperationsNode(Document originalForm, Element foundNode, Node currentGroupOperationsNode) {
		if (currentGroupOperationsNode.getNodeName().equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_MODIFY_INIT)) {
			foundNode.setAttribute(ExtendedXmlParser.MODIFY_REFERENCE_INIT,
					currentGroupOperationsNode.getAttributes().getNamedItem(ExtendedXmlParser.MODIFY_ATTRIBUTE_VALUE).getNodeValue());
		}
		if ((currentGroupOperationsNode.getNodeType() == Node.ELEMENT_NODE) && !currentGroupOperationsNode.getNodeName()
			...
			for (int j = 0; j < foundElementList.getLength(); j++) {
				...
				if (currentGroupOperationsNode.getNodeName().equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_MODIFY_NAME) && currentElement.getNodeName()
						.equals(ExtendedXmlParser.MODIFY_REFERENCE_NAME)) {
					Node childElementText = currentElement.getFirstChild();
					childElementText.setNodeValue(currentGroupOperationsNode.getAttributes().getNamedItem(ExtendedXmlParser.MODIFY_ATTRIBUTE_VALUE).getNodeValue());
				} else if (currentGroupOperationsNode.getNodeName().equalsIgnoreCase(ExtendedXmlParser.MODIFY_REFERENCE_MODIFY_CLASS) && currentElement.getNodeName()
	...
	protected void modifyReferenceParameters(Document originalForm, Element foundNode, Node currentGroupOperationsNode) {
		...
		for (int j = 0; j < paramNodesOperation.getLength(); j++) {
			...
			if (paramNodeOperation.getNodeType() == Node.ELEMENT_NODE) {
				...
				String nodeName = ((Element) paramNodeOperation).getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_NAME).toString();
				String nodeValue = ((Element) paramNodeOperation).getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_VALUE).toString();
				String nodeOperation = ((Element) paramNodeOperation).getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATION);
				String nodeSeparator = ((Element) paramNodeOperation).getAttribute(ExtendedXmlParser.MODIFY_ATTRIBUTE_SEPARATOR);

				Node paramsToModify = this.searchNodeName(nodeName, foundNode);

				if ((nodeOperation.equalsIgnoreCase(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATOR_ADD) && (paramsToModify == null)) || (nodeOperation
				...
				} else if (nodeOperation.equalsIgnoreCase(ExtendedXmlParser.MODIFY_ATTRIBUTE_OPERATOR_ADD) && (paramsToModify != null)) {
					...
					NodeList valueToModify = ((Element) paramsToModify).getElementsByTagName(ExtendedXmlParser.MODIFY_REFERENCE_VALUE);
	 ...
	 * @param attr
	 *            attr attribute of node searched
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			...
			if (actualNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				for (int j = 0; j < childNodesFromActual.getLength(); j++) {
					...
					if ((checkNameValue.getNodeType() == Node.ELEMENT_NODE) && checkNameValue.getNodeName().equalsIgnoreCase("Name")) {
