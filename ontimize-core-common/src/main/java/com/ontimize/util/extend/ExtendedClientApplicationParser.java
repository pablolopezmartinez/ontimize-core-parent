...
public class ExtendedClientApplicationParser {
	...
	protected void executeOperation(Document originalForm, ExtendedClientApplicationOperation operation) throws Exception {
			...
			if (managerNode != null) {
				...
				if (internalOperations.size() > 0) {
					...
					for (int i = 0; i < internalOperations.size(); i++) {
						InnerExtendedClientApplicationOperation internalOperation = (InnerExtendedClientApplicationOperation) internalOperations.get(i);
						...
						if (intOperationName.equalsIgnoreCase(ExtendedClientApplicationParser.DELETE_OP)) {
							...
							if (form != null) {
								Node formNode = this.searchNodeByAttr(managerNode, ExtendedClientApplicationParser.FORM_ATRIBUTE, form);
								if (formNode != null) {
								...
								if (formNode != null) {
									...
									if (iOps.size() > 0) {
										for (int j = 0; j < iOps.size(); j++) {
											InnerExtendedClientApplicationOperation iOp = (InnerExtendedClientApplicationOperation) iOps.get(j);
	 ...
	 *
	 * @param node
	 *            The node in that the search is made
	 * @param attrName
	 *            name of attribute of node searched
	 * @param attrValue
	 *            value of attribute of node searched
	 ...
	 *
	 * @param originalForm
	 *            - The xml of the original file
	 * @param extendDocument
	 *            - The xml containing the operations to apply in originalDocument
	 * @return
	 */
	...
	/**
	 * This method processes a node that contains an operaton and returns an instance of class XmlExtendsOperation with necessary information to apply this operation in original
	 * xml document
	 *
	 * @param node
	...
	protected ExtendedClientApplicationOperation getNodeOperation(Node node) throws Exception {
		...
		if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.REPLACE_OP) || operationName.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_OP)) {

			if (parentNode == null) {
				throw new Exception("Missing " + ExtendedClientApplicationParser.PARENT_ATRIBUTE + " in " + node.getNodeName());
			}
			...
			operation = new ExtendedClientApplicationOperation(ExtendedClientApplicationOperation.UNIQUE_OP, operationName, parentNode.getNodeValue());
			...
			if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_OP)) {
				...
				for (int i = 0; i < lenght; i++) {
					...
					if (this.isInternalOperationNode(internalNode)) {
						InnerExtendedClientApplicationOperation internalOp = this.getInternalNodeOperation(internalNode);
						if (internalOp != null) {
		...
		if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.INSERT_MANAGER_OP) || operationName
				.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_MANAGER_OP) || operationName.equalsIgnoreCase(ExtendedClientApplicationParser.DELETE_MANAGER_OP)) {
			...
			if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.INSERT_MANAGER_OP)) {
				...
				operation = new ExtendedClientApplicationOperation(ExtendedClientApplicationOperation.MANAGER_OP, operationName, null);
				operation.setExtendCode(containedCode);
			}
			...
			if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_MANAGER_OP) || operationName
				...
				if (idNode == null) {
					throw new Exception("Missing " + ExtendedClientApplicationParser.ID_ATRIBUTE + " in " + node.getNodeName());
				}

				operation = new ExtendedClientApplicationOperation(ExtendedClientApplicationOperation.MANAGER_OP, operationName, idNode.getNodeValue());
	...
	protected InnerExtendedClientApplicationOperation getInternalNodeOperation(Node node) throws Exception {
		...
		if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.ATRIBUTE_OP)) {
			...
			if (nameNode == null) {
				throw new Exception("Missing " + ExtendedClientApplicationParser.NAME_ATRIBUTE + " in " + node.getNodeName());
			}

			if (valueNode == null) {
				throw new Exception("Missing " + ExtendedClientApplicationParser.VALUE_ATRIBUTE + " in " + node.getNodeName());
			}

			operation = new InnerExtendedClientApplicationOperation(operationName, nameNode.getNodeValue(), valueNode.getNodeValue(), null);
		}
		...
		if (operationName.equalsIgnoreCase(ExtendedClientApplicationParser.DELETE_OP) || operationName.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_OP)) {

			if (formNode == null) {
				throw new Exception("Missing " + ExtendedClientApplicationParser.FORM_ATRIBUTE + " in " + node.getNodeName());
			}
	...
	protected boolean isExtendNode(Node node) {
		...
		if (nodeName.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_MANAGER_OP) || nodeName.equalsIgnoreCase(ExtendedClientApplicationParser.DELETE_MANAGER_OP) || nodeName
				.equalsIgnoreCase(ExtendedClientApplicationParser.INSERT_MANAGER_OP) || nodeName
						.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_OP) || nodeName.equalsIgnoreCase(ExtendedClientApplicationParser.REPLACE_OP)) {
			return true;
		}
		return false;
	}
	...
	protected boolean isInternalOperationNode(Node node) {
		...
		if (nodeName.equalsIgnoreCase(ExtendedClientApplicationParser.MODIFY_OP) || nodeName.equalsIgnoreCase(ExtendedClientApplicationParser.DELETE_OP) || nodeName
				.equalsIgnoreCase(ExtendedClientApplicationParser.INSERT_OP) || nodeName.equalsIgnoreCase(ExtendedClientApplicationParser.ATRIBUTE_OP)) {
			return true;
		}
		return false;
	}
	...
	public Document getDocumentModel(File f) {
		...
		try {
			...
			ExtendedClientApplicationParser.logger.error(this.getClass().toString() + ": " + e.getClass().toString() + " in " + this.getClass().toString() + ": " + e.getMessage(),
					e);
			return null;
		}
	}
