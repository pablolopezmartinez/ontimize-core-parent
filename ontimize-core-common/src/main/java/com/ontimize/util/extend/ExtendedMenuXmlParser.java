...
public class ExtendedMenuXmlParser extends ExtendedFormXmlParser {
	...
	/**
	 * This method extends functionality of super method adding REMOVE_SEPARATOR_OP. This operation removes the element <SeparadorMenu /> or <ToolBarSeparator /> without attr in
	 * original Xml
	 */
	@Override
			...
			if (operationType.equals(ExtendedMenuXmlParser.REMOVE_SEPARATOR_OP)) {
				...
				if (foundNode != null) {
					...
					for (int i = 0; i < nodeList.getLength(); i++) {
						...
						if (nodeToRemove.getNodeName().equals(ExtendedMenuXmlParser.MENU_SEPARATOR) || nodeToRemove.getNodeName().equals(ExtendedMenuXmlParser.TOOLBAR_SEPARATOR)) {
							numOfSeparators++;
	...
	protected ExtendedXmlOperation getNodeOperation(Node node) throws Exception {
		...
		if (nodeName.equalsIgnoreCase(ExtendedMenuXmlParser.REMOVE_SEPARATOR_OP)) {
			...
			return new ExtendedMenuXmlOperation(nodeName, attrNode.getNodeValue(), Integer.parseInt(position.getNodeValue()));
		} else {
