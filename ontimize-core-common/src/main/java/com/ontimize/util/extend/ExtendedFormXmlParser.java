...
public class ExtendedFormXmlParser extends ExtendedXmlParser {
	...
	protected void executeOperation(Document originalForm, ExtendedXmlOperation operation) throws Exception {
		...
		if (foundNode != null) {
			...
			if (operation.levelsToParent != 0) {
				parentInLevel = this.searchParentInLevel(foundNode, operation.getParentContainerType(), operation.getLevelsToParent());
			} else {
							...
							if (nodeValue.equals("")) {
								...
								if (originalValue.toLowerCase().contains(nodeValue.toLowerCase())) {
									if (originalValue.length() == removeLength) {
										...
										attrToModify.setNodeValue(originalValue.replace(nodeSeparator + nodeValue + nodeSeparator, ";"));
									}
								}
							}
						}
	...
	/**
	 * Recursive method that searches in a node an element of type parentType and it's in the exact level beginning the count of levels in the node. The method makes an inverse
	 * deep search of an element in a node
	 *
	 * @param node
	 *            The node in which search operation is made and in which the search begins
	 * @param parentType
	 *            Type of element to search
	 * @param level
	 *            Number of elements of type parentType than exist before the searched element
