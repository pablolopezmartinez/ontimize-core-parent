...
public class BasicTimePeriodParser implements TimePeriodParser, TimePeriodOperationParser {
	...
	protected String getEquivalentPeriod(String periodDef) throws Exception {
		if (this.alias == null) {
			BasicTimePeriodParser.logger.debug("It has tried to parse an alias file, but alias file reference is null.");
		...
		if (equiv.toString().startsWith("@")) {
			BasicTimePeriodParser.logger.debug("It has tried to parse an alias file. Equivalence begins to @ character (cyclic call). This could provoke a stack overflow error");
			throw new Exception("It has tried to parse an alias file. Equivalence begins to @ character (cyclic call). This could provoke a stack overflow error");
		}
	...
	protected TimePeriod parseAdvancedPeriod(String periodDefinition, Locale locale, String businessCalendarProperties) throws Exception {
		if (periodDefinition != null) {
			...
			if (tokensAt.size() == 1) {
				...
				for (int i = 0; i < tokensAt.size(); i++) {
					AdvancedTimePeriod period = new AdvancedTimePeriod((String) tokensAt.get(i), locale, businessCalendarProperties);
	...
	protected TimePeriod parseSimplePeriod(String periodDefinition, Locale locale, String businessCalendarProperties) throws Exception {
		...
		if (periodSeparatorIndex < 0) {
			...
			if (BasicTimePeriodParser.DEBUG) {
				BasicTimePeriodParser.logger.debug("TimePeriodParser: Found: " + BasicTimePeriodParser.DAY_MONTH_SEPARATOR);
			}
			String sDay = periodDefinition.substring(0, periodDefinition.indexOf(BasicTimePeriodParser.DAY_MONTH_SEPARATOR));
			String sMonth = periodDefinition.substring(periodDefinition.indexOf(BasicTimePeriodParser.DAY_MONTH_SEPARATOR) + 1);
			if (BasicTimePeriodParser.DEBUG) {
			...
			if (strFistDay.indexOf(BasicTimePeriodParser.WORKING_DAY) >= 0) {
				if (BasicTimePeriodParser.DEBUG) {
					BasicTimePeriodParser.logger.debug("TimePeriodParser: Found in start period: " + BasicTimePeriodParser.WORKING_DAY);
				}
				// Working day
				...
				if (BasicTimePeriodParser.DEBUG) {
					BasicTimePeriodParser.logger.debug("TimePeriodParser: Found in end period: " + BasicTimePeriodParser.WORKING_DAY);
				}
	...
	protected int getDay(String s) throws Exception {
		...
		if (BasicTimePeriodParser.DEBUG) {
			BasicTimePeriodParser.logger.debug("TimePeriodParser: Getting Day: Correct: Not Found : " + BasicTimePeriodParser.INTERVAL);
		}
		if (s.indexOf(BasicTimePeriodParser.DAY_MONTH_SEPARATOR) < 0) {
			return Integer.parseInt(s);
		} else {
			if (BasicTimePeriodParser.DEBUG) {
				BasicTimePeriodParser.logger.debug("TimePeriodParser: Getting Day: Found : " + BasicTimePeriodParser.DAY_MONTH_SEPARATOR);
			}
	...
	public int getMonth(String s) throws Exception {
		...
		if (s.indexOf(BasicTimePeriodParser.DAY_MONTH_SEPARATOR) < 0) {
			...
			if (BasicTimePeriodParser.DEBUG) {
				BasicTimePeriodParser.logger.debug("TimePeriodParser: Getting Month: Found : " + BasicTimePeriodParser.DAY_MONTH_SEPARATOR);
			}
	...
	public TimePeriod getCommonPeriod(TimePeriod p1, TimePeriod p2) throws Exception {
		...
		for (int i = 0; i < firstList.size(); i++) {
			...
			if (!(o1 instanceof AdvancedTimePeriod)) {
				throw new Exception(this.getClass().getName() + ": only can join objects from " + AdvancedTimePeriod.class.getName());
			}
			for (int k = 0; k < secondList.size(); k++) {
				...
				AdvancedTimePeriod commonAdvancedPeriod = this.getCommonAdvancedPeriod((AdvancedTimePeriod) o1, (AdvancedTimePeriod) o2);
	...
	protected AdvancedTimePeriod getCommonAdvancedPeriod(AdvancedTimePeriod p1, AdvancedTimePeriod p2) {
		int[] months = this.getCommonInterval(p1.getStartMonth(), p1.getEndMonth(), p2.getStartMonth(), p2.getEndMonth());
		if (months != null) {
			...
			if (days != null) {
				AdvancedTimePeriod timePeriod = new AdvancedTimePeriod(days[0], days[1], months[0], months[1], isWorkingPeriod, p1.getLocale(), p1.getBusinessCalendar());
				return timePeriod;
			}
		}
		return null;
