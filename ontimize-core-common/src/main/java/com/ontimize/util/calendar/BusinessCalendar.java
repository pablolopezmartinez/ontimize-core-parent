...
public class BusinessCalendar implements java.io.Serializable {
	...
	protected class Day implements java.io.Serializable {
		...
		public Day(int day, int month, int year, GregorianCalendar c) {
			...
			this.month = month;
			if ((year != -1) && (c != null)) {
				c.set(year, month, day, c.getActualMinimum(Calendar.HOUR), c.getActualMinimum(Calendar.MINUTE), c.getActualMinimum(Calendar.SECOND));
				this.startDate = c.getTime();
				c.set(year, month, day + 1, c.getActualMinimum(Calendar.HOUR), c.getActualMinimum(Calendar.MINUTE), c.getActualMinimum(Calendar.SECOND));
	...
	public void reload() {
		...
		try {
			...
			BusinessCalendar.logger.error("Key " + BusinessCalendar.COMMON + " not found in calendar file " + this.propertiesFileName, e);
		}
