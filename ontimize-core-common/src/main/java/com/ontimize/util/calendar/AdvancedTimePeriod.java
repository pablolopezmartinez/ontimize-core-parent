...
public class AdvancedTimePeriod implements TimePeriod {
	...
	public AdvancedTimePeriod(String periodDefinitionString, Locale locale, String businessCalendarProperties) throws Exception {
	...
	public AdvancedTimePeriod(int startDay, int endDay, int startMonth, int endMonth, boolean workingPeriod, Locale locale, BusinessCalendar businessCalendar) {
	...
	protected void parseString(String periodDefinitionString) {
		Vector daysMonthsYears = ParseTools.getTokensAt(periodDefinitionString, BasicTimePeriodParser.DAY_MONTH_SEPARATOR);
		...
		if (daysMonthsYears.size() < 2) {
			...
			throw new IllegalArgumentException("Incorrect format: character / only can be used to separate days months and years");
		}
		...
		if (dayDefinition.startsWith(BasicTimePeriodParser.ALL)) {
			...
			if (intervalDays.size() != 2) {
				throw new IllegalArgumentException("Incorrect format: character - must separate two numerical values to define the days");
			}
		...
		if (monthDefinition.equals(BasicTimePeriodParser.ALL)) {
			...
			if (intervalMonths.size() != 2) {
				throw new IllegalArgumentException("Incorrect format: character - must separate two numerical values to define the months");
			}
	...
	protected boolean timeIsInWorkingPeriod(Calendar cal) {
		...
		if (this.startDay == -1) {
			...
			while (workingDays < this.startDay) {
				iDay++;
				boolean holidays = this.businessCalendar.isHoliday(iDay, cal.get(Calendar.MONTH), cal.get(Calendar.YEAR));
				if (!holidays) {
					workingDays++;
				}
			}
			...
			int iDay = this.businessCalendar.workingDayOfMonthToDayOfMonth(this.startDay, cal.get(Calendar.MONTH), cal.get(Calendar.YEAR));
			int iLastDay = this.businessCalendar.workingDayOfMonthToDayOfMonth(this.endDay, cal.get(Calendar.MONTH), cal.get(Calendar.YEAR));
