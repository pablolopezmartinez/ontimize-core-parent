...
/**
 * Interface to be implemented by all database handlers. Two methods are different because they manage the parameter <code>connection</code> in a different mode.
 *
...
public interface SQLHandler {
	 ...
	 *
	 * @param table
	 *            The <code>SQLTableCreation</code> table with whole information to create it.
	 * @throws Exception
	 *
	 ...
	 *
	 * @param table
	 *            The <code>SQLTableDrop</code> table with whole information to drop it.
	 * @throws Exception
	 *
	 * @return a <code>String</code> with the SQL statement
	 */
	 ...
	 *
	 * @param table
	 *            The <code>SQLTableAlter</code> table with whole information to alter it.
	 * @throws Exception
	 *
	 * @return a <code>String</code> with the SQL statement
	 */
	...
	/**
	 * This method creates a database table. At first, autocommit mode in connection is fixed to false, then its SQL statements are grouped into transactions that are terminated by
	 * a call to either the method <code>commit</code> or the method <code>rollback</code>. Moreover, Exceptions are also managed by this method. <br>
	 * <br>
	...
	/**
	 * This method drops a database table. At first, autocommit mode in connection is fixed to false, then its SQL statements are grouped into transactions that are terminated by a
	 * call to either the method <code>commit</code> or the method <code>rollback</code>. Moreover, Exceptions are also managed by this method. <br>
	 * <br>
	 * <u>Call diagram of this method:</u><br>
	 * <br>
	...
	/**
	 * This method alters a database table. At first, autocommit mode in connection is fixed to false, then its SQL statements are grouped into transactions that are terminated by
	 * a call to either the method <code>commit</code> or the method <code>rollback</code>. Moreover, Exceptions are also managed by this method. <br>
	 * <br>
	 * <u>Call diagram of this method:</u><br>
	 * <br>
	...
	/**
	 * This method executes a serial of SQL actions(create,drop or alter) over a database table. At first, autocommit mode in connection is fixed to false, then its SQL statements
	 * are grouped into transactions that are terminated by a call to either the method <code>commit</code> or the method <code>rollback</code>. Moreover, Exceptions are also
	...
	/**
	 * This method executes a serial of SQL actions(create,drop or alter) over a database table in transactional mode. <br>
	 * <br>
	 *
	 * @param sqlTables
	 *            list with all sqlTables
