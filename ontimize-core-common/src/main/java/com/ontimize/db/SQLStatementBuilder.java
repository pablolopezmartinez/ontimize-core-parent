...
public abstract class SQLStatementBuilder {
	...
	static {
		...
		SQLStatementBuilder.registerHandlers.put(SQLStatementBuilder.POSTGRES_HANDLER, new PostgresSQLStatementHandler());
		SQLStatementBuilder.registerHandlers.put(SQLStatementBuilder.SQLSERVER_HANDLER, new SQLServerSQLStatementHandler());
	...
	public static void setUseAsInSubqueries(boolean useAsInSubqueries) {
		SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).setUseAsInSubqueries(useAsInSubqueries);
	}
	...
	/**
	 * This class is used to store a SQL Statement and the required values to be processed in <code>SQLConditionValuesProcessor</code>. This SQLStatement is inserted in brackets
	 * into a condition string after an IN clause
	 *
	 * @author Imatia Innovation S.L.
	 */
	...
	/**
	 * This class is used for avoid using wildcard when the condition string is created in <code>SQLConditionValuesProcessor</code>
	 *
	 * @author Imatia Innovation S.L.
	 */
	...
	/**
	 * This interface defines the basic methods that must be implemented by every class that defines the operation that is used by the <code>Expression</code> when it's processed
	 * by <code>SQLConditionValuesProcessor</code>.
	 *
	 * @author Imatia Innovation S.L
	 */
	...
	/**
	 * This interface defines the basic methods that must be implemented by any class that stores the name of the column that is used by the <code>Expression</code> when it's
	 * processed by <code>SQLConditionValuesProcessor</code>.
	 *
	 * @author Imatia Innovation S.L.
	 */
	...
	/**
	 * Default implementation of <code>Expression</code> interfaces. This class is processed by <code>ExtendedSQLConditionValuesProcessor</code> for created a condition string.
	 * <p>
	...
	public static class BasicExpression implements Expression {
		 ...
		 *
		 * @param lo
		 *            Left operand of the Basic Expression
		 * @param search
		 *            SearchValue associated with the left operand
		 * @param bracket
		 *            Boolean to add brackets to the left operand name
		 */
		...
		public boolean evaluate(Map m) {
			...
			if (lo instanceof Field) {
				...
				if (ro instanceof SearchValue) {
					...
					SQLStatementBuilder.logger.debug("Expression with EQUAL operator is being evaluated -> \"{}\" {} \"{}\" and value for \"{}\" = \"{}\"", lo, o, ro, sField,
							oValue);
					boolean eval = this.evaluateEquals(lo, o, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with the operator \"{}\": are equals -> TRUE", sField, op);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match with the operator \"{}\": \"{}\" is different than \"{}\" -> FALSE", sField,
								op, ro, oValue);
					}
					return eval;
				} else if (op.equals(BasicOperator.NOT_EQUAL)) {
					SQLStatementBuilder.logger.debug("Expression with NOT EQUAL operator is being evaluated -> \"{}\" {} \"{}\" and value for \"{}\" = \"{}\"", lo, o, ro, sField,
							oValue);
					boolean eval = !this.evaluateEquals(lo, BasicOperator.EQUAL_OP, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with the operator \"{}\": \"{}\" is different than \"{}\" -> TRUE", sField, op, ro,
								oValue);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match with the operator \"{}\": are equals -> FALSE", sField, op, ro, oValue);
					}
					return eval;
				} else if (op.equals(BasicOperator.NULL)) {
					SQLStatementBuilder.logger.debug("Expression with NULL operator is being evaluated -> \"{}\"{}and value for \"{}\" = \"{}\"", lo, o, sField, oValue);
					...
					SQLStatementBuilder.logger.debug("Expression with NOT NULL operator is being evaluated -> \"{}\"{}and value for \"{}\" = \"{}\"", lo, o, sField, oValue);
					boolean eval = !this.evaluateNull(lo, o, ro, sField, oValue);
					if (eval) {
					...
					return eval;
				} else if (op.equals(BasicOperator.LESS) || op.equals(BasicOperator.LESS_EQUAL) || op.equals(BasicOperator.MORE) || op.equals(BasicOperator.MORE_EQUAL)) {
					SQLStatementBuilder.logger.debug("Expression with order operator is being evaluated -> \"{}\" {} \"{}\" and value for \"{}\" = \"{}\"", lo, o, ro, sField,
							oValue);
					boolean eval = this.evaluateOrder(lo, o, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with the operator \"{}\":  \"{}\" is \"{}\" than \"{}\"-> TRUE", sField, op, oValue,
								op, ro);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match with the operator \"{}\":  \"{}\" is not \"{}\" than \"{}\"-> FALSE", sField,
								op, oValue, op, ro);
					}
					return eval;
				} else if (op.equals(SQLStatementBuilder.IN)) {
					SQLStatementBuilder.logger.debug("Expression with IN operator is being evaluated -> \"{}\" {} {} and value for \"{}\" = \"{}\"", lo, o, ro, sField, oValue);
					boolean eval = this.evaluateIn(lo, op, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with operator \"{}\": \"{}\" is in \"{}\" -> TRUE", lo, o, oValue, ro);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match with operator \"{}\": \"{}\" isn't in \"{}\" -> FALSE", lo, o, oValue, ro);
					}
					return eval;
				} else if (op.equals(SQLStatementBuilder.NOT_IN)) {
					SQLStatementBuilder.logger.debug("Expression with NOT IN operator is being evaluated -> \"{}\" {} {} and value for \"{}\" = \"{}\"", lo, o, ro, sField, oValue);
					boolean eval = !this.evaluateIn(lo, op, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with operator \"{}\": \"{}\" isn't in \"{}\" -> TRUE", lo, o, oValue, ro);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match with operator \"{}\": \"{}\" is in \"{}\" -> FALSE", lo, o, oValue, ro);
					}
					return eval;
				} else if (op.equals(BasicOperator.LIKE)) {
					SQLStatementBuilder.logger.debug("Expression with LIKE operator is being evaluated -> \"{}\" {} \"{}\" and value for \"{}\" = \"{}\"", lo, o, ro, sField,
							oValue);
					boolean eval = this.evaluateLike(lo, op, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches the pattern with operator \"{}\": \"{}\" matches with \"{}\" -> TRUE", lo, o,
								oValue, ro);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match the pattern with operator \"{}\": \"{}\" don't match with \"{}\" -> FALSE",
								lo, o, oValue, ro);
					}
					return eval;
				} else if (op.equals(BasicOperator.NOT_LIKE)) {
					SQLStatementBuilder.logger.debug("Expression with NOT LIKE operator is being evaluated -> \"{}\" {} \"{}\" and value for \"{}\" = \"{}\"", lo, o, ro, sField,
							oValue);
					boolean eval = !this.evaluateLike(lo, op, ro, sField, oValue);
					if (eval) {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" doesn't match the pattern with operator \"{}\": \"{}\" doesn't match with \"{}\" -> TRUE",
								lo, o, oValue, ro);
					} else {
						SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches the pattern with operator \"{}\": \"{}\" matches with \"{}\" -> FALSE", lo, o,
								oValue, ro);
					}
					return eval;
				} else {
				...
				throw new IllegalArgumentException("Left operand have to be a Field or Expression: " + (lo != SQLStatementBuilder.NULL ? lo.getClass().toString() : "is NULL"));
			}
		}
		...
		protected boolean evaluateLeftOperandAsExpression(Map m, Object lo, Object ro, Object o) {
			...
			if (ro instanceof Expression) {
				...
				if (op.equals(BasicOperator.AND)) {
					res = rLO && rRO;
					SQLStatementBuilder.logger.debug(" AND Operator between \"{}\" and \"{}\" expressions -> {}", rLO, rRO, res);
				} else if (op.equals(BasicOperator.OR)) {
					res = rLO || rRO;
					SQLStatementBuilder.logger.debug(" OR Operator between \"{}\" and \"{}\" expressions -> {}", rLO, rRO, res);
				} else if (op.equals(BasicOperator.OR_NOT)) {
					res = rLO || !rRO;
					SQLStatementBuilder.logger.debug(" OR NOT Operator between \"{}\" and \"{}\" expressions -> {}", rLO, rRO, res);
				} else if (op.equals(BasicOperator.AND_NOT)) {
					res = rLO && !rRO;
					SQLStatementBuilder.logger.debug(" AND NOT Operator between \"{}\" and \"{}\" expressions -> {}", rLO, rRO, res);
				} else {
		 ...
		 * @return
		 */
		protected boolean evaluateRightOperatorAsSearchValue(Map m, Object lo, Object ro, String op, String sField, Object oValue) {
			...
			if (bOp != null) {
				...
				SQLStatementBuilder.logger.debug("SearchValue expression with OR operator is being evaluated -> \"{}\" IN \"{}\" and value for \"{}\" = \"{}\"", lo,
				...
				SQLStatementBuilder.logger.debug("SearchValue expression with BETWEEN operator is being evaluated -> \"{}\" BETWEEN \"{}\" and value for \"{}\" = \"{}\"",
						lo, ((SearchValue) ro).getValue(), sField, oValue);
				boolean eval = this.evaluateBetween(lo, ro, sField, oValue);
				if (eval) {
					SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with the operator BETWEEN: \"{}\" is in range between \"{}\" -> TRUE", lo,
					...
					SQLStatementBuilder.logger.debug(
							"Register for the field \"{}\" doesn't match with the operator BETWEEN: \"{}\" is out of range between \"{}\" -> FALSE", lo, oValue,
				...
				SQLStatementBuilder.logger.debug(
						"SearchValue expression with NOT BETWEEN operator is being evaluated -> \"{}\" NOT BETWEEN \"{}\" and value for \"{}\" = \"{}\"", lo,
						((SearchValue) ro).getValue(), sField, oValue);
				boolean eval = !this.evaluateBetween(lo, ro, sField, oValue);
				if (eval) {
					SQLStatementBuilder.logger.debug("Register for the field \"{}\" matches with the operator NOT BETWEEN: \"{}\" is out of range between \"{}\" -> TRUE",
							lo, oValue, ((SearchValue) ro).getValue());
					SQLStatementBuilder.logger.debug("Expression with SearchValue evaluated with result -> \"true\"");
				} else {
					SQLStatementBuilder.logger.debug(
							"Register for the field \"{}\" doesn't match with the operator NOT BETWEEN: \"{}\" is in range between \"{}\" -> FALSE", lo, oValue,
							((SearchValue) ro).getValue());
					SQLStatementBuilder.logger.debug("Expression with SearchValue evaluated with result -> \"false\"");
				}
				return eval;
			} else if (((SearchValue) ro).getCondition() == 11) {
				throw new IllegalArgumentException("It is not possible to evaluate an operation with an EXIST operator.");
			} else {
		 ...
		 * {@link #evaluateOrder(Object, Object, Object, String, Object)}
		 *
		 * @param ro
		 * @param oValue
		 * @param op
			...
			if (oValue == null) {
				...
				if (((SearchValue) ro).getValue() instanceof List) {
					...
					if ((listValues.size() == 2) && (listValues.get(0) != null) && (listValues.get(1) != null)) {
						Object min;
						Object max;
						if (this.evaluateOrder(lo, BasicOperator.MORE_EQUAL, listValues.get(0), sField, listValues.get(1))) {
	...
	public static interface SQLConditionValuesProcessor {
		 ...
		 *
		 * @param conditions
		 *            condition list that be using for create the string
		 * @param wildcards
	...
	/**
	 * This class provides default implementations for the <code>SQLConditionValuesProcessor</code> interface.
	 *
	 * @author Imatia Innovation S.L.
	 */
	...
	public static class DefaultSQLConditionValuesProcessor implements SQLConditionValuesProcessor {
		...
		/**
		 * Creates a <code>DefaultSQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> is case-insensitive.
		 * <p>
		...
		/**
		 * Creates a <code>DefaultSQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> or is a column of String type is case-insensitive.
		 * <p>
		 * Inserts a upper function in both sides of <code>LIKE</code> conditions (UPPER(Field) LIKE UPPER(Value)) Inserts a upper function in both sides if column type is a String
		 ...
		 *
		 * @param conditions
		 *            a Hashtable specifying pairs of key-value corresponding to the attribute (or column of a table in a database) and the value that is formed by the condition.
		 * @param wildcards
		 *            column list that can use wildcards
		 * @param values
		 *            vector where the value of each processed conditions is stored
		...
		public String createQueryConditions(Hashtable conditions, Vector wildcards, Vector values) {
			...
			while (enumKeys.hasMoreElements()) {
				...
				if (oKey instanceof String) {
					...
					// Check wildcards:
					if ((oValue instanceof String) && ((wildcards == null) || wildcards.isEmpty() || wildcards.contains(oKey))) {
						if ((((String) oValue).indexOf(SQLStatementBuilder.ASTERISK_CHAR) >= 0) || (((String) oValue).indexOf(SQLStatementBuilder.INTERROG) >= 0)) {
							sbStringQuery.append(this.createQueryConditonsWithWildcards(enumKeys, oKey, oValue, bracket));
						} else {
							...
							if (enumKeys.hasMoreElements()) {
								sbStringQuery.append(this.addNonLastPairKeyValue(oKey, bracket, sbCondition.toString()));
							} else {
						...
						if (oValue instanceof SearchValue) {
							StringBuilder sbStringQueryAux = SQLStatementBuilder.createQueryConditionsSearchValue(oValue, values, bracket, oKey);
							...
							values.add(oValue);
							if (enumKeys.hasMoreElements()) {
								sbStringQuery.append(this.addNonLastPairKeyValueInOrder(oKey, bracket, sbCondition.toString()));
							} else {
								sbStringQuery.append(this.addLastPairKeyValueInOrder(oKey, bracket, sbCondition.toString()));
							}

						} else {
							String sCondition = SQLStatementBuilder.EQUAL_XQUESTIONMARK;
							if ((oValue instanceof String) && (((String) oValue).indexOf(SQLStatementBuilder.NOT_EQUAL_ID) == 0)) {
		...
		/**
		 * Method to reduce the complexity of
		 * {@link #createQueryConditions(Hashtable, Vector, Vector)}
		 *
		 * @param sbStringQuery
		 * @param enumKeys
		 * @param oKey
		 ...
		 * @param bracket
		 */
		protected String createQueryConditonsWithWildcards(Enumeration enumKeys, Object oKey, Object oValue, boolean bracket) {
		...
		/**
		 * Mehtod to reduce the complexity of
		 * {@link #createQueryConditions(Hashtable, Vector, Vector)}
		 *
		 * @param sbStringQuery
		 * @param oKey
	...
	public static StringBuilder createQueryConditionsSearchValue(Object oValue, Vector values, boolean bracket, Object oKey) {
		...
		if (((SearchValue) oValue).getCondition() == SearchValue.OR) {
			...
			if (oSearchValue instanceof List) {
				...
				if (!vSearchValues.isEmpty()) {
					sbStringQuery.append(SQLStatementBuilder.createORQueryConditions(values, bracket, oKey, vSearchValues));
				} else {
					return null;
				}
			} else {
		...
		} else if ((((SearchValue) oValue).getCondition() == SearchValue.IN) || (((SearchValue) oValue).getCondition() == SearchValue.EXISTS) || (((SearchValue) oValue)
			...
			if (oSearchValue instanceof List) {
				...
				if (!vSearchValue.isEmpty()) {
					sbStringQuery.append(SQLStatementBuilder.createINQueryConditionsListInstance(oValue, values, oKey, vSearchValue));
				} else {
					return null;
				}
			} else if (oSearchValue instanceof String) {
				sbStringQuery.append(SQLStatementBuilder.createINQueryConditionsStringInstance(oValue, oKey, oSearchValue));
			} else if (oSearchValue instanceof SQLExpression) {
				sbStringQuery.append(SQLStatementBuilder.createINQueryConditionsSQLExpressionInstance(oValue, values, oKey, oSearchValue));
	 ...
	 * @param vSearchValue
	 */
	protected static String createINQueryConditionsListInstance(Object oValue, Vector values, Object oKey, List vSearchValue) {
		StringBuilder sbStringQuery = new StringBuilder();
		sbStringQuery.append(oKey);
		sbStringQuery.append(" ");
	 ...
	 * @param oSearchValue
	 */
	protected static String createINQueryConditionsSQLExpressionInstance(Object oValue, Vector values, Object oKey, Object oSearchValue) {
	 ...
	 * @param sbStringQuery
	 */
	protected static String createNOTBETWEENQueryConditions(Object oValue, Vector values, boolean bracket, Object oKey) {
		// If it is OR, then value is a vector with
		// objects
		StringBuilder sbStringQuery = new StringBuilder();
		Object oSearchValue = ((SearchValue) oValue).getValue();
	...
	/**
	 * This class is used to store the information to create a SQL statement and
	 * the required values.
	 */
	 ...
	 *
	 * @param processor
	 *            the SQLConditionValuesProcessor instance that be used
	 */
	public static void setSQLConditionValuesProcessor(SQLConditionValuesProcessor processor) {
		SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).setSQLConditionValuesProcessor(processor);
	}

	/**
	 * Returns a <code>SQLStatement</code> class that stores the information needed to execute a query that obtains the number of query records
	 *
	 * @param table
	 *            name of the table the query is executed against
	 * @param conditions
	 *            condition list used in the query
	 * @param wildcards
	 *            column list that can use wildcards
	 * @return a <code>SQLStatement</code> class
	 */

	public static SQLStatement createCountQuery(String table, Hashtable conditions, Vector wildcards, Vector countColumns) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createCountQuery(table, conditions, wildcards, countColumns);
	}

	/**
	 * Returns a <code>SQLStatement</code> class that stores the information needed to execute a select query.
	 *
	 * @see SQLStatementBuilder.SQLStatement
	 * @param table
	 *            name of the table the query is executed against
	 * @param requestedColumns
	 *            a Vector specifying the requested column name in the query
	 * @param conditions
	 *            condition list used to created the query
	 * @param wildcards
	 * @return a <code>SQLStatement</code> class
	 */
	public static SQLStatement createSelectQuery(String table, Vector requestedColumns, Hashtable conditions, Vector wildcards) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createSelectQuery(table, requestedColumns, conditions, wildcards);
	}
	 ...
	 *            column list that can use wildcards
	 * @param columnSorting
	 *            column list where query sorting is established
	 * @param recordCount
	 *            number of records requested in the query
	 * @return a <code>SQLStatement</code> class
	 */

	public static SQLStatement createSelectQuery(String table, Vector requestedColumns, Hashtable conditions, Vector wildcards, Vector columnSorting, int recordCount) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createSelectQuery(table, requestedColumns, conditions, wildcards, columnSorting,
				recordCount);
	}
	 ...
	 *            number of records requested in the query
	 * @param forceDistinct
	 *            true if query result cannot have duplicated records
	 * @param descending
	 *            true if sorting should be descending
	 * @return a <code>SQLStatement</code> class
	 */
	...
	public static SQLStatement createSelectQuery(String table, Vector requestedColumns, Hashtable conditions, Vector wildcards, Vector columnSorting) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createSelectQuery(table, requestedColumns, conditions, wildcards, columnSorting);
	}
	...
	public static SQLStatement createSelectQuery(String table, Vector requestedColumns, Hashtable conditions, Vector wildcards, Vector columnSorting, boolean descending) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createSelectQuery(table, requestedColumns, conditions, wildcards, columnSorting,
				descending);
	}
	...
	/**
	 * Returns a <code>SQLStatement</code> class that stores the information needed to execute a insert query.
	 *
	 * @param table
	 *            name of the table the query is executed against
	 * @param attributes
	...
	public static SQLStatement createInsertQuery(String table, Hashtable attributes) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createInsertQuery(table, attributes);
	}

	/**
	 * Returns a <code>SQLStatement</code> class that stores the information needed to execute a update query.
	 *
	 * @see SQLStatementBuilder.SQLStatement
	 * @param table
	 *            name of the table the update is executed against
	...
	public static SQLStatement createUpdateQuery(String table, Hashtable attributesValues, Hashtable keysValues) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createUpdateQuery(table, attributesValues, keysValues);
	}

	/**
	 * Returns a <code>SQLStatement</code> class that stores the information needed to execute a delete query.
	 *
	 * @param table
	 *            name of the table the query is executed against
	 * @param keysValues
	...
	public static SQLStatement createDeleteQuery(String table, Hashtable keysValues) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createDeleteQuery(table, keysValues);
	}
	 ...
	 * permits process complex conditions where condition key doesn't have to be the column of a table in a database. <code>EXPRESSION_KEY</code> is used as condition key to
	 * indicate which is a complex condition
	 * <p>
	 * Using classes that implements <code>Expression, Field, Operator </code> interfaces is necessary to define a complex condition. Each class that implements the
	 * <code>Expression</code> interface has three important parts. The left operand that can only be another expression or a class that implements <code>Field</code> interface The
	 * right operand that can be another expression or a value The operator that has to be a class that implement <code>Operator<code> interface
	 * <p>
	...
	public static class ExtendedSQLConditionValuesProcessor extends DefaultSQLConditionValuesProcessor {
		...
		/**
		 * Creates a <code>ExtendedSQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> is case-insensitive.
		 * <p>
		 * Inserts a upper function in both sides of <code>LIKE</code> conditions (UPPER(Field) LIKE UPPER(Value))
		 *
		 * @param upperLike
		...
		/**
		 * Creates a <code>ExtendedSQLConditionValuesProcessor</code> where every condition that uses <code>LIKE</code> or is a column of String type is case-insensitive.
		 * <p>
		 * Inserts a upper function in both sides of <code>LIKE</code> conditions (UPPER(Field) LIKE UPPER(Value)) Inserts a upper function in both sides if column type is a String
		 * (UPPER(Field) LIKE UPPER(String))
		 *
		...
		/**
		 * Creates the condition string from a expression. The condition string is stored in the <code>StringBuilder</code> param and the required values in a <code>Vector</code>
		 * param
		 *
		 * @param expression
		 *            a class that implements <code>Expression<code> interface
		 ...
		 *            a <code>StringBuilder</code> where the condition string is stored
		 * @param upper
		 *            Boolean that enable the String compare in upper case, i.e., for non case-sensitive.
		 */
		protected void createQueryConditionsFromExpression(Expression expression, Vector values, StringBuilder sb, boolean upper) {
			// Recursive
			Object lo = expression.getLeftOperand();
			Object ro = expression.getRightOperand();
			...
			if (lo instanceof Field) {
				...
				if ((BasicOperator.LIKE_OP.equals(expressionOperator) || BasicOperator.NOT_LIKE_OP.equals(expressionOperator)) && upper) {
					...
					if (ro != null) {
						if (ro instanceof Expression) {
							...
							if (BasicOperator.LIKE_OP.equals(expressionOperator) || BasicOperator.NOT_LIKE_OP.equals(expressionOperator)) {
								...
								sb.append("' ");
								sb.append(")");
							} else if (BasicOperator.IN_OP.equals(expressionOperator) || BasicOperator.NOT_IN_OP.equals(expressionOperator)) {
								// ro has to be a List
		...
		public String createQueryConditions(Hashtable conditions, Vector wildcards, Vector values) {
			...
			if (conditions.containsKey(
					ExtendedSQLConditionValuesProcessor.EXPRESSION_KEY) && (conditions.get(ExtendedSQLConditionValuesProcessor.EXPRESSION_KEY) instanceof Expression)) {
			...
			if (conditions.containsKey(ExtendedSQLConditionValuesProcessor.FILTER_KEY) && (conditions.get(ExtendedSQLConditionValuesProcessor.FILTER_KEY) instanceof Expression)) {
	...
	/**
	 * Returns a <code>SQLStatement</code> class that stores the information needed to execute a select query against two table used a join.
	 *
	 * @param principalTable
	 *            name of the principal table the query is executed against
	 * @param secondaryTable
	 *            name of the secondary table the query is executed against
	 * @param principalKeys
	 *            a Vector specifying the column names of the principal table that be used to combine the two tables
	 * @param secondaryKeys
	 *            a Vector specifying the column names of the secondary table that be used to combine the two tables
	 * @param principalTableRequestedColumns
	 *            column list that be requested in the query from principal table
	 * @param secondaryTableRequestedColumns
	 *            column list that be requested in the query from secondary table
	 * @param principalTableConditions
	 *            a Hashtable specifying conditions that must comply the set of records returned from principal table
	 * @param secondaryTableConditions
	 *            a Hashtable specifying conditions that must comply the set of records returned from secondary table
	 * @param wildcards
	 *            column list which wildcards can be used in
	 * @param columnSorting
	 *            column list where query sorting is established
	 * @param forceDistinct
	 *            true if query result cannot have duplicated records
	...
	public static SQLStatement createJoinSelectQuery(String principalTable, String secondaryTable, Vector principalKeys, Vector secondaryKeys,
			Vector principalTableRequestedColumns, Vector secondaryTableRequestedColumns, Hashtable principalTableConditions, Hashtable secondaryTableConditions, Vector wildcards,
			Vector columnSorting, boolean forceDistinct) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createJoinSelectQuery(principalTable, secondaryTable, principalKeys, secondaryKeys,
				principalTableRequestedColumns, secondaryTableRequestedColumns, principalTableConditions, secondaryTableConditions, wildcards, columnSorting, forceDistinct);
	}

	/**
	 * Returns a <code>String</code> with the qualified name.
	 * <p>
	 * This static methods creates a qualified name from the column name and the table name. The used pattern is table_name.column_name. If the table name or the column name has
	 * special characters, square brackets are used [table_name].[column_name]
	 *
	 * @param col
	 ...
	 * @param secondaryKeys
	 *            a Vector specifying the column names of the secondary table that be used to combine the two tables
	 * @param mainTableRequestedColumns
	 *            column list that be requested in the query from principal table
	 * @param secondaryTableRequestedColumns
	 *            column list that be requested in the query from secondary table
	 * @param mainTableConditions
	 *            a Hashtable specifying conditions that must comply the set of records returned from principal table
	 * @param secondaryTableConditions
	 *            a Hashtable specifying conditions that must comply the set of records returned from secondary table
	 * @param wildcards
	...
	public static SQLStatement createJoinSelectQuery(String mainTable, String secondaryTable, Vector mainKeys, Vector secondaryKeys, Vector mainTableRequestedColumns,
			Vector secondaryTableRequestedColumns, Hashtable mainTableConditions, Hashtable secondaryTableConditions, Vector wildcards, Vector columnSorting, boolean forceDistinct,
			boolean descending) {

		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createJoinSelectQuery(mainTable, secondaryTable, mainKeys, secondaryKeys,
				mainTableRequestedColumns, secondaryTableRequestedColumns, mainTableConditions, secondaryTableConditions, wildcards, columnSorting, forceDistinct, descending);
	}

	/**
	 * Returns a instance of <code>SQLConditionValuesProcessor</code> that be used in this class to create the condition string
	 *
	...
	public static SQLConditionValuesProcessor getQueryConditionsProcessor() {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).getQueryConditionsProcessor();
	}

	public static String createQueryConditionsWithoutWhere(Hashtable conditions, Vector wildcard, Vector values) {
		return SQLStatementBuilder.getSQLStatementHandler(SQLStatementBuilder.DEFAULT_HANDLER).createQueryConditionsWithoutWhere(conditions, wildcard, values);
	}
}
