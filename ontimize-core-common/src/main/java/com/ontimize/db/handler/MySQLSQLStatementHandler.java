...
public class MySQLSQLStatementHandler extends DefaultSQLStatementHandler {
	 ...
	 * @since 5.2071EN-0.1
	 *
	 *        In mysql 5.x.x rsMetaData.getColumnName(i) is not enough to return column names that have been modified with "as" e.g. select customerid as mycustomer from... In this
	 *        case, the modifier "mycustomer" is only found by using method rsMetaData.getColumnLabel(i). This patch fixes two possibilities.
	 *
	 *
	 */
	@Override
	protected String[] getColumnNames(ResultSetMetaData rsMetaData) {
		...
		try {
			...
			for (int i = 1; i <= rsMetaData.getColumnCount(); i++) {
				...
				if ((columnLabeli != null) && !"".equals(columnLabeli) && !columnLabeli.equals(rsMetaData.getColumnName(i))) {
