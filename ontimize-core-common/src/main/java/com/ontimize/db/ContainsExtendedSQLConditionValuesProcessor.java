...
public class ContainsExtendedSQLConditionValuesProcessor extends ExtendedSQLConditionValuesProcessor {
	...
	public void renderQueryConditionsFromExpression(Expression e, StringBuilder sb) {
		if (e.getLeftOperand() instanceof Expression) {
			...
			// Simple expressions
			if (e.getOperator().toString().equals(ContainsOperator.CONTAINS_OP.toString()) || e.getOperator().toString().equals(ContainsOperator.NOT_CONTAINS_OP.toString())) {
	...
	public static Expression queryToStandard(Expression query, String[] cols, int[] types) {
		...
		if (query.getLeftOperand() instanceof Expression) {
			outputExpression.setLeftOperand(ContainsExtendedSQLConditionValuesProcessor.queryToStandard((Expression) query.getLeftOperand(), cols, types));
			outputExpression.setOperator(query.getOperator());
			outputExpression.setRightOperand(ContainsExtendedSQLConditionValuesProcessor.queryToStandard((Expression) query.getRightOperand(), cols, types));
		} else {
			if (query.getOperator().toString().equals(ContainsOperator.CONTAINS_OP.toString()) || query.getOperator().toString()
				...
				if (query.getOperator().toString().equals(BasicOperator.LIKE_OP.toString()) || query.getOperator().toString().equals(BasicOperator.NOT_LIKE_OP.toString())) {
	...
	public static Expression queryToStandard(Expression query, String entityName, List cols, EntityReferenceLocator locator) {
		try {
