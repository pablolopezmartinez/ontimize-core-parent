...
public class LocalEntityInvocationHandler implements InvocationHandler, Entity, DynamicMemoryEntity {
	...
	protected EntityResult cacheData = new EntityResult(EntityResult.OPERATION_SUCCESSFUL, EntityResult.BEST_COMPRESSION);
	...
	/**
	 * Checks if <code>attributesValues</code> contains a value for all columns defined in 'insert_keys' parameter.
	 * <p>
	 *
	 * @param attributesValues
	 * @throws Exception
	 *             if any of the columns defined in 'insert_keys' is not found as key in <code>attributesValues</code>
	 */
	public void checkInsertKeys(Hashtable attributesValues) throws Exception {
		...
		for (int i = 0; i < insertKeys.size(); i++) {
			if (!attributesValues.containsKey(insertKeys.get(i).toString()) && ((autonumericalColumn == null) || !autonumericalColumn.equals(insertKeys.get(i)))) {
	...
	public EntityResult delete(Hashtable keysValues, int sessionId) throws Exception {
		...
		if (index >= 0) {
			...
			LocalEntityInvocationHandler.logger.debug("Delete: keys parameter does not contain any pair key-value valid");
		}
		return null;
	}
