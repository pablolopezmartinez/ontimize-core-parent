...
/**
 * This interface defines the basic methods that must be implemented by any class (entity) that provides and receives data from the Ontimize client classes (for example, forms and
 * trees).
 * <p>
 * Defines four methods corresponding to the basic operations that generally can be done when managing data:<br>
 * <ul>
 * <li>query</li>
 * <li>insert</li>
 * <li>update</li>
 ...
 * </ul>
 * <p>
 * Classes that implement these methods must ensure that the argument objects passed to the methods are not modified. This interface extends java.rmi.Remote so that methods of this
 * interface can be called using RMI, but this is not required, and instances of objects can be used locally.
 *
...
public interface Entity extends java.rmi.Remote {
	 ...
	 * @param sessionId
	 *            a integer identifying the user or session that performs the action.
	...
	/**
	 * This method must implement a standard insert operation with the data contained in <code>attributesValues</code> parameter.<br>
	 * <p>
	 *
	 ...
	 * So, only the records that comply with the conditions specified by <code>keysValues</code> will be updated to the new values specified by <code>attributesValues</code>.
	 * <p>
	 *
	 * @param attributesValues
	 ...
	 * specified by <code>keysValues</code> will be deleted.
	 * <p>
	 *
	 * @param keysValues
