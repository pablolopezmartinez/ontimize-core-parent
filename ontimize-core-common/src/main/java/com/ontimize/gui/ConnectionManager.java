...
public abstract class ConnectionManager {
	...
	public static void setFQDNConversion(String hostnameServid, String ipsIntern, String ipInternaServid) {
		ConnectionManager.logger.debug("FQDN conversion established: {} , {} , {}", hostnameServid, ipsIntern, ipInternaServid);
	...
	public static boolean checkInternalNetwork() {
		...
		// internal network
		if ((ConnectionManager.serverInternalIP != null) && (ConnectionManager.internalIPs != null) && (ConnectionManager.serverHostname != null)) {
			...
			try {
				...
				ConnectionManager.logger.debug("Local IP: {} . Mask length: {} . Binary Local IP: {} Binary Internal IP: {} , time: {}", sLocalIP, maskLength, localBinaryIP,
	...
	public static int getCompresionThreshold(int bytesNumber, long transmisionTime) {
		...
		// // ERROR: this condition is wrong; bytesNumber can be uncompressed, so its value compressed would be much lower
	...
	public static void checkEntityResult(EntityResult res, EntityReferenceLocator locator) {
		if ((res != null) && (EntityResult.OPERATION_WRONG != res.getCode())) {
			...
			// Test the net speed
			int compressionThreshold = ConnectionManager.getCompresionThreshold(res.getBytesNumber(), res.getStreamTime());
			if (compressionThreshold > 0) {
				...
				if ((opt != null) && (locator instanceof ClientReferenceLocator)) {
					try {
						opt.setDataCompressionThreshold(((ClientReferenceLocator) locator).getUser(), locator.getSessionId(), compressionThreshold);
						ConnectionManager.logger.debug("Compression threshold has been established for {} {} in : {}", ((ClientReferenceLocator) locator).getUser(),
