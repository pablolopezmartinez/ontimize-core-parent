...
public class EntityFunctionAttribute implements Serializable {
	...
	/**
	 * By default the query other entities in EntityTable class always make a filter with the entity key.<br>
	 ...
	 *            Entity name
	 * @param column
	 *            Name of the column to apply the function in
	 * @param filter
	 *            List of columns to use as filter in the query statement
	 * @param functionName
	 *            Name of the function (avg, sum, max, min, count(*) ...)
	 */
	...
	public boolean equals(Object obj) {
		if (obj instanceof EntityFunctionAttribute) {
			...
			if ((this.getFunctionStringQuery() != null) && !this.getFunctionStringQuery().equals(comparate.getFunctionStringQuery())) {
				return false;
			}
