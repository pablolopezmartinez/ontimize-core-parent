...
public class TableAttribute extends Hashtable {
	 ...
	 * Parameter 'recordNumberToInitiallyDownload' indicates the number of records to download when a query includes this attribute . If this parameter is less than 0 then no limit
	 * exists.
	 */
	...
	public int hashCode() {
		try {
			return this.entity.hashCode() + (this.keys != null ? this.keys.hashCode() : 0) + (this.parentkeys != null ? this.parentkeys.hashCode() : 0);
		} catch (Exception ex) {
			TableAttribute.logger.error(null, ex);
			return -1;
		}
