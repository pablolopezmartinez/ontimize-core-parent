...
/**
 * Class that implements all logic in xml client permissions. All client parts are defined: menu, form, form manager, tree,...
 * <p>
...
public abstract class XMLClientUtilities {
	...
	public static Hashtable buildClientPermissions(StringBuffer xmlPermissionDescription, Locale l, String businessCalendarFile) throws Exception {
	...
	private static void buildPermissionInfo(CustomNode root, Hashtable permissions, Locale l, String businessCalendarFile) throws Exception {
		// Builds model from xml tree definition.
	...
	private static void buildApplicationPermissionInfo(CustomNode node, Hashtable permissions, Locale l, String businessCalendarFile) throws Exception {
			...
			if (node.getNodeInfo().equalsIgnoreCase(XMLClientUtilities.APPLICATION_ID)) {
				...
				for (int i = 0; i < node.getChildrenNumber(); i++) {
					...
					if (n.isTag()) {
						...
						try {
							if (period != null) {
								tPeriod = com.ontimize.util.calendar.TimePeriodParserManager.getTimePeriodParser().parse((String) period, l, businessCalendarFile);
							}
						} catch (Exception e) {
						...
						if (!vApplicationPermissions.contains(appPermission)) {
							...
							XMLClientUtilities.logger.info("{} permission not added.The permission is already in the permission list", appPermission);
						}
					}
				}
	...
	private static void buildMenuPermissionInfo(CustomNode node, Hashtable permissions, Locale l, String businessCalendarFile) throws Exception {
						...
						if (n.getNodeInfo().equalsIgnoreCase("ELEMENT")) {
							...
							for (int j = 0; j < n.getChildrenNumber(); j++) {
								...
								if (nH.isTag()) {
									...
									// Now we create the permission
									MenuPermission menuPermission = new MenuPermission(sPermissionName, attr.toString(), bRestricted, tPeriod);
									if (!vMenuPermissions.contains(menuPermission)) {
										vMenuPermissions.add(menuPermission);
									} else {
										XMLClientUtilities.logger.info("{} permission not added. The permission is already in the permissions list", menuPermission);
									}
								}
							}
						}
	...
	private static void buildFormPermissionInfo(CustomNode node, Hashtable permissions, Locale l, String businessCalendarFile) throws Exception {
		// Builds model for menu
		if (node.isTag()) {
						...
						if (n.getNodeInfo().equalsIgnoreCase("ELEMENT")) {
							...
							for (int j = 0; j < n.getChildrenNumber(); j++) {
								...
								if (nH.isTag()) {
									...
									if (columnName != null) {
										formPermission = new TableFormPermission(oFormArchiveName.toString(), sPermissionName, attr.toString(), bRestricted, tPeriod, (String) type,
												(String) columnName);
									} else if (type != null) {
										formPermission = new TableFormPermission(oFormArchiveName.toString(), sPermissionName, attr.toString(), bRestricted, tPeriod,
												(String) type);
									} else {
										formPermission = new FormPermission(oFormArchiveName.toString(), sPermissionName, attr.toString(), bRestricted, tPeriod);
									}

									if (!vFormPermissions.contains(formPermission)) {
										vFormPermissions.add(formPermission);
									} else {
										XMLClientUtilities.logger.info("{} permission not added. The permission is already in the permissions list", formPermission);
									}
								}
							}
						}
	...
	private static void buildFMPermissionInfo(CustomNode node, Hashtable permissions, Locale l, String businessCalendarFile) throws Exception {
						...
						if (n.getNodeInfo().equalsIgnoreCase("ELEMENT")) {
							...
							for (int j = 0; j < n.getChildrenNumber(); j++) {
								...
								if (nH.isTag()) {
									...
									// Now we create the permission
									FMPermission fmPermission = new FMPermission(oFMName.toString(), sPermissionName, attr.toString(), bRestricted, tPeriod);
									if (!vFMPermissions.contains(fmPermission)) {
										vFMPermissions.add(fmPermission);
									} else {
										XMLClientUtilities.logger.info("{} permission not added. The permission is already in the permissions list", fmPermission);
									}
								}
							}
						}
	...
	private static void buildTreePermissionInfo(CustomNode node, Hashtable permissions, Locale l, String businessCalendarFile) throws Exception {
		if (node.isTag()) {
						...
						if (n.getNodeInfo().equalsIgnoreCase("ELEMENT")) {
							...
							for (int j = 0; j < n.getChildrenNumber(); j++) {
								...
								if (nH.isTag()) {
									...
									// Now we create the permission
									TreePermission treePermission = new TreePermission(oTreeFileName.toString(), sPermissionName, attr.toString(), bRestricted, tPeriod);
									if (!vTreePermissions.contains(treePermission)) {
										vTreePermissions.add(treePermission);
									} else {
										XMLClientUtilities.logger.info("{} permission not added. The permission is already in the permissions list", treePermission);
									}
								}
							}
						}
	...
	private static StringBuffer dom2String(Node node) {
		return new StringBuffer(XMLClientUtilities.XML_DECLARATION + XMLClientUtilities.dom2StringInternal(node).toString());
	}
	...
	public static Hashtable joinClientPermissions(List permissions) throws Exception {
		...
		boolean nullValues = XMLClientUtilities.checkNullValues(permissions, XMLClientUtilities.ignoreNullOnClientPermissionCombination);
		if (nullValues && !XMLClientUtilities.ignoreNullOnClientPermissionCombination) {
			throw new Exception(XMLClientUtilities.class.getName() + ": Error retrieving client permissions. NULL value found");
		}
		...
		for (int k = 0; k < commonKeys.size(); k++) {
			...
			for (int i = 0; i < permissions.size(); i++) {
				...
				if (previousValue == null) {
					...
					Hashtable permissionResult = XMLClientUtilities.getClientPermissionUnion((Hashtable) previousValue, (Hashtable) oPermissions);
					result.put(key, permissionResult);
				} else if (previousValue instanceof List) {
					List permissionResult = XMLClientUtilities.joinClientPermissionActions((List) previousValue, (List) oPermissions);
					result.put(key, permissionResult);
				}
			}
		}
	...
	public static List joinClientPermissionActions(List value1, List value2) throws Exception {
		...
		for (int i = 0; i < value1.size(); i++) {
			...
			int actionIndex = XMLClientUtilities.getClientPermissionIndex(clientPermission1.getPermissionName(), clientPermission1.getAttribute(), value2);
	...
	protected static void analyzeRestrictiveClientPermission(List result, AbstractClientPermission clientPermission1,
		...
		if ((clientPermission1.getPeriod() == null) && !clientPermission1.isRestricted()) {
			...
			// one
			result.add(clientPermission1);
		} else if ((clientPermission1.getPeriod() != null) && !clientPermission1.isRestricted() && (clientPermission2.getPeriod() != null) && !clientPermission2
				.isRestricted()) {
			...
			analyzeCommonPeriod(result, clientPermission1, clientPermission2);
				...
				.isRestricted()) || ((clientPermission1.getPeriod() != null) && clientPermission1
			...
			// TODO
			throw new Exception(XMLClientUtilities.class.getName() + ": Permissions with periods one restricted and the other not is not supported");
		}
	}

	protected static void analyzeCommonPeriod(List result, AbstractClientPermission clientPermission1, AbstractClientPermission clientPermission2) {
		...
		if (timePeriodParser instanceof TimePeriodOperationParser) {
			try {
				TimePeriod commonPeriod = ((TimePeriodOperationParser) timePeriodParser).getCommonPeriod(clientPermission1.getPeriod(), clientPermission2.getPeriod());
	...
	protected static void analyzeUnionPeriod(List result, AbstractClientPermission clientPermission1, AbstractClientPermission clientPermission2) {
		TimePeriodParser timePeriodParser = TimePeriodParserManager.getTimePeriodParser();
		if (timePeriodParser instanceof TimePeriodOperationParser) {
			try {
				TimePeriod unionPeriod = ((TimePeriodOperationParser) timePeriodParser).getUnionPeriod(clientPermission1.getPeriod(), clientPermission2.getPeriod());
