...
/**
 * This interface defines the methods that must be implemented by a object that provides entity references.<br>
 * This interface extends Remote to provide RMI support. In this way is easy to implement a reference remote locator.<br>
 * Security must be implemented in all classes that use this interface.<br>
 * The method {@link #getEntityReference(String)} has not parameters to identify the user to avoid that all application components need to know this values.
 *
...
public interface EntityReferenceLocator extends Remote {
	 ...
	 *            User password
	 * @param client
	 *            Object to checks if this session is open
	 * @return Integer value not negative used to identifier this client and allows him to get entity references
	 * @throws Exception
	 */
